"""Data repair for Bedford (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bedford DATA is a CitizenServe / SmartGov-style payload with top-level
keys ``main``, ``extra``, and ``location``. Content variants
(INFERRED_SCHEMA):

  - citizenserve_draft:         unsubmitted drafts (main.status == 0)
  - citizenserve_code:          Report a Concern/Complaint
  - citizenserve_building:      building / trade / ROW / event forms
  - citizenserve_registration:  contractor / rental / license shells
  - citizenserve_planning:      variance / site plan / SUP / DRC
...
Known issues repaired:
  - FILE_DATE was derived from main.dateCreated. When dateSubmitted
    falls on a later calendar day (104 sample rows), overwrite with
    the submittal date.

Not repairable from DATA:
  - STATUS_NORMALIZED already matches main.status 1:1
    (draft/active/complete/stopped ↔ In Review/Active/Final/Inactive).
    A handful of code cases have Case Status Closed while main.status
    remains active (or Active while complete); main.status is treated
    as authoritative to stay consistent with STATUS_ORIGINAL.
  - PERMIT_DATE is universally missing. No issuance / approval
    timestamp exists in main or extra; numeric ASI date-like values
    next to contractor names are insurance/license expirations, and
    lastUpdatedDate / expirationDate are not safe approval proxies.
  - FINAL_DATE is universally missing. No completion / finaled /
    signoff field exists in main or extra (garage-sale / event end
    dates are event schedule fields, not finaling stamps).
"""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if "concern" in rtype or "complaint" in rtype:
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Bedford record."""
    main = _main(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bedford permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bedford".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Bedford") & (df["STATE"] == "TX")].copy()

    print(f"Bedford records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = os.path.join(out_dir, "permits_tx_bedford_repaired.parquet")
