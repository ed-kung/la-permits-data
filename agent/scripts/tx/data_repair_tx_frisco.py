"""Data repair for Frisco (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Frisco DATA is a single City permit-portal payload shape with top-level
...
``search_data``, and ``site_info``. Two content variants appear:
  ...
  - permit_info_unstated: PermitStatus blank (mostly late-1990s / 2000
                          legacy rows with IssuedDate only)
...
Known issues repaired:
  - STATUS_NORMALIZED missing for uncommon PermitStatus values
    (PRESCREEN, RES LETTER *, FINAL ACCEPTANC, etc.) and for blank
    STATUS_ORIGINAL on WITHDRAWN / COMPLETE rows → FILLED.
  - Blank PermitStatus with Issued / Approved / Finaled dates →
    STATUS_NORMALIZED FILLED by date inference.
  - Missing FILE_DATE when PermitAppliedDate is present → FILLED.
  - Missing PERMIT_DATE on Active/Final (and other) rows that have
    ApprovedDate but no IssuedDate → FILLED from ApprovedDate.
  - Missing FINAL_DATE on Final rows → FILLED from FinaledDate or
    approved inspection completion.
  - Spurious FINAL_DATE on non-Final rows (ISSUED / EXPIRED / VOID /
    etc. that still carry PermitFinaledDate) → cleared.

Not repairable / left as-is:
  - Unstated rows with no Applied / Issued / Approved / Finaled dates
    → STATUS_NORMALIZED and dates stay missing.
  ...
  - Active/Final rows with neither Issued nor Approved → PERMIT_DATE
    stays missing.
  - FILE_DATE when PermitAppliedDate is blank → stays missing.
"""
...
_STATUS_MAP = {
    ...
    "CLOSED": "Final",
    "FINAL": "Final",
    "FINAL ACCEPTANC": "Final",  # truncated "FINAL ACCEPTANCE"
    "RES LETTER FINAL SENT": "Final",
    # Active
    "ISSUED": "Active",
    "APPROVED": "Active",
    ...
    "PRESCREEN": "In Review",
    "REVIEW - REV RESUB": "In Review",
    "RES LETTER TWO SENT": "In Review",
    # Inactive
    "EXPIRED": "Inactive",
    ...
    "CANCELLED": "Inactive",
    "WITHDRAWN": "Inactive",
    "APPROVED AS NOT": "Inactive",
    "REFERRED TO CODE": "Inactive",
}
...
def _file_date_candidate(d: dict):
    return _safe_to_datetime(_permit_info(d).get("PermitAppliedDate"))


# ── Per-row repair ───────────────────────────────────────────────────────────

def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Frisco permit_info record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Frisco permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Frisco".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Frisco") & (df["STATE"] == "TX")].copy()

    print(f"Frisco records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_frisco_repaired.parquet")
