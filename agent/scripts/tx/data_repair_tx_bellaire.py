"""Data repair for Bellaire (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bellaire DATA is a SmartGov community portal payload (same family as TX
Anna / FL Newberry) with top-level keys ``Department``, ``My Project``,
``Permit Type``, ``Build Status``, ``Permit Number``, ``Permit Details``,
contacts/fees/inspections arrays, and optionally ``Parcel Number`` /
``ProjectDescription``. ``Permit Details`` and ``Permit Inspections`` are
...
Canonical mappings:
  - DATA["Build Status"] (Expired* sticky Inactive; Closed → Final;
    Issued → Active; Application Submitted / Routed to Review /
    Ready for Close Out / Pending Issuance… / Review Complete /
    Pending Letter → In Review), with Closed-date / Issued-date
    overrides                                              → STATUS_NORMALIZED
...
Known issues repaired:
  - ~241 null STATUS_NORMALIZED rows (null Build Status /
    STATUS_ORIGINAL, unmapped review/expired statuses) inferred
    from Build Status and My Project dates → FILLED.
  - Expired* mislabeled Active / In Review → FIXED to Inactive.
  - Closed still labeled Active / In Review → FIXED to Final;
    FINAL_DATE filled from Closed when present.
  - Ready for Close Out / null Build Status with Issued date →
    FIXED/FILLED Active; PERMIT_DATE retained / filled from Issued.
...
Not repairable from DATA:
  ...
  - Final rows with blank Closed and no usable final inspection →
    FINAL_DATE stays missing (most historical Closed rows).
...
# Keys are lower-case. Bellaire uses title-case Build Status values plus
# "Expired: M/D/YYYY".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Explicit Closed / Finaled → Final. Otherwise Closed date → Final,
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bellaire (TX) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bellaire". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Bellaire") & (df["STATE"] == "TX")
    ].copy()

    print(f"Bellaire records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_tx_bellaire_repaired.parquet"
