"""Data repair for Lago Vista (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lago Vista DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). Nearly all sample rows include ``PaymentProcessorModule``
...
Status values observed in sample:
  ...
  - Approved → Active
  - Pending → In Review
  - Expired / Cancelled / Withdrawn / Never Built /
    Denied / Denied (Fees Due) → Inactive
...
Known issues / sample findings (n=2000):
  ...
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active/Final (all 1,752
  ...
  - DateUpdated and other candidate date fields are empty or the .NET
    ...
    cannot be filled for Final rows (all 1,177 Final rows lack
    FINAL_DATE).
"""
...
_STATUS_MAP = {
    ...
    "Cancelled": "Inactive",
    "Withdrawn": "Inactive",
    "Never Built": "Inactive",
    "Denied": "Inactive",
    "Denied (Fees Due)": "Inactive",
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Lago Vista MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lago Vista permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lago Vista".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Lago Vista") & (df["STATE"] == "TX")
    ].copy()

    print(f"Lago Vista records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_lago_vista_repaired.parquet"
