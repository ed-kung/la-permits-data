"""Data repair for Hurst (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hurst DATA is a CivicPlus / EnerGov-style case payload. Two top-level
...
Known issues repaired:
  - Unmapped STATUS_ORIGINAL values left STATUS_NORMALIZED null
    (Rental Resolved, Annual Registration Complete, Withdrew)
    → FILLED from CaseStatus.
  - STATUS_NORMALIZED lagging STATUS_ORIGINAL while CaseStatus has
    already advanced (Complete still "Active"; Issued still
    "In Review"; Expired still "Active") → FIXED from CaseStatus.
  - Missing PERMIT_DATE on Issued rows (including stale In Review
    labels) whose IssueDate is present → FILLED.
  - Missing FINAL_DATE on Complete rows still labeled Active → FILLED
    from FinalDate/FinalizeDate.
  - Spurious FINAL_DATE on non-Final rows (Void / Expired / Submitted /
    Fees Due / Withdrew / Issued / Plan Approval Expired, plus Annual
    Registration Complete mapped to Active) → cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already equals ApplyDate (calendar day) on every row.
  - Complete Rental Management Renewal / Legacy shells with null
    IssueDate → PERMIT_DATE stays missing.
  - One Issued Sign Permit with Issued=False and null IssueDate →
    PERMIT_DATE stays missing.
  - Ten Rental Resolved rows with null IssueDate → PERMIT_DATE stays
    missing.
  - Most Annual Registration Complete rows have no FinalDate (mapped
    to Active; FINAL_DATE not required).
...
_STATUS_MAP = {
    # Final
    "Complete": "Final",
    "Rental Resolved": "Final",
    # Active (issued / in-force registrations)
    "Issued": "Active",
    "Annual Registration Complete": "Active",
    # In Review
    "Submitted": "In Review",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Hurst entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hurst permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hurst".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Hurst") & (df["STATE"] == "TX")].copy()

    print(f"Hurst records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_hurst_repaired.parquet")
