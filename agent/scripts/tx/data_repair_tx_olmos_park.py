"""Data repair for Olmos Park (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Olmos Park DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - Permit Issued (trailing space) → Active
  - Pending (Under Review) → In Review
  - Withdrawn → Inactive

Known issues / sample findings:
  - STATUS_NORMALIZED already matches stripped ``ProjectStatus`` on all
    2,000 rows (``Permit Issued `` has a trailing space in DATA; strip
    before mapping).
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Olmos Park MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Olmos Park permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Olmos Park".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Olmos Park") & (df["STATE"] == "TX")
    ].copy()

    print(f"Olmos Park records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_olmos_park_repaired.parquet"
