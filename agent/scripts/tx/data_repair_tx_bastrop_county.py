"""Data repair for Bastrop County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bastrop County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  - 34 rows with ProjectStatus Waiting on Client / Waiting on Client -
    Floodplain left STATUS_NORMALIZED null → FILLED as In Review.
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Bastrop MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bastrop County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bastrop County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Bastrop County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Bastrop County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_bastrop_county_repaired.parquet"
