"""Data repair for Hutto (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hutto DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  ...
  - FILE_DATE already equals DateCreated (calendar day) on all rows
    (upstream truncates the time component).
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Hutto MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hutto permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hutto".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Hutto") & (df["STATE"] == "TX")
    ].copy()

    print(f"Hutto records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_hutto_repaired.parquet"
