"""Data repair for Marble Falls (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Marble Falls DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). Most sample rows include ``PaymentProcessorModule``
(value ``MGO``); a single row omits that key only:

  - mgo_ppm:   includes ``PaymentProcessorModule``
...
Status values observed in sample:
  ...
  - Stop Work Order Issued → In Review
  - Withdrawn → Inactive
  - Void → Inactive

Known issues / sample findings (n=2000):
  - STATUS_NORMALIZED missing on 1 row with ProjectStatus
    ``Stop Work Order Issued`` (STATUS_ORIGINAL was present but never
    normalized). All other rows already match ProjectStatus.
  ...
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active/Final (all 1,744
  ...
  - DateUpdated and other candidate date fields are empty or the .NET
    ...
    cannot be filled for Final rows (all 907 Final rows lack FINAL_DATE).
"""
...
_STATUS_MAP = {
    ...
    "Issued (Construction)": "Active",
    "Issued": "Active",
    # In review / waiting on applicant / payment / stop-work
    "Pending": "In Review",
    ...
    "Plan Review Completed": "In Review",
    ...
    # Inactive
    "Expired": "Inactive",
    "Project Closed/Permit Expired": "Inactive",
    "Withdrawn by Applicant": "Inactive",
    "Cancelled/Withdrawn": "Inactive",
    "Denied": "Inactive",
    "Denied - Awaiting Revisions": "Inactive",
    "Cancelled": "Inactive",
    "Void": "Inactive",
    "Withdrawn": "Inactive",
}
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if "customer response" in lower:
        return "In Review"
    if "awaiting revisions" in lower and "denied" not in lower:
        return "In Review"
    if "ready to issue" in lower:
        return "In Review"
    if "plan review" in lower:
        return "In Review"
    if "coc" in lower or "co issued" in lower or "final" in lower:
        return "Final"
    if "closed" in lower and "expir" in lower:
        return "Inactive"
    if "closed" in lower or "complete" in lower:
        return "Final"
    if "issued" in lower and "ready" not in lower and "stop work" not in lower:
        return "Active"
    if (
        "expire" in lower
        or "void" in lower
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Marble Falls
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Marble Falls permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Marble Falls".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Marble Falls") & (df["STATE"] == "TX")
    ].copy()

    print(f"Marble Falls records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_marble_falls_repaired.parquet"
