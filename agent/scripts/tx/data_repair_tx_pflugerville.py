"""Data repair for Pflugerville (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pflugerville DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  ...
  - DateUpdated is also the .NET sentinel; no completion / CO /
    ...
    1,656 Final (Closed / Finaled) rows.
"""
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Pflugerville
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pflugerville permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pflugerville".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pflugerville") & (df["STATE"] == "TX")
    ].copy()

    print(f"Pflugerville records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_pflugerville_repaired.parquet"
