"""Data repair for League City (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

League City DATA is a CivicPlus / EnerGov-style case payload with two
top-level key-set variants that share the same entity/details date and
status fields:
...
Canonical mappings:
  - details.PermitStatus when Complete / Certificate of Occupancy
    Issued, else entity.CaseStatus        → STATUS_NORMALIZED
  - entity.ApplyDate                      → FILE_DATE
  - entity.IssueDate (else details.IssueDate)
                                          → PERMIT_DATE
  - entity.FinalDate (else details.FinalizeDate)
                                          → FINAL_DATE (Final only;
    years outside 1900–2035 treated as missing)

Known issues repaired:
  - Missing STATUS_NORMALIZED on Comments Available Online,
    Pending Issuance of Permits, Pending Inspections, Active - FMO,
    and one In Review row → FILLED.
  - Stale STATUS_ORIGINAL left STATUS_NORMALIZED wrong when portal
    CaseStatus advanced (Expired/Complete/CO Issued still labeled
    Active; Issued still labeled In Review) → FIXED.
  - Spurious FINAL_DATE on non-Final rows (especially Expired /
    Inactive) that still carry portal FinalDate / FinalizeDate →
    cleared (FIXED).
  - Missing PERMIT_DATE / FINAL_DATE filled from IssueDate /
    FinalDate when present.

Not repairable / left as-is:
  - FILE_DATE already matches ApplyDate on all sample rows.
  - ~18 Final rows with null IssueDate → PERMIT_DATE stays missing.
  - ~27 Final (Certificate of Occupancy Issued) rows with null
...
_STATUS_MAP = {
    ...
    "Comments Available Online": "In Review",
    "Pending Issuance of Permits": "In Review",
    "Pending Inspections": "In Review",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Prefer Final portal statuses on either field (details.PermitStatus can
    advance ahead of entity.CaseStatus). Otherwise map entity.CaseStatus,
    falling back to PermitStatus.
    """
    ent = _entity(d)
    det = _details(d)
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one League City entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    League City permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "League City".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "League City") & (df["STATE"] == "TX")].copy()

    print(f"League City records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_league_city_repaired.parquet")
