"""Data repair for San Marcos (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Marcos DATA is a flat MyGovernmentOnline (MGO) / MyPermitNow project
payload (``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). Two top-level key-sets appear in the sample:
...
Status values observed in sample:
  - Completed → Final
  - Issued → Active
  - In Review / Applied / Awaiting Revision /
    Ready to Issue / Addendum Awaiting Revision → In Review
  - Void / Dormant → Inactive

Known issues / sample findings:
  - STATUS_NORMALIZED already matches ProjectStatus 1:1; no fills/fixes.
...
_STATUS_MAP = {
    ...
    "In Review": "In Review",
    "Applied": "In Review",
    "Awaiting Revision": "In Review",
    "Addendum Awaiting Revision": "In Review",
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if "licensed" in lower:
        return "Final"
    if "finaled" in lower or "final" in lower:
        return "Final"
    ...
    if "complete" in lower and "issued" not in lower:
        return "Final"
    if "ready to issue" in lower or "ready-to-issue" in lower:
        return "In Review"
    if "applied" in lower:
        return "In Review"
    if "awaiting revision" in lower or "awaiting" in lower:
        return "In Review"
    if "permitted" in lower:
        return "Active"
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the San Marcos
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Marcos permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Marcos".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "San Marcos") & (df["STATE"] == "TX")
    ].copy()

    print(f"San Marcos records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_san_marcos_repaired.parquet"
