"""Data repair for La Marque (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

La Marque DATA is a SmartGov community portal payload (same family as TX
Bellaire / Anna) with top-level keys ``Department``, ``My Project``,
``Permit Type``, ``Build Status``, ``Permit Number``, ``Permit Details``,
contacts/fees/inspections/parcels arrays, and ``Application Number``.
Every sample row shares the same key set; usable vs empty is distinguished
by whether canonical fields are populated.

Variants (INFERRED_SCHEMA):
  - smartgov_minimal:  SmartGov core with usable Build Status / dates /
...
Canonical mappings:
  - DATA["Build Status"] (Expired* sticky Inactive;
    All work completed / Closed → Final; Issued → Active;
    Pending → In Review), with Closed-date / Issued-date overrides
                                                         → STATUS_NORMALIZED
...
Known issues / sample findings (n=2000):
  - 1,973 rows are empty SmartGov shells (UUID-like PERMIT_NUMBER,
    null STATUS_ORIGINAL, blank My Project) → not repairable from DATA.
  - 27 rows have usable payload; 26 have Build Status.
  - Among usable rows, STATUS_NORMALIZED already matches Build Status
    except: (1) one null-status row with Submitted/Created only → fill
    In Review; (2) three Pending rows that also carry an Issued date →
    Fixed to Active (Issued-date override, same as Bellaire).
  - FILE_DATE already equals Submitted (else Created) on all usable rows.
  - PERMIT_DATE already matches Issued/Approved where present; cannot
    fill Active/Final rows that lack both Issued and Approved
    (e.g. one Final burial permit with Closed only).
  - FINAL_DATE already equals Closed on all Final rows; no spurious
    FINAL_DATE on non-Final rows.

Status values observed in sample:
  - All work completed → Final
  - Issued → Active
  - Pending → In Review (unless Issued date present → Active)
  - Expired: M/D/YYYY → Inactive
"""
...
# Keys are lower-case. La Marque uses title-case Build Status values plus
# "Expired: M/D/YYYY" and "All work completed".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "all work completed": "Final",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive for Expired / Cancelled when no Closed date.
    Explicit Closed / Finaled / All work completed → Final. Otherwise
    Closed date → Final, Issued date → Active. Null Build Status falls
    back to date inference.
    """
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    La Marque (TX) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "La Marque". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "La Marque") & (df["STATE"] == "TX")
    ].copy()

    print(f"La Marque records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_tx_la_marque_repaired.parquet"
        ...
        tmp = pd.DataFrame(
            {
                ...
                "permit": city.loc[changed, "PERMIT_NUMBER"],
                "status_o": city.loc[changed, "STATUS_ORIGINAL"],
            }
        )
    ...
    if len(af_miss):
        print("  ", af_miss["PERMIT_NUMBER"].tolist())
    final_miss = repaired[
