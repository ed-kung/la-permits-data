"""Data repair for Coppell (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Coppell DATA is a CivicPlus / EnerGov-style case payload with two
...
Known issues repaired:
  - Missing STATUS_NORMALIZED on
    ``Comments Entered - Awaiting Re-submission`` (10) → FILLED as
    In Review.
  - Spurious FINAL_DATE on Active (Issued, 2) and Inactive (Void, 4)
    rows that still carry a portal FinalDate / FinalizeDate → cleared
    (FIXED).

Not repairable / left as-is:
  - FILE_DATE already matches ApplyDate on all sample rows.
  - Active (4) / Final (44) rows with null IssueDate and details.Issued
    False → PERMIT_DATE stays missing (no issuance timestamp in DATA).
"""
...
_STATUS_MAP = {
    ...
    "Comments Entered - Awaiting Re-submission": "In Review",
    # Inactive
    "Expired": "Inactive",
    "Void": "Inactive",
    "Denied": "Inactive",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Coppell entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Coppell permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Coppell".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Coppell") & (df["STATE"] == "TX")].copy()

    print(f"Coppell records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_coppell_repaired.parquet")
