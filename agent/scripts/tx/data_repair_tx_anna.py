"""Data repair for Anna (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Anna DATA is a SmartGov community portal payload (same family as FL
Newberry / Longwood / Redington Shores) with top-level keys
...
Canonical mappings:
  - DATA["Build Status"] (Expired* / Cancelled sticky Inactive;
    Closed / Finaled → Final; Issued / Approved → Active;
    Ready To Issue / Under Review / Pending → In Review), with
    Closed-date / Issued-date overrides              → STATUS_NORMALIZED
...
Known issues repaired:
  - ~865 null STATUS_NORMALIZED rows (null Build Status /
    STATUS_ORIGINAL, or Expired* left unmapped) inferred from Build
    Status and My Project dates → FILLED
    (Closed→Final, Issued→Active, Expired→Inactive, else
    Submitted/Created/Approved→In Review).
  - Expired* mislabeled Active / In Review → FIXED to Inactive.
  - Closed / Finaled still labeled In Review or null → FIXED/FILLED
    Final; FINAL_DATE filled from Closed when present.
  - Issued labeled In Review or null → FIXED/FILLED Active;
  ...
  - Spurious FINAL_DATE on non-Final statuses cleared after status
    resolution (null-status rows that carried Closed but were not yet
    labeled Final).

Not repairable from DATA:
  - Fully empty SmartGov shells (no Build Status, Permit Number,
    Permit Type, or My Project dates) → status/dates stay missing.
  - Final / Finaled rows with blank Closed and no passed Building
    Final / COO inspection → FINAL_DATE stays missing.
...
# Keys are lower-case. Anna uses title-case Build Status values plus
# "Expired: M/D/YYYY".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if not pd.isna(current_permit):
        ...
        # Inactive includes Expired / Cancelled shells that were
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Anna (TX) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Anna". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Anna") & (df["STATE"] == "TX")
    ].copy()

    print(f"Anna records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_tx_anna_repaired.parquet"
