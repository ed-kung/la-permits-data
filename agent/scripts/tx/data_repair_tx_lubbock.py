"""Data repair for Lubbock (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lubbock DATA is a CivicPlus / EnerGov-style case payload. Two top-level
...
Canonical mappings:
  ...
  - entity.FinalDate (else details.FinalizeDate,
    else latest Passed inspection date)       → FINAL_DATE (Final only)

Known issues repaired:
  - STATUS_NORMALIZED missing for uncommon CaseStatus values
    (Pending Applicant Action, Under Review - Online,
    Approved - WSSC/Small Cell, LPL/ROW Denied) → FILLED.
  - STATUS_NORMALIZED stale vs CaseStatus (STATUS_ORIGINAL lagged:
    Completed still ``issued`` / ``reviewed``, Issued still ``reviewed``,
    Expired still ``issued``, Canceled still ``reviewed``) → FIXED.
  - Missing PERMIT_DATE when IssueDate is present (often after status
    was wrongly In Review) → FILLED.
  - Missing FINAL_DATE on Final / newly-Final rows with FinalDate /
    FinalizeDate, or with a Passed processing_status inspection date
    → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Expired / Void / Approved
    stamped with FinalDate) → cleared (FIXED).

Not repairable / left as-is:
  - Active / Final rows with null IssueDate → PERMIT_DATE stays missing.
  - Final rows with null FinalDate, FinalizeDate, and no Passed
    inspection date → FINAL_DATE stays missing.
  - FILE_DATE is already complete and correct vs ApplyDate; no changes
    expected.
"""
...
_STATUS_MAP = {
    ...
    "Issued": "Active",
    "Active": "Active",
    "Approved - WSSC/Small Cell": "Active",
    # In Review
    "Reviewed": "In Review",
    "Under Review": "In Review",
    "Under Review - Online": "In Review",
    "Pending Applicant Action": "In Review",
    # Inactive
    "Expired": "Inactive",
    "Canceled": "Inactive",
    "Void": "Inactive",
    "LPL/ROW Denied": "Inactive",
}
...
def _latest_passed_inspection_date(d: dict):
    """Latest date from a Passed processing_status inspection entry."""
    ...
    for item in ps:
        ...
        if status != "passed":
            continue
        # Lubbock inspections expose scheduled_date / requested_date only
        # (no completed_date in the sample).
        for key in (
            "completed_date",
            "complete_date",
            "inspection_date",
...
def _final_date_candidate(d: dict):
    """Prefer entity.FinalDate; then details.FinalizeDate; then Passed insp."""
    ent = _entity(d)
    det = _details(d)
    ...
    if finalize is not pd.NaT and not pd.isna(finalize):
        return finalize
    return _latest_passed_inspection_date(d)
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Lubbock entity_* record."""
    ent = _entity(d)
    ...
    # -- FINAL_DATE ← FinalDate / FinalizeDate / Passed insp (Final only) --
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lubbock permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lubbock".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Lubbock") & (df["STATE"] == "TX")].copy()

    print(f"Lubbock records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_lubbock_repaired.parquet")
