"""Data repair for Travis County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Travis County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - In-Review / Open / Unpaid / Post Review → In Review
  - Void / Dormant → Inactive

Known issues / sample findings:
  - 1 row missing STATUS_NORMALIZED (``Post Review``) → FILLED as
    In Review. All other rows already match ProjectStatus 1:1.
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Travis County
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Travis County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Travis County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Travis County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Travis County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_travis_county_repaired.parquet"
