...
Known issues / sample findings (n=2000):
  - STATUS_NORMALIZED missing on 4 rows with ProjectStatus
    ``Denied - Awaiting Revisions`` (STATUS_ORIGINAL was present but
    never normalized). All other rows already match ProjectStatus.
  ...
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active/Final (all 1,811
  ...
  - DateUpdated and other candidate date fields are empty or the .NET
    ...
    cannot be filled for Final rows (all 1,240 Final rows lack FINAL_DATE).
"""
...
_STATUS_MAP = {
    ...
    "Issued": "Active",
    # In review / waiting on applicant / payment
    "Pending": "In Review",
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Taylor MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Taylor permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Taylor".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Taylor") & (df["STATE"] == "TX")
    ].copy()

    print(f"Taylor records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_taylor_repaired.parquet"
