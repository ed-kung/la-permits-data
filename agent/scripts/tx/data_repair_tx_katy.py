"""Data repair for Katy (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Katy DATA is a CivicPlus / EnerGov-style case payload. Every sample row
shares the same top-level key set (classified as ``entity_rich``):
...
Known issues repaired:
  - Unmapped STATUS_ORIGINAL values left STATUS_NORMALIZED null
    (requires resubmittal, review approved) → FILLED from CaseStatus /
    PermitStatus.
  - STATUS_NORMALIZED lagging portal status when details.PermitStatus
    is already Complete while entity.CaseStatus / STATUS_ORIGINAL still
    say Issued → FIXED to Final; FINAL_DATE filled from FinalizeDate.
  - Spurious FINAL_DATE on non-Final rows (Void / Expired / Denied /
    Issued / Review Approved) → cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already equals entity.ApplyDate (calendar day) on every row.
  - Four Complete rows with Issued=False and null IssueDate → PERMIT_DATE
    stays missing.
  - Four Complete rows with null FinalDate and FinalizeDate → FINAL_DATE
    stays missing.
  - processing_status is null on every sample row (no inspection fallback).
"""
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Katy entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Katy permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Katy".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Katy") & (df["STATE"] == "TX")].copy()

    print(f"Katy records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_katy_repaired.parquet")
