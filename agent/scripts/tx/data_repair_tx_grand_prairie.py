"""Data repair for Grand Prairie (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Grand Prairie DATA is a CivicPlus / EnerGov-style case payload. Two
...
Known issues repaired:
  - STATUS_NORMALIZED stale vs CaseStatus (STATUS_ORIGINAL lagged behind
    portal updates: Issued still ``in review``, Complete/Closed still
    ``expired`` / ``issued``, Expired still ``issued`` / ``on hold``)
    → FIXED from CaseStatus.
  - Missing PERMIT_DATE on Issued rows that already carry IssueDate
    (status was wrongly In Review) → FILLED.
  - FINAL_DATE wrong vs FinalDate / FinalizeDate on a few Complete rows
    → FIXED.
  - Missing FINAL_DATE on Final rows with FinalDate / FinalizeDate, or
    with a Passed processing_status inspection date → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Issued / Approved / Expired /
    etc. stamped with FinalizeDate) → cleared (FIXED).

Not repairable / left as-is:
  - Active / Final rows with null IssueDate → PERMIT_DATE stays missing
    (common on Complete / Closed / Certificate Issued / Approved).
  - Final rows with null FinalDate, FinalizeDate, and no Passed
    inspection date → FINAL_DATE stays missing (majority of Complete /
    Closed legacy rows).
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Grand Prairie entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Grand Prairie permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Grand Prairie".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Grand Prairie") & (df["STATE"] == "TX")].copy()

    print(f"Grand Prairie records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_grand_prairie_repaired.parquet")
