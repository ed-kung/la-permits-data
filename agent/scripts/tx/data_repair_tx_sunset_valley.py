"""Data repair for Sunset Valley (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Sunset Valley DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings (n=570):
  ...
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active/Final (all 322
  ...
  - DateUpdated and other candidate date fields are empty or the .NET
    ...
    cannot be filled for Final rows (all 28 Final rows lack FINAL_DATE).
"""
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Sunset Valley
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sunset Valley permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Sunset Valley".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Sunset Valley") & (df["STATE"] == "TX")
    ].copy()

    print(f"Sunset Valley records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_sunset_valley_repaired.parquet"
