"""Data repair for Edinburg (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Edinburg DATA is a CivicPlus / EnerGov-style case payload. Two top-level
...
Canonical mappings:
  ...
  - entity.IssueDate (reject year < 1900)     → PERMIT_DATE
...
Known issues repaired:
  - STATUS_NORMALIZED stale vs CaseStatus (STATUS_ORIGINAL lagged:
    Complete still ``issued``, Expired / Expired - No Activity still
    ``issued``, Issued still ``in review``, Stop Work Order coded as
    In Review) → FIXED.
  - PERMIT_DATE missing when a real IssueDate is present (Issued rows
    still labeled In Review) → FILLED.
  - PERMIT_DATE / IssueDate SQL sentinel ``1753-01-01`` → cleared
    (FIXED).
  - Spurious FINAL_DATE on non-Final rows (Issued often carries
    FinalDate/FinalizeDate while still Issued; Void occasionally
    stamped) → cleared (FIXED).
  - Newly-Final Complete rows already carry FinalDate; no extra fill
    needed beyond status fix.

Not repairable / left as-is:
  - Approved (Active) rows with null IssueDate → PERMIT_DATE stays
    missing (approved but not issued).
  - Complete Certificate of Occupancy / similar shells with null
...
_STATUS_MAP = {
    ...
    "Incomplete Submission": "In Review",
    # Inactive
    "Expired": "Inactive",
    "Expired - No Activity": "Inactive",
    "Void": "Inactive",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Edinburg entity_* record."""
    ent = _entity(d)
    ...
    # -- PERMIT_DATE ← IssueDate (reject SQL 1753 sentinel) --
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Edinburg permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Edinburg".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Edinburg") & (df["STATE"] == "TX")].copy()

    print(f"Edinburg records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_edinburg_repaired.parquet")
