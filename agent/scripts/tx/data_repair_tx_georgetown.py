"""Data repair for Georgetown (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Georgetown DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  - Closed → Final
  - Cond CO Issued → Final
  - Permit Issued → Active
  - In Review / Awaiting Revisions / Ready to Issue → In Review
  - Expired / Denied → Inactive

Known issues / sample findings:
  - STATUS_NORMALIZED already matches ProjectStatus 1:1 on all 2,001
    sample rows; no fills/fixes expected.
  ...
  - DateUpdated and ScheduledDueDate are null or the .NET sentinel; no
    finaled / completion timestamp exists → FINAL_DATE cannot be filled
    for Final rows.
  - ``ProjectStatusIsPermit`` is False for In Review, Expired, Denied,
    and Cond CO Issued; status text still drives normalization.
"""
...
_STATUS_MAP = {
    ...
    "Expired": "Inactive",
    "Denied": "Inactive",
    "Project Withdrawn/Canceled": "Inactive",
    "Project Withdrawn/Cancelled": "Inactive",
    "Withdrawn": "Inactive",
    "Withdrawn/Cancelled": "Inactive",
    "Withdrawn/Canceled": "Inactive",
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    lower = raw.lower()
    # Terminal abandon / non-response closures before generic "closed"→Final.
    ...
    if "ready to issue" in lower:
        return "In Review"
    if "awaiting" in lower or "revision" in lower:
        return "In Review"
    if (
        "expire" in lower
        or "void" in lower
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion timestamp exists in the Georgetown MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Georgetown permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Georgetown".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Georgetown") & (df["STATE"] == "TX")
    ].copy()

    print(f"Georgetown records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_georgetown_repaired.parquet"
