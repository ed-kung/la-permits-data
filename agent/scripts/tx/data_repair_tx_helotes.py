"""Data repair for Helotes (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Helotes DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). In the TX sample every row includes
``PaymentProcessorModule`` (value ``MGO``):
...
Status values observed in sample:
  ...
  - In Review / Pending / Pending (In Review) → In Review

Known issues / sample findings:
  - STATUS_NORMALIZED matches ProjectStatus on 88/89 rows; the single
    ``Closed (Completed)`` row has missing STATUS_NORMALIZED → fill Final.
  - FILE_DATE already equals DateCreated (calendar day) on all rows.
  - DateIssued and DateUpdated are the sentinel ``0001-01-01T00:00:00``
    on every sample row → PERMIT_DATE cannot be filled for Active/Final.
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Helotes MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Helotes permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Helotes".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Helotes") & (df["STATE"] == "TX")
    ].copy()

    print(f"Helotes records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_helotes_repaired.parquet"
