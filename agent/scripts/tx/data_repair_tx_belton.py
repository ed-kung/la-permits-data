"""Data repair for Belton (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Belton DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - No Applicant Response - Closed → Inactive
  - No Inspections - Permit expired → Inactive
  - Withdrawn → Inactive

Known issues repaired:
  - 165 rows with ProjectStatus ``No Applicant Response - Closed``
    were normalized as In Review; reclassified to Inactive (FIXED).
    ``ProjectStatusIsPermit`` is False for these rows (same as
    expired / withdrawn), and the portal status is a terminal
    non-completion closure rather than an open review.

Known issues / sample findings left as-is:
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Belton MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Belton permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Belton".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Belton") & (df["STATE"] == "TX")
    ].copy()

    print(f"Belton records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_belton_repaired.parquet"
