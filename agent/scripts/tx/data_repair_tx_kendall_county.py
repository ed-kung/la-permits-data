"""Data repair for Kendall County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Kendall County DATA is a flat MyGovernmentOnline (MGO) / MyPermitNow
project payload (``ProjectStatus``, ``DateCreated``, ``DateIssued``,
applicant/site fields, etc.). The sample is structurally uniform:
...
Status values observed in sample:
  - Closed → Final
  - Permitted / Active → Active
  - Pending → In Review
  - Expired / Inactive → Inactive

Known issues / sample findings (n=2000):
  - STATUS_NORMALIZED already matches ProjectStatus 1:1 (0 mismatches).
...
_STATUS_MAP = {
    ...
    "Permitted": "Active",
    "Permitted (No Maintenance": "Active",  # truncated portal label seen elsewhere
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Kendall County
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Kendall County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Kendall County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Kendall County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Kendall County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_kendall_county_repaired.parquet"
