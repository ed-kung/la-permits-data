"""Data repair for Irving (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Irving DATA is a single City permit-portal payload shape with top-level
keys ``contacts``, ``fees``, ``inspections``, ``permit_info``,
``search_data``, and ``site_info``. Most rows have a populated
``PermitStatus`` (schema label: ``permit_info``); a small legacy subset
has blank status (``permit_info_unstated``).

Canonical mappings:
  - permit_info.PermitStatus       → STATUS_NORMALIZED
    (blank status: infer from Finaled / Issued / Approved dates)
  - permit_info.PermitAppliedDate  → FILE_DATE
    (fallback: PermitIssuedDate when Applied is blank — legacy NONE rows)
...
Known issues repaired:
  - STATUS_NORMALIZED missing for NONE, PENDING INSPECT, and blank
    PermitStatus → FILLED.
  - Missing FILE_DATE on legacy NONE (PARENT) rows with blank Applied
    but populated Issued → FILLED from IssuedDate.
  - Missing PERMIT_DATE when Issued is blank but Approved is present
    → FILLED from ApprovedDate.
  - Missing FINAL_DATE on Final rows with blank FinaledDate → FILLED
    from latest approved/pass inspection Completed date when available.
  - Spurious FINAL_DATE on non-Final rows (PAID / CANCELLED that still
    carry PermitFinaledDate) → cleared (FIXED).
...
Not repairable / left as-is:
  ...
  - Final rows with neither FinaledDate nor approved inspection
    Completed dates (common for NO INSP REQUESTED / NO FINAL REQ
    -CLOSED) → FINAL_DATE stays missing.
  - When present, FILE/PERMIT/FINAL already match Applied/Issued/Finaled
    at day resolution; no date FIXEDs expected beyond clearing spurious
    finals.
"""
...
_STATUS_MAP = {
    ...
    "CLOSED": "Final",
    "FINALED": "Final",
    "NO FINAL REQ -CLOSED": "Final",
    "NO FINAL INSP REQUES": "Final",  # truncated "NO FINAL INSP REQUESTED"
    "NO INSP REQUESTED": "Final",
    "CO": "Final",
    ...
    "APPROVED": "Active",
    "ACTIVATED": "Active",
    "PENDING INSPECT": "Active",  # under inspection (CO / post-approval)
    "NONE": "Active",  # legacy PARENT rows with Issued, blank Applied
    # In Review
    "SUBMITTED": "In Review",
...
def _is_approved_inspection_result(result) -> bool:
    """Irving uses APPROVED/PASS, APPROVED WITH EXCEPT, rare APPROVEDA/PASS."""
...
def _file_date_candidate(d: dict):
    """Prefer AppliedDate; fall back to IssuedDate for legacy NONE rows."""
    pi = _permit_info(d)
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Irving permit_info record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Irving permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Irving".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Irving") & (df["STATE"] == "TX")].copy()

    print(f"Irving records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_irving_repaired.parquet")
