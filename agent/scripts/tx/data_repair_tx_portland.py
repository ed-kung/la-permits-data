"""Data repair for Portland (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Portland DATA is a SmartGov community portal payload (same family as TX
La Marque / Bellaire / Anna) with top-level keys ``Department``,
``My Project``, ``Permit Type``, ``Build Status``, ``Permit Number``,
``Permit Details``, contacts/fees/inspections/parcels arrays, and
``Application Number``. Most rows also carry ``Parcel Number`` and
``ProjectDescription``.

Variants (INFERRED_SCHEMA):
  - smartgov_full:      ProjectDescription + Parcel Number + core keys
  - smartgov_no_desc:   Parcel Number + core keys, no ProjectDescription
  - smartgov_no_parcel: core keys without Parcel Number / description
  - smartgov_minimal:   My Project present; missing Department / Permit Type
  - smartgov_empty:     SmartGov keyset present but all canonical fields blank
  - missing / unknown

Canonical mappings:
  - DATA["Build Status"] (Expired* sticky Inactive unless later
    Closed; Withdrawn/Cancelled sticky Inactive even if Closed;
    Closed / FINALED / Cerificate of Occupancy → Final;
    Issued → Active; Pending / Technically Completed /
    Application Complete / Ready To Issue / Received → In Review),
...
Known issues / sample findings (n=2000):
  - ~412 null STATUS_NORMALIZED rows: mostly Expired* never mapped
    (~178), null Build Status with Issued/Closed dates (~177 Active /
    ~28 Final fillable), plus a few In Review shells.
  - Mis-maps: Expired* labeled Active (~8); Cerificate of Occupancy /
    CLOSED with Closed date still labeled Active (~17 → Final);
    ISSUED labeled In Review when Issued date present (~4 → Active);
    Application Complete wrongly labeled Final (~6 → In Review).
  - FILE_DATE already matches Submitted (else Created) on nearly all
    rows; ~11–15 missing values fillable from My Project.
  - PERMIT_DATE missing on ~63 fillable Active/Final/Inactive rows;
    FINALED / Cerificate of Occupancy rows often lack Closed so
    FINAL_DATE cannot always be filled from My Project (inspection
    fallback helps when a passed Building Final / COO exists).
  - ~11 empty SmartGov shells (blank My Project + null Build Status)
    are not repairable from DATA.
"""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if "Department" in keys or "Permit Type" in keys:
        return "smartgov_no_parcel"
    return "smartgov_minimal"


# ── Status mapping ──────────────────────────────────────────────────────────

# Keys are lower-case. Portland mixes title-case, ALL-CAPS, and the agency
# typo "Cerificate of Occupancy". Expired values are normalized to "Expired".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "all work completed": "Final",
    "certificate of completion": "Final",
    "certificate of occupancy": "Final",
    "cerificate of occupancy": "Final",  # agency typo in source
    "issued": "Active",
    "active": "Active",
    "approved": "Active",
    "renewed": "Active",
    ...
    "routed to review": "In Review",
    "review complete": "In Review",
    "ready for close out": "In Review",
    "pending issuance - resolve alerts and collect unpaid fees": "In Review",
    "pending letter": "In Review",
    "pending": "In Review",
...
def _map_build_status(bs: Optional[str]) -> Optional[str]:
    if bs is None:
        return None
    return _STATUS_MAP.get(bs.lower())
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive for Expired / Withdrawn when no Closed date.
    Explicit Closed / Finaled / CO → Final. Otherwise Closed date → Final,
    Issued date → Active. Null Build Status falls back to date inference.
    """
    ...
    if bs == "Expired":
        # Expired stays Inactive unless later Closed/Finaled.
        if closed is not pd.NaT or (bs_low in _FINAL_BUILD_STATUSES):
            return "Final"
        return "Inactive"

    if bs_low is not None and bs_low in _INACTIVE_BUILD_STATUSES:
        # Withdrawn / cancelled / denied stay Inactive. SmartGov often stamps
        # Closed when an application is administratively closed after
        # withdrawal; that is not a permit final.
        return "Inactive"
...
def _is_passed_inspection_status(status: str) -> bool:
    s = status.strip().lower()
    if not s:
        return False
    if s in ("passed", "approved", "completed"):
        return True
    # Portland third-party inspector wording, e.g. "Bureau Veritas Passed"
    if s.endswith(" passed") or s.endswith(" approved"):
        return True
    return False
...
def _final_inspection_date(d: dict):
    ...
    for insp in inspections:
        ...
        if not _is_passed_inspection_status(status):
            continue
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if not pd.isna(current_permit):
        ...
        # Inactive includes Expired shells that were previously issued /
        # approved; fill from Issued, else Approved.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Portland (TX) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Portland". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Portland") & (df["STATE"] == "TX")
    ].copy()

    print(f"Portland records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_tx_portland_repaired.parquet"
    ...
    print("\nSTATUS_NORMALIZED changes (before → after), summary:")
    ...
    print("\nCoverage by STATUS_NORMALIZED (after, usable schemas only):")
    usable_df = repaired[repaired["INFERRED_SCHEMA"] != "smartgov_empty"]
    for status in ["Active", "Final", "In Review", "Inactive"]:
        sub = usable_df[usable_df["STATUS_NORMALIZED"] == status]
        if len(sub) == 0:
            continue
