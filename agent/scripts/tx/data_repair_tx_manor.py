"""Data repair for Manor (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Manor DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings (n=2000):
  ...
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active/Final (all 1,754
  ...
  - DateUpdated and other candidate date fields are empty or the .NET
    ...
    cannot be filled for Final rows (all 1,456 Final rows lack
    FINAL_DATE).
"""
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Manor MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Manor permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Manor".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Manor") & (df["STATE"] == "TX")
    ].copy()

    print(f"Manor records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_manor_repaired.parquet"
