"""Data repair for Hays County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hays County DATA is a flat MyGovernmentOnline (MGO) / MyPermitNow project
...
Status values observed in sample:
  - Closed → Final
  - Permitted / Active /
    Permitted (No Maintenance → Active
  - Pending → In Review
  - Expired / Inactive / Transferred → Inactive

Known issues / sample findings:
  - 4 ``Transferred`` rows were labeled In Review; remapped to Inactive
    (transfer out of this portal is a terminal / non-review state).
...
_STATUS_MAP = {
    ...
    "Permitted": "Active",
    "Permitted (No Maintenance": "Active",  # truncated portal label in sample
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Hays County
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hays County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hays County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Hays County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Hays County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_hays_county_repaired.parquet"
