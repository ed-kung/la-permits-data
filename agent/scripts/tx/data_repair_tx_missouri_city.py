"""Data repair for Missouri City (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Missouri City DATA is a CivicPlus / EnerGov-style case payload. Two
...
Known issues repaired:
  - STATUS_NORMALIZED missing for uncommon CaseStatus values
    (Application incomplete - Attention required; Review Disapproved -
    Response required; Fees Invoiced - Payment Required) → FILLED as
    In Review.
  - Spurious FINAL_DATE on non-Final rows (Issued / Void / Denied /
    Withdrawn / Expired / In Review / On Hold often carry
    FinalDate/FinalizeDate while CaseStatus is not Complete) → cleared
    (FIXED).

Not repairable / left as-is:
  - Complete rows with null IssueDate (mostly Certificate of Occupancy
    / similar shells) → PERMIT_DATE stays missing.
  - Complete rows with null FinalDate/FinalizeDate → FINAL_DATE stays
    missing.
...
_STATUS_MAP = {
    ...
    "Fees Paid": "In Review",
    "On Hold": "In Review",
    "Application incomplete - Attention required": "In Review",
    "Review Disapproved - Response required": "In Review",
    "Fees Invoiced - Payment Required": "In Review",
    # Inactive
    "Expired": "Inactive",
    "Void": "Inactive",
    "Denied": "Inactive",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Missouri City entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Missouri City permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Missouri City".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Missouri City") & (df["STATE"] == "TX")].copy()

    print(f"Missouri City records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_missouri_city_repaired.parquet")
