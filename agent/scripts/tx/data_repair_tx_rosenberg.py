"""Data repair for Rosenberg (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Rosenberg DATA is a CivicPlus / EnerGov-style case payload. Two
...
Known issues repaired:
  - Spurious FINAL_DATE on non-Final rows (Void / Issued / Expired
    often carry FinalDate/FinalizeDate while CaseStatus is not
    Complete or Closed) → cleared (FIXED).

Not repairable / left as-is:
  - STATUS_NORMALIZED already matches CaseStatus 1:1 on all sample
    rows; no fills/fixes expected.
  - FILE_DATE already equals ApplyDate (calendar day) on every row.
  - Active/Final rows with null IssueDate (Approved shells; some
    Complete/Closed CO / reconnect / legacy cases) → PERMIT_DATE
    stays missing.
  - Complete rows with null FinalDate/FinalizeDate (28 legacy
    shells) → FINAL_DATE stays missing.
...
_STATUS_MAP = {
    ...
    "Waiting for resubmittal": "In Review",
    "Submitted": "In Review",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Rosenberg entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Rosenberg permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Rosenberg".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Rosenberg") & (df["STATE"] == "TX")].copy()

    print(f"Rosenberg records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_rosenberg_repaired.parquet")
