"""Data repair for Pearland (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pearland DATA is a single City permit-portal payload shape with top-level
keys ``contacts``, ``fees``, ``inspections``, ``permit_info``,
``search_data``, and ``site_info``. All sample rows have a populated
``PermitStatus`` (schema label: ``permit_info``).
...
Known issues repaired:
  - STATUS_NORMALIZED missing for uncommon PermitStatus values
    (APPROVED, FEES DUE; ACTIVATED) → FILLED.
  - Missing PERMIT_DATE when Issued is blank but Approved is present
    → FILLED from ApprovedDate (common on CLOSED / Final rows).
  - Missing FINAL_DATE on Final rows with blank FinaledDate → FILLED
    from latest APPROVED inspection Completed date when available.
  - Spurious FINAL_DATE on non-Final rows (ISSUED that still carry
...
Not repairable / left as-is:
  ...
  - FILE_DATE is already complete and correct vs PermitAppliedDate;
    no changes expected.
"""
...
def _last_approved_inspection_date(d: dict):
    ...
    for insp in d.get("inspections") or []:
        if not isinstance(insp, dict):
            continue
        result = str(insp.get("Result") or "").strip().upper()
        if result not in {"APPROVED", "APPROVED W/ EXCEPTN"}:
            continue
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Pearland permit_info record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pearland permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pearland".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Pearland") & (df["STATE"] == "TX")].copy()

    print(f"Pearland records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_pearland_repaired.parquet")
