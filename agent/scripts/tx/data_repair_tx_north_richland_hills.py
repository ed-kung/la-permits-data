"""Data repair for North Richland Hills (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

North Richland Hills DATA is a CivicPlus / EnerGov-style case payload
with two top-level key sets:
...
Known issues repaired:
  - 23 missing STATUS_NORMALIZED rows whose CaseStatus is
    ``On Hold Awaiting New Documents`` or
    ``Submittal Incomplete; Awaiting Additional Info.``
    → FILLED as In Review.
  - 6 Active rows where CaseStatus is still Issued but
    PermitStatus has advanced to Complete → FIXED to Final;
    FINAL_DATE filled from FinalizeDate / FinalDate when present.
  - Spurious FINAL_DATE on non-Final rows (Active / In Review /
    Inactive Void/Denied/Withdrawn/Expired that still carry
    FinalDate in DATA) → cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already equals ApplyDate (calendar day) on every row.
  - PERMIT_DATE already equals IssueDate when present; 1 Active +
    2 Final rows with Issued=False and null IssueDate stay missing.
  - processing_status is null on every sample row → no inspection
    fallback for FINAL_DATE.
"""
...
_STATUS_MAP = {
    ...
    "Fees Paid": "In Review",
    "On Hold": "In Review",
    "On Hold Awaiting New Documents": "In Review",
    "Resubmitted": "In Review",
    "Submittal Incomplete; Awaiting Additional Info.": "In Review",
    # Inactive
    "Expired": "Inactive",
    "Void": "Inactive",
    "Denied": "Inactive",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Prefer Final, then Active, on either field (details.PermitStatus can
    advance ahead of entity.CaseStatus — observed on 6 Issued/Complete
    rows). Otherwise map entity.CaseStatus, falling back to PermitStatus.
    """
    ent = _entity(d)
    det = _details(d)
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one North Richland Hills entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    North Richland Hills permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "North Richland Hills".
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "North Richland Hills") & (df["STATE"] == "TX")
    ].copy()

    print(f"North Richland Hills records: {len(city):,}\n")
    ...
    if len(flagged):
        ...
        if "STATUS_ORIGINAL" in city.columns:
            ...
            print(
                flagged.groupby(
                    ["BEFORE", "STATUS_NORMALIZED", "STATUS_NORMALIZED_FLAG"]
                )
                .size()
                .to_string()
            )
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_tx_north_richland_hills_repaired.parquet"
        )
