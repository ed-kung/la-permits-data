"""Data repair for Ellis County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Ellis County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - Permit Issued/Complete → Active
    (Ellis-local label for the MGO ``Permit Issued`` state used by
    peer TX cities; upstream incorrectly normalized to Final)
  - Pending (Under Review) → In Review
  - Withdrawn → Inactive

Known issues / sample findings:
  - 790 rows with ``Permit Issued/Complete`` labeled Final → FIXED to
    Active (peer MGO cities map ``Permit Issued`` → Active; Ellis has
    zero Active rows before repair).
  ...
  - DateUpdated and RequestInspections carry no usable timestamps;
    no finaled / completion / CO date exists → FINAL_DATE cannot be
    filled for Final rows.
"""
...
_STATUS_MAP = {
    ...
    "Project Closed/Complete": "Final",
    # Active / issued
    # Ellis uses "Permit Issued/Complete" for the same MGO issued state
    # that peer TX cities label "Permit Issued" → Active.
    "Permit Issued/Complete": "Active",
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    lower = raw.lower()
    # Ellis-specific: "… Issued/Complete" means issuance complete, not
    # project finaled (check before the generic "complete" → Final rule).
    ...
    if "closed" in lower:
        return "Final"
    if "complete" in lower and "issued" not in lower:
        return "Final"
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Ellis County
    # MGO payload (DateUpdated is also the .NET sentinel; RequestInspections
    # is a boolean flag only). Clear only if a non-Final row somehow
    # carries a FINAL_DATE.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Ellis County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Ellis County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Ellis County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Ellis County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_ellis_county_repaired.parquet"
