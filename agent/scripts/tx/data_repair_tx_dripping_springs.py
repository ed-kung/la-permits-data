"""Data repair for Dripping Springs (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Dripping Springs DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  - Closed → Final
  - Permit Issued / Permit Issued (Construction) → Active
  - Pending (Under Review) / Pending (Payment Needed) /
    Pending (Review Complete) → In Review
  - Expired / Withdrawn by Applicant → Inactive
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Dripping
    # Springs MGO payload (DateUpdated is also the .NET sentinel on
    # every sample row). Clear only if a non-Final row somehow carries
    # a FINAL_DATE.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Dripping Springs permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Dripping Springs".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Dripping Springs") & (df["STATE"] == "TX")
    ].copy()

    print(f"Dripping Springs records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_dripping_springs_repaired.parquet"
