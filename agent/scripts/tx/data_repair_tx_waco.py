"""Data repair for Waco (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Waco DATA is a CivicPlus / EnerGov-style case payload. Two top-level
...
Known issues repaired:
  - STATUS_NORMALIZED lagging STATUS_ORIGINAL while CaseStatus has
    already advanced (Issued still "In Review"; Finaled still
    "Active"; Expired still "Active") → FIXED from CaseStatus.
  - Missing PERMIT_DATE on Issued rows whose STATUS_ORIGINAL was
    stale → FILLED from IssueDate.
  - Missing FINAL_DATE on Finaled rows still labeled Active → FILLED
    from FinalDate/FinalizeDate.
  - Spurious FINAL_DATE on non-Final rows (Withdrawn / Expired /
    Canceled / Void / Issued / Review Only often carry FinalDate)
    → cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already equals ApplyDate (calendar day) on every row.
  - Two Finaled Right-of-Way / Gas shells with null IssueDate →
...
_STATUS_MAP = {
    ...
    "In Review": "In Review",
    "On Hold": "In Review",
    "Waiting on Response": "In Review",
    "Paid In Full": "In Review",
    "Review Only": "In Review",
    # Inactive
    "Withdrawn": "Inactive",
    "Expired": "Inactive",
    "Canceled": "Inactive",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Waco entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Waco permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Waco".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Waco") & (df["STATE"] == "TX")].copy()

    print(f"Waco records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_waco_repaired.parquet")
