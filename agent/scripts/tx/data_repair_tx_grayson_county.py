"""Data repair for Grayson County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Grayson County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  ...
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active rows.
  - No Final / closed / complete ProjectStatus appears in the sample;
...
_STATUS_MAP = {
    # Final / completed (not observed in Grayson sample; keep peer MGO labels)
    "Closed": "Final",
    "Completed/Closed": "Final",
    "Closed/Completed": "Final",
    "Project Closed/Complete": "Final",
    # Active / issued — Grayson uses "Permitted" for the issued state
    "Permitted": "Active",
    "Active": "Active",
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Grayson County
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Grayson County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Grayson County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Grayson County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Grayson County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_grayson_county_repaired.parquet"
