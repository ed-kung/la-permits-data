"""Data repair for Burnet (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Burnet DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  - One row has STATUS_ORIGINAL ``permit issued`` / STATUS_NORMALIZED
    Active while ProjectStatus is Closed → FIXED to Final.
  ...
  - ``ProjectStatusIsPermit`` is False for Withdrawn; status text still
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Burnet MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Burnet permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Burnet".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Burnet") & (df["STATE"] == "TX")
    ].copy()

    print(f"Burnet records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_burnet_repaired.parquet"
