"""Data repair for Cedar Park (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Cedar Park DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  - Closed / CO Issued / TCO Issued → Final
  - Issued (Construction) → Active
  - Pending (Under Review) / Approved/Not Issued /
    NOV Sent / 120 Notice Sent → In Review
  - Expired → Inactive

Known issues / sample findings:
  - STATUS_NORMALIZED already matches ProjectStatus 1:1; no fills/fixes
    expected on the current sample.
...
_STATUS_MAP = {
    ...
    "Issued": "Active",
    # In review / pre-issuance / notices
    "Pending (Under Review)": "In Review",
    "Approved/Not Issued": "In Review",
    "NOV Sent": "In Review",
    "120 Notice Sent": "In Review",
    "In Review": "In Review",
    # Inactive
    "Expired": "Inactive",
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    lower = raw.lower()
    # CO / TCO before generic "issued" heuristic.
    if "tco" in lower or "co issued" in lower or "certificate of occup" in lower:
        return "Final"
    if "approved" in lower and "not issued" in lower:
        return "In Review"
    if "nov" in lower or "notice sent" in lower or "120 notice" in lower:
        return "In Review"
    if "closed" in lower or "complete" in lower or "final" in lower:
        return "Final"
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Cedar Park MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Cedar Park permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Cedar Park".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Cedar Park") & (df["STATE"] == "TX")
    ].copy()

    print(f"Cedar Park records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_cedar_park_repaired.parquet"
