"""Data repair for Bulverde (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bulverde DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  - 38 rows with ProjectStatus ``Closed (Completed)`` have missing
    STATUS_NORMALIZED (STATUS_ORIGINAL ``closed (completed)``); these
    should be Final. All other statuses already match 1:1.
  ...
  - DateUpdated and other candidate date fields are null or the .NET
    sentinel; no finaled / completion timestamp exists → FINAL_DATE
    cannot be filled for Final rows.
  - ``ProjectStatusIsPermit`` is False only for Withdrawn; status text
    still drives normalization.
"""
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion timestamp exists in the Bulverde MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bulverde permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bulverde".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Bulverde") & (df["STATE"] == "TX")
    ].copy()

    print(f"Bulverde records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_bulverde_repaired.parquet"
