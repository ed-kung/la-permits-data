"""Data repair for Harker Heights (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Harker Heights DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). One top-level key-set appears in the sample:

  - mgo_ppm:   includes ``PaymentProcessorModule`` (value ``MGO``)
  - mgo_base:  same key set without ``PaymentProcessorModule`` (defensive)
  - missing / unknown
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Harker Heights
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Harker Heights permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Harker Heights".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Harker Heights") & (df["STATE"] == "TX")
    ].copy()

    print(f"Harker Heights records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_harker_heights_repaired.parquet"
