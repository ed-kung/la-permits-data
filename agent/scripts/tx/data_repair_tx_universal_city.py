"""Data repair for Universal City (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Universal City DATA is a SmartGov community portal payload (same family
as TX Portland / La Marque / Bellaire) with top-level keys ``Department``,
...
Canonical mappings:
  - DATA["Build Status"] (Expired* sticky Inactive; Voided sticky
    Inactive; Closed → Final; Issued → Active; Received / In Review /
    Approved Pending Payment / Required Inspections Complete - In Final
    Review → In Review), with Closed-date / Issued-date overrides
                                                         → STATUS_NORMALIZED
  - My Project.Submitted (fallback Created)              → FILE_DATE
  - My Project.Issued (fallback Approved)                → PERMIT_DATE
  - My Project.Closed (fallback latest passed Building
    Final / COO inspection when present)                 → FINAL_DATE

Known issues / sample findings (n=2000):
  - ~887 null STATUS_NORMALIZED: Expired* never mapped (~209), null
    Build Status with Issued/Closed/Submitted dates (~594 fillable),
    Closed / Issued / Approved-Pending never mapped (~84), plus ~15
    empty SmartGov shells that cannot be repaired.
  - Mis-maps: Closed labeled Active (~34 → Final); Expired* labeled
    Active / In Review (~25 → Inactive); Issued labeled In Review
    (~2 → Active); Closed labeled In Review (~1 → Final).
  - FILE_DATE already equals Submitted on every populated row; ~70 of
    81 missing values fillable from My Project (11 empty shells remain).
  - PERMIT_DATE matches Issued wherever both present; ~98 Active/Final
    gaps fillable from Issued/Approved after status repair.
  - FINAL_DATE matches Closed on nearly all Final rows; ~88 Final gaps
    fillable from Closed; 1 Closed-date mismatch (FIX); Permit Inspections
    arrays are empty in this sample so no inspection fallback fires.
"""
...
# Keys are lower-case. Expired values are normalized to "Expired" first.
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    ...
    "cerificate of occupancy": "Final",  # agency typo seen in sibling cities
    "issued": "Active",
    "active": "Active",
    "approved": "Active",
    "renewed": "Active",
    ...
    "approved, pending payment or license information": "In Review",
    "required inspections complete - in final review": "In Review",
}
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive for Expired / Voided / Withdrawn when no Closed date.
    ...
    if bs_low is not None and bs_low in _INACTIVE_BUILD_STATUSES:
        # Voided / withdrawn / cancelled / denied stay Inactive.
        return "Inactive"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Universal City (TX) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Universal City". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Universal City") & (df["STATE"] == "TX")
    ].copy()

    print(f"Universal City records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_tx_universal_city_repaired.parquet"
