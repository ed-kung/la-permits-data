"""Data repair for Bee Cave (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bee Cave DATA is a flat MyGovernmentOnline (MGO) project payload
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Bee Cave MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bee Cave permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bee Cave".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Bee Cave") & (df["STATE"] == "TX")
    ].copy()

    print(f"Bee Cave records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_bee_cave_repaired.parquet"
