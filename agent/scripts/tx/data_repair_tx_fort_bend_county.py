"""Data repair for Fort Bend County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Fort Bend County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  - STATUS_NORMALIZED already matches ProjectStatus 1:1 on all 2,000
  ...
  - DateUpdated and ScheduledDueDate are null or the .NET sentinel; no
    ...
    for Final rows.
  - ``ProjectStatusIsPermit`` is False for Pending (Under Review) and
    Project Withdrawn/Canceled; status text still drives normalization.
"""
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion timestamp exists in the Fort Bend County
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fort Bend County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Fort Bend County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Fort Bend County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Fort Bend County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_fort_bend_county_repaired.parquet"
