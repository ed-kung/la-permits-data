"""Data repair for McKinney (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

McKinney DATA is a CivicPlus / EnerGov-style case payload. Two top-level
...
Canonical mappings:
  ...
  - entity.FinalDate (else details.FinalizeDate,
    else latest Passed / final-Approved inspection)
                                              → FINAL_DATE (Final only)

Known issues repaired:
  - STATUS_NORMALIZED missing for uncommon CaseStatus values
    (Revisions Necessary, Submitted - Internal, and a couple of
    lagged STATUS_ORIGINAL rows whose CaseStatus is To Be Issued / Void)
    → FILLED.
  - STATUS_NORMALIZED stale vs CaseStatus (STATUS_ORIGINAL lagged:
    Complete still ``issued`` / ``expired`` / ``payment pending``,
    Issued still ``expired`` / ``payment pending``, Expired/Void still
    ``issued``) → FIXED.
  - Missing PERMIT_DATE when IssueDate is present → FILLED.
  - Missing FINAL_DATE on Final / newly-Final rows with FinalDate /
    FinalizeDate → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Issued often stamps
    FinalDate with ExpireDate; also Expired / Void / In Review) →
    cleared (FIXED).

Not repairable / left as-is:
  - Active / Final rows with null IssueDate → PERMIT_DATE stays missing.
  - Final rows with null FinalDate, FinalizeDate, and no usable
    inspection date (common on older HTE-Permit Complete rows) →
...
_STATUS_MAP = {
    ...
    "Submitted - Internal": "In Review",
    "To Be Issued": "In Review",
    "Revisions Necessary": "In Review",
    # Inactive
    "Cancelled": "Inactive",
    "Denied": "Inactive",
    "Expired": "Inactive",
...
def _latest_final_inspection_date(d: dict):
    """Latest usable inspection date for finaling fallback.

    McKinney inspections use Approved / Disapproved (not Passed). Prefer
    Passed when present; otherwise Approved* entries whose description
    mentions a final inspection.
    """
    ...
    for item in ps:
        ...
        is_final_approved = (
            "final" in desc
            and status.startswith("approv")
        )
        if not (is_passed or is_final_approved):
            continue
        for key in (
            "completed_date",
            "complete_date",
...
def _final_date_candidate(d: dict):
    """Prefer entity.FinalDate; then details.FinalizeDate; then inspection."""
    ent = _entity(d)
    det = _details(d)
    ...
    if finalize is not pd.NaT and not pd.isna(finalize):
        return finalize
    return _latest_final_inspection_date(d)
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one McKinney entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    McKinney permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "McKinney".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "McKinney") & (df["STATE"] == "TX")].copy()

    print(f"McKinney records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_mckinney_repaired.parquet")
