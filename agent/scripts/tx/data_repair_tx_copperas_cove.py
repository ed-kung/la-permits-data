"""Data repair for Copperas Cove (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Copperas Cove DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - Pending (Under Review) / Pending Payment → In Review
...
_STATUS_MAP = {
    ...
    # In review / pre-issuance / payment
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Copperas Cove
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Copperas Cove permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Copperas Cove".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Copperas Cove") & (df["STATE"] == "TX")
    ].copy()

    print(f"Copperas Cove records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_copperas_cove_repaired.parquet"
