"""Data repair for Laredo (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Laredo DATA is a municipal permit-portal payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED missing on 78 detail_only rows (REVIEWED FOR
    CODE COMP / PR - IN PLAN REVIEW / APPLICATION PRESCREEN /
    CANCELLED ADMINISTRATIVE) → FILLED from Application Status.
  - PERMIT_DATE was taken from "Permit Date". When Issue Date differs
    (498 sample rows), Permit Date is often the application day
    (Active, typically Issue − 1) or a later completion stamp
    (Final, median +84 days) → FIXED to Issue Date.
  - FINAL_DATE on one C.O. ISSUED row matched an approved ML ROUGH-IN
    rather than ML FINAL → FIXED from approved *FINAL* insp.
  - Two Final rows whose only approved insp lacked "FINAL" in the
    name → FIXED to Permit Date completion stamp.

Not repairable / left as-is:
  - FILE_DATE already matches Application Date on every sample row.
  - Application Status disagreements with permit status (e.g.
    CERTIFICATE OF OCCUPANCY / EXPIRED while Status remains
    PERMIT PRINTED) — permit status is treated as authoritative,
    consistent with STATUS_ORIGINAL.
  - In Review rows with blank Issue Date still carry PERMIT_DATE from
    Permit Date (often equal to Application Date); no issuance stamp
    exists to replace or justify clearing under the Issue-first rule.
  - Active PERMIT PRINTED rows that already have an approved trade
...
_APP_STATUS_MAP = {
    "REVIEWED FOR CODE COMP": "In Review",
    "PR - IN PLAN REVIEW": "In Review",
    "APPLICATION PRESCREEN": "In Review",
    "CANCELLED ADMINISTRATIVE": "Inactive",
    "REJECTED": "Inactive",
    "WITHDRAWN": "Inactive",
    "EXPIRED": "Inactive",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Laredo full_portal / detail_only record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Laredo permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Laredo".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Laredo") & (df["STATE"] == "TX")].copy()

    print(f"Laredo records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_laredo_repaired.parquet")
