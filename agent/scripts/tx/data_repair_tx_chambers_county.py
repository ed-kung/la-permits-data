"""Data repair for Chambers County (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Chambers County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - Permit Issued / waiting-for-*Certificate* → Active
  - Pending / Payment / Revisions / Plan Approval /
    inspections & schedules / Customer Request For Power /
    Master Open → In Review
  - Permit Expired / Withdrawn → Inactive

Known issues repaired:
  - STATUS_NORMALIZED null for uncommon waiting/schedule ProjectStatus
    values (fire/septic inspection, drainage approval, septic permit,
    revisions, schedule food/fire-marshal inspection) → FILLED as
    In Review.
  ...
  - DateUpdated and other candidate date fields are empty or the .NET
    sentinel; no finaled / completion timestamp exists → FINAL_DATE
    cannot be filled for Final rows.
"""
...
_STATUS_MAP = {
    ...
    "Closed": "Final",
    "CO Issued": "Final",
    # Active / issued / post-issuance certificate collection
    "Permit Issued": "Active",
    "Issued": "Active",
    "Issued/Open": "Active",
    "Issued (Construction)": "Active",
    "Waiting for Certificate of Compliance": "Active",
    "Waiting for Drainage Certificate": "Active",
    "Waiting for Elevation Certificate": "Active",
    "Waiting for Final Elevation Certificate": "Active",
    # In review / pre-issuance / waiting gates / inspections
    "Pending (Under Review)": "In Review",
    "Waiting for Payment": "In Review",
    "Waiting for Revisions": "In Review",
    "Waiting for Drainage Approval": "In Review",
    "Waiting for Septic Permit": "In Review",
    "Waiting for Fire Inspection": "In Review",
    "Waiting for Septic Inspection": "In Review",
    "Schedule Food Inspection": "In Review",
    "Schedule Fire Marshal Inspection": "In Review",
    "Plan Approval": "In Review",
    "Customer Request For Power": "In Review",
    "Master Open": "In Review",
    "In Review": "In Review",
    "Open": "In Review",
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    lower = raw.lower()
    # Certificate-collection waits before generic "waiting" → In Review.
    if "certificate" in lower and "waiting" in lower:
        return "Active"
    ...
    if (
        ...
        return "Inactive"
    if "coc" in lower or "co issued" in lower:
        return "Final"
    if "closed" in lower or "complete" in lower:
        return "Final"
    if "issued" in lower and "ready" not in lower and "not issued" not in lower:
        return "Active"
    if (
        "pending" in lower
        or "review" in lower
        ...
        or "payment" in lower
        or "fee" in lower
        or "plan approval" in lower
        or "request for power" in lower
        or lower == "open"
        or "master open" in lower
    ):
        return "In Review"
    if "active" in lower:
        return "Active"
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Chambers County
    # MGO payload (DateUpdated is also the .NET sentinel on every sample
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Chambers County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Chambers County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Chambers County") & (df["STATE"] == "TX")
    ].copy()

    print(f"Chambers County records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_chambers_county_repaired.parquet"
