"""Data repair for Lewisville (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lewisville DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  - Closed / Licensed → Final
  - Permit Issued / Approved → Active
  - Pending Payment / Pending Additional Info /
    Under Review / On Hold → In Review
...
Known issues / sample findings:
  ...
  - DateUpdated is also the .NET sentinel; no completion / CO /
    ...
    1,222 Final (Closed / Licensed) rows.
"""
...
_STATUS_MAP = {
    # Final / completed
    "Closed": "Final",
    "Licensed": "Final",  # contractor / registration licenses (trailing space in portal)
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Lewisville
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lewisville permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lewisville".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Lewisville") & (df["STATE"] == "TX")
    ].copy()

    print(f"Lewisville records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_lewisville_repaired.parquet"
