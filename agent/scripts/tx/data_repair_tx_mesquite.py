"""Data repair for Mesquite (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Mesquite DATA is a CivicPlus / EnerGov-style case payload. Three
top-level key-set variants appear in the sample (all share the same
...
Known issues repaired:
  - STATUS_NORMALIZED derived from lagged STATUS_ORIGINAL while
    CaseStatus already advanced (Finaled still Issued/In Review;
    Expired/Void still Active/In Review; Issued still In Review;
    Denied still In Review; CO In Review labeled Final) → FIXED.
  - Missing PERMIT_DATE on Issued/Finaled rows whose STATUS_ORIGINAL
    lagged (awaiting payment / in review / resubmission) → FILLED
    from IssueDate.
  - Missing FINAL_DATE on Finaled rows that carry FinalDate /
    FinalizeDate → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Issued / Void / CO In
    Review / Warranty remapped) → cleared (FIXED).

Not repairable / left as-is:
  - Finaled rows with null IssueDate (7) → PERMIT_DATE stays missing.
  - Finaled rows with null FinalDate/FinalizeDate and empty
    processing_status (7 legacy FDP stubs) → FINAL_DATE stays missing.
...
_STATUS_MAP = {
    # Final
    "Finaled": "Final",
    "Warranty": "Final",  # post-final warranty period; carries FinalDate
    # Active
    "Issued": "Active",
    # In Review
    "Applied Online": "In Review",
    "In Review": "In Review",
    "CO In Review": "In Review",
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Repair one Mesquite entity_* record."""
    ent = _entity(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Mesquite permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Mesquite".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Mesquite") & (df["STATE"] == "TX")].copy()

    print(f"Mesquite records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_tx_mesquite_repaired.parquet")
