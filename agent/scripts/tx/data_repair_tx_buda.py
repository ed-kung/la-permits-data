"""Data repair for Buda (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Buda DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). One top-level key-set variant appears in the sample:
...
Known issues / sample findings:
  ...
  - ``ProjectStatusIsPermit`` is False for CO Issued / Permit Not
    Required / Withdrawn/Cancelled; status text still drives
    normalization (CO Issued → Final).
"""
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Buda MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Buda permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Buda".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Buda") & (df["STATE"] == "TX")
    ].copy()

    print(f"Buda records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_buda_repaired.parquet"
