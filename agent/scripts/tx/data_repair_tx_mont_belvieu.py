"""Data repair for Mont Belvieu (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Mont Belvieu DATA is a flat MyGovernmentOnline (MGO) / MyPermitNow
project payload (``ProjectStatus``, ``DateCreated``, ``DateIssued``,
applicant/site fields, etc.). Every sample row includes
``PaymentProcessorModule`` = ``MGO``:
...
Known issues / sample findings (n=2000):
  - STATUS_NORMALIZED is already 1:1 with ProjectStatus for every row;
    no FILLED/FIXED status changes on the sample.
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Mont Belvieu
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Mont Belvieu permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Mont Belvieu".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Mont Belvieu") & (df["STATE"] == "TX")
    ].copy()

    print(f"Mont Belvieu records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_mont_belvieu_repaired.parquet"
