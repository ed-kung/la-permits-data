"""Data repair for Kerrville (TX) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Kerrville DATA is a flat MyGovernmentOnline (MGO) project payload
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Kerrville MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Kerrville permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Kerrville".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Kerrville") & (df["STATE"] == "TX")
    ].copy()

    print(f"Kerrville records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_tx_kerrville_repaired.parquet"
