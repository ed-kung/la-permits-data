"""Data repair for Manhattan Beach (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Manhattan Beach DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - 145 non-Final rows carry FINAL_DATE copied from entity.FinalDate
    (Void, Withdrawn, Expired, History, Issued, In Review, Partial
    Fees Paid). These are not permit sign-offs under CaseStatus in
    {Finaled, Closed} → cleared.

Not repairable / left as-is:
  - STATUS_NORMALIZED already matches CaseStatus for all sample rows.
  - FILE_DATE already matches entity.ApplyDate for all sample rows;
    none missing.
  - 6 Active/Final rows lack IssueDate (Issued=False) → PERMIT_DATE
    stays missing; FILE_DATE is not used as a proxy.
  - 2 Final rows (Finaled / Closed) lack FinalDate and FinalizeDate →
    FINAL_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - One Issued row has details.PermitStatus=Finaled and FinalizeDate
    but null entity.FinalDate; CaseStatus remains authoritative →
    stays Active.
"""
...
_STATUS_MAP = {
    ...
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Partial Fees Paid": "In Review",
    "Submitted": "In Review",
    "Submitted - Online": "In Review",
    "Incomplete Submittal": "In Review",
    "On Hold": "In Review",
    "Pending Workflow Requirements": "In Review",
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single MB record."""
    ...
    # Prefer entity.FinalDate. details.FinalizeDate is only applied when
    # status is Final; on non-Finaled/Closed rows it often appears with
    # Void/Withdrawn/Expired/In Review and must not populate FINAL_DATE.
    ...
    if effective_status == "Final":
        ...
        # while CaseStatus is Void / Withdrawn / Expired / Issued / etc.).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Manhattan Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Manhattan Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Manhattan Beach") & (df["STATE"] == "CA")].copy()

    print(f"Manhattan Beach records: {len(city):,}\n")
