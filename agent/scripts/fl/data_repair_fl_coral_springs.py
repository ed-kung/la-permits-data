"""Data repair for Coral Springs (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Coral Springs DATA is an Accela-style portal payload with top-level keys
...
and ``search_data``. Content variants (INFERRED_SCHEMA):
  ...
  - accela_finaled:         finaled, no issued (common on legacy APPROVED)
  - accela_approved:        PermitApprovedDate only (no issued/finaled)
  - accela_applied:         PermitAppliedDate only
  - accela_status_only:     PermitStatus / search STATUS, no dates
  - accela_shell:           empty permit_info status + empty dates
  - missing / unknown

Canonical mappings:
  - permit_info.PermitStatus (else search_data.STATUS)
...
Known issues repaired:
  - 7 shell rows with blank permit_info.PermitStatus but search_data
    STATUS in {CLOSED, CERT OF OCCUPANCY, CERT OF COMPLETION}
    → STATUS_NORMALIZED FILLED as Final.
  - 17 STATUS_ORIGINAL=approved rows carry PermitFinaledDate (and often
    final inspections) while STATUS_NORMALIZED=Active → FIXED to Final.
  - PERMIT_DATE missing on Active/Final when Issued is blank but
    PermitApprovedDate exists → FILLED from approved date.
  - FINAL_DATE missing on Final rows with blank PermitFinaledDate but
    approved final / other inspections → FILLED from inspections.
  - After Active→Final upgrades, FINAL_DATE on those rows is retained;
    any remaining non-Final FINAL_DATE would be cleared (FIXED).

Not repairable from DATA:
  - 9 status-only rows (7 conversion shells + 2 cancelled) have blank
    PermitAppliedDate and no alternate application timestamp →
    FILE_DATE stays missing.
  - Many Active/Final rows have neither Issued nor Approved dates →
    PERMIT_DATE stays missing.
  - Many Final rows have empty inspection lists and blank
    PermitFinaledDate → FINAL_DATE stays missing.
"""
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    if finaled is not pd.NaT and not pd.isna(finaled):
        # Agency stamped a finaled date → treat as Final even if PermitStatus
        # is still APPROVED (legacy conversion rows).
        return "Final"
    return expected
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # -- PERMIT_DATE ← Issued (any status) else Approved (Active/Final only) --
    has_issued = issued is not pd.NaT and not pd.isna(issued)
    has_approved = approved is not pd.NaT and not pd.isna(approved)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Coral Springs permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Coral Springs".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Coral Springs"].copy()

    print(f"Coral Springs records: {len(city):,}\n")
