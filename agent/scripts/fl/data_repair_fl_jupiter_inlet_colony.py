"""Data repair for Jupiter Inlet Colony (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Jupiter Inlet Colony DATA is a flat MyGovernmentOnline (MGO) project
payload (``ProjectStatus``, ``DateCreated``, ``DateIssued``,
applicant/site fields, etc.). Two layout variants appear:
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Jupiter Inlet
    # Colony MGO payload (DateUpdated is also the .NET sentinel on every
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Jupiter Inlet Colony permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Jupiter Inlet Colony".
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Jupiter Inlet Colony") & (df["STATE"] == "FL")
    ].copy()

    print(f"Jupiter Inlet Colony records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_jupiter_inlet_colony_repaired.parquet"
