"""Data repair for Madeira Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Madeira Beach DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - Substantial Damage Determination under review → In Review
  - Private Provider Under Review Pending → In Review
  - Stop Work Order - Open → In Review
  - Expired → Inactive

Known issues / sample findings:
  - Upstream left 64 ProjectStatus values unmapped
    (Substantial Damage Determination under review,
    Private Provider Under Review Pending, Stop Work Order - Open)
    → STATUS_NORMALIZED null; repair FILLs them to In Review.
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if (
        ...
        or "substantial damage" in lower
        or lower == "open"
    ):
        return "In Review"
    if "active" in lower:
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Madeira Beach
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Madeira Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Madeira Beach".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Madeira Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Madeira Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_madeira_beach_repaired.parquet"
