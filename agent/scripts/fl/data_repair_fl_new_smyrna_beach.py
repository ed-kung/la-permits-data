"""Data repair for New Smyrna Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

New Smyrna Beach DATA is a Tyler EnerGov payload with top-level keys
contacts, details, entity, fees, processing_status, and optionally
reviews / holds / attachments / more_info (same family as Melbourne /
Palm Beach Gardens).
...
Known issues repaired:
  - Stale STATUS_ORIGINAL ``fees due`` while entity.CaseStatus is
    ``Issued`` → FIXED to Active; PERMIT_DATE filled from IssueDate.
  - CaseStatus=Issued with PermitStatus=Complete and FinalizeDate
    (entity lags details; FinalDate often blank) → FIXED to Final;
    FINAL_DATE filled from FinalizeDate.
  - PERMIT_DATE cleared on In Review when a leftover IssueDate /
    upstream PERMIT_DATE is present (Fees Due, On Hold, etc.).
  - FINAL_DATE cleared on non-Final rows that still carry FinalDate
    (Active Issued shells, Fees Due, Void / Withdrawn).
...
Not repairable from DATA:
  - FILE_DATE already populated for all sample rows; entity ApplyDate
    ...
    from UTC offset).
  - One Issued shell (Issued=false) and one Complete shell lack
    IssueDate → PERMIT_DATE stays missing for those Active / Final rows.
"""
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    final = _final_date(d)
    # Entity lagged as Issued while details show Complete + final date.
    if (
        mapped == "Active"
        and _present(final)
        and permit_raw
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    New Smyrna Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "New Smyrna Beach". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "New Smyrna Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"New Smyrna Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_new_smyrna_beach_repaired.parquet"
        )
