"""Data repair for Green Cove Springs (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Green Cove Springs DATA is a SmartGov community portal payload (same
family as Sunny Isles Beach / Port Orange) with top-level keys
...
Canonical mappings:
  - DATA["Build Status"] (Expired* / Closed / Finaled / CO / Issued / …),
  ...
  - My Project.Closed (fallback latest
    passed Final / CO inspection)           → FINAL_DATE

Known issues repaired:
  - Null STATUS_NORMALIZED (~638): null Build Status shells, Expired*,
    Insufficient Submittal / Ready for Issuance / Council Approval…,
    and a few Closed/Issued rows with blank STATUS_ORIGINAL → FILLED
    from Build Status + My Project dates.
  - Closed / Finaled / CO shells kept as Active / In Review / Inactive
    (STATUS_ORIGINAL lagged as issued / expired / in review) → FIXED
    to Final.
  - Issued shells still labeled In Review → FIXED to Active.
  - Expired* mislabeled Active / In Review / Final → FIXED to Inactive.
  - Missing FILE_DATE / PERMIT_DATE / FINAL_DATE filled from My Project
    (Approved used when Issued is the SmartGov `` - -`` placeholder);
    FINAL_DATE filled from passed Final inspections when Closed blank;
    spurious FINAL_DATE on non-Final → cleared.

Not repairable from DATA:
  - 2 empty-shell records (null Build Status, blank My Project) →
    STATUS_NORMALIZED / FILE_DATE stay missing.
  - Finaled shells with blank Closed and no usable Final inspection →
    FINAL_DATE stays missing.
  - Active/Final with blank Issued and blank Approved → PERMIT_DATE
    stays missing.
"""
...
# Keys are lower-case; Green Cove Springs uses title-case Build Status labels.
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "co": "Final",
    ...
    "canceled": "Inactive",
    "in review": "In Review",
    "insufficient submittal": "In Review",
    "ready for issuance": "In Review",
    "resubmission required": "In Review",
    "submitted": "In Review",
    "council approval with conditions": "In Review",
}
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive for Expired. Explicit Closed / Finaled / CO → Final.
    ...
    if bs is not None and bs.lower() in ("closed", "finaled", "co"):
        return "Final"
...
def _final_inspection_date(d: dict):
    ...
    for insp in inspections:
        ...
        if not (
            ...
            or re.search(r"certificate of occupancy|\bcoo\b|\bco\b", name, re.IGNORECASE)
        ):
            continue
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    # -- FINAL_DATE (completion / Closed, else Final/CO inspection) --
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Green Cove Springs (FL) permit records using information from the raw
    DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Green Cove Springs". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Green Cove Springs") & (df["STATE"] == "FL")
    ].copy()

    print(f"Green Cove Springs records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_green_cove_springs_repaired.parquet"
    ...
    print("\nSTATUS still null (by Build Status):")
    gap = Counter()
    for idx in repaired.index:
