"""Data repair for Groveland (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Groveland DATA is a uniform civic / eTRAKiT-style payload with top-level
keys contacts, fees, inspections, permit_info, search_data, and
site_info. Canonical fields always live under ``permit_info``:
...
Known issues repaired:
  - Stale STATUS_ORIGINAL on FINALED shells still labeled Active
    (issued / approved), Inactive (expired), or In Review (intake)
    → Final; these also carry PermitFinaledDate with a missing
    FINAL_DATE.
  - ISSUED / APPROVED kept as In Review despite current PermitStatus
    (and often PermitIssuedDate / PermitApprovedDate) → Active.
  - EXPIRED kept as Active from stale STATUS_ORIGINAL → Inactive.
  - Missing PERMIT_DATE filled from IssuedDate or ApprovedDate on
    Active / Final / Inactive rows.
  - Missing FINAL_DATE on Final filled from PermitFinaledDate (or a
    passed FINAL / CO / CC inspection when finaled is blank).

Not repairable from DATA:
  - One empty CONVERTED shell (permit #16) lacks all permit_info
    dates → FILE_DATE / PERMIT_DATE stay missing.
  - Legacy CONVERTED Final rows never carry PermitFinaledDate or
    inspections → FINAL_DATE stays missing (~163).
  - A few FINALED / zoning shells lack IssuedDate, ApprovedDate, and
    a usable final inspection → rare PERMIT_DATE / FINAL_DATE gaps.
"""
...
# Full pass only — PARTIAL APPROVED on a Final* inspection is not close-out.
_PASS_RESULTS = {
    "approved",
    "completed",
    "passed",
...
_STATUS_MAP = {
    # Final
    "finaled": "Final",
    "converted": "Final",  # legacy migrated shells
    "closed": "Final",
    "final inspection complete": "Final",
    # Active — post-issuance / approved
    "issued": "Active",
...
def _expected_status(raw_status: Optional[str], issued, finaled) -> Optional[str]:
    ...
    # Explicit finaled date upgrades non-inactive rows (e.g. FINALED
    # shells still carrying a stale Active label from STATUS_ORIGINAL).
    if _present(finaled):
        return "Final"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Groveland permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Groveland".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Groveland"].copy()

    print(f"Groveland records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "groveland_repaired_sample.parquet"
        )
