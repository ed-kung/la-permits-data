"""Data repair for Bay County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bay County DATA is a single civic eTRAKiT-style family
(permit_info / search_data / site_info / inspections / fees / contacts).
Content variants (INFERRED_SCHEMA) further split records by which
...
Canonical mappings:
  - PermitStatus (+ Issued/Finaled for APPROVED) → STATUS_NORMALIZED
...
Known issues repaired:
  - Null STATUS_NORMALIZED on BLDR APP / ADDR APP / C/O → FILLED.
  - Unissued APPROVED labeled Active → FIXED to In Review.
  - Spurious FINAL_DATE on non-Final rows (ISSUED / APPLIED / PAID
    that still carry PermitFinaledDate) → cleared.
  - Missing FINAL_DATE on Final rows filled from approved inspections
    when PermitFinaledDate is blank.

Not repairable from DATA:
  - FILE_DATE stays missing when PermitAppliedDate is blank (9 sample
    rows, mostly APPLIED / ISSUED / ADDR APP).
  - Active ISSUED rows with blank PermitIssuedDate have no PERMIT_DATE.
  - Some FINALED rows lack both FinaledDate and a usable approved
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bay County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bay County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Bay County") & (df["STATE"] == "FL")
    ].copy()

    print(f"Bay County records: {len(city):,}\n")
    ...
    print(ch.groupby(["STATUS_ORIGINAL", "BEFORE", "STATUS_NORMALIZED"]).size().to_string())

    print("\nFINAL_DATE flags:")
    print(repaired["FINAL_DATE_FLAG"].value_counts(dropna=False).to_string())

    if agent_data_path:
        out_path = os.path.join(agent_data_path, "bay_county_repaired_sample.parquet")
