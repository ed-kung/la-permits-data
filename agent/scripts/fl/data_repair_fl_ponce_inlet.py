"""Data repair for Ponce Inlet (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Ponce Inlet DATA is a Tyler EnerGov payload with top-level keys
attachments, contacts, details, entity, fees, holds, more_info,
processing_status, and reviews (reviews always empty in this sample;
processing_status / holds / fees vary).
...
Key-set variants (INFERRED_SCHEMA prefixes):
  - energov_full: reviews/holds/attachments/more_info keys present
...
Known issues repaired:
  - CaseStatus=No Longer Renting left STATUS_NORMALIZED null → FILLED
    as Inactive.
  - CaseStatus=Sold incorrectly mapped to Final → FIXED to Inactive
    (rental-license terminal state, not a finaled building permit).
  - CaseStatus=Issued with PermitStatus=Complete and FinalizeDate
    (entity lags details) → FIXED to Final; FINAL_DATE FILLED.
  - Spurious PERMIT_DATE on In Review → FIXED clear.
  - Spurious FINAL_DATE on Active / Inactive / In Review → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - 13 Complete / administrative shells with Issued=False and blank
    IssueDate stay without PERMIT_DATE.
  - Rental renewals where ApplyDate is after a license-period
    IssueDate (often Oct 1) retain source chronology (FILE > PERMIT).
  - Issued rentals that still carry a prior-cycle FinalDate keep
    Active status; FINAL_DATE is cleared (not upgraded) because
    PermitStatus remains Issued.
"""
...
_STATUS_MAP = {
    ...
    # Inactive (includes rental-license terminal states)
    "Expired": "Inactive",
    "Void": "Inactive",
    "Denied": "Inactive",
    "Withdrawn": "Inactive",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Complete/Final) and a final date is present, upgrade to Final.
    """
    case_raw = None
    ...
    if (
        ...
        and _present(final)
        and permit_raw
        and permit_raw.lower() in {"complete", "completed", "final"}
    ):
        return "Final"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Ponce Inlet permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Ponce Inlet". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Ponce Inlet") & (df["STATE"] == "FL")
    ].copy()

    print(f"Ponce Inlet records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_ponce_inlet_repaired.parquet"
        )
