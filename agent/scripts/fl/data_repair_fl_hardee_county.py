"""Data repair for Hardee County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hardee County DATA is a city portal payload with top-level permit fields
(Status, Permit Date, Issued Date, Date of CO, CC, or Closed Date,
Expiration Date, Permit Type, …) plus nested contractors / fees /
inspections / payments / property_info / reviews. Key-set variants add
plan_reviews and optionally record_type_from_contractor_box.
...
Canonical fields:
  ...
  - Date of CO, CC, or Closed Date, else latest passed final-ish
    inspection completed_date → FINAL_DATE
...
Known issues repaired:
  - Unmapped STATUS_ORIGINAL values (c.o issued, problem file,
    contacted and ready for issuance, waiting on fire department,
    zoning department) left STATUS_NORMALIZED null → FILLED from
    DATA Status.
  - Spurious PERMIT_DATE with blank / sentinel Issued Date
    → FIXED clear.
  - Sentinel 01/01/1900 PERMIT_DATE / FINAL_DATE → FIXED.
  - Missing FINAL_DATE on Final shells → FILLED from Closed / CO
    date or passed final inspections.
  - FINAL_DATE on non-Final shells, or with no supporting DATA date
    → FIXED clear.

Not repairable from DATA:
  - Blank Status shells (mostly MILTON Damage Assessment) with no
    Issued / Closed dates → STATUS_NORMALIZED stays missing.
  - One empty-shell row with blank Permit Date → FILE_DATE stays
    missing.
  - Active / Final shells with blank Issued Date → PERMIT_DATE
    remains missing.
  - Final shells with blank Closed / CO date and no passed final
    inspection → FINAL_DATE remains missing.
"""
...
def _final_date(d: dict):
    dt = _safe_to_datetime(d.get("Date of CO, CC, or Closed Date"))
    if _present(dt):
        return dt
...
_STATUS_MAP = {
    ...
    "open": "In Review",
    "contacted and ready for issuance": "In Review",
    "waiting on fire department": "In Review",
    "zoning department": "In Review",
    "problem file": "Inactive",
    "voided": "Inactive",
    "void": "Inactive",
    "expired": "Inactive",
}
...
def _expected_status(row, d: dict) -> Optional[str]:
    """Infer STATUS_NORMALIZED from DATA Status / STATUS_ORIGINAL."""
    status = _norm_text(d.get("Status")).lower()
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # -- PERMIT_DATE --
    # Canonical source is Issued Date only. Unsupported or sentinel
    ...
    if effective_status == "Final":
        if _has_usable_date(row["FINAL_DATE"]):
            if _present(final_dt):
                closed = _safe_to_datetime(d.get("Date of CO, CC, or Closed Date"))
                if _present(closed) and _dates_equal(row["FINAL_DATE"], closed):
                    pass
                elif not _dates_equal(row["FINAL_DATE"], final_dt):
                    supporting = [
                        _safe_to_datetime(d.get("Date of CO, CC, or Closed Date")),
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hardee County permit records using information from the raw DATA JSON
    column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hardee County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Hardee County"].copy()

    print(f"Hardee County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "hardee_county_repaired_sample.parquet"
        )
