"""Data repair for Naples (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Naples DATA is an EnerGov / Civic community-development payload with
top-level keys ``Summary``, ``Locations``, ``Fees``, plus either
``Permits`` (list) or ``Permit Info`` (dict), and optionally
``project_id``. Variants (INFERRED_SCHEMA):
...
Known issues repaired:
  - Returned for Correction / Submittals Incomplete /
    Revision – Upload Documents left STATUS_NORMALIZED
    missing → FILLED as In Review.
  - Rows whose STATUS_ORIGINAL stayed at "permit(s) issued"
    / "issued" while Application Status advanced to
    Finaled / Closed → FIXED Active → Final; FINAL_DATE
    filled or corrected from Date Finaled.
  - In Plan Check / Pending / Ready for Issuance / On Hold /
    Historical rows that already carry Issued Date → FIXED
    to Active and PERMIT_DATE filled.
  - Spurious PERMIT_DATE on Inactive (Expired / Canceled)
    cleared.
  - Spurious FINAL_DATE on non-Final statuses cleared;
    mismatched FINAL_DATE overwritten from Date Finaled.

Not repairable from DATA:
  - Closed Final rows with blank Summary["Date Finaled"]
    → FINAL_DATE stays missing.
  - Closed Final rows with blank Summary["Issued Date"]
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    if effective_status == "Final":
        if _present(finaled):
            if pd.isna(current_final) or not _present(
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Naples (FL) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Naples". Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Naples") & (df["STATE"] == "FL")].copy()

    print(f"Naples records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_naples_repaired.parquet"
