"""Data repair for Miami Lakes (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Miami Lakes DATA is a uniform civic / eTRAKiT-style payload with
top-level keys contacts, fees, inspections, permit_info, search_data,
and site_info. ``search_data`` has two key-set variants (with / without
FOLIO), but canonical fields always live under ``permit_info``:
...
Known issues repaired:
  - Null STATUS_NORMALIZED for NULL/VOID / REQUIRED / blank / ISSUED
    shells → FILLED from PermitStatus (or issued-date signal).
  - Mislabels vs PermitStatus: FINALED kept as Active/In Review/
    Inactive; ISSUED kept as In Review/Inactive; DENIED as In Review;
    EXPIRED as Active; READY as Inactive; NULL/VOID as In Review
    → FIXED.
  - ISSUED / RENEWED / READY rows carrying PermitFinaledDate upgraded
    to Final.
  - READY / ON REVIEW / PENDING with a real issue date upgraded to
    Active.
  - Missing FILE_DATE on Final shells filled from IssuedDate when
    AppliedDate is blank.
  - Missing / stale PERMIT_DATE filled or corrected from IssuedDate
    (else ApprovedDate).
  - Missing FINAL_DATE on Final filled from PermitFinaledDate or a
    passed FINAL / CO / CC inspection.
  - Spurious FINAL_DATE on non-Final (Active / Inactive) cleared.
  - Spurious PERMIT_DATE on In Review cleared.

Not repairable from DATA:
  - Many FINALED / CLOSED shells lack both IssuedDate and ApprovedDate
    → PERMIT_DATE stays missing on some Active/Final rows.
  - Many Final shells lack PermitFinaledDate and a usable passed final
    inspection → FINAL_DATE stays missing.
  - Empty PermitStatus shell with only an applied date stays
...
# Full pass only — PARTIAL APPROVAL on a Final* inspection is not close-out.
_PASS_RESULTS = {
    "approved",
    "completed",
    "passed",
...
_STATUS_MAP = {
    ...
    "closed": "Final",
    "co issued": "Final",
    # Active — post-issuance / renewed / temporary CO
    "issued": "Active",
    "renewed": "Active",
    "tcc issued": "Active",
...
def _expected_status(raw_status: Optional[str], issued, finaled) -> Optional[str]:
    ...
    # Explicit finaled date upgrades non-inactive rows (ISSUED/RENEWED/
    # READY shells that still carry PermitFinaledDate).
    if _present(finaled):
        return "Final"
    ...
    # (e.g. READY / ON REVIEW after PermitIssuedDate).
    ...
    if effective_status in ("Active", "Final", "Inactive"):
        if _present(permit_src):
            if pd.isna(current_permit):
                ...
                not _present(issued)
                and _present(approved)
                and not _dates_equal(current_permit, approved)
            ):
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Miami Lakes permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Miami Lakes".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Miami Lakes"].copy()

    print(f"Miami Lakes records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "miami_lakes_repaired_sample.parquet"
        )
