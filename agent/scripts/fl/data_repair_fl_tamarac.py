"""Data repair for Tamarac (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Tamarac DATA is the same city-portal family as Ormond Beach /
St. Petersburg / Punta Gorda / Pompano Beach / Margate / North Port /
Lake Mary. This sample has three sub-schemas:
...
Canonical mappings:
  - Status for Permit Number (permit_status), overridden to
    Inactive when Application Status is CANCELLED / VOID /
    NULL AND VOID / etc.
                                                     → STATUS_NORMALIZED
...
Known issues repaired:
  - ~399 fees_detail / mini_set rows with null STATUS_NORMALIZED
    filled from Application Status (CLOSED, IN PLAN CHECK, VOID,
    CANCELLED, DUPLICATE, etc.).
  - CLOSED / PERMIT PRINTED / PLAN CHECK rows whose Application
    Status is VOID / CANCELLED incorrectly labeled Final / Active /
...
Not repairable from DATA:
  - mini_set rows (157) have no Application Date → FILE_DATE stays
    missing.
...
_APP_MAP = {
    ...
    "C/O ISSUED": "Final",
    "C.O. ISSUED": "Final",
    # APPROVED / WEB APPROVED alone (no Status for Permit Number) is
    # pre-issuance plan approval, not an issued permit.
    "APPROVED": "In Review",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Tamarac permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Tamarac". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Tamarac") & (df["STATE"] == "FL")
    ].copy()

    print(f"Tamarac records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_tamarac_repaired.parquet"
