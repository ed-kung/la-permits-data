"""Data repair for Palm Springs (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Palm Springs DATA is a CityView / CentralSquare community portal payload
with top-level keys ``id``, ``type``, ``number``, ``status``, ``details``,
``timeline``, ``customFields``, ``contacts``, and optional
``lastUpdDate`` / ``entryForms`` / ``canMakeOperations``. Variants
(INFERRED_SCHEMA):
...
Known issues repaired:
  - Open rows with real Closed stamps (mostly Historical Permits)
    mislabeled In Review → FIXED to Final.
  - Open rows with Issued but no real Closed → FIXED to Active.
  - Open rows with Expired / Cancelled detail status → FIXED to
    Inactive.
  - One Issued row with a real Closed stamp labeled Active → Final.
  - 136 FINAL_DATE values of 1899-11-30 (SQL/empty-date sentinel)
    cleared; spurious FINAL_DATE on non-Final statuses cleared.
  - Spurious PERMIT_DATE on In Review / Inactive cleared after
    status overrides.

Not repairable from DATA:
  - 37 Munis shells with blank details.created → FILE_DATE stays
    missing (issued is present but is not a file/application date).
  - ~19 Issued Active rows with blank details.issued (timeline has
    no timestamps) → PERMIT_DATE stays missing.
  - ~346 Final rows with blank details.issued → PERMIT_DATE stays
    missing.
  - 2 Closed Finals with blank details.closed → FINAL_DATE stays
    missing.
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Palm Springs (FL) permit records using information from the raw DATA JSON.
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Palm Springs") & (df["STATE"] == "FL")].copy()
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_palm_springs_repaired.parquet"
