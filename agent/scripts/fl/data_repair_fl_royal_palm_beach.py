"""Data repair for Royal Palm Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Royal Palm Beach DATA is the same city-portal family as Fort Pierce /
Tamarac / Ormond Beach / St. Petersburg / Punta Gorda / Pompano Beach /
Margate / North Port / Lake Mary. This sample has two sub-schemas:
...
Known issues repaired:
  - 16 fees_detail rows with null STATUS_NORMALIZED filled from
    Application Status (CLOSED, IN PLAN CHECK, APPROVED, VOID) when
    mappable; blank Application Status left null.
  - Stale STATUS_ORIGINAL (plan check / permit printed / permit
    revoked) that disagrees with Status for Permit Number
    (FINAL INSPECTION COMPLETE → Final, PERMIT PRINTED → Active).
  - VOID Application Status on otherwise Active / Final / In Review
    permit shells → FIXED to Inactive.
...
Not repairable from DATA:
  ...
  - fees_detail with blank Application Status → STATUS_NORMALIZED
    stays missing.
"""
...
_APP_MAP = {
    ...
    "PERMIT ISSUED": "Active",
    "ISSUED": "Active",
    # Issued but awaiting Notice of Commencement filing.
    "NOC NEEDED": "Active",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Royal Palm Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Royal Palm Beach". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Royal Palm Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Royal Palm Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_royal_palm_beach_repaired.parquet"
