"""Data repair for Palm Beach Gardens (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Palm Beach Gardens DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - Unmapped CaseStatus values left STATUS_NORMALIZED null → FILLED:
      Void -* variants → Inactive; Verify Application (+ On Hold),
      Pending Issuance - On Hold → In Review; Pending Closure → Active;
      Pending Closure - On Hold → Active when IssueDate present else
      In Review.
  - Spurious FINAL_DATE on non-Final shells (Void closures carrying
      FinalDate/FinalizeDate, one In Review with leftover FinalDate)
      → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - 36 Complete Final shells (mostly Revisions) have Issued=False and
    blank IssueDate → PERMIT_DATE remains missing.
  - processing_status is empty on all sample rows, so no inspection
...
# Exact CaseStatus / PermitStatus → STATUS_NORMALIZED (strip before lookup).
_STATUS_MAP = {
    # Final
    "Complete": "Final",
    "Completed": "Final",
    ...
    # Active (issued / in inspection / pending closure)
    "Issued": "Active",
    "Permit Issued/Printed": "Active",
    "Inspections": "Active",
    "Approved": "Active",
    "Pending Closure": "Active",
    # In Review
    "In Review": "In Review",
    "In Review - Requires Resubmittal": "In Review",
    "Submitted - Online": "In Review",
    "Submitted": "In Review",
    "Verify Application": "In Review",
    "Verify Application - On Hold": "In Review",
    "Pending Issuance - On Hold": "In Review",
    "Pending Issuance": "In Review",
    "On Hold": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Sufficiency Check": "In Review",
...
def _map_status(raw: Optional[str]) -> Optional[str]:
    ...
    for key, val in _STATUS_MAP.items():
        if key.lower() == raw.lower():
            return val
    # Palm Beach Gardens uses many Void - <reason> CaseStatus values.
    if raw.lower().startswith("void"):
        return "Inactive"
    return None
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    case_raw = _case_status(d)
    if case_raw:
        # Pending Closure - On Hold: Active only when already issued.
        if case_raw.lower() == "pending closure - on hold":
            issue = _entity_date(d, "IssueDate", "IssueDate")
            return "Active" if _present(issue) else "In Review"
        mapped = _map_status(case_raw)
        if mapped is not None:
            return mapped
    return _map_status(_permit_status(d))
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Palm Beach Gardens permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Palm Beach Gardens". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Palm Beach Gardens") & (df["STATE"] == "FL")
    ].copy()

    print(f"Palm Beach Gardens records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_palm_beach_gardens_repaired.parquet"
        )
