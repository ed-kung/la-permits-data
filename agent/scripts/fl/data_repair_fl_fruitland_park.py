"""Data repair for Fruitland Park (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Fruitland Park DATA is a uniform civic / eTRAKiT-style payload with
top-level keys contacts, fees, inspections, permit_info, search_data,
and site_info. Canonical fields always live under ``permit_info``:
...
Known issues repaired:
  - Stale STATUS_ORIGINAL on FINALED shells still labeled Active
    (issued / approved) or In Review (paid online) → Final; these
    also carry PermitFinaledDate with a missing FINAL_DATE.
  - ISSUED / APPROVED kept as In Review despite current PermitStatus
    (and often PermitIssuedDate / PermitApprovedDate) → Active.
  - EXPIRED / VOID kept as Active or In Review from stale
...
Not repairable from DATA:
  - One VOID shell lacks all permit_info dates → FILE_DATE stays
    missing.
  - A few FINALED shells lack PermitFinaledDate and a usable full-pass
    final inspection (PARTIAL APPROVED only) → FINAL_DATE stays
    missing.
  - Some Active APPROVED / ISSUED shells lack both IssuedDate and a
    usable ApprovedDate after other gaps → rare PERMIT_DATE gaps.
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fruitland Park permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Fruitland Park".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Fruitland Park"].copy()

    print(f"Fruitland Park records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "fruitland_park_repaired_sample.parquet"
        )
