"""Data repair for Sanford (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Sanford DATA is a municipal portal payload (CitizenServe-style form
...
submittal dates come from Review workflow starts (especially
``Application Intake``). Final / CO dates come from passed Final* / CO
inspections, with Certificate Review Completion and top-level
``Completion Date`` as fallbacks.
...
Canonical fields:
  ...
  - DATA["Status:"] (blank / unmapped inferred from Issue Date /
    passed Final* inspections)
      → STATUS_NORMALIZED
  - Application Intake Start else earliest non-post-issuance Review
    Start/Completion (on/before Issue) else ROW Start Date
      → FILE_DATE
  - Permit Details["Issue Date:"]         → PERMIT_DATE
  - Latest passed Final*/CO inspection
    (else Certificate Review Completion / Completion Date)
      → FINAL_DATE

Key-set variants (INFERRED_SCHEMA prefixes):
  - portal_core:        core permit keys only (minimal form)
  - portal_row:         right-of-way extras (Start Date / ROW Name)
  - portal_commercial:  commercial Type of work extras
  - portal_form:        residential Type of Work / owner-builder extras
...
Known issues repaired:
  - Null STATUS_NORMALIZED for Finaled - CO / Finaled - CC /
    Information Required / Corrections Requested / blank Status:
    → FILLED from Status map or Issue / Final* inference.
  - FILE_DATE often copied from latest Review Completion (plan-review
    finish) rather than Application Intake Start → FIXED when an
    intake / early-review source exists; post-issue FILE values cleared
    when no application source exists.
  - Missing PERMIT_DATE on issued / finaled shells with a usable
    Issue Date → FILLED; OOR sentinel Issue years (195x / 203x–204x)
    cleared / ignored; spurious PERMIT_DATE on In Review cleared.
  - FINAL_DATE missing on every sample row → FILLED for Closed /
    Finaled - CO / Finaled - CC rows with a final/CO stamp.
  - Spurious FINAL_DATE on non-Final cleared (none expected pre-repair).

Not repairable from DATA:
  - ~600+ rows lack Application Intake / on-or-before-Issue Review
    dates → FILE_DATE stays missing (mostly older shells with empty
    Reviews).
  - Active/Final rows with blank Issue Date, or Issue Date years in the
    2030–2042 corruption band / other OOR sentinels → PERMIT_DATE stays
    missing.
  - Closed / Finaled shells without a usable Final*/CO inspection
    (excluding 2000-01-01 / 2018-01-01 migration placeholders),
    Certificate Review Completion, or Completion Date → FINAL_DATE
    stays missing.
  - Empty Status: shells with no Reviews / Inspections remain null.
"""
...
_MIN_YEAR = 1980
# Sanford Issue Date / inspection stamps include bogus 2030–2042 years;
# keep the ceiling at the current data year (+1 buffer).
_MAX_YEAR = 2026

# Migration placeholders that appear as "Approved" Final* inspection dates.
_SENTINEL_DATES = {
    (2000, 1, 1),
    (2018, 1, 1),
}
...
_INTAKE_TASK_RE = re.compile(
    r"application intake|permit application review",
    re.IGNORECASE,
)
...
def _cert_review_completion(d: dict):
    ...
    for r in reviews:
        ...
        if not re.search(r"certificate review", task, re.IGNORECASE):
            continue
        status = _insp_status_token(r.get("Status"))
        if status and not (
            status.startswith("approved")
            or status.startswith("complete")
            or status.startswith("review complete")
        ):
            continue
...
def _final_date(d: dict):
    ...
    if _present(final):
        return final
    cert = _cert_review_completion(d)
    if _present(cert):
        return cert
    return _safe_to_datetime(d.get("Completion Date"))
...
def _file_date(d: dict):
    ...
    Priority: Application Intake Start → earliest non-certificate Review
    Start (≤ Issue) → Intake Completion (≤ Issue) → earliest
    non-certificate Review Completion (≤ Issue) → ROW Start Date
    (≤ Issue).
    ...
    intake_starts = [dt for dt in intake_starts if _on_or_before(dt, issue)]
    if intake_starts:
    ...
    start = _safe_to_datetime(d.get("Start Date"))
    if _on_or_before(start, issue):
        return start
...
def _schema_base(data_dict: dict) -> str:
    ...
    if "ROW Name" in keys or "Start Date" in keys or "Completion Date" in keys:
        return "portal_row"
    if "Type of work - commercial" in keys:
        return "portal_commercial"
    if "Type of Work" in keys or "Is this an owner/builder project?" in keys:
        return "portal_form"
    nkeys = len(keys)
    extras = keys - _CORE_KEYS - {""}
    if nkeys <= 16 and not extras:
        return "portal_core"
    if not extras or extras <= {"Contractor", "Architect/Engineer", "Property Owner",
                                 "Construction Value", "Plan Review Contact Person",
                                 "Primary Permit Number",
                                 "Is this a sub-permit that needs to be attached to a primary Building Permit?"}:
        return "portal_core"
    return "portal_form"
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    if raw is None:
        ...
        if has_issue:
            return "Active"
        # Completely empty historic shells stay null rather than fake In Review.
        reviews = d.get("Reviews")
        insp = d.get("Inspections")
        has_workflow = (
...
def _apply_date(repairs: dict, row, field: str, candidate) -> None:
    ...
    if pd.isna(current) or not _has_usable_date(current):
        if pd.isna(current):
            ...
            # Sentinel / OOR current value replaced by a usable candidate.
            repairs[field] = cand
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # (upstream often copied latest Plan Review Completion).
    if _present(file_dt):
    ...
    # FINAL_DATE ← Final*/CO inspection / certificate completion for Final only.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sanford permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Sanford". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Sanford") & (df["STATE"] == "FL")
    ].copy()

    print(f"Sanford records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_sanford_repaired.parquet"
        )
