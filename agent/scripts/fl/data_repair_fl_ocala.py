"""Data repair for Ocala (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Ocala DATA is a uniform civic / eTRAKiT-style payload with top-level keys
contacts, fees, inspections, permit_info, search_data, and site_info.
Canonical fields:

  - permit_info.PermitStatus (+ issuance gating for APPROVED;
    empty status with an issued date → Active)
      → STATUS_NORMALIZED
...
Known issues repaired:
  - 76 unmapped portal statuses left STATUS_NORMALIZED null
    (ON APPLICATION, IN REVIEW - EPLANS / INSPECT, E-APPLICATION,
    AWAITING FINAL PAY, empty legacy shells, etc.) → FILLED.
  - SUSPENDED labeled In Review → FIXED to Inactive.
  - CLOSED-NO FINAL INSP labeled Final → FIXED to Inactive
    (administratively closed without a final inspection / finaled date).
  ...
  - Empty PermitStatus shells with PermitIssuedDate → FILLED Active.
  - Missing PERMIT_DATE on Active/Final filled from IssuedDate, else
    ApprovedDate (approval counts as permit date when issuance blank).
  - Missing FINAL_DATE on Final rows filled from PermitFinaledDate,
    search_data FINALED / CO ISSUED, or Approved final inspections.
  - Spurious FINAL_DATE on non-Final rows cleared.

Not repairable from DATA:
  - 568 FILE_DATE gaps: PermitAppliedDate blank on legacy rows
    (mostly pre-2004 APPROVED / empty-status shells). No substitute
    application date exists in DATA.
  - A few FINALED rows still lack FINAL_DATE after inspection / CO
    fallbacks.
  - Empty-status shells with neither status nor issued date stay
...
_STATUS_MAP = {
    ...
    # Active
    "issued": "Active",
    "issued pending noc": "Active",
    # In Review
    "paid": "In Review",
    "in review": "In Review",
    "in review - eplans": "In Review",
    "in review inspect": "In Review",
    "hold": "In Review",
    "on application": "In Review",
    "e-application": "In Review",
    "e-fees paid": "In Review",
    "awaiting final pay": "In Review",
    "approved to issue": "In Review",
    "plan corrections needed": "In Review",
    "incomplete application": "In Review",
    "pending site": "In Review",
    "fees due": "In Review",
    "conditional app": "In Review",
    # Inactive
    "expired": "Inactive",
    "expired - re-issued": "Inactive",
    "expired reissued": "Inactive",
    "void": "Inactive",
    "withdrawn": "Inactive",
    "revoked": "Inactive",
    "denied": "Inactive",
    "suspended": "Inactive",
    # Administratively closed without a final inspection / finaled stamp.
    "closed-no final insp": "Inactive",
}
    ...
    if permit_src is not pd.NaT:
        if pd.isna(row["PERMIT_DATE"]):
            if effective_status in ("Active", "Final", "Inactive"):
                # Inactive (expired/void/revoked) often still carries issuance.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Ocala permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Ocala".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Ocala"].copy()

    print(f"Ocala records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "ocala_repaired_sample.parquet")
