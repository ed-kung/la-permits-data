"""Data repair for Atlantic Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Atlantic Beach DATA is an Accela-style portal payload with top-level keys
...
Canonical mappings:
  ...
  - PermitIssuedDate else PermitApprovedDate
    (Approved fallback: Active/Final only) → PERMIT_DATE
  - PermitFinaledDate else latest approved
    final-ish inspection else latest
    approved inspection (Final only)       → FINAL_DATE

Known issues repaired:
  - 8 rows with null STATUS_NORMALIZED
    (awaiting corrections, in-active, pre issuance)
    → FILLED from PermitStatus.
  ...
  - 1 Inactive (withdrawn) row incorrectly carried FINAL_DATE
    → cleared (FIXED).

Not repairable from DATA:
  - 1,111 CONV-legacy rows have blank PermitAppliedDate and no
    alternate application timestamp → FILE_DATE stays missing.
  - Active/Final rows with neither Issued nor Approved dates →
    PERMIT_DATE stays missing.
  - Final rows with empty / non-approved inspections and blank
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Atlantic Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Atlantic Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Atlantic Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Atlantic Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(agent_data_path, "atlantic_beach_repaired_sample.parquet")
