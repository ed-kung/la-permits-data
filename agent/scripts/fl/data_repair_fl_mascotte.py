"""Data repair for Mascotte (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Mascotte DATA is a city-portal payload with top-level keys Status,
Permit Date, Permit Number, inspections, fees, payments, property_info,
plus optional Description / Final Inspection Date / Project Type /
Square Footage*, reviews, and/or plan_reviews /
record_type_from_contractor_box.

Canonical mappings:
  - Status (plus Final Inspection Date / passed FINAL-ish
    inspection overrides; blank Status inferred Final only
    from a dated passed FINAL-ish inspection) → STATUS_NORMALIZED
  - Permit Date                               → FILE_DATE
  - Latest approved review/plan_review
    completed_date (Building* preferred;
    fallback review_date / date)              → PERMIT_DATE
  - Final Inspection Date, else latest passed
    FINAL-ish inspection completed_date       → FINAL_DATE
...
Known issues repaired:
  - Unmapped Status values (Waiting Payment, Needs Additional Info,
    Issued / Work started, Final / Completed, Intake / Applied for,
    PRIVATE PROVIDER, APPROVED WITH CONDITIONS) left STATUS_NORMALIZED
    null or wrong → FILLED / FIXED.
  - Issued shells that already carry Final Inspection Date → FIXED
    to Final; FINAL_DATE filled.
  - Blank-Status shells with a dated passed FINAL-ish inspection
    → FILLED as Final; FINAL_DATE filled from that stamp.
  - Missing PERMIT_DATE on Active/Final filled from approved review
    completion stamps; spurious PERMIT_DATE on In Review / Inactive /
    null status cleared.
  - Missing FINAL_DATE on Final filled from Final Inspection Date or
    passed FINAL-ish inspections; spurious FINAL_DATE on non-Final
    cleared.

Not repairable from DATA:
  - ~1,000 blank-Status shells without a dated passed FINAL-ish
    inspection → STATUS_NORMALIZED stays missing.
  - FILE_DATE missing exactly when Permit Date is blank (empty shells);
    inspection dates are not treated as application dates.
  - Many Active/Final rows lack review completion stamps (and payment
    dates are a poor proxy) → PERMIT_DATE stays missing.
  - Many Final / Complete rows lack Final Inspection Date and a dated
    passed FINAL-ish inspection → FINAL_DATE stays missing.
"""
...
def _permit_date(d: dict):
    ...
    Prefer Building* approvals when present (plan_reviews often mix
    Intake / Payment / Zoning stamps). Else use the latest approved
    non-payment completed_date, then any approved completed_date,
    then approved review_date / date when completed is blank. Never
    use Permit Date (application stamp).
    """
...
_STATUS_MAP = {
    ...
    "needs additional info": "In Review",
    "intake": "In Review",
    "intake / applied for": "In Review",
    "void": "Inactive",
    "expired": "Inactive",
    "denied": "Inactive",
}
...
def _expected_status(row, d: dict) -> Optional[str]:
    """Infer STATUS_NORMALIZED from DATA Status + date/inspection overrides."""
    ...
    # Blank Status shells: only infer Final from a dated passed FINAL-ish
    # inspection (undated Pass rows are too weak to normalize alone).
    if not raw:
        if _has_passed_final_inspection(d, require_date=True):
            return "Final"
        return None

    # Strong Final signals override Issued/Active labels.
    if mapped != "Inactive":
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Mascotte permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Mascotte".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Mascotte"].copy()

    print(f"Mascotte records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "mascotte_repaired_sample.parquet"
        )
