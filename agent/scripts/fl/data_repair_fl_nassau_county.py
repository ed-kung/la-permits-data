"""Data repair for Nassau County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Nassau County DATA is a uniform city-portal payload with top-level keys
app, fees, permit, init_info, permit_list, and inspection_list. Content
variants (INFERRED_SCHEMA) split by whether ``permit`` carries a parseable
...
Known issues repaired:
  - Null STATUS_NORMALIZED for ENTERED IN ERROR / DENIED / WITHDRAWN /
    EXPIRED / ACTIVE PENDING|READY TO ISSUE|COMPLETE / HOLD shells
    → FILLED from app.Status + Permit Status.
  - COMPLETE / COMPLETE or Permit Status COMPLETED labeled Active
    → FIXED to Final.
  - Unissued ACTIVE / ACTIVE|PENDING with Permit Status REVIEWING
    labeled Active → FIXED to In Review.
  - ENTERED IN ERROR / VOID labeled Active → FIXED to Inactive.
  - Missing PERMIT_DATE on Active / Final / Inactive filled from
    Issued Date when present; spurious In Review PERMIT_DATE cleared.
  - FINAL_DATE never ingested → FILLED from latest PASSED inspection
    for Final rows; spurious non-Final FINAL_DATE cleared.

Not repairable from DATA:
  - FILE_DATE already matches Application Received Date for every row.
  - Final rows with no dated PASSED inspections → FINAL_DATE stays
    missing (~193 of originally-Final COMPLETE shells; more after
    status fills that also lack inspection dates).
  - Active / Final shells with blank Issued Date (REVIEWING / empty
    permit) → PERMIT_DATE stays missing after status reclassification
    to In Review where appropriate.
"""
...
def _expected_status(
    ...
    # ACTIVE / COMPLETE with ISSUED (not COMPLETED) is still open.
    if app_key == "ACTIVE / COMPLETE" and ps == "ISSUED":
        return "Active"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Nassau County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Nassau County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Nassau County") & (df["STATE"] == "FL")
    ].copy()

    print(f"Nassau County records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(
            agent_data_path, "nassau_county_repaired_sample.parquet"
        )
