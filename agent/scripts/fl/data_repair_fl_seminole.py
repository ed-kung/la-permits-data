"""Data repair for Seminole (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Seminole DATA is a Tyler EnerGov payload with a single key-set in this
sample: contacts, details, entity, fees, processing_status.
...
Key-set variants (INFERRED_SCHEMA prefixes):
  - energov: fees present (only key-set observed)
...
Known issues repaired:
  ...
  - Spurious PERMIT_DATE on In Review rows that still carry IssueDate
    (mostly On Hold with Issued=True) → FIXED clear.
...
Not repairable from DATA:
  ...
  - Thirteen Final shells (mostly Variance / Zoning Clearance) have
    Issued=False and blank IssueDate → PERMIT_DATE stays missing.
  - Inactive Void shells missing PERMIT_DATE also lack IssueDate.
  - All Final shells already have FINAL_DATE; no fill needed.
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Seminole permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Seminole". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Seminole") & (df["STATE"] == "FL")
    ].copy()

    print(f"Seminole records: {len(city):,}\n")
    ...
    if len(af_miss):
        ...
        for idx in af_miss.index:
            ...
            ps_counts[raw] += 1
            ctype = (_entity(d).get("CaseType") or "").strip() or "__EMPTY__"
            type_counts[ctype] += 1
        print("  by CaseStatus:", dict(ps_counts))
        print("  by CaseType:", dict(type_counts))

    if AGENT_DATA_PATH:
        out_dir = os.path.join(AGENT_DATA_PATH, "repaired")
        os.makedirs(out_dir, exist_ok=True)
        out_path = os.path.join(
            out_dir, "permits_fl_seminole_repaired.parquet"
        )
