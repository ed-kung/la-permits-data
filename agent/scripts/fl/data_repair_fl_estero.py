"""Data repair for Estero (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Estero DATA is a city-portal payload with top-level Applications,
Fees and Payments, Permit Information, Inspections History,
Permit Requirements, and Plan Review History. Two key-casing variants
appear in the sample:

  - city_portal_pascal: StatusDesc / AppDate / ApprovedByDate
                        (Permit Information usually a one-element list)
  - city_portal_lower:  statusdesc / appdate / approvedbydate
                        (Permit Information often a bare dict)

Canonical mappings:
  - Permit Information StatusDesc/statusdesc      → STATUS_NORMALIZED
  - earliest Applications AppDate/appdate         → FILE_DATE
  - earliest Applications ApprovedByDate/
    approvedbydate                                → PERMIT_DATE
  - latest Pass/Passed inspection with "FINAL"
    in inspectiondesc                             → FINAL_DATE

Known issues repaired:
  - Lowercase-schema rows left STATUS_NORMALIZED
    (and STATUS_ORIGINAL) null — especially
    "Permit Closed" / "Permit Issued" → FILLED.
  - Trailing whitespace / CR-LF on "Voided" → mapped.
  - "Incomplete" → In Review.
  - PERMIT_DATE copied from FILE_DATE on In Review
    (unissued) rows → cleared.
  - When ApprovedByDate exists and differs from the
    FILE_DATE copy, PERMIT_DATE overwritten → FIXED.
  - FINAL_DATE missing on all rows → FILLED from
    Pass/Passed FINAL inspections for Final records.

Not repairable from DATA:
  - 4 shell rows with empty Applications (and no
    other dates) → FILE_DATE stays missing.
  - Most Active/Final rows lack ApprovedByDate →
    PERMIT_DATE left as upstream FILE_DATE copy when
    present; lowercase rows with no issuance date
    stay missing.
  - Final rows with empty inspection history or no
    Pass/Passed FINAL inspection → FINAL_DATE stays
    missing.
"""
...
def _permit_info(d: dict) -> Optional[dict]:
    """Return the primary Permit Information dict (list or bare object)."""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    info = _permit_info(data_dict)
    if info is None:
        return "city_portal_unknown"

    if "StatusDesc" in info or "AppDate" in info or "PermitNumber" in info:
        return "city_portal_pascal"
    if "statusdesc" in info or "appdate" in info or "permitid" in info:
        return "city_portal_lower"
    return "city_portal_unknown"
...
def _earliest_app_date(d: dict):
    dates = []
    for a in _applications(d):
        dates.append(_safe_to_datetime(a.get("AppDate")))
        dates.append(_safe_to_datetime(a.get("appdate")))
...
def _earliest_approved_by_date(d: dict):
    dates = []
    for a in _applications(d):
        dates.append(_safe_to_datetime(a.get("ApprovedByDate")))
        dates.append(_safe_to_datetime(a.get("approvedbydate")))
    dates = [x for x in dates if x is not pd.NaT and not pd.isna(x)]
    return min(dates) if dates else pd.NaT


_PASS_RESULTS = {"Passed", "Pass"}


def _final_date_from_inspections(d: dict):
    """Latest Pass/Passed inspection whose description contains FINAL."""
    ...
    for insp in hist:
        ...
        if status not in _PASS_RESULTS:
            continue
...
def _repair_city_portal(row, d: dict, repairs: dict) -> None:
    """Repair a city_portal_* record (pascal or lower key casing)."""
    ...
    # FINAL_DATE from Pass/Passed FINAL inspections (Final only)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Estero permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Estero".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Estero") & (df["STATE"] == "FL")
    ].copy()

    print(f"Estero records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(
            agent_data_path, "estero_repaired_sample.parquet"
        )
