"""Data repair for Greenacres (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Greenacres DATA is a single Logos / TRAKiT-style portal payload with
...
whether Permit Details carries Application Received Date:

  - logos_completed / logos_completed_app
  - logos_issued / logos_issued_app
  - logos_created / logos_created_app
  - logos_pending / logos_pending_app
  - logos_status_only
  - missing / unknown
...
Canonical mappings:
  ...
  - Permit Details.Application Received
    Date, else Application Rcvd note /
    APPLICATION RECEIVED note text, else
    status date for Created / Pending
    Payment, else earliest non-archival
    note (bounded by lifecycle date)     → FILE_DATE
...
Known issues repaired:
  - FILE_DATE missing on nearly all Issued / Completed rows and on
    Pending Payment rows; filled primarily from Application Received
    Date when the newer Permit Details schema includes that field.
  - Two bare "Application Created" rows (no embedded status date)
    get FILE_DATE from Application Received Date.
  - A few Permit Created rows used the create/status stamp as
    FILE_DATE while Application Received Date is earlier → FIXED.

Not repairable from DATA:
  - Older Completed rows omit Application Received Date and lack
    usable Notes (~1,480 FILE_DATE gaps remain after repair).
  ...
  - All Permit Completed rows already embed a completion date, so
    FINAL_DATE needs no fill; non-Final correctly lack it.
"""
...
def _file_date_candidate(d: dict, status_base: Optional[str], status_dt, row) -> object:
    ...
    # Explicit Permit Details field (newer Logos schema).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Greenacres permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Greenacres". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Greenacres") & (df["STATE"] == "FL")
    ].copy()

    print(f"Greenacres records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_greenacres_repaired.parquet"
