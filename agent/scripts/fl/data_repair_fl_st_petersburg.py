"""Data repair for St. Petersburg (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

St. Petersburg DATA is the same city-portal family as Punta Gorda /
Pompano Beach / Margate / North Port. This sample has two sub-schemas:
...
Known issues repaired:
  - 133 fees_detail rows with null STATUS_NORMALIZED filled from
    Application Status (IN PROCESS / VOID / CLOSED / PENDING
    VERIFICATION).
  - CLOSED + VOID / ABANDONED / EXPIRED incorrectly labeled Final
    → FIXED to Inactive.
  ...
  - Spurious PERMIT_DATE on all In Review rows → cleared.
  - Active rows missing PERMIT_DATE filled from Issue Date
    (often older permits with blank Permit Date).
...
_APP_MAP = {
    ...
    "PLAN CHECK": "In Review",
    "HOLD FOR NOTICE OF COMM": "In Review",
    "HOLD FOR STORM DAMAGE": "In Review",
    "EPLAN MISSING INFO": "In Review",
    "CHANGE OF CONTRACTOR HOLD": "In Review",
    "TO BE ISSUED": "In Review",
    "VOID": "Inactive",
    "ABANDONED": "Inactive",
    "EXPIRED": "Inactive",
...
def _legacy_expected_status(sp_raw, app_raw) -> Optional[str]:
    ...
    if app_upper in _INACTIVE_APP_OVERRIDE or app in _INACTIVE_APP_OVERRIDE:
        ...
        if (
            app_upper in {"EXPIRED", "PERMIT EXPIRED", "PERMITS EXPIRED"}
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    St. Petersburg permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "St. Petersburg".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "St. Petersburg") & (df["STATE"] == "FL")
    ].copy()

    print(f"St. Petersburg records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(
            agent_data_path, "st_petersburg_repaired_sample.parquet"
        )
