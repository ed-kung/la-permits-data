"""Data repair for Debary (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Debary DATA comes from a citizen-portal scrape in the same family as
Cocoa / Casselberry / Bay Harbor Islands. Every row has the same core
keys (``Status:``, ``Permit Details``, ``Reviews``, ``Inspections``,
``Issue Date``, ``Permit Type``, …) but form fields and workflow depth
vary. Top-level ``Issue Date`` is always null in this sample; the real
issuance stamp lives under ``Permit Details['Issue Date:']``.
``Reviews`` / ``Inspections`` are usually lists but occasionally a bare
dict. Content variants (INFERRED_SCHEMA):
...
Known issues repaired:
  - STATUS_NORMALIZED missing for blank Status: shells that still carry
    Issue Date (2) and for Status:=CO Issued Date (1) → FILLED
    (Active / Final respectively).
  - FILE_DATE often equals a late Review Completion even when an earlier
    Review Start exists → FIXED to earliest Start.
  - FILE_DATE missing filled from Reviews when present, else Issue Date.
  - FINAL_DATE missing on all rows; filled for Closed/Final from passed
    inspections, review Completions, or Issue Date.
  - Active/Final PERMIT_DATE gaps (esp. Status:=Approved with blank
    Issue Date, and Closed shells) filled from Issue Date or latest
    review Completion.

Not repairable / left as-is:
  - ~1,500+ rows with no dated Reviews and no usable application stamp
    beyond Issue Date get FILE_DATE only from Issue Date (weak proxy).
  - Closed shells with blank Issue Date and empty Reviews / Inspections
    stay missing PERMIT_DATE / FINAL_DATE.
  - A few In Review rows carry Issue Date / PERMIT_DATE while Status:
    is still Payment Required; status text is kept.
"""
...
def _permit_date_candidate(d: dict):
    ...
    # Closed / Approved shells sometimes omit Issue Date; latest approved /
    # latest review Completion is the best issuance proxy in DATA.
...
def _final_date_candidate(d: dict, raw_status: str):
    ...
    if _clean_text(raw_status) in ("Closed", "CO Issued Date", "CO Issued"):
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Debary permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Debary".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Debary"].copy()

    print(f"Debary records: {len(city):,}\n")
