"""Data repair for Miami Springs (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Miami Springs DATA is a uniform civic / eTRAKiT-style payload with
...
Known issues repaired:
  - Null STATUS_NORMALIZED for IN APPROVAL (with issue date) → Active.
  - Stale STATUS_ORIGINAL on CLOSED shells still labeled Active
    (issued / permit issued) → Final; three of these also carry
    PermitFinaledDate with a missing FINAL_DATE.
  - ISSUED / REVISIONS kept as In Review despite PermitIssuedDate
    → Active.
...
Not repairable from DATA:
  - All 160 FINAL INSPECTION COMPLETE shells lack PermitFinaledDate
    and inspections → FINAL_DATE stays missing.
  - Most older CLOSED Final shells lack finaled stamps and usable
    final inspections → FINAL_DATE stays missing.
  - Some CLOSED / ISSUED Active/Final shells lack both IssuedDate and
...
_STATUS_MAP = {
    ...
    # Active — post-issuance / approved / printed
    "issued": "Active",
...
def _expected_status(raw_status: Optional[str], issued, finaled) -> Optional[str]:
    ...
    # Explicit finaled date upgrades non-inactive rows (e.g. CLOSED shells
    # still carrying a stale Active label from STATUS_ORIGINAL).
    if _present(finaled):
        return "Final"
    ...
    # (e.g. REVISIONS / IN APPROVAL after PermitIssuedDate).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Miami Springs permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Miami Springs".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Miami Springs"].copy()

    print(f"Miami Springs records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "miami_springs_repaired_sample.parquet"
        )
