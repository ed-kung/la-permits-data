"""Data repair for Pembroke Park (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pembroke Park DATA is a single city-portal schema in this sample (same
family as Hillsboro Beach / Deerfield Beach / Gadsden County):
...
Canonical mappings:
  ...
  - latest Passed/Complete inspection with
    "FINAL" in inspectiondesc                     → FINAL_DATE
...
Known issues repaired:
  - Archived StatusDesc mapped to In Review
    (STATUS_ORIGINAL="archived") → FIXED to Inactive.
  ...
  - FINAL_DATE missing on all rows → FILLED from
    Passed/Complete FINAL inspections for Final records.

Not repairable from DATA:
  - FILE_DATE already matches earliest AppDate on
    every sample row (no fills/fixes).
  - Active/Final/Inactive rows lacking ApprovedByDate
    keep the upstream FILE_DATE copy as PERMIT_DATE
    (fee DatePaid is not a reliable issuance proxy).
  - Final rows with empty inspection history or no
    Passed/Complete FINAL inspection → FINAL_DATE
    stays missing.
"""
...
# Inspection statuses that count as a successful close-out.
_PASSED_STATUSES = {"Passed", "Complete"}
...
def _final_date_from_inspections(d: dict):
    """Latest Passed/Complete inspection whose description contains FINAL."""
...
def _repair_city_portal(row, d: dict, repairs: dict) -> None:
    ...
    # FINAL_DATE from Passed/Complete FINAL inspections (Final only)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pembroke Park permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pembroke Park".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pembroke Park") & (df["STATE"] == "FL")
    ].copy()

    print(f"Pembroke Park records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_pembroke_park_repaired.parquet"
