"""Data repair for Orchid (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Orchid DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues / sample findings:
  - One row (ProjectNumber 2024-225) has upstream STATUS_NORMALIZED /
    STATUS_ORIGINAL as Active / permit issued while DATA.ProjectStatus
    is Pending (Under Review) (ProjectStatusID 6720) → FIXED to In Review.
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Orchid MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Orchid permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Orchid".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Orchid") & (df["STATE"] == "FL")
    ].copy()

    print(f"Orchid records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_orchid_repaired.parquet"
