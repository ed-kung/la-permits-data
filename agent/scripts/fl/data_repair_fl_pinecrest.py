"""Data repair for Pinecrest (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pinecrest DATA is an Accela / eTRAKiT-style portal payload with top-level
keys ``fees``, ``contacts``, ``site_info``, ``inspections``,
``permit_info``, and ``search_data`` (same family as Key West /
Parkland / Estero). Content variants (INFERRED_SCHEMA):
...
Canonical mappings:
  - permit_info.PermitStatus
    ...
    + blank status with Issued → Active (archive CERT shells)
                                           → STATUS_NORMALIZED
...
Known issues repaired:
  - Unmapped PermitStatus values left STATUS_NORMALIZED null
    (VERIFIED, ONLINE REWORKS, CC, blank archive shells) → FILLED.
  - Upstream sometimes normalized from stale STATUS_ORIGINAL while
    PermitStatus is FINALED / EXPIRED / ACTIVE → FIXED.
  - Rows with PermitFinaledDate still labeled Active / In Review
    → FIXED to Final and FINAL_DATE filled.
  - PERMIT_DATE missing when Issued is blank but Approved exists
    on Active/Final → FILLED from approved date.
  - Final rows missing FINAL_DATE filled from PermitFinaledDate
    or passed inspections (PASS / APPROVED / PARTIAL / VERIFIED).
  - Non-Final rows incorrectly carrying FINAL_DATE are cleared.

Not repairable from DATA:
  - ~805 rows with blank PermitAppliedDate (mostly CONV/CNTY
    historical shells) → FILE_DATE stays missing; issued /
    inspection / fee dates are not application dates.
  - CLOSED / status-only Finals and some FINALED shells with no
    Issued/Approved → PERMIT_DATE stays missing.
...
# Pinecrest portal inspection Result values that count as successful.
_PASS_RESULTS = {
    "PASS",
    "PASSED",
    "APPROVED",
...
_STATUS_MAP = {
    ...
    "FINALED": "Final",
    "CLOSED": "Final",
    "CC": "Final",  # certificate / completion closeout code
    "COMPLETED": "Final",
    # Active / issued / approved
    "ACTIVE": "Active",
    "APPROVED": "Active",
    # In review / pre-issuance / hold / rework
    "SUBMITTED": "In Review",
    "UNDER REVIEW": "In Review",
    "ON HOLD": "In Review",
    "ONLINE REWORKS": "In Review",
    "VERIFIED": "In Review",  # fee / zoning intake; no issuance stamp
    # Inactive
    "VOID": "Inactive",
    "EXPIRED": "Inactive",
    "WITHDRAWN": "Inactive",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    if finaled is not pd.NaT and not pd.isna(finaled):
        ...
        # PermitStatus / STATUS_ORIGINAL still say ACTIVE / UNDER REVIEW.
        return "Final"

    if expected is not None:
        return expected

    # Blank-status archive CERT shells with an Issued stamp but no
    # Finaled date: treat as Active (issued historical permit).
    issued = _safe_to_datetime(pi.get("PermitIssuedDate"))
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pinecrest permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pinecrest". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pinecrest") & (df["STATE"] == "FL")
    ].copy()

    print(f"Pinecrest records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_pinecrest_repaired.parquet"
