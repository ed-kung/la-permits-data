"""Data repair for Cocoa (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Cocoa DATA comes from a citizen-portal scrape. Every row has the same
core keys (``Status:``, ``Permit Details``, ``Reviews``, ``Inspections``,
``Issue Date``, ``Permit Type``, …) but form fields and workflow depth
vary. ``Reviews`` / ``Inspections`` are usually lists but occasionally a
bare dict. Content variants (INFERRED_SCHEMA):
...
Known issues repaired:
  - One row with STATUS_ORIGINAL=closed / STATUS_NORMALIZED=Final while
    DATA Status:=Approved → FIXED to Active.
  - FILE_DATE often equals Issue Permit Completion or Issue Date even
  ...
  - FINAL_DATE missing on all rows; filled for Closed/Final from passed
    final / other inspections, review Completions, or Issue Date.
  - Active/Final PERMIT_DATE gaps (esp. Status:=Approved with blank
    Issue Date, and some Closed shells) filled from Issue Date or
    latest review Completion.

Not repairable / left as-is:
  - One Under Review shell with empty Reviews / Inspections / Issue Date
    cannot get FILE_DATE.
  - ~78 Issued/Active rows have approved final-named inspections but
    Status: remains Issued; status text is kept (not promoted to Final).
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Cocoa permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Cocoa".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Cocoa"].copy()

    print(f"Cocoa records: {len(city):,}\n")
