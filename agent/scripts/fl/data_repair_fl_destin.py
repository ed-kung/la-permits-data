"""Data repair for Destin (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Destin DATA is a Tyler EnerGov payload with top-level keys contacts,
...
Known issues repaired:
  - Null STATUS_NORMALIZED for Decal Ready / Decal to be placed
    (and case variants) → FILLED to In Review.
  - Spurious PERMIT_DATE / FINAL_DATE equal to the 1900-01-01 sentinel
    cleared (or replaced when a real source exists).
  - Spurious FINAL_DATE on non-Final rows (Issued / Inactive / In Review
    carrying FinalDate or the sentinel) cleared.
  - Spurious PERMIT_DATE on In Review cleared.
  - Missing FINAL_DATE on Final filled from FinalDate / FinalizeDate or
    a passed Final inspection in processing_status.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row (entity
    first; one details/entity UTC day-boundary difference left as-is
    because entity agrees with the stored FILE_DATE).
  - 95 Complete / Final rows have blank IssueDate → PERMIT_DATE stays
    missing.
  - Many Complete shells lack a usable FinalDate and final inspection
    → FINAL_DATE stays missing after sentinel cleanup.
"""
...
_STATUS_MAP = {
    # Final — completed / administratively closed after work
    "Complete": "Final",
    "Administratively Closed": "Final",
    # Active — issued / open permit
    "Issued": "Active",
    # In Review — pre-issuance / hold / fees / decal workflow
    "In Review": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "On Hold": "In Review",
    ...
    "Decal Ready": "In Review",
    "Decal to be placed": "In Review",
    "Decal To Be Placed": "In Review",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Destin permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Destin".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Destin"].copy()

    print(f"Destin records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_destin_repaired.parquet")
