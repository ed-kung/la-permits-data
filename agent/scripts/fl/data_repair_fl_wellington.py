"""Data repair for Wellington (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Wellington DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - STATUS_ORIGINAL lag: CaseStatus advanced to Closed / Certificate
    of Occupancy Issued / Permit Issued/Printed while STATUS_ORIGINAL
    still held an earlier value (plan check, permit issued/printed,
    notice of commencement hold, expired) → FIXED.
  - Unmapped CaseStatus values (Notice of Commencement Hold, No Permit
    Obtained; trailing-space Void / Abandoned / Plan Check) left
    STATUS_NORMALIZED null → FILLED.
  - Missing PERMIT_DATE on issued shells whose STATUS_ORIGINAL still
    said plan check / NOC hold → FILLED from IssueDate after status fix.
  - Missing FINAL_DATE on Closed / CO Issued shells whose upstream
    status lagged as Active / In Review → FILLED from FinalDate.
  - Spurious FINAL_DATE on Void / Expired Inactive shells → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - A handful of Closed Final shells have Issued=False and blank
    IssueDate → PERMIT_DATE remains missing.
"""
...
_STATUS_MAP = {
    ...
    "Completed": "Final",
    "Finaled": "Final",
    # Active (issued; NOC hold is post-issuance administrative hold)
    "Permit Issued/Printed": "Active",
    "Issued": "Active",
    "Notice of Commencement Hold": "Active",
    # In Review
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Wellington permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Wellington". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Wellington") & (df["STATE"] == "FL")
    ].copy()

    print(f"Wellington records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_wellington_repaired.parquet"
        )
