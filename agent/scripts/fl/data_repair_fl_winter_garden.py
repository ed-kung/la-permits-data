"""Data repair for Winter Garden (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Winter Garden DATA is the same city-portal family as Oviedo / Lake Mary,
...
Canonical mappings:
  ...
  - Latest successful (APPROVED / WAIVED /
    PARTIALLY APPROVED) inspection excluding
    Notice of Commencement (Final rows only)        → FINAL_DATE
...
def _issue_date(detail: dict):
    """Permit issuance date (Winter Garden field name: Issue Date)."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Winter Garden permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Winter Garden".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Winter Garden"].copy()

    print(f"Winter Garden records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "winter_garden_permits_repaired.parquet"
        )
