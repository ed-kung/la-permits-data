"""Data repair for Fernandina Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Fernandina Beach DATA is a Tyler EnerGov payload with top-level keys
...
treated as null. Unlike some sibling cities, Fernandina Beach often has
populated ``processing_status`` inspection lists.
...
Known issues repaired:
  - Null STATUS_NORMALIZED for Void -Duplicate / Awaiting Corrections
    → FILLED.
  - Mis-normalized rows where STATUS_ORIGINAL is stale vs live
    CaseStatus (Complete→Active/Inactive/In Review, Issued→In Review/
    Inactive, Expired→Active/In Review, etc.) → FIXED.
  - Missing FINAL_DATE on Complete shells that were mislabeled Active /
    Inactive / In Review → FILLED from FinalDate after reclassification.
  - Spurious FINAL_DATE on non-Final (Expired / Void / Issued / In Review)
    → cleared.
  - Spurious PERMIT_DATE on In Review → cleared.
  - Missing PERMIT_DATE on Issued / Complete shells with IssueDate that
    were labeled In Review → FILLED after status fix (or kept for
    Active / Final / Inactive).

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - 1 Complete row has blank IssueDate (Issued=false) → PERMIT_DATE
    stays missing after mapping to Final.
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fernandina Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Fernandina Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Fernandina Beach"].copy()

    print(f"Fernandina Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_fernandina_beach_repaired.parquet"
        )
