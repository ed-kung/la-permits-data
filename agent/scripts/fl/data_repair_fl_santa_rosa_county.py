"""Data repair for Santa Rosa County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Santa Rosa County DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). Content variants (INFERRED_SCHEMA):

  - mgo_imported: TypeList contains ``Imported Fee`` (legacy fee
    shells migrated into MGO)
  - mgo_modern:   all other portal records
  - missing / unknown
...
Known issues repaired:
  - ``Administratively Closed`` labeled Final → FIXED to Inactive.
    These are legacy Imported Fee shells with no issuance or
    completion date; they are closed without a true final/sign-off.

Not repairable from DATA:
  - DateIssued is the sentinel ``0001-01-01T00:00:00`` on every sample
    row → PERMIT_DATE cannot be filled for Active/Final.
  - No finaled / completion / CO date field exists in the payload →
    FINAL_DATE cannot be filled.
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if "coc" in lower or "co issued" in lower or "final" in lower:
        return "Final"
    if "closed/complete" in lower or lower == "closed" or lower.endswith(" closed"):
        # Prefer exact Closed/Complete above; generic "closed" → Final
        # unless already handled as administrative.
        return "Final"
    ...
    if (
        ...
        return "Inactive"
    if "pending" in lower or "review" in lower or "hold" in lower:
        return "In Review"
    if "active" in lower:
        return "Active"
    return None
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Santa Rosa MGO
    # payload. Leave FINAL_DATE unchanged (universally missing in sample).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Santa Rosa County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Santa Rosa County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Santa Rosa County") & (df["STATE"] == "FL")
    ].copy()

    print(f"Santa Rosa County records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(
            agent_data_path, "santa_rosa_county_repaired_sample.parquet"
        )
