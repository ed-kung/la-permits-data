"""Data repair for Inverness (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Inverness DATA has two families in this sample:

  - converted: legacy migrated shells with uppercase STATUS,
               APPLIC DATE, Permit Date (= APPLIC DATE), ACTUAL END DATE
  - city_portal*: modern portal payloads with Status, Permit Date,
                  nested inspections / fees / payments / property_info,
                  and optional reviews / plan_reviews /
                  record_type_from_contractor_box

Canonical mappings:
  - STATUS / Status                         → STATUS_NORMALIZED
  - APPLIC DATE (converted) /
    Permit Date (portal)                    → FILE_DATE
  - (no distinct Issued Date in DATA)       → PERMIT_DATE
  - ACTUAL END DATE (converted Final) /
    latest passed FINAL-ish inspection
    completed_date (portal Final)           → FINAL_DATE

Key-set variants (INFERRED_SCHEMA prefixes):
  - converted:                 uppercase STATUS + APPLIC DATE
...
``_status_only``). Note: Inverness has no Issued Date field, so
``_issued*`` suffixes do not appear in this sample.

Known issues repaired:
  - All 1,491 converted shells have null STATUS_NORMALIZED
    (STATUS_ORIGINAL also null) → FILLED from STATUS
    (FINALED → Final; WITHDRAWN / EXPIRED / CLOSED APPLICATION
    → Inactive).
  - FINAL_DATE on converted Inactive shells (copied from
    ACTUAL END DATE) → cleared.
  - FINAL_DATE missing on all portal CLOSED (Final) shells
    → FILLED from passed FINAL-ish inspections when present.

Not repairable from DATA:
  - PERMIT_DATE is missing on every row and DATA has no Issued /
    Approved date distinct from FILE_DATE. Converted Permit Date
    always equals APPLIC DATE; portal has only Permit Date
    (already used as FILE_DATE). Left missing.
  - Portal CLOSED shells with empty inspections or no passed
    FINAL-ish inspection → FINAL_DATE stays missing.
  - Portal OPEN mapped to In Review (matches upstream / peer
    portal cities); no Issued Date to promote to Active.
"""
...
_FINAL_INSP_RE = re.compile(r"final|fnl|certificate|\bco\b|\bcc\b", re.IGNORECASE)
...
def _is_converted(d: dict) -> bool:
    """Legacy migrated shells use uppercase STATUS + APPLIC DATE."""
    return "STATUS" in d and "APPLIC DATE" in d
...
def _file_date(d: dict):
    if _is_converted(d):
        return _safe_to_datetime(d.get("APPLIC DATE"))
    return _safe_to_datetime(d.get("Permit Date"))


def _permit_date(d: dict):
    """Inverness DATA has no distinct issuance stamp.

    Converted ``Permit Date`` always equals ``APPLIC DATE``; portal
    payloads expose only ``Permit Date`` (used as FILE_DATE). Return
    NaT so PERMIT_DATE is not filled from the application stamp.
    """
    return pd.NaT
...
def _final_date(d: dict):
    if _is_converted(d):
        return _safe_to_datetime(d.get("ACTUAL END DATE"))
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if _is_converted(data_dict):
        base = "converted"
    elif "Permit Date" in keys or "Permit Number" in keys:
...
def _expected_status(row, d: dict) -> Optional[str]:
    """Infer STATUS_NORMALIZED from DATA STATUS/Status / STATUS_ORIGINAL."""
    raw = _norm_text(d.get("STATUS") or d.get("Status")).lower()
    orig = _norm_text(row.get("STATUS_ORIGINAL")).lower()

    if raw in _STATUS_MAP:
        return _STATUS_MAP[raw]
    if orig in _STATUS_MAP:
        return _STATUS_MAP[orig]
    return None
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # -- PERMIT_DATE --
    # No distinct Issued Date in DATA. Do not copy FILE_DATE / Permit Date.
    # Clear only if a non-usable sentinel is present (none observed).
    if _has_usable_date(row["PERMIT_DATE"]):
        ...
        # else: leave existing value (no better source to overwrite with)
    else:
    ...
    if effective_status == "Final":
        if _has_usable_date(row["FINAL_DATE"]):
            if _present(final_dt):
                if not _dates_equal(row["FINAL_DATE"], final_dt):
                    supporting = []
                    if _is_converted(d):
                        supporting.append(
                            _safe_to_datetime(d.get("ACTUAL END DATE"))
                        )
                    supporting.append(_final_from_inspections(d))
                    if not any(
                ...
                # Keep existing FINAL_DATE when DATA has no alternate stamp
                # (portal CLOSED with no passed final inspection).
                pass
        else:
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Inverness permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Inverness".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Inverness"].copy()

    print(f"Inverness records: {len(city):,}\n")
    ...
    for status in ["Active", "Final", "In Review", "Inactive"]:
        sub = repaired[repaired["STATUS_NORMALIZED"] == status]
        n_has = sub["PERMIT_DATE"].apply(_has_usable_date).sum()
        print(f"  {status:15s}: {n_has:>4,} / {len(sub):>4,} ({(n_has/len(sub) if len(sub) else 0):.1%})")

    print("\nFINAL_DATE by STATUS_NORMALIZED (after repair):")
    for status in ["Active", "Final", "In Review", "Inactive"]:
        sub = repaired[repaired["STATUS_NORMALIZED"] == status]
        n_has = sub["FINAL_DATE"].apply(_has_usable_date).sum()
        print(f"  {status:15s}: {n_has:>4,} / {len(sub):>4,} ({(n_has/len(sub) if len(sub) else 0):.1%})")

    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "inverness_repaired_sample.parquet"
        )
