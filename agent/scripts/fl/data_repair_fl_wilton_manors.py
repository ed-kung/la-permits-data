"""Data repair for Wilton Manors (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Wilton Manors DATA is a CitizenServe-style municipal portal payload.
...
from the earliest Review Start / Completion on or before Issue (there
is no Application Intake task). The portal does not use Final* /
certificate inspection type labels; final / sign-off is proxied by the
latest Approved / Passed inspection date for Final rows.

Canonical fields:

  - DATA["Status:"]                       → STATUS_NORMALIZED
  - earliest non-post-issuance Review
    Start, else earliest Review Completion
    (on/before Issue)
                                          → FILE_DATE
  - Permit Details["Issue Date:"]
    (rejecting ``01/01/2000`` sentinel)   → PERMIT_DATE
  - Latest Approved/Passed inspection
    (floored at Issue when present)       → FINAL_DATE

Key-set variants (INFERRED_SCHEMA prefixes):
  - portal_form_checklist: residential/
    contractor form extras + Permit Checklist
  - portal_form:           form extras, no checklist
  - portal_core:           minimal colon-key portal shell
  - portal_core_sqft:      core + Square Footage / dwelling units
  - portal_form_window:    form + window/door openings extras
...
Known issues repaired:
  - FILE_DATE often equals latest Review Completion (Plan Review /
    Review Complete) rather than earliest Review Start → FIXED;
    post-issue FILE cleared when no application source.
  - PERMIT_DATE copied from placeholder Issue Date ``01/01/2000`` on
    legacy Closed / Finaled / Void / Approved shells → FIXED (cleared);
    one Active Issued row missing PERMIT_DATE filled from real Issue.
  - FINAL_DATE missing on every sample row → FILLED for Final rows
    with Approved/Passed inspections.

Not repairable from DATA:
  - Shells with empty / undated Reviews → FILE_DATE stays missing.
  - Legacy shells with blank or ``01/01/2000`` Issue Date and no other
    issuance stamp → PERMIT_DATE stays missing.
  - Final shells without Approved/Passed inspections → FINAL_DATE
    stays missing.
  - STATUS_NORMALIZED already matches Status: for every sample row.
"""
...
# Legacy migration placeholder stamped as Issue Date on many Closed /
# Finaled / Void shells (and a handful of Approved).
_SENTINEL_DATES = {
    (2000, 1, 1),
}
...
def _latest_passed_inspection(d: dict):
    """Latest Approved/Passed inspection date (any trade type)."""
...
def _final_date(d: dict):
    """Final / sign-off proxy: latest Approved inspection, ≥ Issue."""
...
def _file_date(d: dict):
    ...
    earliest non-post-issuance Review Start / Completion on/before Issue.
...
def _schema_base(data_dict: dict) -> str:
    ...
    has_form = (
        "Valuation (Job Cost)" in keys
        or "Property Owners Email" in keys
        or "Contractor" in keys
    )
    has_checklist = "Permit Checklist" in keys
    has_window = "# of Window/Door Openings" in keys
    has_sqft = "Square Footage" in keys or "# of Dwelling Units" in keys

    if has_form and has_window:
        return "portal_form_window"
    if has_form and has_checklist:
        return "portal_form_checklist"
    if has_form:
        return "portal_form"
    if has_sqft and not (keys - _CORE_KEYS - {
        "Square Footage", "# of Dwelling Units",
    }):
        return "portal_core_sqft"
    if has_sqft and not has_form:
        return "portal_core_sqft"
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # PERMIT_DATE ← Issue Date for issued lifecycles (reject 01/01/2000).
    ...
    # FINAL_DATE ← latest Approved inspection for Final only.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Wilton Manors permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Wilton Manors". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Wilton Manors") & (df["STATE"] == "FL")
    ].copy()

    print(f"Wilton Manors records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_wilton_manors_repaired.parquet"
        )
