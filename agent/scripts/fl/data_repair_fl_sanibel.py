"""Data repair for Sanibel (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Sanibel DATA is a Tyler EnerGov payload with top-level keys contacts,
details, entity, fees, processing_status, and optionally reviews / holds /
attachments / more_info.

Canonical fields:

  - entity.CaseStatus (fallback details.PermitStatus; values are stripped
    because some CaseStatus strings carry a trailing space)
      → STATUS_NORMALIZED
...
Known issues repaired:
  - Null STATUS_NORMALIZED for Issued Dev Permit / Closed HB 447
    (STATUS_ORIGINAL never mapped) → FILLED.
  - Certificate of Completion still labeled Active because
    STATUS_ORIGINAL stayed at issued bldg permit → FIXED to Final.
  - Issued Bldg Permit still labeled In Review (stale STATUS_ORIGINAL
    in review / ready to issue) → FIXED to Active; PERMIT_DATE filled
    from IssueDate.
  - Canceled still labeled In Review → FIXED to Inactive.
  - Spurious FINAL_DATE on Inactive / In Review / Active Issued rows
    cleared; FINAL_DATE filled when CaseStatus upgrades to Final and
    FinalDate / FinalizeDate was already present.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - Many Converted / Closed HB 447 / Closed/Complete Final shells lack
    FinalDate / FinalizeDate → FINAL_DATE stays missing.
  - Closed/Complete / Converted shells with Issued=False and null
    IssueDate → PERMIT_DATE stays missing.
  - Ready to Issue rows that still carry IssueDate in EnerGov keep
    In Review (CaseStatus trusted); PERMIT_DATE cleared.
"""
...
# Sanibel has legacy Converted rows back to the mid-1970s.
_MIN_YEAR = 1970
_MAX_YEAR = 2035
...
# _expected_status; keys are stripped forms).
_STATUS_MAP = {
    # Final — completed / closed / occupancy
    "Certificate of Completion": "Final",
    "Cerificate of Occupancy": "Final",  # agency typo preserved in source
    ...
    # Active — issued
    "Issued": "Active",
    "Issued Bldg Permit": "Active",
    "Issued Dev Permit": "Active",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sanibel permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Sanibel".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Sanibel"].copy()

    print(f"Sanibel records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_sanibel_repaired.parquet")
