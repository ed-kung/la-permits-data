"""Data repair for Margate (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Margate DATA comes from the same city portal family as Boca Raton /
North Port / Lake Mary, with three sub-schemas in this sample:

  - permit_status: detail/fees plus permit_status_detail,
                   insp_status_detail (full permit + inspections)
  - fees_detail:   detail + fees + fees_total only (almost all are
                   empty shells; a few have Application Date/Status)
  - application:   mini_set with top-level application_status only

Canonical mappings:
  - Status for Permit Number (permit_status), overridden to
    Inactive when Application Status is NULL AND VOID /
    CANCELLED / PERMITS EXPIRED (except when Status for Permit
    Number already shows FINAL INSPECTION COMPLETE or
    C.O. ISSUED)                                     → STATUS_NORMALIZED
...
Known issues repaired:
  - fees_detail / application rows with null
    STATUS_NORMALIZED filled from Application Status.
  - CLOSED / PERMIT PRINTED / TO BE ISSUED rows with
    Application Status NULL AND VOID / CANCELLED /
    PERMITS EXPIRED incorrectly labeled Final / Active /
    In Review → FIXED to Inactive.
  - PERMIT_DATE ingested from portal "Permit Date"
  ...
  - Unsupported FINAL_DATE cleared when remapped away
    from Final or when no APPROVED inspection supports it.

Not repairable from DATA:
  - ~1,460 fees_detail shells have only an Application #
    → status and all dates stay missing.
  - Two application mini_set rows have CLOSED status but
    no date fields → FILE/PERMIT/FINAL stay missing.
  - Final rows with empty / non-APPROVED inspection
...
# Application Status (fees_detail / top-level detail / application mini_set)
_APP_MAP = {
    "CLOSED": "Final",
    "CERTIFICATE OF COMPLETION": "Final",
...
# Completion evidence that should win over a later PERMITS EXPIRED flag.
_COMPLETION_SP = {
    "FINAL INSPECTION COMPLETE",
    "C.O. ISSUED",
    "FINALED",
    "CLOSED MANUALLY - FINALED",
...
def _legacy_expected_status(sp_raw, app_raw) -> Optional[str]:
    ...
    if app_upper in _INACTIVE_APP_OVERRIDE or app in _INACTIVE_APP_OVERRIDE:
        ...
        # and APP only says PERMITS EXPIRED (post-completion admin flag).
        if (
            app_upper == "PERMITS EXPIRED"
...
def _repair_application(row, d: dict, repairs: dict) -> None:
    ...
    if isinstance(app, str) and (
        app.strip().upper() in _INACTIVE_APP_OVERRIDE
        or app.strip() in _INACTIVE_APP_OVERRIDE
    ):
        expected = "Inactive"
    else:
        expected = _map_app(app)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Margate permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Margate".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Margate") & (df["STATE"] == "FL")
    ].copy()

    print(f"Margate records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(agent_data_path, "margate_repaired_sample.parquet")
