"""Data repair for Cape Coral (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Cape Coral DATA is a Tyler EnerGov-style payload with top-level keys
...
/ ``more_info``. Three key-set variants appear in the sample:
  ...
  - entity_basic:         entity + details (+ contacts,
                          processing_status) without fees
...
Known issues repaired:
  - STATUS_NORMALIZED missing for CaseStatus ``Awaiting Customer``
    → FILLED as In Review.
  - Spurious FINAL_DATE on non-Final rows (Void, Denied) copied from
    entity.FinalDate, which is a case-closure stamp rather than a
    permit sign-off → cleared (FIXED).
...
Not repairable from DATA:
  ...
  - PERMIT_DATE already matches entity.IssueDate whenever IssueDate is
    present. Active ``Approved`` / one ``Issued`` shell and five Closed
    Final rows have Issued=False / null IssueDate → PERMIT_DATE cannot
    be filled.
  - Four Closed Final rows have null FinalDate / FinalizeDate (and no
    usable passed final inspection in processing_status) → FINAL_DATE
    cannot be filled.
"""
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    """Populate *repairs* with corrected values for a single Cape Coral record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Cape Coral permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Cape Coral".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Cape Coral"].copy()

    print(f"Cape Coral records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "cape_coral_repaired_sample.parquet")
