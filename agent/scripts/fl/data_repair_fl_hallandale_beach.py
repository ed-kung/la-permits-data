"""Data repair for Hallandale Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hallandale Beach DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED for CaseStatus Required (never mapped from
    STATUS_ORIGINAL ``required``) → FILLED as In Review. These shells
    have Issued=False and blank IssueDate.
  - Spurious FINAL_DATE on non-Final rows cleared:
      * Inactive VOID / Expired carrying FinalDate
      * Active with FinalDate preceding IssueDate (agency quirk)
      * Required → In Review carrying FinalDate

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - Active / Final rows with blank IssueDate stay without PERMIT_DATE
    (zoning Active shells; Finaled / Closed without issuance stamp).
  - One Finaled shell lacks FinalDate / FinalizeDate → FINAL_DATE
    stays missing. CompleteDate / ClosedDate are always null here.
"""
...
_MIN_YEAR = 1990  # Hallandale Beach sample ApplyDate range starts in 1990
_MAX_YEAR = 2035
...
_STATUS_MAP = {
    ...
    "Submitted": "In Review",
    "In-Review": "In Review",
    "Required": "In Review",  # unmapped STATUS_ORIGINAL "required"
    # Inactive — canceled / void / denied / expired / revoked
    "Cancelled": "Inactive",
    "Canceled": "Inactive",
    "Expired": "Inactive",
    "REVOKED": "Inactive",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hallandale Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hallandale Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Hallandale Beach"].copy()

    print(f"Hallandale Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_hallandale_beach_repaired.parquet"
        )
