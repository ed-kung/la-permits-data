"""Data repair for Juno Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Juno Beach DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  - Closed → Final
  - Issued → Active
  - In Review / Stop Work Order → In Review
  - Expired → Inactive
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Juno Beach MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Juno Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Juno Beach".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Juno Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Juno Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_juno_beach_repaired.parquet"
