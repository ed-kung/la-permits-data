"""Data repair for Palm Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Palm Beach DATA is a Tyler EnerGov payload with top-level keys
attachments, contacts, details, entity, fees, holds, more_info,
processing_status, and reviews (processing_status / reviews are always
null or empty in this sample; holds/fees vary).
...
Known issues repaired:
  - CaseStatus=Issued with PermitStatus=Final and FinalizeDate
    ...
    FinalizeDate.
  - Spurious FINAL_DATE on Active / Inactive shells that still carry
    FinalDate / FinalizeDate → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - 348 Final + 4 Active shells with Issued=False and blank IssueDate
    stay without PERMIT_DATE (no issuance stamp in DATA). Most are
    CaseStatus=Completed review/administrative shells.
  - 2 Completed (Final) shells with blank FinalDate / FinalizeDate
    stay without FINAL_DATE.
  - Inactive shells missing PERMIT_DATE also lack IssueDate in DATA.
  - processing_status is always null in this sample, so inspection
...
# Palm Beach uses "Completed" (not "Complete") and "Cancelled" / "Abandoned";
# CaseStatus "Cancelled " arrives with a trailing space (stripped above).
_STATUS_MAP = {
    # Final
    "Final": "Final",
    "Complete": "Final",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Palm Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Palm Beach". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Palm Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Palm Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_palm_beach_repaired.parquet"
        )
