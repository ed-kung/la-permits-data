"""Data repair for Bay Harbor Islands (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bay Harbor Islands DATA comes from a citizen-portal scrape. Every row has
the same core keys (``Status:``, ``Permit Details``, ``Reviews``,
``Inspections``, ``Issue Date``, ``Permit Type``, …) but form fields and
workflow depth vary. Content variants (INFERRED_SCHEMA):
...
Known issues repaired:
  - STATUS_NORMALIZED missing for Application Abandoned (20),
    Zoning Needed (1), Partial Approval - Foundation Only (1) → FILLED.
  - Approved with Conditions / Change of Contractor labeled In Review
    → FIXED to Active.
  - FILE_DATE often equals a late Review Completion even when an
    earlier Review Start exists → FIXED to earliest Start.
  - FILE_DATE missing on most rows; filled from Reviews when present,
    else Issue Date as application-date proxy.
  - FINAL_DATE missing on all rows; filled from passed final / other
    inspections, review Completions, or Issue Date for Closed.
  - Active/Final PERMIT_DATE gaps filled from Issue Date or review
    Completions.

Not repairable / left as-is:
  - 2 rows with blank Status: and no STATUS_ORIGINAL stay missing.
  - Rows with no Reviews and no Issue Date cannot get FILE_DATE /
    PERMIT_DATE from DATA.
  - FINAL_DATE equal to Issue Date on Closed rows with no inspections
    or reviews is a weak proxy (permit closed without recorded
    fieldwork).
"""
...
_STATUS_MAP = {
    ...
    "Partial Approval - Foundation Only": "Active",
    "Change of Contractor": "Active",
    "Pending": "In Review",
    "Online Application Received": "In Review",
    "On Hold": "In Review",
    "Zoning Needed": "In Review",
    "Corrections": "In Review",
    "Expired": "Inactive",
    "Expired w/no Inspections": "Inactive",
    "Application Abandoned": "Inactive",
    "Withdrawn": "Inactive",
...
def _apply_status(repairs: dict, current, raw_status: Optional[str]) -> Optional[str]:
    ...
    if raw_status is None or not _clean_text(raw_status):
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bay Harbor Islands permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bay Harbor Islands".  Must
...
if __name__ == "__main__":
    ...
    bhi = df[df["JURISDICTION"] == "Bay Harbor Islands"].copy()

    print(f"Bay Harbor Islands records: {len(bhi):,}\n")

    repaired = data_repair(bhi)
    ...
    for field in ["STATUS_NORMALIZED", "FILE_DATE", "PERMIT_DATE", "FINAL_DATE"]:
        ...
        before_missing = bhi[field].isna().sum()
    ...
    print("  Before:")
    for s, c in bhi["STATUS_NORMALIZED"].value_counts(dropna=False).items():
