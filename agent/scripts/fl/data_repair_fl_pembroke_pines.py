"""Data repair for Pembroke Pines (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pembroke Pines DATA is a Tyler EnerGov payload (entity / details /
contacts / fees / processing_status; a minority also have
reviews/holds/attachments/more_info). Canonical fields:
...
Known issues repaired:
  - Stale STATUS_ORIGINAL / STATUS_NORMALIZED lag: CaseStatus Complete
    still labeled Active → FIXED to Final (4 rows); CaseStatus Issued
    still labeled In Review → FIXED to Active (3 rows).
  - Missing PERMIT_DATE filled from IssueDate on rows remapped to
    Active (2 rows).
  - Missing FINAL_DATE on Final / remapped-Complete rows filled from
    FinalDate / FinalizeDate, or from Passed final inspections when
    entity FinalDate is null (~700+ fills).
  - Spurious FINAL_DATE on Active / In Review / Inactive cleared —
    FinalDate there is not a completion date for those CaseStatus
    values.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - Legacy Sub Application rows (671) have no IssueDate / FinalDate /
    inspections → PERMIT_DATE and FINAL_DATE stay missing.
  - ~157 Complete rows lack IssueDate → PERMIT_DATE stays missing.
  - ~51 Complete rows lack FinalDate and any Passed final inspection
...
# EnerGov inspection result strings observed in Pembroke Pines
# (status values often end with a trailing period, e.g. "Passed.").
_INSP_PASS = {"passed", "approved"}
...
def _final_inspection_date(d: dict):
    ...
    Pembroke Pines uses EnerGov result strings such as ``Passed.`` (trailing
    period). Partial Pass is intentionally excluded.
    """
    ...
    for insp in ps:
        if not isinstance(insp, dict):
            continue
        status = str(insp.get("status") or "").strip().lower().rstrip(".")
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # Clear on In Review (e.g. mislabeled Issued rows that carried a date).
    ...
    # Prefer entity FinalDate over inspections (FinalDate is often the
    # administrative close day after the last Passed final inspection).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pembroke Pines permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pembroke Pines".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Pembroke Pines"].copy()

    print(f"Pembroke Pines records: {len(city):,}\n")
