"""Data repair for Lake Worth Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lake Worth Beach DATA is the same city-portal family as Tarpon Springs /
Winter Garden / Oviedo, with two sub-schemas in this sample:
...
Canonical mappings:
  - Inactive Application Status (VOIDED / EXPIRED /
    CANCELLED / …) overrides permit status; else
...
Known issues repaired:
  - STATUS_NORMALIZED null on all fees_detail rows
    → FILLED from Application Status.
  - VOIDED / EXPIRED / CANCELLED apps whose
    Status for Permit Number is CLOSED were
    labeled Final → FIXED to Inactive.
  - Stale STATUS_ORIGINAL mislabels (CLOSED kept
    as Active; PERMIT PRINTED kept as In Review;
    FINAL INSPECTION COMPLETE kept as In Review /
    Active) → FIXED from portal status fields.
  - PERMIT_DATE was populated from the portal
    ...
    Inactive.
  - Spurious PERMIT_DATE on In Review (copied from
    "Permit Date", often with blank Issue Date)
    → cleared.
  - FINAL_DATE filled / corrected from latest
    non-NOC successful inspection; cleared on
    non-Final and on fees_detail shells.
...
Not repairable from DATA:
  ...
  - Active/Final with blank Issue Date → PERMIT_DATE
    stays missing (or is cleared if it only reflected
    "Permit Date").
"""
...
def _issue_date(detail: dict):
    """Permit issuance date (Lake Worth Beach field name: Issue Date)."""
...
def _repair_permit_status(row, d: dict, repairs: dict) -> None:
    ...
    if effective_status == "In Review":
        ...
        if pd.notna(current) and _present(permit_stamp) and _dates_equal(
            current, permit_stamp
        ):
            _clear_date(repairs, row, "PERMIT_DATE")
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lake Worth Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lake Worth Beach".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Lake Worth Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Lake Worth Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_lake_worth_beach_repaired.parquet"
