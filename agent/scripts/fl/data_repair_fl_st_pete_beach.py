"""Data repair for St. Pete Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

St. Pete Beach DATA is a city permit-portal payload (same family as
Daytona Beach Shores) with top-level ``Status``, ``Permit Date``,
``Permit Number``, ``permit_id``, nested ``fees`` / ``payments`` /
``contractors`` / ``inspections`` / ``property_info``, and either
``reviews`` or ``plan_reviews``. Historic shells also carry
``APPLICATION TYPE`` / ``OWNER NAME`` / ``Permit Number Old``.

INFERRED_SCHEMA prefixes:
  - historic:      APPLICATION TYPE / Permit Number Old present
  - plan_reviews:  plan_reviews key (no reviews array)
  - portal:        standard modern shells (EXPIRATION DATE / reviews)

Suffix is a slug of DATA["Status"] (or ``blank``).

Canonical mappings:
  - DATA["Status"] (+ blank+completed inspections → Final)
                                           → STATUS_NORMALIZED
  - DATA["Permit Date"]                    → FILE_DATE
    (application / record stamp — present on In Review / Online
     Application rows; not an issuance date)
  - (no issuance field in DATA)            → PERMIT_DATE left missing
  - Latest successful final-named inspection completed_date,
    else latest successful inspection (Final only)
                                           → FINAL_DATE

Known issues repaired:
  - Open mapped to In Review → FIXED to Active (portal has a separate
    In Review / Online Application workflow for pre-issuance).
  - Null STATUS_NORMALIZED on Online Application, ADDITIONAL
    DOCUMENTS NEEDED, SWO, and blank-Status historic shells that
    already have completed inspections → FILLED.
  - FILE_DATE 1899-12-30 sentinel (invalid Permit Date) → cleared.
...
Not repairable from DATA:
  - No Issued / Approved date field. Permit Date is the file date
    (present on unissued In Review rows) so it must not be copied
    into PERMIT_DATE. Active / Final PERMIT_DATE stays missing.
  - 148 blank-Status historic shells with empty inspections →
...
_FINAL_TYPE_RE = re.compile(r"final|fnl|\bbfinal\b", re.I)

# Inspection status prefixes that count as successful completion.
_PASS_PREFIXES = (
    "pass",
    "complete",
    "approved",
...
def _schema_family(d: Optional[dict]) -> str:
    ...
    if "Status" not in keys or "Permit Date" not in keys:
        return "unknown"
    if "APPLICATION TYPE" in keys or "Permit Number Old" in keys:
        return "historic"
...
# uses separate In Review / Online Application labels for pre-issuance.
_STATUS_MAP = {
    "Closed": "Final",
    "Open": "Active",
    "In Review": "In Review",
    "Hold": "In Review",
    "Online Application": "In Review",
    "ADDITIONAL DOCUMENTS NEEDED": "In Review",
    "Void": "Inactive",
    "Expired": "Inactive",
    "Withdrawn": "Inactive",
    "Cancelled": "Inactive",
    "Abandoned": "Inactive",
    "SWO": "Inactive",  # stop-work order
}


def _has_completed_inspection(d: dict) -> bool:
    ...
    if raw:
        ...
        for key, val in _STATUS_MAP.items():
            if key.lower() == raw.lower():
                return val
        return None
    # Blank Status: historic shells with completed inspections are Final.
    if _has_completed_inspection(d):
        return "Final"
    return None
...
def _insp_status_passed(status: str) -> bool:
    ...
    if not s:
        return False
    if s.startswith("fail") or s.startswith("incomplete") or s.startswith("pending"):
        return False
    if s.startswith("cancelled") or s.startswith("yellow tag"):
        return False
    return any(s.startswith(p) for p in _PASS_PREFIXES)
...
def _final_date_from_inspections(d: dict):
    """Prefer latest passed final-named insp; else latest any passed insp."""
    final_dates = []
    pass_dates = []
    for insp in d.get("inspections") or []:
        if not isinstance(insp, dict):
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    ...
    if _present(permit_date):
        ...
        # Clear stored sentinel / out-of-range FILE_DATE when Permit Date
        # itself is unusable (e.g. 1899-12-30).
    ...
    # (also present on In Review / Online Application). Do not copy it.
    # Clear any unsupported stamp if somehow present.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    St. Pete Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "St. Pete Beach".
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "St. Pete Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"St. Pete Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_st_pete_beach_repaired.parquet"
