"""Data repair for Okeechobee County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Okeechobee County DATA is a uniform civic / eTRAKiT-style payload with
top-level keys contacts, fees, inspections, permit_info, search_data,
and site_info. ``search_data`` has two key-set variants (legacy
Permit No / SITE_* vs newer Permit Number / Address), but canonical
fields always live under ``permit_info``:
...
Known issues repaired:
  - Null STATUS_NORMALIZED for CLOSED FINAL / 1ST HOLD / FRONT HOLD /
    BUILDING PLAN REVIEW / ZONING PLAN REVIEW → FILLED.
  - APPROVED (no issue date) labeled Active → FIXED to In Review
    (approved but not issued).
  - UNKNOWN labeled Final with no issue/final dates → FIXED to In Review.
  - ISSUED / INSPECTED rows carrying PermitFinaledDate still labeled
    Active → FIXED to Final.
  - READY / APPLIED / APPROVED / hold statuses with an issue date
    upgraded to Active.
  ...
  - Missing FINAL_DATE on Final filled from PermitFinaledDate or a
    passed FINAL / CO inspection.
  - Spurious FINAL_DATE on non-Final (Active / Inactive VOID) cleared.
  - Spurious PERMIT_DATE on In Review cleared.

Not repairable from DATA:
  - 2 FILE_DATE gaps (VOID / empty shell) have blank PermitAppliedDate
    and no IssuedDate.
  - Some ISSUED / INSPECTED / FINALED rows have blank IssuedDate and
    ApprovedDate → PERMIT_DATE stays missing.
  - Most CLOSED FINAL / FINALED shells lack PermitFinaledDate and
    usable final inspections → FINAL_DATE stays missing.
  - Empty PermitStatus shells with only an applied date stay
    STATUS_NORMALIZED null when no stronger signal exists.
"""
...
_STATUS_MAP = {
    ...
    "finaled": "Final",
    "closed final": "Final",
    # Active — post-issuance / under inspection
    "issued": "Active",
    "inspected": "Active",
    # In Review — pre-issuance / holds / plan review
    "applied": "In Review",
    "approved": "In Review",
    "ready": "In Review",
    "1st hold": "In Review",
    "front hold": "In Review",
    "zoning hold": "In Review",
    "building plan review": "In Review",
    "zoning plan review": "In Review",
    # Inactive
    "void": "Inactive",
    "expired": "Inactive",
    "denied": "Inactive",
...
def _expected_status(raw_status: Optional[str], issued, finaled) -> Optional[str]:
    raw_key = "" if raw_status is None else str(raw_status).strip().lower()

    # Terminal inactive statuses win even if a finaled date is present.
    if raw_key in _INACTIVE_STATUSES:
        return "Inactive"

    # Explicit finaled date upgrades non-inactive rows (ISSUED/INSPECTED
    # shells that still carry PermitFinaledDate).
    if _present(finaled):
        return "Final"
    ...
    if raw_key == "unknown":
        # No lifecycle evidence beyond an application date in the sample.
        if _present(issued):
            return "Active"
        return "In Review"
    ...
    # (e.g. READY, APPLIED, APPROVED, 1ST HOLD after PermitIssuedDate).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Okeechobee County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Okeechobee County".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Okeechobee County"].copy()

    print(f"Okeechobee County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "okeechobee_county_repaired_sample.parquet"
        )
