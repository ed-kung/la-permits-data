"""Data repair for Southwest Ranches-Lauderdale-by-the-Sea (FL) permits.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

DATA is a CitizenServe-style municipal portal payload shared by Southwest
Ranches and Lauderdale-by-the-Sea. Every sample row has colon-suffixed
keys (``Status:``, ``Permit #:``, ``Permit Details``, ``Reviews``,
``Inspections``). Top-level ``Issue Date`` is always null; the usable
issue stamp lives under ``Permit Details["Issue Date:"]``. Application /
submittal dates come from the earliest Review Start / Completion on or
before Issue (there is no Application Intake task). The portal does not
use Final* / certificate inspection type labels; final / sign-off is
proxied by the latest Approved / Passed inspection date for Final rows.
...
Key-set variants (INFERRED_SCHEMA prefixes):
  - portal_form: contractor / building-type
    form extras (Builder, Electrician, …)
...
Known issues repaired:
  - 54 null STATUS_NORMALIZED values for on-hold / pending / abandoned /
    blank Status: labels that upstream never mapped → FILLED.
  - Pending Zoning/Engineering Final (and similar) rows that already
    carry Issue Date → FIXED/FILLED to Active.
  - FILE_DATE often equals latest Review Completion rather than earliest
    Review Start → FIXED; post-issue FILE cleared when no application
    source.
  - FINAL_DATE missing on every sample row → FILLED for Final
    (Closed) rows with Approved/Passed inspections.

Not repairable from DATA:
  - Shells with empty / undated Reviews → FILE_DATE stays missing.
  - Shells with blank Issue Date and no other issuance stamp →
    PERMIT_DATE stays missing (mostly Inactive / In Review; a few
    Approved Active shells).
  - Final (Closed) shells without Approved/Passed inspections →
    FINAL_DATE stays missing.
  - Blank Status: with no workflow and no Issue Date stays null.
"""
...
_STATUS_MAP = {
    ...
    "Pending Engineering": "In Review",
    "Pending Zoning/Engineering Final": "In Review",
    ...
    "Voided": "Inactive",
    "Abandoned": "Inactive",
    "Abandoned/ Voided Permit": "Inactive",
}
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Southwest Ranches-Lauderdale-by-the-Sea permit records using DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION ==
        "Southwest Ranches-Lauderdale-by-the-Sea". Must contain columns
        STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, FINAL_DATE, and DATA.
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Southwest Ranches-Lauderdale-by-the-Sea")
        & (df["STATE"] == "FL")
    ].copy()

    print(f"Southwest Ranches-Lauderdale-by-the-Sea records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir,
            "permits_fl_southwest_ranches_lauderdale_by_the_sea_repaired.parquet",
        )
