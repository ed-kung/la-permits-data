"""Data repair for North Miami Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

North Miami Beach DATA is a Tyler EnerGov payload with top-level keys
...
treated as null. In the FL sample, ``processing_status`` is always null,
so the inspection fallback is unused but kept for robustness.
...
Known issues repaired:
  - Null STATUS_NORMALIZED for In Review-ProjectDox / Prescreen /
    Waiting for Master Approval / Pending Application Correction /
    Prescreen Correction / Approved / Issued / On Hold → FILLED.
  - Mis-normalized CaseStatus (Finaled→Active/Inactive, Issued→Final/
    Inactive, Expired→Active) → FIXED.
  - Missing FINAL_DATE on Finaled shells that were mislabeled and on
    Finaled rows reclassified to Final → FILLED from FinalDate.
  - Wrong FINAL_DATE on one Finaled row (stale value vs FinalDate)
    → FIXED.
  - Spurious FINAL_DATE on non-Final (Issued / On Hold / Void /
    Expired) → cleared.
  - Spurious PERMIT_DATE on In Review → cleared.
  - Missing PERMIT_DATE on Issued shells with IssueDate → FILLED.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - 78 Finaled shells have blank IssueDate → PERMIT_DATE stays missing.
  - All 19 Approved shells lack IssueDate (Issued=false) → PERMIT_DATE
    stays missing after mapping to Active.
  - processing_status is null throughout the sample → no inspection
    fallback for FINAL_DATE / PERMIT_DATE.
"""
...
_STATUS_MAP = {
    ...
    "Finaled": "Final",
    # Active — issued / plan-approved open permit
    "Issued": "Active",
    "Approved": "Active",
    # In Review — pre-issuance / hold / online review workflow
    "In Review": "In Review",
    "In Review-ProjectDox": "In Review",
    "On Hold": "In Review",
    "Prescreen": "In Review",
    "Prescreen Correction": "In Review",
    "Pending Application Correction": "In Review",
    "Submitted": "In Review",
    "Submitted - Online": "In Review",
    "Waiting for Master Approval": "In Review",
    "Stop Work Order": "In Review",
    # Inactive — expired / void
    "Expired": "Inactive",
    "Void": "Inactive",
}
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    North Miami Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "North Miami Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "North Miami Beach"].copy()

    print(f"North Miami Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_north_miami_beach_repaired.parquet"
        )
