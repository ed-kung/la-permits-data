"""Data repair for Palmetto Bay (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Palmetto Bay DATA is a single Accela-style portal payload (same family
as North Miami) with top-level keys ``main``, ``details``, ``actions``,
``fees``, ``routing``, etc. Content variants (INFERRED_SCHEMA) are
labeled by portal status plus which canonical dates are populated:
...
Canonical mappings:
  - main.Status
    (final → Final; issued / approved → Active;
     pending → In Review; canceled / expired → Inactive)
                                                → STATUS_NORMALIZED
  - main.Applied                                → FILE_DATE
  - main.Issued else main.Approved else
    completed collissue / issue action          → PERMIT_DATE
  - main.Final else completed
    ``final - FINALIZE PERMIT`` action          → FINAL_DATE

Known issues repaired:
  - Stale STATUS_ORIGINAL lags main.Status
    (issued/pending/approved kept after portal
    moved to final / canceled / issued) → FIXED.
  - One Final row with null STATUS_ORIGINAL /
    STATUS_NORMALIZED despite main.Status=final
    → FILLED.
  - FILE_DATE missing when Applied is present
    → FILLED.
  - Active ``approved`` (and sparse Final /
    issued) rows missing PERMIT_DATE are FILLED
    from Issued, else Approved.
  - Non-Final rows (esp. canceled Inactive)
    incorrectly carrying main.Final as FINAL_DATE
    are cleared (FIXED); Active→Final status
    fixes also FILLED FINAL_DATE from main.Final.

Not repairable from DATA:
  - One empty ``LIEN SEARCH`` shell (blank main)
    keeps upstream STATUS / dates unchanged.
  - Rare Final rows with blank Issued and Approved
    → PERMIT_DATE stays missing (only Final date
    present).
"""
...
def _expected_status(d: dict) -> Optional[str]:
    """Map main.Status; fall back to date inference."""
    raw = _raw_status(d)
    if raw is None:
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Palmetto Bay permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Palmetto Bay". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Palmetto Bay") & (df["STATE"] == "FL")
    ].copy()

    print(f"Palmetto Bay records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_palmetto_bay_repaired.parquet"
