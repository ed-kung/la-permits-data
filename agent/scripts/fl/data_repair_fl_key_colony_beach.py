"""Data repair for Key Colony Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Key Colony Beach DATA is a CitizenServe-style municipal portal payload.
Every sample row has colon-suffixed keys (``Status:``, ``Permit #:``,
``Permit Details``, ``Reviews``, ``Inspections``). Top-level
``Issue Date`` is almost always null (one row polluted with work-
description text); the usable issue stamp lives under
``Permit Details["Issue Date:"]``. Application / submittal dates come
from ``Application Intake`` / earliest Review Start; final / sign-off
dates come from passed Final* inspections (especially
``FINAL BUILDING``).

Canonical fields:

  - DATA["Status:"] (blank/unmapped inferred; Issued upgraded to Final
    when FINAL BUILDING passed; In Review upgraded to Active when Issue
    Date exists)
      → STATUS_NORMALIZED
...
Key-set variants (INFERRED_SCHEMA prefixes):
  - portal_owner: owner-contact extras (Property Owner Name, …)
...
Known issues repaired:
  - Null STATUS_NORMALIZED on blank Status shell with no workflow
    remains null (no DATA signal); Issued+FINAL BUILDING → Final;
    Online Application Received (and other In Review labels) carrying
    Issue Date → Active.
  - FILE_DATE often equals latest Review Completion (Plan Review /
    Review Complete) rather than Application Intake / earliest Review
    Start → FIXED; post-issue FILE cleared when no application source.
  - Spurious PERMIT_DATE on In Review cleared; 2 Active/Final shells
    missing PERMIT_DATE filled from Permit Details Issue Date.
  - FINAL_DATE missing on every sample row → FILLED for Closed /
    upgraded-Final rows with a Final*/CO stamp.
...
Not repairable from DATA:
  ...
  - Closed / Issued shells with blank Issue Date → PERMIT_DATE stays
    missing.
  - Closed shells without a usable Final* inspection → FINAL_DATE
    stays missing.
  - One blank Status shell with empty Reviews/Inspections/Issue Date
...
# Whole-permit closeout (status lag Issued → Final). Trade finals alone
# (Final Electric / Final Roof / …) are not enough to reclassify.
_PRIMARY_FINAL_RE = re.compile(
    r"final\s*building|final\s*bldg|certificate of occupancy|"
...
_COMMON_FORM_KEYS = {
    ...
    "Intended Use (Specify Residential or Business)",
    "Type of Construction (Specify concrete block, frame, etc.)",
    "Is this a rental property?",
    "Are you the property owner?",
    "Contractor Approval Document (i.e. contract, proposal, affidavit)",
    "Will you be using a Private Provider",
    "click here for private provider packet.",
    "Property Owner Name",
...
def _issue_date(d: dict):
    """Permit Details Issue Date (top-level Issue Date is null/polluted)."""
...
def _final_date(d: dict):
    """Final / CO / sign-off date proxy (any Final* trade or building)."""
    return _final_from_inspections(d, primary_only=False)
...
def _file_date(d: dict):
    ...
    Prefer Application Intake; fall back to earliest non-post-issuance
    Review Start / Completion on/before Issue. Upstream FILE_DATE
    frequently equals the latest Review Completion.
    """
    issue = _issue_date(d)
...
def _schema_base(data_dict: dict) -> str:
    ...
    if "Property Owner Name" in keys or "Property Owner Email" in keys:
        return "portal_owner"
    if (
        "Will you be using a Private Provider" in keys
        or "click here for private provider packet." in keys
    ):
        return "portal_pp"
    if (
        "Is this a rental property?" in keys
        or "Are you the property owner?" in keys
    ):
        return "portal_res"
    extras = keys - _CORE_KEYS - _COMMON_FORM_KEYS
    if not extras:
        return "portal_core"
    return "portal_form"
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    if mapped == "In Review" and has_issue:
        return "Active"
    # Issued lagging behind whole-permit FINAL BUILDING → Final.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Key Colony Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Key Colony Beach". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Key Colony Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Key Colony Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_key_colony_beach_repaired.parquet"
        )
