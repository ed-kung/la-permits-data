"""Data repair for Sunny Isles Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Sunny Isles Beach DATA is a SmartGov community portal payload (same
family as Port Orange / Auburndale) with top-level keys ``Department``,
``My Project``, ``Permit Type``, ``Build Status``, ``Permit Number``,
``Permit Details``, contacts/fees/inspections arrays, and optionally
``Parcel Number`` / ``ProjectDescription``. Variants (INFERRED_SCHEMA):
...
Canonical mappings:
  - DATA["Build Status"] (Expired* / CLOSED / FINALED / …),
...
Known issues repaired:
  - 368 null STATUS_NORMALIZED rows: REVISED / null Build Status /
    Expired* / review statuses inferred from Build Status + My Project
    dates → FILLED.
  - CLOSED / RENEWED / READY TO ISSUE rows with Closed stamps kept as
    Active or In Review → FIXED to Final.
  - Expired* mislabeled Active / In Review → FIXED to Inactive.
  - ACTIVE kept as In Review despite Issued → FIXED to Active.
  - Missing FILE_DATE / PERMIT_DATE / FINAL_DATE filled from My Project
    when the canonical stamp exists; FINAL_DATE mismatched to Closed
    → FIXED; spurious FINAL_DATE on non-Final → cleared.

Not repairable from DATA:
  - 7 empty-shell records (null Build Status, blank My Project dates)
    → STATUS_NORMALIZED / FILE_DATE stay missing.
  - FINALED / Closed finals with blank Closed and empty
    Permit Inspections → FINAL_DATE stays missing.
...
# Keys are lower-case; Sunny Isles Beach uses UPPERCASE Build Status labels.
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "active": "Active",
    ...
    "ready for review - zoning only": "In Review",
    "complete - zoning": "In Review",
}
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sunny Isles Beach (FL) permit records using information from the raw
    DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Sunny Isles Beach". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Sunny Isles Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Sunny Isles Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_sunny_isles_beach_repaired.parquet"
