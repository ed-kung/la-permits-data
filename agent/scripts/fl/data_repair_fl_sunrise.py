"""Data repair for Sunrise (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Sunrise DATA is a Tyler EnerGov payload (entity / details / contacts /
processing_status, optionally fees and reviews/holds/attachments/
more_info). Canonical fields:
  ...
  - entity.FinalDate (fallback details.FinalizeDate,
    else latest Approved final-ish inspection in
...
Known issues repaired:
  - Stale STATUS_ORIGINAL / STATUS_NORMALIZED lag: CaseStatus Closed
    still labeled Active → FIXED to Final (9 rows).
  - To Be Issued (Paid / Awaiting Payment) labeled Active → FIXED to
    In Review; spurious PERMIT_DATE on those rows cleared.
  - Withdrawn labeled Active (STATUS_ORIGINAL still ``issued``) →
    FIXED to Inactive.
  - Missing FINAL_DATE on Final rows filled from FinalDate /
    FinalizeDate, or from Approved final inspections in
    processing_status when entity FinalDate is null.
  - Spurious FINAL_DATE on Inactive (Voided / Voided (Error) /
    Withdrawn) and In Review (On Hold) cleared — FinalDate there is a
    void/close stamp, not a completion date.
  - Missing PERMIT_DATE filled from IssueDate when present for
    Active / Final / Inactive.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - ~750 Active/Final rows have CaseStatus Issued/Closed/CO but
    Issued=false and null IssueDate → PERMIT_DATE stays missing.
  - ~24 Final (mostly Certificate of Occupancy) rows have neither
    FinalDate nor an Approved final inspection → FINAL_DATE stays
    missing.
"""
...
_STATUS_MAP = {
    ...
    "Expired": "Inactive",
    "Voided": "Inactive",
    "Voided (Error)": "Inactive",
    "Withdrawn": "Inactive",
    # In Review
    "In Review": "In Review",
    "Submitted": "In Review",
    "Submitted - Online": "In Review",
    "To Be Issued (Awaiting Payment)": "In Review",
    "To Be Issued (Paid)": "In Review",
    "On Hold": "In Review",
}
...
def _final_inspection_date(d: dict):
    """Latest Approved processing_status inspection that looks final."""
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # Clear on In Review (e.g. To Be Issued rows that carried a date).
    ...
    # FINAL_DATE ← FinalDate / FinalizeDate, else Approved final inspection.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sunrise permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Sunrise".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Sunrise"].copy()

    print(f"Sunrise records: {len(city):,}\n")
