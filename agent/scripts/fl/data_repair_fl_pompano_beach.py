"""Data repair for Pompano Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pompano Beach DATA is the same city-portal family as Margate / North Port
/ Boca Raton, with two sub-schemas in this sample:
...
Canonical mappings:
  ...
  - Latest successful FINAL/FNL/CLOSEOUT inspection;
    else latest non-NOC successful inspection
    (APPROVED / APPROVED WITH EXCEPTION / SATISFACTORY)
                                                     → FINAL_DATE
...
Known issues repaired:
  - 169 fees_detail rows with null STATUS_NORMALIZED filled from
    Application Status (mostly IN PLAN CHECK / VOID / WITHDRAWN).
  - CLOSED / PERMIT PRINTED rows with Application Status VOID /
    WITHDRAWN / SUPERSEDED / PERMIT EXPIRED incorrectly labeled
    Final / Active → FIXED to Inactive; spurious FINAL_DATE cleared.
  - PERMIT_DATE entirely missing upstream despite Issue Date on
    most permit_status rows → FILLED from Issue Date.
  - Missing / drifted FINAL_DATE on Final rows filled or corrected
    from inspection history (including APPROVED WITH EXCEPTION).

Not repairable from DATA:
  - fees_detail rows have no Issue Date / inspections → PERMIT_DATE
    and FINAL_DATE stay missing (FILE_DATE already present).
  - Some Final (CLOSED) rows have empty or non-successful inspection
    history → FINAL_DATE stays missing (Last Maintained is a
    maintenance stamp, not used as completion).
  - Active/Final rows with blank Issue Date → PERMIT_DATE stays
    missing.
"""
...
def _final_date_from_inspections(insp_detail) -> pd.Timestamp:
    """Latest successful FINAL/FNL/CLOSEOUT date; else latest non-NOC success."""
...
def _repair_permit_status(row, d: dict, repairs: dict) -> None:
    ...
    # Prefer inspection-derived completion; leave an existing FINAL_DATE
    # alone when DATA has no successful inspection candidate.
    ...
    if issue is not pd.NaT and not pd.isna(issue):
        if effective_status in ("Active", "Final", "Inactive"):
            ...
            # Rare pre-issue stamps on PLAN CHECK / TO BE ISSUED — keep if
            # present in DATA, but do not invent; clear only if current
            # PERMIT_DATE exists with no Issue Date support (handled below).
            _apply_date(repairs, row, "PERMIT_DATE", issue)
    elif effective_status == "In Review":
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pompano Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pompano Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pompano Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Pompano Beach records: {len(city):,}\n")
    ...
    # Unmapped Application Status on fees_detail
    n_unmapped = 0
    unmapped_apps = {}
    for idx in repaired.index:
        d = _safe_parse(repaired.at[idx, "DATA"]) or {}
        schema = repaired.at[idx, "INFERRED_SCHEMA"]
        if not str(schema).startswith("fees_detail"):
            continue
        detail = d.get("detail") or {}
        app = detail.get("Application Status") if isinstance(detail, dict) else None
        if _map_app(app) is None and not (
            str(app or "").strip().upper() in _INACTIVE_APP_OVERRIDE
        ):
            n_unmapped += 1
            unmapped_apps[app] = unmapped_apps.get(app, 0) + 1
    print(f"Unmapped fees_detail Application Status: {n_unmapped} {unmapped_apps}")

    if agent_data_path:
        out_path = os.path.join(
            agent_data_path, "pompano_beach_repaired_sample.parquet"
        )
