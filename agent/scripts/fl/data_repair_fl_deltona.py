"""Data repair for Deltona (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Deltona DATA is a Tyler EnerGov payload with top-level keys contacts,
details, entity, fees, processing_status, and optionally reviews / holds /
...
Canonical fields:
  ...
  - entity.CaseStatus (fallback details.PermitStatus)
      → STATUS_NORMALIZED
    APPROVED is issuance-gated (Active only when IssueDate / Issued;
    otherwise In Review). ISSUED / ISSUED ONLINE / APPROVED rows that
    already carry FinalDate / FinalizeDate are upgraded to Final.
...
Known issues repaired:
  - Null STATUS_NORMALIZED for SUBMITTED ON-LINE / UNDER E-REVIEW /
    ENGINEERING REVIEW (never mapped from STATUS_ORIGINAL) → FILLED
    as In Review.
  - APPROVED animal-tag shells labeled Active with Issued=False and
    blank IssueDate → FIXED to In Review.
  - ISSUED / ISSUED ONLINE / APPROVED rows that already carry
    FinalDate while STATUS_NORMALIZED stayed Active → FIXED to Final;
    FINAL_DATE retained.
  - Spurious FINAL_DATE on Inactive (EXPIRED / CANCELLED) cleared.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - Three FINALED shells lack FinalDate / FinalizeDate → FINAL_DATE
    stays missing.
  - APPROVED (and other) shells with Issued=False / null IssueDate
    stay without PERMIT_DATE after status correction to In Review.
"""
...
def _is_issued_flag(d: dict) -> bool:
    ...
    if "Issued" in details:
        return bool(details.get("Issued"))
    entity = d.get("entity") if isinstance(d.get("entity"), dict) else {}
    if "Issued" in entity:
        return bool(entity.get("Issued"))
    return False
...
# Exact CaseStatus → STATUS_NORMALIZED (lookup is case-insensitive via
# _expected_status; keys are stripped uppercase forms as seen in DATA).
_STATUS_MAP = {
    # Final — completed / finaled
    "FINALED": "Final",
    "COMPLETED": "Final",
    # Active — issued (APPROVED handled with issuance gating below)
    "ISSUED": "Active",
    "ISSUED ONLINE": "Active",
    "APPROVED": "Active",
    # In Review — pre-issuance
    "SUBMITTED": "In Review",
    "SUBMITTED ON-LINE": "In Review",
    "UNDER E-REVIEW": "In Review",
    "UNDER REVIEW": "In Review",
    "ENGINEERING REVIEW": "In Review",
    "READY TO ISSUE": "In Review",
    # Inactive — canceled / void / denied / expired
    "VOID": "Inactive",
    "CANCELLED": "Inactive",
    "CANCELED": "Inactive",
    "EXPIRED": "Inactive",
...
# Statuses that become Final when FinalDate / FinalizeDate is already set
# (agency often leaves CaseStatus at ISSUED / APPROVED after finaling).
_FINAL_UPGRADE_FROM = {
    "ISSUED",
    "ISSUED ONLINE",
    "APPROVED",
...
# APPROVED is Active only when an issuance date (or Issued flag) is present.
_ISSUANCE_GATED = {
    "APPROVED",
}
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    has_issue = _present(issue) or _is_issued_flag(d)
    has_final = _present(final)

    if has_final and raw_key in _FINAL_UPGRADE_FROM:
        return "Final"

    if raw_key in _ISSUANCE_GATED:
        return "Active" if has_issue else "In Review"

    if raw_key in _STATUS_MAP:
        return _STATUS_MAP[raw_key]
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Deltona permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Deltona".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Deltona"].copy()

    print(f"Deltona records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_deltona_repaired.parquet")
