"""Data repair for Highland Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Highland Beach DATA is a SmartGov community portal payload (same family
as Sunny Isles Beach / Port Orange / Auburndale) with top-level keys
...
Canonical mappings:
  - DATA["Build Status"] (Expired* / Issued / …), with Closed-date /
...
Known issues repaired:
  - Expired* rows left with null STATUS_NORMALIZED → FILLED Inactive.
  - Null Build Status applications with Submitted/Created only
    → FILLED In Review.
  - Inactive Expired shells with blank Issued but usable Approved
    → FILLED PERMIT_DATE from Approved.
  - Spurious FINAL_DATE on non-Final (none observed in sample) cleared.

Not repairable from DATA:
  - ~98% of sample rows are empty SmartGov shells (null Build Status,
    blank My Project dates, null Permit Number / Permit Type) → status
    and dates remain missing.
  - No Closed / Finaled Build Status and all Closed stamps blank → no
    Final records and FINAL_DATE stays empty.
"""
...
# Keys are lower-case; Highland Beach Build Status is title-ish
# ("Issued", "Expired: M/D/YYYY").
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "issued": "Active",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Highland Beach (FL) permit records using information from the raw
    DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Highland Beach". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Highland Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Highland Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_highland_beach_repaired.parquet"
