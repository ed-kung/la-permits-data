"""Data repair for Pensacola (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pensacola DATA is a flat MyGovernmentOnline (MGO) project payload
...
_STATUS_MAP = {
    ...
    # Active / issued
    "Issued": "Active",
    # In review / pre-issuance (Open is distinct from Issued in this portal)
    "Open": "In Review",
    "In Review": "In Review",
    # Inactive
    "Expired": "Inactive",
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Pensacola MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pensacola permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pensacola".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pensacola") & (df["STATE"] == "FL")
    ].copy()

    print(f"Pensacola records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(agent_data_path, "pensacola_repaired_sample.parquet")
