"""Data repair for North Lauderdale (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

North Lauderdale DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - CaseStatus=Issued with PermitStatus=Complete and FinalizeDate
    ...
    FinalizeDate / final inspection.
  - Spurious PERMIT_DATE on In Review rows that still carry IssueDate
    → FIXED clear.
  - Spurious FINAL_DATE on In Review 40-year inspection shells that
    carry FinalDate without being Complete → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row
    (details.ApplyDate can be +1 calendar day from UTC offset).
  - Two Complete shells with Issued=False and blank IssueDate stay
    without PERMIT_DATE (no issuance stamp in DATA).
  - Three Complete shells where FinalDate precedes IssueDate remain
    as agency date-order quirks (both stamps match DATA).
"""
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    and a final date is present, upgrade to Final.
    """
    case_raw = None
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    North Lauderdale permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "North Lauderdale". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "North Lauderdale") & (df["STATE"] == "FL")
    ].copy()

    print(f"North Lauderdale records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_north_lauderdale_repaired.parquet"
        )
