"""Data repair for St. Lucie County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

St. Lucie County DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - Unmapped CaseStatus values (On Hold (See Conditions), Admin
    Closeout) left STATUS_NORMALIZED null → FILLED.
  - CaseStatus=Issued while PermitStatus=Completed\\Finaled and a
    finalize stamp exists (entity lags details) → FIXED to Final;
    FINAL_DATE FILLED from FinalizeDate / final inspection.
  - Missing PERMIT_DATE on Issued rows that carry IssueDate → FILLED.
  - Spurious PERMIT_DATE on In Review / Notified / Fees Due rows that
    still carry IssueDate → FIXED clear.
  - Spurious FINAL_DATE on Active / In Review shells that still show
    CaseStatus=Issued / Notified → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Active / Final shells with Issued=False and blank IssueDate stay
    without PERMIT_DATE (no issuance stamp in DATA).
  - Admin Closeout Final shell has FinalizeDate but no IssueDate →
...
# Note: agency uses a literal backslash in "Completed\\Finaled".
_STATUS_MAP = {
    # Final
    "Completed\\Finaled": "Final",
    "Complete": "Final",
    "Closed": "Final",
    ...
    "Notified": "In Review",
    "On Hold": "In Review",
    "On Hold (See Conditions)": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Submitted": "In Review",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Completed\\Finaled) and a final date is present, upgrade to Final.
    """
    case_raw = None
    ...
    final = _final_date(d)
    permit_mapped = _map_status(permit_raw)
    # Entity lagged as Issued while details show Completed\\Finaled + stamp.
    if mapped == "Active" and permit_mapped == "Final" and _present(final):
        return "Final"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    St. Lucie County permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "St. Lucie County". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "St. Lucie County") & (df["STATE"] == "FL")
    ].copy()

    print(f"St. Lucie County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_st_lucie_county_repaired.parquet"
        )
