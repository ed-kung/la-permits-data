"""Data repair for Aventura (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Aventura DATA is an Accela-style portal payload with top-level keys
``fees``, ``contacts``, ``site_info``, ``inspections``, ``permit_info``,
and ``search_data``. Content variants (INFERRED_SCHEMA):
  ...
  - accela_status_only:     PermitStatus set, no usable dates
  - accela_shell:           empty permit_info (status + dates blank);
                            search_data.Application may still exist
  - missing / unknown

Canonical mappings:
  - permit_info.PermitStatus               → STATUS_NORMALIZED
  - PermitAppliedDate else
    search_data.Application                → FILE_DATE
  - PermitIssuedDate else search_data.Issued
    else PermitApprovedDate (Active/Final) → PERMIT_DATE
  - PermitFinaledDate else latest approved
    final-ish inspection else latest
    approved inspection (Final only)       → FINAL_DATE

Known issues repaired:
  - 10 shell rows missing FILE_DATE (blank PermitAppliedDate) but with
    search_data.Application → FILLED.
  - FINAL_DATE missing on Final rows with blank PermitFinaledDate
    (all CERTIFICATE OF OCCUPANCY / COMPLETION plus a few FINALED)
    → FILLED from approved final inspections when available.

Not repairable from DATA:
  - 10 shell rows have blank PermitStatus and no search STATUS →
    STATUS_NORMALIZED stays missing.
  - Active/Final rows with neither Issued nor Approved dates
    (1,777 in sample, mostly CONV FINALED without issuance fields) →
    PERMIT_DATE stays missing. Fee Paid Date is not a safe proxy
    (rarely equals Issued when both exist).
  - A few Final rows with blank PermitFinaledDate and no approved
...
_PASS_RESULTS = {"APPROVED", "APPROVED W EXCEPTION"}
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    approved = _safe_to_datetime(pi.get("PermitApprovedDate"))
    finaled = _safe_to_datetime(pi.get("PermitFinaledDate"))
    sd_application = _safe_to_datetime(sd.get("Application"))
    sd_issued = _safe_to_datetime(sd.get("Issued"))

    # -- FILE_DATE ← PermitAppliedDate else search_data.Application --
    file_candidate = applied
    if file_candidate is pd.NaT or pd.isna(file_candidate):
        file_candidate = sd_application
    _apply_date(repairs, row, "FILE_DATE", file_candidate)

    # -- PERMIT_DATE ← Issued else search Issued else Approved (Active/Final) --
    has_issued = issued is not pd.NaT and not pd.isna(issued)
    has_sd_issued = sd_issued is not pd.NaT and not pd.isna(sd_issued)
    has_approved = approved is not pd.NaT and not pd.isna(approved)
    ...
    if has_issued:
        ...
        if pd.isna(row["PERMIT_DATE"]):
            repairs["PERMIT_DATE"] = sd_issued
            repairs["PERMIT_DATE_FLAG"] = "FILLED"
        elif not _dates_equal(row["PERMIT_DATE"], sd_issued):
            repairs["PERMIT_DATE"] = sd_issued
            repairs["PERMIT_DATE_FLAG"] = "FIXED"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Aventura permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Aventura".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Aventura") & (df["STATE"] == "FL")
    ].copy()

    print(f"Aventura records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(agent_data_path, "aventura_repaired_sample.parquet")
