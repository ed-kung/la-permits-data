"""Data repair for Ormond Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Ormond Beach DATA is the same city-portal family as St. Petersburg /
Punta Gorda / Pompano Beach / Margate / North Port / Lake Mary. This
sample has three sub-schemas:
  ...
  - mini_set:      sparse application_status / address / parcel only
                   (no dates)

Canonical mappings:
  - Status for Permit Number (permit_status), overridden to
    Inactive when Application Status is CANCELLED / EXPIRED PERMIT /
    REVOLKED PERMIT / REJECTED / etc.
                                                     → STATUS_NORMALIZED
  - Application Status (fees_detail / mini_set)      → STATUS_NORMALIZED
  ...
  - Later of (a) successful FINAL/CO inspection or
    ...
    rows                                             → FINAL_DATE
    (insp_status_detail is often truncated at 8 rows,
    so Permit Date is needed for C.O. / admin close)

INFERRED_SCHEMA is ``permit_status_{sp_slug}``,
``fees_detail_{app_slug}``, or ``mini_set_{app_slug}``.

Known issues repaired:
  - ~1,024 fees_detail / mini_set rows with null STATUS_NORMALIZED
    filled from Application Status (ADMINISTRATIVE CLOSURE, CLOSED,
    CANCELLED, IN PLAN CHECK, etc.).
  - CLOSED + CANCELLED / EXPIRED PERMIT / REVOLKED PERMIT incorrectly
    labeled Final or Active → FIXED to Inactive.
  ...
  - Final rows missing / wrong FINAL_DATE filled from inspections
    ...
    (admin close / C.O. stamp). Truncated inspection lists had
    produced early intermediate dates as FINAL_DATE on many
    C.O. ISSUED rows.

Not repairable from DATA:
  - mini_set rows (23) have no Application Date → FILE_DATE stays
    missing.
  - Active/Final/Inactive with blank Issue Date → PERMIT_DATE
    cleared or left missing (cannot invent issuance from Permit Date).
  - fees_detail / mini_set Finals with no Issue Date / inspections /
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if data_dict.get("mini_set") is True or (
        "application_status" in keys and "permit_status_detail" not in keys and "detail" not in keys
    ):
        app = data_dict.get("application_status")
        return f"mini_set_{_slug(app)}"
...
# Application Status (fees_detail / mini_set / top-level detail)
_APP_MAP = {
    "CLOSED": "Final",
...
def _final_date_candidate(d: dict, detail: dict) -> pd.Timestamp:
    ...
    insp_status_detail is frequently truncated at 8 rows for C.O. ISSUED
    records, so take the later of inspection-derived and Permit-Date
    candidates when both exist.
    """
...
def _repair_mini_set(row, d: dict, repairs: dict) -> None:
    """Repair a mini_set shell (status only; no date fields in DATA)."""
    app = (d.get("application_status") or "").strip()
    if app.upper() in _INACTIVE_APP_OVERRIDE or app in _INACTIVE_APP_OVERRIDE:
        expected = "Inactive"
    else:
        expected = _map_app(app)
    ...
    # No Application / Issue / Permit dates available.
    if effective_status in ("Active", "Final", "Inactive", "In Review"):
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Ormond Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Ormond Beach". Must contain
    ...
    for idx in out.index:
        ...
        elif schema.startswith("mini_set"):
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Ormond Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Ormond Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_ormond_beach_repaired.parquet"
