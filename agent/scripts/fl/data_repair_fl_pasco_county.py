"""Data repair for Pasco County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pasco County DATA is an Accela Citizen Access payload. Rows typically
include ``status``, ``date``, ``search_data``, ``tasks``, and often
``inspections``. Legacy / migrated rows may have empty task event
histories (``accela_shell``) or empty ``status`` with no workflow.

Canonical mappings:
  - DATA.status                                              → STATUS_NORMALIZED
  - DATA.date / search_data.Date                             → FILE_DATE
  - Earliest Permit Issuance / Permit Issued (else
    Application Submittal marked Permit Issued / Issue
    Permit)                                                  → PERMIT_DATE
  - Permit Completion Review Final Release / Final
    CO/CC/Meter Release; else Closed / Case Complete;
    else Inspection Final Inspection Complete; else
    final inspection Pass dates                              → FINAL_DATE

Known issues repaired:
  - STATUS_NORMALIZED derived from STATUS_ORIGINAL lags
    DATA.status (e.g. permit issued / conditional approval /
    pending client while Accela shows Closed / Abandoned /
    Expired Permit) → FIXED.
  - Unmapped statuses (Abandoned, Active Mainframe) left null
    → FILLED.
  - Missing PERMIT_DATE filled from Permit Issuance / Issued
    workflow (and Application Submittal issuance marks) for
    Active / Final rows.
  - Incorrect PERMIT_DATE overwritten with Accela issuance
    event date → FIXED.
  - Missing FINAL_DATE filled for Final rows from completion /
    Closed / final-inspection signals (including shell rows
    that only retain inspection Status Date).
  - Incorrect FINAL_DATE (often final-inspection Pass while a
    later Final Release / Closed event exists) → FIXED.
  - Spurious FINAL_DATE on non-Final rows → cleared (FIXED).

Not repairable / left as-is:
  - Rows with empty DATA.status and no workflow (legacy
    ``accela_shell`` stubs, often FILE_DATE 2016-04-08) →
    STATUS_NORMALIZED / PERMIT_DATE / FINAL_DATE stay missing.
  - Complete / Closed shell rows with no dated issuance events
    → PERMIT_DATE stays missing.
  - Final rows with neither completion/Closed events nor a
...
_STATUS_MAP = {
    # Final — closed / complete
    "Closed": "Final",
    "Complete": "Final",
    # Active — issued / ongoing
    "Permit Issued": "Active",
    "Issued": "Active",
    "Active Mainframe": "Active",
    # In Review — intake / pending issuance
    "NEW_STATUS": "In Review",
    "Ready To Issue": "In Review",
    "Submitted": "In Review",
    # Inactive — expired / cancelled / withdrawn / deleted
...
def _file_date_from_data(d: dict):
    ...
    intake = _event_dates(
        d.get("tasks") or [],
        {"Application Submittal", "Application Acceptance"},
        lambda m: "accepted" in (m or "").strip().lower(),
    )
    if intake:
        return min(intake)
    return pd.NaT
...
def _permit_date_from_tasks(tasks: list):
    """Earliest issuance date from Permit Issuance / Application Submittal."""

    def _is_issued(m: str) -> bool:
        ml = (m or "").strip().lower()
        if "ready" in ml or "revision" in ml or "note" in ml:
            return False
        return ml in ("permit issued", "issued", "issue permit", "complete")
    ...
    # Older / alternate workflows mark issuance on Application Submittal.
    app_issued = _event_dates(
        tasks,
        {"Application Submittal"},
        lambda m: (m or "").strip().lower() in ("permit issued", "issue permit"),
    )
    if app_issued:
        return min(app_issued)
...
def _final_date_from_data(d: dict):
    """Best finalization date from completion / Closed / inspections."""
    tasks = d.get("tasks") or []

    def _is_final_release(m: str) -> bool:
        ml = (m or "").strip().lower()
        if any(x in ml for x in ("pending", "tbd", "not required", "cancel", "void")):
            return False
        return ("final" in ml and "release" in ml) or ml in {
            "final co/cc/meter release",
            "final release",
        }

    completion = _event_dates(tasks, {"Permit Completion Review"}, _is_final_release)
    if completion:
    ...
    # Last resort for trade finals labeled without "Final" in the title
    # (e.g. older shell imports where only a single Pass inspection remains).
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for one Pasco County record."""
    tasks = d.get("tasks") or []
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pasco County permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pasco County".  Must contain
...
if __name__ == "__main__":
    ...
    pasco = df[df["JURISDICTION"] == "Pasco County"].copy()

    print(f"Pasco County records: {len(pasco):,}\n")

    repaired = data_repair(pasco)
    ...
    for field in ["STATUS_NORMALIZED", "FILE_DATE", "PERMIT_DATE", "FINAL_DATE"]:
        ...
        before_missing = pasco[field].isna().sum()
    ...
    print("  Before:")
    for s, c in pasco["STATUS_NORMALIZED"].value_counts(dropna=False).items():
