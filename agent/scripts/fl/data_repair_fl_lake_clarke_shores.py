"""Data repair for Lake Clarke Shores (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lake Clarke Shores DATA is a flat MyGovernmentOnline (MGO) project
payload (``ProjectStatus``, ``DateCreated``, ``DateIssued``,
applicant/site fields, etc.). Schema variants:
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Lake Clarke
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lake Clarke Shores permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lake Clarke Shores".
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Lake Clarke Shores") & (df["STATE"] == "FL")
    ].copy()

    print(f"Lake Clarke Shores records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_lake_clarke_shores_repaired.parquet"
