"""Data repair for Auburndale (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Auburndale DATA is a SmartGov community portal payload with top-level
keys ``Department``, ``My Project``, ``Permit Type``, ``Build Status``,
...
Canonical mappings:
  - DATA["Build Status"] (Expired* / Closed / Approved / …)
                                           → STATUS_NORMALIZED
  ...
  - My Project.Closed (fallback latest
    passed/completed Final inspection)     → FINAL_DATE

Known issues repaired:
  - 5 rows with null STATUS_NORMALIZED and Build Status
    ``Expired: 10/7/2022`` → FILLED as Inactive (other Expired*
    rows were already mapped).
  - FINAL_DATE universally missing; Closed is always the SmartGov
    blank placeholder (`` - -``). Final rows with a Passed /
    Approved / Completed inspection whose name contains ``Final``
    → FILLED from that inspection date.

Not repairable from DATA:
  - FILE_DATE already matches Submitted on every sample row.
  - Active (Build Status Approved) rows have blank Issued and
    Approved stamps and no inspections → PERMIT_DATE stays missing.
  - 5 Closed rows with blank Issued/Approved and empty inspections
    → PERMIT_DATE stays missing.
  - Closed rows with no usable Final inspection (and blank Closed)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Auburndale (FL) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Auburndale". Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Auburndale") & (df["STATE"] == "FL")].copy()

    print(f"Auburndale records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_auburndale_repaired.parquet"
