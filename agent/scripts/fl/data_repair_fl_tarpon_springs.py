"""Data repair for Tarpon Springs (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Tarpon Springs DATA is the same city-portal family as Winter Garden /
Oviedo / Lake Mary, with two sub-schemas in this sample:
...
Known issues repaired:
  ...
  - 7 Active rows whose Status for Permit Number is
    FINAL INSPECTION COMPLETE (stale STATUS_ORIGINAL
    "permit printed") → FIXED to Final.
  - PERMIT_DATE was populated from the portal
    "Permit Date" field (often a later admin /
    closeout stamp, with a large 07/21/15 batch)
    instead of "Issue Date" → FIXED to Issue Date
  ...
  - FINAL_DATE filled / corrected from latest non-NOC
    successful inspection; year-typo result dates
    (~365 days off schedule) use the later date.
...
def _issue_date(detail: dict):
    """Permit issuance date (Tarpon Springs field name: Issue Date)."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Tarpon Springs permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Tarpon Springs".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Tarpon Springs"].copy()

    print(f"Tarpon Springs records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "tarpon_springs_permits_repaired.parquet"
        )
