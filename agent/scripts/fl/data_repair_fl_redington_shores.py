"""Data repair for Redington Shores (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Redington Shores DATA is a SmartGov community portal payload (same family
as Longwood / Lighthouse Point / Highland Beach) with top-level keys
...
Canonical mappings:
  - DATA["Build Status"] (Expired* sticky Inactive; Closed / Issued /
    Disapproved / Pending / Under Review / Payment Pending /
    Resubmittal / …), with Closed-date / Issued-date overrides
                                                  → STATUS_NORMALIZED
  ...
  - My Project.Closed (fallback latest passed
    Final / COO inspection)                       → FINAL_DATE

Known issues repaired:
  - ~1,615 null STATUS_NORMALIZED rows (mostly null Build Status /
    STATUS_ORIGINAL) inferred from My Project dates → FILLED
    (Closed→Final, Issued→Active, else Submitted/Created/Approved→
    In Review).
  - Closed Build Status (or Closed date) still labeled Active /
    In Review because STATUS_ORIGINAL lagged as "issued" / "pending"
    → FIXED to Final; FINAL_DATE filled from Closed.
  - Expired* still labeled Active (STATUS_ORIGINAL="issued") or left
    null → FIXED/FILLED Inactive.
  - Issued Build Status labeled In Review / Inactive / null →
    FIXED/FILLED Active; PERMIT_DATE filled from Issued.
  - Disapproved labeled In Review → FIXED Inactive.
...
Not repairable from DATA:
  - ~8 fully empty SmartGov shells (no Build Status, Permit Number,
  ...
  - Final rows with blank Closed and no passed Final/COO inspection →
    FINAL_DATE stays missing (or keeps a pre-existing scraped value
    when no Closed source exists).
"""
...
# Keys are lower-case. Redington Shores uses title-case Build Status
# values ("Issued", "Closed", "Pending", "Under Review", "Disapproved",
# "Payment Pending", "Resubmittal") plus "Expired: M/D/YYYY".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "issued": "Active",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive for Expired / DENIED / Disapproved when no Closed
    date. Explicit CLOSED / FINALED → Final. Otherwise Closed date →
    Final, Issued date → Active (unless Build Status maps to Inactive
    without an Issued stamp). Null Build Status falls back to date
    inference.
    """
    ...
    if bs == "Expired" or (bs is not None and bs.lower() in ("denied", "disapproved")):
        # Disapproved / Expired remain Inactive even if earlier Issued
        # dates exist, unless the project was later Closed/Finaled.
        if closed is not pd.NaT or (bs is not None and bs.lower() in ("closed", "finaled")):
            return "Final"
        return "Inactive"
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if not pd.isna(current_permit):
        ...
        # Inactive includes Expired / Disapproved shells that were
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Redington Shores (FL) permit records using information from the raw
    DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Redington Shores". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Redington Shores") & (df["STATE"] == "FL")
    ].copy()

    print(f"Redington Shores records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_redington_shores_repaired.parquet"
