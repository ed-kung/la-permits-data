"""Data repair for Altamonte Springs (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Altamonte Springs DATA is a Tyler EnerGov-style payload with top-level
keys ``entity``, ``details``, ``contacts``, ``processing_status``, and
...
Known issues repaired:
  - STATUS_NORMALIZED lagged behind CaseStatus for Finaled / Closed /
    Issued rows (STATUS_ORIGINAL still "issued" / "submitted - online"
    / "in review") → FIXED to Final or Active.
  - STATUS_NORMALIZED missing for ``In Production`` and
    ``Voided Work Class Incorrect`` → FILLED as In Review / Inactive.
  - Missing PERMIT_DATE filled from IssueDate for Active / Final rows
    when IssueDate is present.
  - Missing FINAL_DATE filled from FinalDate / FinalizeDate for Final
    rows (notably Finaled rows previously labeled Active).
  - Spurious FINAL_DATE on Inactive (Expired / Void) rows → cleared
    (FIXED).

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for all sample records.
  - Most Converted / Closed / Complete Final rows lack FinalDate /
    FinalizeDate → FINAL_DATE stays missing (Converted especially).
  - Public-records ``Complete`` and other Final shells with Issued=False
    / null IssueDate → PERMIT_DATE cannot be filled.
"""
...
_STATUS_MAP = {
    ...
    "Void": "Inactive",
    "Denied": "Inactive",
    "Voided Work Class Incorrect": "Inactive",
    # In Review
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # historical stamp (e.g. Expired after finaling) and must not remain on
    # FINAL_DATE.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Altamonte Springs permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Altamonte Springs".  Must
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Altamonte Springs"].copy()

    print(f"Altamonte Springs records: {len(city):,}\n")
    ...
    print("\nPERMIT_DATE_FLAG detail:")
    for flag in ["FILLED", "FIXED"]:
        sub = repaired[repaired["PERMIT_DATE_FLAG"] == flag]
        print(f"  {flag} ({len(sub)}):")
        for idx in sub.index[:10]:
            d = _safe_parse(city.loc[idx, "DATA"])
            cs = _case_status(d) if d else None
            print(
                f"    {cs!r}: before={city.loc[idx, 'PERMIT_DATE']!r} "
                f"after={repaired.loc[idx, 'PERMIT_DATE']!r} "
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "altamonte_springs_repaired_sample.parquet")
