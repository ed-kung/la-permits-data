"""Data repair for Escambia County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Escambia County DATA is a flat MyGovernmentOnline (MGO) project payload
...
Known issues repaired:
  - Null STATUS_NORMALIZED on ``Closed - No Inspection`` → FILLED Final.
  - ``COC Issued`` (Certificate of Completion) labeled Active → FIXED
    to Final.
  - ``Contract Terminated`` labeled In Review → FIXED to Inactive.
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    lower = raw.lower()
    if "coc" in lower or "co issued" in lower or "final" in lower or "closed" in lower:
        return "Final"
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Escambia MGO
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Escambia County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Escambia County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Escambia County") & (df["STATE"] == "FL")
    ].copy()

    print(f"Escambia County records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(
            agent_data_path, "escambia_county_repaired_sample.parquet"
        )
