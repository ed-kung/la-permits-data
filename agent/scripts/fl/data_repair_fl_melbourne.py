"""Data repair for Melbourne (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Melbourne DATA is a Tyler EnerGov payload with top-level keys contacts,
details, entity, fees, processing_status, and optionally reviews /
holds / attachments / more_info. One shell lacks fees.
...
Key-set variants (INFERRED_SCHEMA prefixes):
  ...
  - energov_minimal: entity/details/contacts/processing_status only
...
Known issues repaired:
  - Null STATUS_NORMALIZED for ``Awaiting Review`` → FILLED In Review.
  - Stale STATUS_ORIGINAL / STATUS_NORMALIZED lagging CaseStatus
    (issued kept after Complete; fees due / in review kept after
    Expired / Issued / Complete; etc.) → FIXED.
  - CaseStatus=Issued with PermitStatus=Complete and FinalDate
    (entity lags details) → FIXED to Final.
  - PERMIT_DATE filled from IssueDate for Active / Final (and Inactive
    when issued); cleared on In Review when a spurious IssueDate
    appears.
  - FINAL_DATE filled for Final from FinalDate / FinalizeDate /
    final inspection; cleared on non-Final (incl. Expired shells that
    still carry FinalDate).

Not repairable from DATA:
  - FILE_DATE already populated for all sample rows; entity ApplyDate
    is the canonical source (details ApplyDate can be +1 calendar day
    from UTC offset).
  - Some Expired / Issued shells lack IssueDate → PERMIT_DATE stays
    missing for those Inactive / rare Active rows.
  - One Complete shell lacks FinalDate / FinalizeDate but has passed
    final inspections → filled via processing_status; residual Final
    gaps only if no final date and no usable final inspection.
"""
...
_STATUS_MAP = {
    ...
    "Expired": "Inactive",
    "Void": "Inactive",
    "Void -Duplicate": "Inactive",
    "Denied": "Inactive",
    "Withdrawn": "Inactive",
    "Plan Approval Expired": "Inactive",
    "Permit Application Expired": "Inactive",
}
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Prefer entity.CaseStatus. When entity lags details (Issued vs Complete)
    and a final date is present, upgrade to Final. When CaseStatus and
    PermitStatus disagree otherwise, prefer the mapped PermitStatus if
    CaseStatus is missing; otherwise keep CaseStatus.
    """
    case_raw = None
    ...
    final = _final_date(d)
    # Entity lagged as Issued while details show Complete + FinalDate.
    if (
        mapped == "Active"
        and _present(final)
        and permit_raw
        and permit_raw.lower() == "complete"
    ):
        return "Final"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Melbourne permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Melbourne".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Melbourne") & (df["STATE"] == "FL")
    ].copy()

    print(f"Melbourne records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_melbourne_repaired.parquet")
