"""Data repair for Satellite Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Satellite Beach DATA is a CitizenServe-style municipal portal payload.
Every sample row has the same colon-suffixed shell (``Status:``,
``Permit #:``, ``Permit Details``, ``Reviews``, ``Inspections``). Usable
issue stamps live under ``Permit Details["Issue Date:"]`` (top-level
``Issue Date`` is also populated on some rows). ``Reviews`` is usually a
single dict rather than a list. Application / submittal dates come from
the earliest Completeness Check / Permit Review Start (else Completion)
on or before Issue. Final / sign-off is proxied by the latest Pass /
...
Canonical fields:
  ...
  - Latest Pass/Approved inspection
    (floored at Issue when present)       → FINAL_DATE

Key-set variants (INFERRED_SCHEMA prefixes):
  - portal_core: minimal colon-key portal shell (all sample rows)
...
Known issues repaired:
  - Admin Close left STATUS_NORMALIZED null → FILLED as Inactive.
  - Under Review / Online Application Received rows that already carry
    Issue Date → FIXED to Active (keep PERMIT_DATE).
  - FILE_DATE often equals Review Completion rather than earlier Review
    Start → FIXED; post-issue FILE cleared when no on/before-Issue
    application source (legacy reopen Completeness Checks).
  - FINAL_DATE missing on every sample row → FILLED for Final rows
    with Pass/Approved inspections.
  - Spurious PERMIT_DATE / FINAL_DATE on pure In Review cleared.

Not repairable from DATA:
  - Shells with empty / undated Reviews → FILE_DATE stays missing.
  - Issued / Approved / Final shells with blank Issue Date →
    PERMIT_DATE stays missing.
  - Final shells without Pass/Approved inspections → FINAL_DATE
    stays missing.
"""
...
_NON_FILE_TASK_RE = re.compile(
    ...
    r"certificate review|admin co fee|remodel final|wire lath",
    re.IGNORECASE,
)
...
def _latest_passed_inspection(d: dict):
    """Latest Pass/Approved inspection date (any trade type)."""
...
def _normalize_reviews(d: dict) -> list:
    """CitizenServe often returns a single review dict instead of a list."""
...
def _file_date(d: dict):
    ...
    Upstream FILE_DATE frequently equals Review Completion rather than
    the earlier Completeness Check Start.
    """
    issue = _issue_date(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Satellite Beach permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Satellite Beach". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Satellite Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"Satellite Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_satellite_beach_repaired.parquet"
        )
