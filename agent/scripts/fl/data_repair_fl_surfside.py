"""Data repair for Surfside (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Surfside DATA is a Tyler EnerGov payload with top-level keys contacts,
details, entity, fees, processing_status, and optionally reviews /
holds / attachments / more_info.
...
treated as null. Most Surfside rows are the basic key set; a small
...
Known issues repaired:
  - Mis-normalized rows where STATUS_ORIGINAL is stale vs live
    CaseStatus (Complete→Active/Inactive, Issued→In Review,
    Expired→Active/In Review) → FIXED.
  - Missing FINAL_DATE on Complete shells that were mislabeled Active /
    Inactive → FILLED from FinalDate after reclassification.
  - Missing PERMIT_DATE on Issued shells that were labeled In Review
    → FILLED after status fix to Active.
  - Spurious FINAL_DATE on non-Final (Issued / Expired /
    Plan Approval Expired / Fees Paid) → cleared.
  - Spurious PERMIT_DATE on In Review (Fees Due / Fees Paid / In Review)
    → cleared.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - Many Issued / Complete shells have blank IssueDate
    (details.Issued=false) → PERMIT_DATE stays missing on those
    Active / Final rows.
  - Most Complete shells lack FinalDate / FinalizeDate; processing_status
    is usually empty or only "Legacy Inspection" → FINAL_DATE stays
    missing on the majority of Final rows.
  - A few ApplyDate > IssueDate inversions exist in source EnerGov
    dates and are preserved when both fields are kept.
"""
...
_STATUS_MAP = {
    # Final — completed / finaled
    "Complete": "Final",
    # Active — issued open permit
    "Issued": "Active",
    # In Review — pre-issuance / hold / fees / online review workflow
    "In Review": "In Review",
    "On Hold": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Surfside permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Surfside".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Surfside") & (df["STATE"] == "FL")
    ].copy()

    print(f"Surfside records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_surfside_repaired.parquet")
