"""Data repair for Palm Beach Shores (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Palm Beach Shores DATA is a flat MyGovernmentOnline (MGO) project payload
...
Status values observed in sample:
  ...
  - Pending (Under Review) / Stop Work Order → In Review
  - Expired → Inactive
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the Palm Beach
    # Shores MGO payload (DateUpdated is also the .NET sentinel on every
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Palm Beach Shores permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Palm Beach Shores".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Palm Beach Shores") & (df["STATE"] == "FL")
    ].copy()

    print(f"Palm Beach Shores records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_palm_beach_shores_repaired.parquet"
