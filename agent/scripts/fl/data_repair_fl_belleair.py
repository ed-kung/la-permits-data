"""Data repair for Belleair (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Belleair DATA is a CitizenServe / SmartGov-style payload with top-level
keys ``main``, ``extra``, and ``location``. Content variants
(INFERRED_SCHEMA):

  - citizenserve_draft:     unsubmitted drafts (main.status == 0)
  - citizenserve_building:  Building Permit forms
  - citizenserve_planning:  Variance Application forms
  - citizenserve_other:     residual form types (e.g. Temporary Use)
  - unknown / missing

Canonical mappings:
  - main.status (0/1/2/-1)                         → STATUS_NORMALIZED
  - main.dateSubmitted (else dateCreated;
    else extra['Date of Application'])            → FILE_DATE
  - (none reliable)                                → PERMIT_DATE
  - (none reliable)                                → FINAL_DATE

Known issues repaired:
  - FILE_DATE was derived from main.dateCreated. When dateSubmitted
    falls on a later calendar day (263 sample rows), overwrite with
    the submittal date.
...
Not repairable from DATA:
  ...
  - PERMIT_DATE is universally missing. No issuance / approval
    timestamp exists in main or extra; lastUpdatedDate and
    expirationDate are not safe approval proxies.
...
def _preferred_file_date(main: dict, extra: dict) -> Optional[date]:
    """Application/submittal date: dateSubmitted → dateCreated → Date of Application."""
    ...
    if created is not None:
        return created
    app = _safe_to_datetime(extra.get("Date of Application"))
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Belleair record."""
    main = _main(d)
    extra = _extra(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Belleair permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Belleair".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Belleair") & (df["STATE"] == "FL")].copy()

    print(f"Belleair records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(agent_data_path, "belleair_repaired_sample.parquet")
