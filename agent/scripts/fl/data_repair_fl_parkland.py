"""Data repair for Parkland (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Parkland DATA is an Accela-style portal payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED follows a stale STATUS_ORIGINAL for ~17 rows
    where DATA.PermitStatus is already FINALED / CANCELLED
    (e.g. STATUS_ORIGINAL=issued → Active while PermitStatus=FINALED).
  - ISSUED / APPROVED rows that already carry PermitFinaledDate are
    upgraded to Final.
  - PERMIT_DATE missing on Active/Final when Issued is blank but
    PermitApprovedDate exists → FILLED from approved date.
  - After Active/In Review → Final upgrades, FINAL_DATE is filled from
    PermitFinaledDate (or inspections).
  - Non-Final rows incorrectly carrying FINAL_DATE are cleared.

Not repairable from DATA:
  - 16 rows with blank PermitAppliedDate and no alternate application
    timestamp → FILE_DATE stays missing (issued/approved/inspection
    dates are not application dates).
  - Many Final rows have neither Issued nor Approved dates →
    PERMIT_DATE stays missing.
  - 2 Final rows have blank PermitFinaledDate and empty inspections →
...
_STATUS_MAP = {
    # Final / completed / certificate of occupancy
    "FINALED": "Final",
    "CO''ED": "Final",
    "CO'ED": "Final",
    "COED": "Final",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Parkland permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Parkland".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Parkland"].copy()

    print(f"Parkland records: {len(city):,}\n")
    ...
    if agent_data_path:
        out_path = os.path.join(agent_data_path, "parkland_repaired_sample.parquet")
