"""Data repair for Lighthouse Point (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lighthouse Point DATA is a SmartGov community portal payload (same family
as Highland Beach / Sunny Isles Beach / Port Orange / Auburndale) with
top-level keys ``Department``, ``My Project``, ``Permit Type``,
``Build Status``, ``Permit Number``, ``Permit Details``,
...
Canonical mappings:
  - DATA["Build Status"] (Expired* / DENIED sticky Inactive; Closed /
    Issued / SENT TO …), with Closed-date / Issued-date overrides
                                              → STATUS_NORMALIZED
  ...
  - My Project.Closed (fallback latest
    passed Final / COO inspection)            → FINAL_DATE

Known issues repaired:
  - Null STATUS_NORMALIZED on Expired*, DENIED, SENT TO *, Issued, and
    null-Build-Status rows with usable My Project dates → FILLED.
  - Closed Build Status (or Closed date) still labeled Active / In Review
    because STATUS_ORIGINAL lagged as "issued" / "ready to issue"
    → FIXED to Final; FINAL_DATE filled from Closed.
  - Expired* still labeled Active (STATUS_ORIGINAL="issued") → FIXED
    Inactive.
  - Issued Build Status labeled In Review / null (STATUS_ORIGINAL still
    "ready to issue" / "sent to …") → FIXED/FILLED Active; PERMIT_DATE
    filled from Issued.
  - Spurious FINAL_DATE on null-status shells that already have Closed
    → status becomes Final (FINAL kept); non-Final FINAL cleared.

Not repairable from DATA:
  - ~10 fully empty SmartGov shells (no Build Status, Permit Number,
    Permit Type, or My Project dates) → status/dates stay missing.
  - Legacy Closed rows with blank Issued / Closed and inspections whose
    Status is empty → PERMIT_DATE / FINAL_DATE stay missing.
  - Out-of-range Issued/Approved stamps (e.g. year 2090) rejected.
"""
...
# Keys are lower-case. Lighthouse Point mixes title case ("Issued",
# "Closed", "Ready To Issue") with UPPERCASE department queues
# ("SENT TO ELECTRICAL", "DENIED") and "Expired: M/D/YYYY".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "issued": "Active",
    ...
    "in review": "In Review",
    "under clerical review": "In Review",
    "awaiting corrections from applicant": "In Review",
    "ready to issue": "In Review",
    "ready to issue - awaiting payment from applicant": "In Review",
    "sent to administration": "In Review",
    "sent to electrical": "In Review",
    "sent to engineering": "In Review",
    "sent to fire": "In Review",
    "sent to plumbing": "In Review",
    "sent to structure": "In Review",
    "sent to zoning": "In Review",
}
...
def _map_build_status(bs: Optional[str]) -> Optional[str]:
    ...
    if mapped is not None:
        return mapped
    # Department queue labels beyond the explicit map.
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive for Expired / DENIED. Explicit CLOSED / FINALED →
    Final. Otherwise Closed date → Final, Issued date → Active. Null
    Build Status falls back to date inference.
    """
    bs = _normalize_build_status(d.get("Build Status"))
    closed = _mp_date(d, "Closed")
    issued = _mp_date(d, "Issued")

    if bs == "Expired" or (bs is not None and bs.lower() == "denied"):
        return "Inactive"

    if bs is not None and bs.lower() in ("closed", "finaled"):
        return "Final"
...
def _final_inspection_date(d: dict):
    """Latest passed Final / Certificate of Occupancy inspection date."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lighthouse Point (FL) permit records using information from the raw
    DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lighthouse Point". Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Lighthouse Point") & (df["STATE"] == "FL")
    ].copy()

    print(f"Lighthouse Point records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_lighthouse_point_repaired.parquet"
