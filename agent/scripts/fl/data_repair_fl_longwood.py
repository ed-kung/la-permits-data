"""Data repair for Longwood (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Longwood DATA is a SmartGov community portal payload (same family as
Lighthouse Point / Highland Beach / Auburndale) with top-level keys
...
Canonical mappings:
  - DATA["Build Status"] (Expired* sticky Inactive; Closed / Finaled /
    Issued / Return To Applicant / Ready To Issue / …), with Closed-date
    / Issued-date overrides                   → STATUS_NORMALIZED
...
Known issues repaired:
  - Null STATUS_NORMALIZED on Expired*, Return To Applicant, Issued, and
    null-Build-Status rows with usable My Project dates → FILLED.
  - Closed Build Status (or Closed date) still labeled Active / Inactive /
    In Review because STATUS_ORIGINAL lagged as "issued" / "expired" /
    "ready to issue" → FIXED to Final; FINAL_DATE filled from Closed.
  ...
  - Issued Build Status labeled In Review / null (STATUS_ORIGINAL still
    "pending" / "ready to issue" / "expired") → FIXED/FILLED Active;
    PERMIT_DATE filled from Issued.
  - FINAL_DATE matching Approved instead of Closed → FIXED to Closed;
    Active/Final missing PERMIT_DATE filled from Issued/Approved.
  - Spurious FINAL_DATE on non-Final rows → cleared.

Not repairable from DATA:
  - ~9 fully empty SmartGov shells (no Build Status, Permit Number,
    Permit Type, or My Project dates) → status/dates stay as scraped
    (two already carry Inactive + FILE_DATE from STATUS_ORIGINAL only).
  - Closed / Final rows with blank Issued and blank Approved →
    PERMIT_DATE stays missing.
  - Final rows with blank Closed and no passed Final/COO inspection →
...
# Keys are lower-case. Longwood uses title case Build Status values
# ("Issued", "Closed", "Ready To Issue", "Return To Applicant",
# "In review", "Pending", "Finaled") plus "Expired: M/D/YYYY".
_STATUS_MAP = {
    "closed": "Final",
    "finaled": "Final",
    "issued": "Active",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Longwood (FL) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Longwood". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Longwood") & (df["STATE"] == "FL")
    ].copy()

    print(f"Longwood records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_fl_longwood_repaired.parquet"
