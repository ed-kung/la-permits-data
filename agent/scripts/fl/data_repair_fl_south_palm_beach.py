"""Data repair for South Palm Beach (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

South Palm Beach DATA is a flat MyGovernmentOnline (MGO) project payload
(``ProjectStatus``, ``DateCreated``, ``DateIssued``, applicant/site
fields, etc.). Every sample row includes ``PaymentProcessorModule``
(``MGO``):
...
Status values observed in sample:
  ...
  - Pending (Under Review) / Fee Payment → In Review

Known issues / sample findings:
  - Upstream left all 21 ``Fee Payment`` rows with null
    STATUS_NORMALIZED → FILLED to In Review.
  - Remaining ProjectStatus values already matched STATUS_NORMALIZED.
  ...
  - DateUpdated is likewise always the .NET sentinel; no finaled /
    completion / CO date field exists → FINAL_DATE cannot be filled
    for Final rows.
"""
...
_STATUS_MAP = {
    ...
    "Permit Issued": "Active",
    "Issued": "Active",
    # In review / pre-issuance / administrative hold
...
def _repair_mgo(row, d: dict, repairs: dict) -> None:
    ...
    # -- FINAL_DATE --
    # No finaled / completion / CO timestamp exists in the South Palm
    # Beach MGO payload (DateUpdated is also the .NET sentinel on every
    # sample row). Clear only if a non-Final row somehow carries a
    # FINAL_DATE.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    South Palm Beach permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "South Palm Beach".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "South Palm Beach") & (df["STATE"] == "FL")
    ].copy()

    print(f"South Palm Beach records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_south_palm_beach_repaired.parquet"
