"""Data repair for DeSoto County (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

DeSoto County DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED for Ready to Issue - PENDING / Owner Builder,
    Requires Resubmittal, and one Issued shell → FILLED from CaseStatus.
  - Stale STATUS_ORIGINAL mismatches: Complete still labeled Active /
    In Review; Expired labeled Active; Issued labeled In Review → FIXED.
  - After Complete→Final upgrades, FINAL_DATE filled from FinalDate /
    FinalizeDate; one Active Complete row with a stale FINAL_DATE is
    FIXED to the current FinalDate.
  - PERMIT_DATE filled for Issued/Complete rows that had IssueDate but
    blank PERMIT_DATE under an In Review / null label.
  - Spurious FINAL_DATE on non-Final rows (Denied / Void closeout stamps,
    Issued shells carrying FinalDate) cleared.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - ~6 In Review pre-issuance shells (Ready to Issue*, Requires
    Resubmittal) have Issued=False and null IssueDate → PERMIT_DATE
    correctly stays missing.
  - processing_status inspections are not needed for FINAL_DATE; every
    Complete row already carries FinalDate / FinalizeDate.
"""
...
_STATUS_MAP = {
    ...
    # Active — issued
    "Issued": "Active",
    # In Review — pre-issuance / hold / stop-work
    "In Review": "In Review",
    "Submitted": "In Review",
    "On Hold": "In Review",
    "Ready to Issue": "In Review",
    "Ready to Issue - PENDING": "In Review",
    "Ready to Issue - Owner Builder": "In Review",
    "Requires Resubmittal": "In Review",
    "Stop Work Order": "In Review",
    # Inactive — expired / void / denied
    "Expired": "Inactive",
    "Void": "Inactive",
    "Denied": "Inactive",
}
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    # Complete rows always carry FinalDate in this sample; inspections unused.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    DeSoto County permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "DeSoto County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "DeSoto County"].copy()

    print(f"DeSoto County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_desoto_county_repaired.parquet")
