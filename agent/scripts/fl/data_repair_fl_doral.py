"""Data repair for Doral (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Doral DATA is a Tyler EnerGov payload with top-level keys contacts,
details, entity, fees, processing_status (always null in this sample),
and optionally reviews / holds / attachments / more_info.
...
Known issues repaired:
  - Null STATUS_NORMALIZED for ``Submitted - In Office`` (In Review)
    and ``Inspect - PW`` (Active) → FILLED from CaseStatus.
  - Spurious PERMIT_DATE on In Review (On Hold / Fees Due with an
    IssueDate) cleared.
  - Spurious FINAL_DATE on non-Final rows (Cancel / Void / Process
    Expired cancel stamps, plus sparse Active / In Review FinalDate
    stamps) cleared; Final rows aligned to FinalDate when present.
...
Not repairable from DATA:
  ...
  - processing_status is always null → no inspection-based FINAL_DATE
    fallback.
  - ~31 Final Closed/No Further Action + ~2 Active Inspect shells have
    Issued=False and null IssueDate → PERMIT_DATE stays missing.
  - ~2 Final Closed/No Further Action rows have null FinalDate /
    FinalizeDate → FINAL_DATE stays missing.
"""
...
_STATUS_MAP = {
    # Final — completed / closed / certificate issued
    ...
    "Submitted - In Office": "In Review",
    # Inactive — canceled / expired / void / denied
    "Cancel": "Inactive",
    "Denied": "Inactive",
    "Expired": "Inactive",
    "Process Expired": "Inactive",
    "Void": "Inactive",
}
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Doral permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Doral".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Doral"].copy()

    print(f"Doral records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_doral_repaired.parquet")
