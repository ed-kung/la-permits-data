"""Data repair for Kissimmee (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Kissimmee DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED for unmapped CaseStatus values
    (Submitted-Online, ROW Utilization Issued, Disapproved Review,
    Abandoned (Closed), Pending Permit Issuance, Masterfile Closed,
    etc.) → FILLED.
  - Incorrect STATUS_NORMALIZED for Superseded (was Final) → FIXED
    to Inactive.
  - Spurious FINAL_DATE on non-Final rows cleared:
      * Active Issued carrying FinalDate (agency stamp without
        Finaled / CO / Closed status)
      * Inactive On Hold-Expired Certifications/Registration
      * Abandoned (Closed) shells that become Inactive

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - PERMIT_DATE already matches IssueDate whenever present; Active /
    Final gaps mirror blank IssueDate (esp. Closed / Finaled /
    Superseded shells).
  - Final Closed / Superseded rows lacking FinalDate /
    FinalizeDate stay without FINAL_DATE. CompleteDate /
    ClosedDate are always null here.
"""
...
# _expected_status; keys are stripped forms as seen in DATA).
_STATUS_MAP = {
    # Final — completed / occupancy / closed
    "Finaled": "Final",
    "Certificate of Occupancy": "Final",
    "Certificate of Completion": "Final",
    "Closed": "Final",
    "Masterfile Closed": "Final",
    # Active — issued / open permit
    "Issued": "Active",
    "ROW Utilization Issued": "Active",
    ...
    "Fees Due": "In Review",
    "On Hold": "In Review",
    "Pending Permit Issuance": "In Review",
    "Approved-Pending Main Permit Issuance": "In Review",
    "Insufficient Submittal": "In Review",
    "Revision Fees Due": "In Review",
    "Extension/Reinstatement Fees Due": "In Review",
    # Inactive — canceled / void / denied / expired / abandoned / superseded
    "Expired": "Inactive",
    "Void": "Inactive",
    "Canceled": "Inactive",
    "Cancelled": "Inactive",
    "Abandoned (Closed)": "Inactive",
    "Disapproved Review": "Inactive",
    "Masterfile Expired": "Inactive",
    "On Hold-Expired Certifications/Registration": "Inactive",
    "On Hold-Main Permit Expired": "Inactive",
    "Superseded": "Inactive",
}
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    if _present(issue):
        if effective_status in ("Active", "Final", "Inactive"):
            ...
            # Keep IssueDate if already present (e.g. Extension/Revision
            # fees due after issuance); otherwise leave blank.
            pass
    elif effective_status == "In Review":
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Kissimmee permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Kissimmee".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Kissimmee"].copy()

    print(f"Kissimmee records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_kissimmee_repaired.parquet"
        )
