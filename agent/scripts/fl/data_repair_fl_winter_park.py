"""Data repair for Winter Park (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Winter Park DATA is a Tyler EnerGov payload with top-level keys contacts,
...
treated as null. Most Winter Park rows are the basic key set; a minority
also carry reviews / holds / attachments / more_info.
...
Known issues repaired:
  - Null STATUS_NORMALIZED on unmapped CaseStatus values
    (Address Approved, Process For Issuance, Permit Application
    Expired) → FILLED to Final / In Review / Inactive.
  - Spurious FINAL_DATE on non-Final shells (Issued, Void, Fees Due,
    Reinstated, Stop Work Order, Denied, Permit Application Expired)
    → cleared.
  - Spurious PERMIT_DATE on In Review (Fees Due / In Review /
    Stop Work Order) → cleared.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - A handful of Complete / Expired shells lack IssueDate
    (details.Issued=false) → PERMIT_DATE stays missing.
  - Complete shells already carry FinalDate in this sample; residual
    Active/Final PERMIT_DATE gaps are blank IssueDate only.
"""
...
_STATUS_MAP = {
    # Final — completed / finaled / closed address approvals
    "Complete": "Final",
    "Closed": "Final",
    "Address Approved": "Final",
    # Active — issued / reinstated open permit
    "Issued": "Active",
    "Reinstated": "Active",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Winter Park permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Winter Park".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Winter Park") & (df["STATE"] == "FL")
    ].copy()

    print(f"Winter Park records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_winter_park_repaired.parquet")
