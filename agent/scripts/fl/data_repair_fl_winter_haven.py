"""Data repair for Winter Haven (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Winter Haven DATA is a single Logos / TRAKiT-style portal payload with
top-level keys Permit Summary, Permit Details, Inspections, Notes,
Conditions, Payment Summary, etc. Top-level keys are uniform; content
variants (INFERRED_SCHEMA) are labeled by StatusValue lifecycle and
whether an embedded lifecycle date is present:

  - logos_completed / logos_completed_bare
  - logos_issued
  - logos_expired / logos_expired_bare
  - logos_pending / logos_pending_bare
  - logos_pending_review / logos_pending_review_bare
  - logos_created
  - missing / unknown
...
Known issues repaired:
  - STATUS_NORMALIZED often mirrors stale STATUS_ORIGINAL
    (permit issued / pending payment) while StatusValue has advanced
    to Permit Completed or Permit Issued → FIXED to Final / Active.
  - Null STATUS_NORMALIZED on many Permit Expired rows → FILLED
    Inactive (STATUS_ORIGINAL embeds the expiry date string and was
    not mapped).
  ...
  - FILE_DATE almost entirely missing; filled only where Created or
    Pending Payment carries a usable as-of / create stamp.

Not repairable from DATA:
  - No Application Received Date, Notes, Inspections, or Conditions in
    this sample → FILE_DATE remains missing for nearly all Issued /
  ...
  - Bare "Permit Completed" (no embedded date, no inspections) cannot
    supply FINAL_DATE.
"""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if status_base == "Pending Payment":
        return f"{base}_pending{suffix}" if (has_status_dt or has_app) else f"{base}_pending_bare"
    if status_base == "Pending Review":
        return f"{base}_pending_review{suffix}" if has_status_dt else f"{base}_pending_review_bare"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Winter Haven permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Winter Haven". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Winter Haven") & (df["STATE"] == "FL")
    ].copy()

    print(f"Winter Haven records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_winter_haven_repaired.parquet"
