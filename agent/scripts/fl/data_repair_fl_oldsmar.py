"""Data repair for Oldsmar (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Oldsmar DATA is a nested city permit-portal payload with top-level keys
Parcel, Permit, Contacts, and optionally Notes / InspectionsCompleted /
InspectionsScheduled. Canonical dates and status live under Permit.Main
(colon-suffixed keys). Occupancy & Warranty carries CO dates that are
secondary to Closed Date.
...
Known issues repaired:
  - FINAL_DATE missing on a few COMPLT rows where Closed Date is blank /
    N/A but InspectionsCompleted has a COMPLETE timestamp → FILLED from
    the latest completed inspection (calendar day).

Not repairable from DATA:
  - Hollow scrape shells (empty Permit No / Receipt / Issued / Closed)
    → dates stay missing; status left as agency Permit Status.
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    """Repair an Oldsmar nested-portal record."""
    main = _permit_main(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Oldsmar permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Oldsmar".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[df["JURISDICTION"] == "Oldsmar"].copy()

    print(f"Oldsmar records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "oldsmar_repaired_sample.parquet")
