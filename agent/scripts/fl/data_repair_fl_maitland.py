"""Data repair for Maitland (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Maitland DATA is a Tyler EnerGov payload with top-level keys
...
Known issues repaired:
  - STATUS_ORIGINAL lag: CaseStatus advanced to Complete / Issued /
    Expired while STATUS_ORIGINAL still held an earlier value
    (issued, in review, fees due, on hold, expired) → FIXED.
  - Unmapped CaseStatus values (In Review - Requires Resubmittal,
    Sufficiency Check, Sufficiency Check - Requires Resubmittal,
    Permit Processing Fee Due, Expired (Not Issued)) left
    STATUS_NORMALIZED null → FILLED.
  - Missing PERMIT_DATE on Issued / Complete shells whose upstream
    status lagged as In Review / null → FILLED from IssueDate after
    status fix.
  - Missing FINAL_DATE on Complete shells whose upstream status lagged
    as Active / In Review / Inactive → FILLED from FinalDate.
  - Spurious FINAL_DATE / PERMIT_DATE on In Review and non-Final shells
    (Fees Due / Issued with leftover FinalDate, Void with FinalDate)
    → FIXED clear.

Not repairable from DATA:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - One Complete Final shell has Issued=False and blank IssueDate →
...
_STATUS_MAP = {
    ...
    "Sufficiency Check - Requires Resubmittal": "In Review",
    "Submitted - Online": "In Review",
    "Submitted": "In Review",
    "On Hold": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Permit Processing Fee Due": "In Review",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    if mapped is None:
        mapped = _map_status(_permit_status(d))
    return mapped
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Maitland permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Maitland". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Maitland") & (df["STATE"] == "FL")
    ].copy()

    print(f"Maitland records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(
            out_dir, "permits_fl_maitland_repaired.parquet"
        )
