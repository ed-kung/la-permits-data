"""Data repair for Cutler Bay (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Cutler Bay DATA is a single Accela-style portal payload with top-level
...
Canonical mappings:
  - main.Status (with completed ``final - FINALIZE PERMIT``
    override for canceled rows that were fully finaled)
                                           → STATUS_NORMALIZED
...
Known issues repaired:
  - 2 canceled rows with a completed FINALIZE PERMIT action (and
    main.Final) were labeled Inactive → FIXED to Final.
  - 57 canceled rows carried main.Final as FINAL_DATE while
    STATUS_NORMALIZED=Inactive; those stamps are cancel/close dates
    for non-finaled cancels → FINAL_DATE cleared (FIXED).
  - Active ``approved`` rows (and sparse Final/Inactive) missing
    PERMIT_DATE filled from Approved / issue actions when Issued is
    blank.
  - 30 Final rows with blank main.Final filled FINAL_DATE from the
    completed FINALIZE PERMIT action.

Not repairable from DATA:
  - 19 Final rows have blank main.Final and empty / non-finalize
    actions → FINAL_DATE stays missing.
  - 3 Final and 1 Active ``approved`` row have neither Issued,
    Approved, nor a completed issue action → PERMIT_DATE stays
    missing.
  - In Review (pending) correctly lack issuance / final dates.
"""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    return (
        f"accela_{status}_"
        f"{_date_suffix(_present(applied), _present(issued), _present(final))}"
    )
...
def _expected_status(d: dict) -> Optional[str]:
    """Map main.Status; upgrade canceled→Final when finalize completed."""
    raw = _raw_status(d)
    if raw is None:
        return None
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Cutler Bay permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Cutler Bay". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Cutler Bay") & (df["STATE"] == "FL")
    ].copy()

    print(f"Cutler Bay records: {len(city):,}\n")
    ...
    if agent_data_path:
        ...
        out_path = out_dir / "permits_fl_cutler_bay_repaired.parquet"
