"""Data repair for Treasure Island (FL) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Treasure Island DATA is a Tyler EnerGov payload with top-level keys
...
EnerGov uses ``1900-01-01`` as a missing-date sentinel; those values are
treated as null. Most Treasure Island rows are the basic key set; a
minority also carry reviews / holds / attachments / more_info.
...
Known issues repaired:
  - Null STATUS_NORMALIZED on unmapped CaseStatus values
    (Waiting on Response to Review Comments, Waiting on Water & Nav
    Permit, Review(s) Complete*, Tabled) → FILLED to In Review.
  - Mis-normalized Complete → Active/Inactive and Expired → Active
    (STATUS_ORIGINAL lagged behind CaseStatus) → FIXED.
  - Spurious FINAL_DATE on non-Final shells and EnerGov 1900-01-01
    sentinels on Complete shells → cleared (FIXED).
  - Spurious PERMIT_DATE on In Review (legacy Issued=true shells
    still labeled In Review) → cleared.
  - FINAL_DATE filled on Complete shells that were mislabeled Active/
    Inactive but already carry a real FinalDate in DATA.

Not repairable from DATA:
  - FILE_DATE already matches ApplyDate for every sample row.
  - Active/Final shells with blank IssueDate (details.Issued=false)
    → PERMIT_DATE stays missing.
  - Complete shells whose only FinalDate is the 1900-01-01 sentinel
    (and no processing_status / CompleteDate / ClosedDate) →
    FINAL_DATE stays missing after sentinel clear.
"""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if not _present(final):
        final = _final_from_processing_status(data_dict)
...
_STATUS_MAP = {
    ...
    "Complete": "Final",
    "Closed": "Final",
    # Active — issued / open permit
    "Issued": "Active",
    "Reinstated": "Active",
    # In Review — pre-issuance / waiting / online review workflow
    "In Review": "In Review",
    "Submitted": "In Review",
    "Submitted - Online": "In Review",
    "Ready to Issue": "In Review",
    "Incomplete Application": "In Review",
    "Waiting on Response to Review Comments": "In Review",
    "Waiting on Water & Nav Permit": "In Review",
    "Review(s) Complete": "In Review",
    "Review(s) Complete - Complete": "In Review",
    "Review(s) Complete - Needs Corrections": "In Review",
    "Tabled": "In Review",
    "On Hold": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Awaiting Corrections": "In Review",
    "Process For Issuance": "In Review",
    # Inactive — expired / void / denied / withdrawn
    "Expired": "Inactive",
    "Void": "Inactive",
    "Denied": "Inactive",
    "Withdrawn": "Inactive",
...
def _apply_date(repairs: dict, row, field: str, candidate) -> None:
    ...
    if pd.isna(current) or not _has_usable_date(current):
        # Missing or sentinel / out-of-range stored value.
        if pd.isna(current):
            repairs[field] = cand
...
def _repair_record(row, d: dict, repairs: dict) -> None:
    ...
    if not _present(final):
        final = _final_from_processing_status(d)
    ...
    # Clear In Review rows and drop EnerGov 1900-01-01 sentinels.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Treasure Island permit records using the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Treasure Island".  Must
    ...
    for col in ("FILE_DATE", "PERMIT_DATE", "FINAL_DATE"):
        # Parquet may load these as datetime64[s]; promote to ns so
        # EnerGov timestamps with fractional seconds can be written back.
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Treasure Island") & (df["STATE"] == "FL")
    ].copy()

    print(f"Treasure Island records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = os.path.join(out_dir, "permits_fl_treasure_island_repaired.parquet")
