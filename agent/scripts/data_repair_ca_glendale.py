"""Data repair for Glendale (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Glendale DATA is a Tyler EnerGov-style payload with top-level keys
``entity``, ``details``, ``contacts``, ``processing_status``, and
``fees``, plus optional ``reviews`` / ``holds`` / ``attachments`` /
``more_info``. Two key-set variants appear in the sample:
...
Known issues repaired:
  - STATUS_NORMALIZED was derived from STATUS_ORIGINAL, which disagrees
    with entity.CaseStatus on ~26 sample rows (stale portal status vs
    current case status). Remap from CaseStatus: e.g. CaseStatus=Final
    wrongly labeled Active/Inactive/In Review; Issued→In Review;
    Expired→Active; Void→In Review.
  - Pending Expiration rows have IssueDate and Issued=True; treat as
    Active (permit issued, nearing expiry) rather than In Review.
  - FINAL_DATE was populated from entity.FinalDate for ~287 non-Final
    CaseStatus rows (Issued, Expired, Fees Due, etc.). On short-term
    permits (film, street use, ROW) FinalDate often equals IssueDate or
    ExpireDate and is not a finaling / sign-off date — clear those.
  - One COO Final row has FINAL_DATE four days after entity.FinalDate;
    overwrite with FinalDate.
  - After status remaps, fill PERMIT_DATE from IssueDate for Active/Final
    rows that previously lacked it (Issued mislabeled as In Review).

Not repairable from DATA:
  - ~34 Active/Final rows (mostly Building Code Modification / Change of
    Address / legacy) have null IssueDate and Issued=False, so
    PERMIT_DATE cannot be filled. FILE_DATE is not used as a proxy.
...
_STATUS_MAP = {
    ...
    "COO": "Final",
    "Complete": "Final",
    # Active (issued / still valid)
    "Issued": "Active",
    "Permit Extended": "Active",
    "Pending Expiration": "Active",
    # Inactive
    "Expired": "Inactive",
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Glendale record."""
    ...
    if effective_status == "Final":
        ...
        # Spurious FINAL_DATE on non-Final rows. Common for Issued /
        # Expired short-term permits where entity.FinalDate mirrors
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Glendale permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Glendale".  Must contain
...
if __name__ == "__main__":
    ...
    glendale = df[(df["JURISDICTION"] == "Glendale") & (df["STATE"] == "CA")].copy()

    print(f"Glendale records: {len(glendale):,}\n")

    repaired = data_repair(glendale)
    ...
    for field in ["STATUS_NORMALIZED", "FILE_DATE", "PERMIT_DATE", "FINAL_DATE"]:
        ...
        before_missing = glendale[field].isna().sum()
    ...
    print("  Before:")
    for s, c in glendale["STATUS_NORMALIZED"].value_counts(dropna=False).items():
