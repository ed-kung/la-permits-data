"""Data repair for Claremont (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Claremont DATA is a single flat portal schema (all sample rows share the
same top-level keys). Canonical fields:
...
finaling / completion date. System Status values are compound strings
(e.g. ``Final Final``, ``Open Plan check``).

Known issues repaired:
  - STATUS_NORMALIZED: 24 Estimate rows mapped to Final → In Review;
    158 Open Issued rows mapped to In Review → Active (all have
    Issue Date); 2 Open Completed → Final; 1 Final Final row with
    stale STATUS_ORIGINAL=issued labeled Active → Final.
  - FINAL_DATE was populated from Expire Date for ~1,880 sample rows
    instead of Final Date. Overwrite with Final Date when present;
    clear Expire-derived values when Final Date is absent or status
    is not Final.

Not repairable from DATA:
  - 27 Final Final rows have empty Final Date; Expire Date is not
    used as a proxy.
  - 23 Issued Issued and 14 Final Final rows lack Issue Date;
    PERMIT_DATE left null (no safe proxy in Approvals/Inspections).
"""
...
# Full System Status string (as in DATA) → STATUS_NORMALIZED
_STATUS_MAP = {
    # Final
    "Final Final": "Final",
    "Final": "Final",
    "Open Completed": "Final",
    "Completed": "Final",
    # Active
    ...
    "Open Issued": "Active",
    # In Review
    "Open Plan check": "In Review",
    "Open Open": "In Review",
    "Open Copy": "In Review",
    "Open": "In Review",
    "Estimate Estimate": "In Review",
    "Estimate": "In Review",
    "Hold Hold": "In Review",
    "Hold": "In Review",
    # Inactive
    ...
    "Expired": "Inactive",
    "Void Void": "Inactive",
    "Void Rescind": "Inactive",
    "Void": "Inactive",
    "Canceled Cancelled": "Inactive",
    "Canceled": "Inactive",
    "Cancelled": "Inactive",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Claremont record."""
    dd = _data_details(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Claremont permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Claremont".  Must contain
...
if __name__ == "__main__":
    ...
    claremont = df[(df["JURISDICTION"] == "Claremont") & (df["STATE"] == "CA")].copy()

    print(f"Claremont records: {len(claremont):,}\n")

    repaired = data_repair(claremont)
    ...
    for field in ["STATUS_NORMALIZED", "FILE_DATE", "PERMIT_DATE", "FINAL_DATE"]:
        ...
        before_missing = claremont[field].isna().sum()
    ...
    print("  Before:")
    for s, c in claremont["STATUS_NORMALIZED"].value_counts(dropna=False).items():
    ...
    # Spot-check: repaired FINAL_DATE vs Final Date / Expire Date
    import json as _json

    parsed = repaired["DATA"].apply(
        lambda x: _json.loads(x) if isinstance(x, str) else x
    )
    fin = parsed.apply(lambda d: (d.get("Data Details") or {}).get("Final Date"))
    exp = parsed.apply(lambda d: (d.get("Data Details") or {}).get("Expire Date"))
    ...
    match_fin = (
        final_dt.dt.normalize() == fin_dt.dt.normalize()
    ) & final_dt.notna() & fin_dt.notna()
    match_exp = (
        final_dt.dt.normalize() == exp_dt.dt.normalize()
    ) & final_dt.notna() & exp_dt.notna() & (
        fin_dt.isna() | (final_dt.dt.normalize() != fin_dt.dt.normalize())
    )
    print(f"\nFINAL_DATE matches Final Date: {match_fin.sum()}")
    print(f"FINAL_DATE matches Expire only (bad): {match_exp.sum()}")
