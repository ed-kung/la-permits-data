"""Data repair for Redwood City (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Redwood City DATA is a civic portal payload. All sample rows share the
same top-level keys: ``fees``, ``contacts``, ``site_info``,
``inspections``, ``permit_info``, ``search_data``. Canonical fields live
under ``permit_info`` (with ``search_data`` as a redundant mirror for
APPLIED / ISSUED / APPROVED / STATUS):

  - PermitStatus                          → STATUS_NORMALIZED
  - PermitAppliedDate (fallback: SD
    APPLIED)                              → FILE_DATE
  - PermitIssuedDate (fallback: SD ISSUED,
    then PermitApprovedDate / SD APPROVED) → PERMIT_DATE
  - PermitFinaledDate (fallback: passed
...
Content variants (same keys; differ by which fields are populated):
  ...
  - permit_info_empty_dates:    status/desc may exist, no usable dates
  - permit_info_empty:          blank permit_info shell
  - legacy_no_status:           blank PermitStatus but dates present

Known issues repaired:
  - 637 blank PermitStatus rows left STATUS_NORMALIZED null. When dates
    exist, infer Final / Active / In Review from Finaled / Issued|Approved
    / Applied (~449 FILLED). Empty shells stay null.
  - NO PMT REQ and NOT APPROVED were mapped to In Review → FIXED to
    Inactive.
  - ISSUED / INVESTIGATE rows that already have PermitFinaledDate kept
    as Active / In Review → FIXED to Final (finaled date authoritative
    unless status is a terminal inactive label).
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    (or SD ISSUED) present → FILLED.
  - Spurious FINAL_DATE on non-Final rows (esp. CANCEL - NFR close
    timestamps) → cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate / SD APPLIED whenever
    either source is populated; ~197 rows have neither.
  - Many Final FINALED/CLOSED rows lack PermitFinaledDate and have no
    dated final inspection → FINAL_DATE stays missing.
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if applied is pd.NaT:
        applied = _sd_date(data_dict, "APPLIED")
    if issued is pd.NaT:
        issued = _sd_date(data_dict, "ISSUED")
    ...
    if not raw_status and not has_any_date:
        # Distinguish blank shells from rows that only have desc/type text.
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"final\s*building|final\s*bldg|\*\*final\b|^final$|"
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if not raw:
        raw = _normalize_status_key(_search_data(d).get("STATUS"))
...
def _preferred_file_date(d: dict):
    ...
    if applied is not pd.NaT:
        return applied
    return _sd_date(d, "APPLIED")


# ── Per-record repair logic ─────────────────────────────────────────────────

def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Redwood City record."""
    # -- STATUS_NORMALIZED --
    ...
    if effective_status == "Final":
        ...
        # Spurious FINAL_DATE on non-Final rows (e.g. CANCEL - NFR close).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Redwood City permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Redwood City".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Redwood City") & (df["STATE"] == "CA")].copy()

    print(f"Redwood City records: {len(city):,}\n")

    repaired = data_repair(city)

    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "redwood_city_repaired_sample.parquet")
