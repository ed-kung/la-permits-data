"""Data repair for Lassen County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lassen County DATA is a custom portal payload with top-level keys
``Department``, ``My Project``, ``Permit Type``, ``Build Status``,
``Permit Number``, ``Permit Details``, contacts/fees/inspections arrays,
and optionally ``Parcel Number`` / ``ProjectDescription``. Three key-set
variants plus empty shells appear in the sample:

  - my_project_with_description:  core keys + ProjectDescription
  - my_project_with_parcel:       core keys + Parcel Number
                                  (no ProjectDescription)
  - my_project_basic:             core keys without Parcel Number /
                                  ProjectDescription
  - empty_shell:                  My Project {} and null Build Status /
                                  Permit Type / Department

Canonical fields live under ``Build Status`` and ``My Project``:
  - Build Status (+ date overrides) → STATUS_NORMALIZED
  - My Project.Submitted (fallback Created) → FILE_DATE
  - My Project.Issued (fallback Approved) → PERMIT_DATE
  - My Project.Closed → FINAL_DATE

Known issues repaired:
  - Null Build Status (and unmapped labels such as Application is under
    review / AUDITED / Expired without STATUS_ORIGINAL) → STATUS
    FILLED from Build Status and/or Closed / Issued / Approved /
    Submitted dates.
  - Closed shells mis-tagged Active or Inactive (STATUS_ORIGINAL
    expired while Build Status is Closed) → FIXED to Final;
    FINAL_DATE FILLED from Closed when missing.
  - Ready To Issue with Issued stamp left In Review → FIXED to Active.
  - Incomplete Application with Closed stamp left In Review → FIXED
    to Inactive; junk FINAL_DATE cleared.
  - Missing FILE_DATE when Submitted is present → FILLED.
  - Missing PERMIT_DATE on Active/Final when Issued or Approved is
    present → FILLED.
  - Junk FINAL_DATE on non-Final rows (Expired / Incomplete closures;
    Inactive empty shells) → cleared.

Not repairable / left as-is:
  - empty_shell rows with no My Project dates and no Build Status
    (3 fully blank; 10 with upstream dates only — no DATA to verify).
  - Build Status FINAL (13) with blank Closed → FINAL_DATE stays
    missing.
  - Active/Final shells with neither Issued nor Approved (mostly
    Closed with only Submitted + Closed) → PERMIT_DATE stays missing.
  - Created is never present without Submitted, so FILE_DATE fallback
    to Created is unused in the sample.
"""
...
def _is_blank_date(val) -> bool:
    ...
    if isinstance(val, float) and math.isnan(val):
        return True
    s = str(val).strip()
    return s in _BLANK_DATE_STRINGS
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if "ProjectDescription" in keys:
        return "my_project_with_description"
    if data_dict.get("Parcel Number") is not None:
        return "my_project_with_parcel"
    return "my_project_basic"
...
_STATUS_MAP = {
    ...
    "Application is under review": "In Review",
    "Pending": "In Review",
}
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Sticky Inactive: Expired. Incomplete Application with a Closed stamp
    is treated as Inactive (closed incomplete), not Final. Explicit
    Closed / FINAL labels → Final. Otherwise Closed date → Final, Issued
    date → Active (overrides review-pipeline labels), Approved date or
    Approved/AUDITED labels → Active, Submitted-only → In Review.
    """
    bs = _normalize_build_status(d.get("Build Status"))
    closed = _mp_date(d, "Closed")
    issued = _mp_date(d, "Issued")
    approved = _mp_date(d, "Approved")
    submitted = _mp_date(d, "Submitted")

    if bs == "Expired":
        return "Inactive"

    if bs == "Incomplete Application":
        return "Inactive" if closed is not pd.NaT else "In Review"

    if bs in ("Closed", "FINAL"):
        return "Final"
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if not pd.isna(current_permit):
        if permit_src is not pd.NaT and not _dates_equal(current_permit, permit_src):
            # Prefer Issued when present; do not overwrite an existing
            # upstream date with Approved-only when Issued is blank and
            # the current value already differs (empty-shell leftovers).
            issued = _mp_date(d, "Issued")
            if issued is not pd.NaT:
            ...
            and _mp_date(d, "Issued") is pd.NaT
        ):
    ...
    # -- FINAL_DATE (completion / Closed) --
    # Non-Final statuses often carry junk Closed stamps (Expired /
    # Incomplete); only keep FINAL_DATE when status is Final.
    closed = _mp_date(d, "Closed")
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lassen County permit records using information from the raw DATA JSON
    column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lassen County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Lassen County") & (df["STATE"] == "CA")
    ].copy()

    print(f"Lassen County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_lassen_county_repaired.parquet"
