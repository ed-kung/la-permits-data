"""Data repair for Hawthorne (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hawthorne DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - One row has entity.CaseStatus=Issued (and IssueDate present) but
    STATUS_ORIGINAL='submitted - online', so STATUS_NORMALIZED was
    incorrectly set to In Review → Active; PERMIT_DATE filled from
    IssueDate.
  - 19 non-Final rows carry FINAL_DATE copied from entity.FinalDate
    (Issued, In Review, Plan Approval Expired, Void). These are not
    permit sign-offs under CaseStatus=Complete → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate (UTC day) for all
    sample rows; none missing.
  - All Active/Final rows already have PERMIT_DATE matching IssueDate
    except the one mislabeled Issued row above.
  - All CaseStatus=Complete (Final) rows already have FINAL_DATE
    matching entity.FinalDate.
  - details.PermitStatus=Complete with FinalizeDate but CaseStatus not
    Complete (41 rows) is treated as a stale details label, not a
    finaling signal; CaseStatus is authoritative.
...
_STATUS_MAP = {
    ...
    "Void": "Inactive",
    "Denied": "Inactive",
    # In Review (pre-issuance / hold / enforcement)
    "In Review": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Submitted": "In Review",
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Hawthorne record."""
    ...
    # status is Final; on non-Complete rows it often appears with a stale
    # details.PermitStatus=Complete label and must not populate FINAL_DATE.
    ...
    if effective_status == "Final":
        ...
        # while CaseStatus is Issued / In Review / Plan Approval Expired / Void).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hawthorne permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hawthorne".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Hawthorne") & (df["STATE"] == "CA")].copy()

    print(f"Hawthorne records: {len(city):,}\n")
