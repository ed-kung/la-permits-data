"""Data repair for Pacifica (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pacifica DATA is a flat city portal payload. Every sample row shares the
same top-level keys:
...
Known issues repaired:
  - 3 rows with blank STATUS_ORIGINAL / STATUS_NORMALIZED → FILLED
    from Check-List inference (In Review early-stage shells).
  - STATUS_ORIGINAL lagging DATA.Status: Permit Finaled. still Active
    → FIXED to Final; Permit Expired. still Active → FIXED to Inactive;
    Ready for inspections. still In Review → FIXED to Active;
    Inspections in process. still Inactive → FIXED to Active;
    Cancelled Permit. still In Review → FIXED to Inactive.
...
def _repair_row(row, d: dict, repairs: dict) -> None:
    """Apply Pacifica repairs into *repairs* dict."""
    ...
    # Pacifica DATA has no issuance or finaled date fields. Expiration
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pacifica (CA) permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pacifica". Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pacifica") & (df["STATE"] == "CA")
    ].copy()

    print(f"Pacifica records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_pacifica_repaired.parquet"
