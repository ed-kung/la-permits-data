"""Data repair for Mendocino County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Mendocino County DATA is a civic portal payload (same shape as Belmont).
All sample rows share the same top-level keys: ``fees``, ``contacts``,
``site_info``, ``inspections``, ``permit_info``, ``search_data``.
Canonical fields live under ``permit_info`` (``search_data`` only mirrors
Address / RECORDID / Permit Number — no date mirrors):
  ...
  - PermitFinaledDate (fallback: latest
    final / legacy type-33 inspection)    → FINAL_DATE
...
Known issues repaired:
  - Blank PermitStatus (legacy conversions) left STATUS_NORMALIZED
    null → FILLED from Issued/Approved (Active) or Applied (In Review).
  - READY FOR APPLICANT left null → FILLED In Review.
  - Stale STATUS_NORMALIZED vs PermitStatus (FINALED mislabeled
    Active/Inactive; ISSUED mislabeled Inactive/In Review; EXPIRED /
    SUPERSEDED / CANCELLED / UNSATISFACTORY mislabeled; SATISFACTORY
    mislabeled In Review) → FIXED.
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    present (common on TRANSPORTATION / ENCROACHMENT) → FILLED.
  - Final missing FINAL_DATE with blank PermitFinaledDate but a final /
    legacy type-33 inspection → FILLED from inspection.
  - Spurious FINAL_DATE on Inactive rows that still carry
    PermitFinaledDate → cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate wherever Applied exists;
    7 rows lack Applied (and usually lack any application date in DATA).
    Do not backfill FILE_DATE from Issued.
  - ~30 Final rows lack PermitFinaledDate and a usable final inspection
    (including COMPLETED / SATISFACTORY shells).
  - A few Final rows with neither Issued nor Approved → PERMIT_DATE
    stays missing.
"""
...
_STATUS_MAP = {
    ...
    "CORRECTION LETTER": "In Review",
    "READY FOR APPLICANT": "In Review",
    "READY": "In Review",
    "IN PROGRESS": "In Review",
    "REVISION": "In Review",
    "BOND REQUIRE": "In Review",
    "EXPIRED BY DATE": "Inactive",
    "EXPIRED": "Inactive",
    "VOID": "Inactive",
    "CANCELLED": "Inactive",
    "CANCELED": "Inactive",
    "CANCELLED 8-": "Inactive",
    "SUPERSEDED": "Inactive",
    "ABANDONED": "Inactive",
    "DENIED": "Inactive",
    "UNSATISFACTORY": "Inactive",
}
...
_FINAL_INSP_OK = {
    ...
    "COMPLETED",
    "COMPLETE",
    "1",  # legacy numeric pass code paired with Type "33"
}
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then CANCELLED* / EXPIRED* prefixes."""
    if not raw:
        return None
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if raw in _INACTIVE_KEEP or (
        raw.startswith("CANCELED")
        or raw.startswith("CANCELLED")
    ...
    if raw:
        if "FINAL" in raw or "COMPLETED" in raw or "SATISFACTORY" in raw:
            return "Final"
        if (
            "EXPIRE" in raw
            or "VOID" in raw
...
def _is_final_inspection(item: dict) -> bool:
    ...
    # Legacy Mendocino codes: Type "33" + Result "1" = final pass.
    if typ == "33" and result_u in {"1", "FINALED", "APPROVED", "COMPLETED", "COMPLETE"}:
        return True
...
def _final_from_inspections(d: dict):
    """Latest completion date from a final / legacy type-33 inspection."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Mendocino County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Mendocino County".  Must
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Mendocino County") & (df["STATE"] == "CA")].copy()

    print(f"Mendocino County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_mendocino_county_repaired.parquet"
