"""Data repair for Hayward (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hayward DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED null for CaseStatus Reviewed and Approved (2) and
    Non-Exempt (1) → FILLED as In Review.
  - 440 non-Final rows carry FINAL_DATE copied from entity.FinalDate
    (Issued, Expired, Cancelled, On Hold, Exempt, In Review, Non-Exempt).
    These are not permit sign-offs under CaseStatus=Complete → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate (UTC day) for all
    sample rows; none missing.
  - All Active/Final rows with IssueDate already have matching
    PERMIT_DATE. Two Final (Complete) rows have null IssueDate and
    Issued=False → PERMIT_DATE stays missing.
  - Five Final (Complete) rows have null FinalDate / FinalizeDate and
    no usable finaling inspection → FINAL_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - details.ApplyDate can differ from entity.ApplyDate by a timezone
    day-boundary; entity.ApplyDate is authoritative (already matches
    FILE_DATE).
"""
...
# entity.CaseStatus (Title Case, as in DATA; lookup uses stripped value)
_STATUS_MAP = {
    # Final
    "Complete": "Final",
    # Active
    ...
    "Cancelled": "Inactive",
    "Denied": "Inactive",
    # In Review (pre-issuance / hold / soft-story classification)
    "In Review": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Submitted": "In Review",
    "Submitted - Online": "In Review",
    "Completeness Check": "In Review",
    "In-Complete": "In Review",
    "On Hold": "In Review",
    "Exempt": "In Review",
    "Non-Exempt": "In Review",
    "Reviewed and Approved": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Hayward record."""
    ...
    # legacy stamp coincident with Apply/Issue, an expire-adjacent close,
    # or a soft-story program date — not a Complete sign-off.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hayward permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hayward".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Hayward") & (df["STATE"] == "CA")].copy()

    print(f"Hayward records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "hayward_repaired_sample.parquet")
