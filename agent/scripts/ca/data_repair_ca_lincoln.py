"""Data repair for Lincoln (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lincoln DATA is an Accela Citizen Access scrape (same family as Perris /
Eastvale / Menifee). All sample rows share the full portal key set;
content variants (INFERRED_SCHEMA):

  - portal_issued_finaled:   Permit Issuance Issued + final-inspection /
                             Final*-Passed inspection evidence
  - portal_issued:           Issued present, no finaling date
  - portal_final_insp_only:  Final evidence present, no Issued
  - portal_application_only: Application Acceptance / top-level date only
...
Canonical mappings:
  ...
  - Earliest Permit Issuance Marked as Issued / Permit Issued
                                                             → PERMIT_DATE
  - Earliest Inspection Marked as Final Inspection Complete
    / Inspections Complete (fallback: Final CO Issued,
    Pass/Passed inspection titled Final*)                    → FINAL_DATE

Known issues repaired:
  - FILE_DATE brought forward when an Application Acceptance
    Accepted* mark is earlier than DATA.date → FIXED.
  - Finaled rows missing FINAL_DATE while a Passed Final* /
    Final Solar inspection exists (workflow Inspection task still
    TBD) → FILLED.
  - Spurious FINAL_DATE on Active Issued and Inactive Expired
    shells → FIXED (cleared).

Not repairable / left as-is:
  - STATUS_NORMALIZED already matches DATA.status for every sample
    row (no stale STATUS_ORIGINAL lag).
  - 28 Active/Final shells (mostly Finaled with Permit Issuance TBD
    / empty events) have no dated Issued mark → PERMIT_DATE stays
    missing.
  - 7 Finaled shells lack Final Inspection Complete, Inspections
    Complete, Final CO Issued, and Final*-titled Pass inspections
    → FINAL_DATE stays missing.
  - Passed Final inspections on still-Issued shells do **not**
    promote status to Final (agency CaseStatus lag).
"""
...
def _final_date_from_data(d: dict):
    ...
    Prefer the earliest Final Inspection Complete / Inspections Complete
    workflow mark (matches existing FINAL_DATE when both present), then
    Final CO Issued, then Pass/Passed Final*-titled inspections
    (including Final Solar).
    """
    tasks = d.get("tasks") or []
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels are sticky. Finaled / CofO Issued / Closed
    map to Final. A dated Permit Issuance Issued promotes In Review →
    Active. Final-inspection evidence alone does **not** promote Issued
    → Final when the portal status is still Issued.
    """
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lincoln permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lincoln".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Lincoln") & (df["STATE"] == "CA")].copy()

    print(f"Lincoln records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_lincoln_repaired.parquet"
