"""Data repair for El Cajon (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

El Cajon DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED on ``OFC``, ``Fees Due (Post-Issuance)``,
    and ``Not Accepted …`` shells → FILLED.
  - ``Issued`` shells whose details.PermitStatus is ``Complete`` (and
    FinalizeDate is present) left Active → FIXED to Final.
  - Issued / Fees Paid shells with FinalDate/FinalizeDate strictly
    after IssueDate left Active/In Review → FIXED to Final.
  - Post-issuance ``Fees Due`` / ``Fees Paid`` / ``On Hold`` / ``Pending``
    rows with IssueDate left In Review → FIXED to Active.
...
Not repairable / left as-is:
  ...
  - 15 ``Issued`` Active shells and ~433 ``Complete``/``Closed`` Final
    shells (mostly Encroachment Permit Application) have null IssueDate
    → PERMIT_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Void / Canceled / Plan Approval Expired is a
...
_STATUS_MAP = {
    ...
    "Denied": "Inactive",
    "Rejected": "Inactive",
    "Not Accepted - emailed missing information": "Inactive",
...
_INACTIVE_LABELS = {
    ...
    "Denied",
    "Rejected",
    "Not Accepted - emailed missing information",
}
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    El Cajon often stamps FinalDate on still-Issued / Fees Paid shells
    where FinalDate ≤ IssueDate (or IssueDate is absent). Those are junk
    workflow stamps, not completion. Require either an explicit Complete
    / Closed / Finaled label, or FinalDate/FinalizeDate strictly after
    IssueDate.
    """
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Void / Withdrawn / Plan Approval
    Expired / Rejected / Not Accepted) are sticky even when FinalDate is
    present as a case-closure stamp. Explicit Complete / Closed / Finaled
    ...
    / Fees Paid shells). IssueDate → Active overrides review-pipeline
    labels (Fees Due / Fees Paid / On Hold / Pending / OFC).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single El Cajon record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    El Cajon permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "El Cajon".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "El Cajon") & (df["STATE"] == "CA")
    ].copy()

    print(f"El Cajon records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_el_cajon_repaired.parquet"
