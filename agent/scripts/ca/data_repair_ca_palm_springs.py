"""Data repair for Palm Springs (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Palm Springs DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED missing for 6 CaseStatus values the upstream
    mapper did not know (Corrections Requested, Outstanding COA's)
    → FILLED as In Review.
  - Spurious FINAL_DATE on non-Final rows (Void, Denied, Expired,
    On Hold, Submitted - Online) copied from entity.FinalDate, which
    is a case-closure stamp rather than a permit sign-off → cleared.

Not repairable from DATA:
  - FILE_DATE already matches the UTC calendar date of entity.ApplyDate
    for all sample records.
  - PERMIT_DATE already matches entity.IssueDate whenever IssueDate is
    present. 1 Final (Complete) Revision row has Issued=False / null
    IssueDate, so PERMIT_DATE cannot be filled.
  - 390 Final (Complete) rows (mostly ApplyDate 2001–2013) have null
    FinalDate / FinalizeDate, so FINAL_DATE cannot be filled.
"""
...
_STATUS_MAP = {
    ...
    "Denied": "Inactive",
    "Canceled": "Inactive",
    # In Review (pre-issuance / fees / corrections)
    "In Review": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "On Hold": "In Review",
    "Ready to Review": "In Review",
    "Submitted": "In Review",
    "Submitted - Online": "In Review",
    "Corrections Requested": "In Review",
    "Outstanding COA's": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Palm Springs record."""
    ...
    # case-closure stamp (Void / Denied / etc.) and must not remain on
    # FINAL_DATE.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Palm Springs permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Palm Springs".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Palm Springs") & (df["STATE"] == "CA")].copy()

    print(f"Palm Springs records: {len(city):,}\n")
    ...
    print("\nFINAL_DATE_FLAG FIXED by CaseStatus:")
    fixed_final = repaired[repaired["FINAL_DATE_FLAG"] == "FIXED"]
    from collections import Counter
    cs_counts = Counter()
    for idx in fixed_final.index:
        d = _safe_parse(city.loc[idx, "DATA"])
        cs_counts[_case_status(d) if d else None] += 1
    for cs, n in cs_counts.most_common():
        print(f"  {cs!r}: {n}")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "permits_ca_palm_springs_repaired.parquet")
