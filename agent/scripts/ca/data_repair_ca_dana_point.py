"""Data repair for Dana Point (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Dana Point DATA is a civic portal payload (same shape as San Clemente /
Hemet / Belmont). All sample rows share the same top-level keys:
...
``search_data``. Canonical fields live under ``permit_info``:
  ...
  - PermitFinaledDate (fallback: latest
    final / C of O inspection)            → FINAL_DATE
...
Content variants (same keys; differ by which fields are populated):
  ...
  - legacy_no_status:           blank / <NONE> PermitStatus but dates present

Known issues repaired:
  - Blank / <NONE> PermitStatus (mostly HISTORICAL shells) with Issued /
    Approved / Finaled dates → STATUS_NORMALIZED FILLED from dates.
  - Stale STATUS_NORMALIZED vs PermitStatus (FINALED mislabeled Active
    because STATUS_ORIGINAL lagged at 'permit issued') → FIXED to Final.
  - PERMIT ISSUED / APPROVED mislabeled In Review → FIXED to Active.
  - Rows with PermitFinaledDate but non-inactive PermitStatus
    (e.g. PERMIT ISSUED) → FIXED to Final.
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    present → FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present (incl. after
    status fix) → FILLED; blank FinaledDate but a final inspection →
    FILLED.
  - Spurious FINAL_DATE on non-Final rows (CANCELLED close stamps) →
    cleared (FIXED).
  - Out-of-range PermitFinaledDate / FINAL_DATE years (outside 1990–2035,
    e.g. 1958 or 5009 typos) → rejected / cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate whenever both present;
    rows with blank AppliedDate stay missing (no reliable application
    date; IssuedDate is not used as a FILE substitute).
  - Some Final / Active rows lack both Issued and Approved → PERMIT_DATE
    stays missing.
  - Some Final rows lack PermitFinaledDate and usable final inspections
    → FINAL_DATE stays missing.
  - EXPIRED_RENEWED kept as In Review (upstream mapping); not treated as
    Inactive via the EXPIRED* prefix.
"""
...
def _safe_to_datetime(val):
    """Parse a date value, returning pd.NaT on failure or out-of-range year."""
...
_STATUS_MAP = {
    ...
    "ISSUED": "Active",
    "APPROVED": "Active",
    "APPROVED/CONDIT": "Active",
    "ACTIVE": "Active",
    "ACTIVE-BUILT": "Active",
    "ACTIVE-PRECISE": "Active",
    # In Review
    "UNDER REVIEW": "In Review",
    "DUE": "In Review",
    "EXPIRED_RENEWED": "In Review",
    "PENDING SUBMITTAL": "In Review",
    "PENDING AA SENDOUT": "In Review",
    "PENDING FEES OWED": "In Review",
    "PAID": "In Review",
    "SUSPENDED": "In Review",
    # Inactive
    "CANCELLED": "Inactive",
    ...
    "DENIED": "Inactive",
    "EXPIRED": "Inactive",
    "EXPIRED LETTER": "Inactive",
    "EXPIRED_CANCELL": "Inactive",
    "VOID": "Inactive",
    "INACTIVE": "Inactive",
}
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"plumbing\s*final|electrical\s*final|mechanical\s*final|"
    r"final-complete|final-building|final-roof|final-electrical|"
    r"final-fire|encroach\s*final|construc\s*final|"
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then EXPIRED* / CANCELLED* prefixes (except EXPIRED_RENEWED)."""
    if not raw:
        return None
    if raw in _STATUS_MAP:
        return _STATUS_MAP[raw]
    if raw.startswith("CANCELLED") or raw.startswith("CANCELED"):
        return "Inactive"
    # EXPIRED_RENEWED is mapped above to In Review; other EXPIRED* → Inactive.
    if raw.startswith("EXPIRED"):
        return "Inactive"
    if raw.startswith("FINAL") or raw.startswith("COMPLETED"):
        return "Final"
    return None
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if (
        raw in _INACTIVE_KEEP
        or (raw.startswith("EXPIRED") and raw != "EXPIRED_RENEWED")
        or raw.startswith("CANCEL")
    ):
    ...
    if raw:
        if "FINAL" in raw or "COMPLETED" in raw or "CLOSED" in raw:
            return "Final"
        if (
            "EXPIRE" in raw
            or "VOID" in raw
            ...
            return "Inactive"
        if "ACTIVE" in raw or "ISSUE" in raw or "APPROV" in raw:
            return "Active"
        if (
            "REVIEW" in raw
            or "PENDING" in raw
    ...
    # Blank status: infer from dates (common on HISTORICAL shells).
...
def _is_final_inspection(item: dict) -> bool:
    ...
    if _FINAL_TITLE_RE.search(typ) and result_u in _FINAL_INSP_OK:
        return True
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if effective_status == "Final":
        if preferred_final is not pd.NaT:
            ...
            # Existing FINAL_DATE is unparseable or outside 1990–2035.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Dana Point permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Dana Point".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Dana Point") & (df["STATE"] == "CA")].copy()

    print(f"Dana Point records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_dana_point_repaired.parquet"
