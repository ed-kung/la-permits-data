"""Data repair for Soledad (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Soledad DATA is a SmartGov community portal payload with top-level keys
``Department``, ``My Project``, ``Permit Type``, ``Build Status``,
``Permit Number``, ``Permit Details``, contacts/fees/inspections arrays,
and optionally ``Parcel Number``. Two key-set variants appear in the
sample:

  - my_project_with_parcel:  core keys + Parcel Number
  - my_project_basic:        core keys without Parcel Number

``Build Status``, ``Permit Number``, ``Application Number``,
``Permit Details``, and ``Permit Inspections`` are null/empty for every
sample row. Canonical fields therefore come from ``My Project`` dates:

  - Closed / Issued / Approved / Submitted presence → STATUS_NORMALIZED
...
Known issues repaired:
  - STATUS_NORMALIZED (and STATUS_ORIGINAL) missing on every sample row
    → FILLED from My Project date presence (Closed → Final, Issued →
    Active, Approved/Submitted/Created only → In Review).
  - Missing PERMIT_DATE on Final shells that have Closed but blank
    Issued → FILLED from Approved when available.
  - Spurious PERMIT_DATE on In Review shells (no Issued) → cleared.
  - Spurious FINAL_DATE on non-Final shells → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches My Project.Submitted at calendar-day
    resolution for every sample row.
  - FINAL_DATE already matches My Project.Closed whenever Closed is
    present; no Final row lacks Closed in the sample.
  - Active/Final shells with neither Issued nor Approved → PERMIT_DATE
    stays missing (none in sample after Approved fallback).
  - In Review shells (Submitted-only or Approved-without-Issued) →
    PERMIT_DATE / FINAL_DATE stay missing by design.
  - A handful of portal chronology inversions (Issued before Submitted;
    Closed before Issued) are retained because they match My Project.
  - Application Expires is a validity window, not an Inactive signal.
  - Build Status is null for every sample row (no label mapping used).
"""
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if not has_any_date and not mp:
        return "empty_shell"
...
def _expected_status(d: dict) -> Optional[str]:
    """Derive STATUS_NORMALIZED from My Project dates.

    Build Status is null for every Soledad sample row, so status is
    inferred entirely from date presence:

      Closed → Final
      Issued (no Closed) → Active
      Approved / Submitted / Created only → In Review
    """
    ...
    if _mp_date(d, "Issued") is not pd.NaT:
        return "Active"
    if (
        _mp_date(d, "Approved") is not pd.NaT
        or _mp_date(d, "Submitted") is not pd.NaT
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    # -- PERMIT_DATE (issuance / Issued, else Approved for Active/Final) --
    ...
    if effective_status in ("Active", "Final"):
        ...
        elif permit_src is not pd.NaT:
            repairs["PERMIT_DATE"] = permit_src
            repairs["PERMIT_DATE_FLAG"] = "FILLED"
    elif not pd.isna(current_permit) and issued is pd.NaT:
        # In Review / Inactive shells should not carry an issuance date.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Soledad permit records using information from the raw DATA JSON
    column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Soledad".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Soledad") & (df["STATE"] == "CA")
    ].copy()

    print(f"Soledad records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_soledad_repaired.parquet"
    ...
    print("\nChronology inversions (retained from portal):")
