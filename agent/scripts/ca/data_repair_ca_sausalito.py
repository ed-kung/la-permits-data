"""Data repair for Sausalito (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Sausalito DATA is a civic portal payload (same shape as Mill Valley /
Hillsborough / Rohnert Park). All sample rows share top-level keys:
``fees``, ``contacts``, ``site_info``, ``inspections``,
``permit_info``, ``search_data``. Canonical fields live under
``permit_info`` (``search_data`` only carries Address / Permit # /
RECORDID / Accessor Parcel Number in this sample):
...
Known issues repaired:
  - STATUS_NORMALIZED null for blank PermitStatus (legacy shells) →
    FILLED from dates (Issued → Active; Applied-only → In Review).
  - FINALED shells left Active / In Review / Inactive because
    STATUS_ORIGINAL lagged (issued / waiting for payment / expired /
    approve pend payment) → FIXED to Final.
  - ISSUED / ON HOLD / AWAITING RESUBMITTAL / UNDER REVIEW /
    REV UNDER REVIEW shells that already carry PermitIssuedDate left
    In Review → FIXED to Active.
  - ISSUED / CCTV FAIL / CCTV PASS shells that already carry
    PermitFinaledDate left Active / In Review → FIXED to Final.
  - NOT APPROVED left In Review → FIXED to Inactive.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    → FILLED (common on APPROVED encroachment shells and FINALED with
    Approved only).
  - Final missing FINAL_DATE when PermitFinaledDate or a passed final
    inspection date exists → FILLED.
  - Spurious FINAL_DATE on non-Final rows (WITHDRAWN close stamp) →
    cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate whenever Applied is
    present; 861 shells lack any application date (ISSUED without
    Applied, blank-status Issued-only shells, empty legacy) → stay
    missing.
  - 56 CONVERTED sewer-lateral shells (mapped Final) have neither
    Issued, Approved, nor Finaled → PERMIT_DATE / FINAL_DATE stay
    missing.
  - Some FINALED shells lack FinaledDate and a usable final inspection
    → FINAL_DATE stays missing.
  - Three blank-status shells with neither Applied nor Issued stay
...
_STATUS_MAP = {
    ...
    "CLOSED": "Final",
    "CONVERTED": "Final",
    "EP NOT REQUIRED": "Final",
    # Active (approval / issuance / open work)
    "ISSUED": "Active",
    "APPROVED": "Active",
    "ACTIVE": "Active",
    # In Review (intake / fee / CCTV / pre-issuance workflow)
    "UNDER REVIEW": "In Review",
    "REV UNDER REVIEW": "In Review",
    "CCTV FAIL": "In Review",
    "CCTV PASS": "In Review",
    "RESPONSE SENT TO APP": "In Review",
    "AWAITING RESUBMITTAL": "In Review",
    "ON HOLD": "In Review",
    "PROCESSING PENDING": "In Review",
    "APPROVE PEND PAYMENT": "In Review",
    "WAITING FOR PAYMENT": "In Review",
    "INCOMPLETE": "In Review",
    "READY TO ISSUE": "In Review",
    "RECEIVED": "In Review",
    "APPROVAL PENDING PARENT": "In Review",
    "EP FAILED REVIEW": "In Review",
    # Inactive
    "CANCELLED": "Inactive",
    "CANCELED": "Inactive",
    "EXPIRED": "Inactive",
...
def _lookup_status(raw: str) -> Optional[str]:
    ...
    if "REVOKE" in raw or "WITHDRAW" in raw or "ABANDON" in raw:
        return "Inactive"
    if "DENIED" in raw or "NOT APPROVED" in raw or "STOP WORK" in raw:
        return "Inactive"
    if raw.startswith("FINAL") or "CERTIFICATE OF OCC" in raw:
        return "Final"
    if raw == "CLOSED" or raw.startswith("CLOSE") or raw == "CONVERTED":
        return "Final"
    if raw.startswith("ISSUED") or raw == "ACTIVE":
        return "Active"
    if "APPROV" in raw and "PEND" not in raw and "READY TO PAY" not in raw:
        return "Active"
    if (
        "SUBMIT" in raw
        or "REVIEW" in raw
...
def _is_inactive_keep(raw: str) -> bool:
    ...
    if "REVOKE" in raw or "WITHDRAW" in raw or "ABANDON" in raw or "DENIED" in raw:
        return True
    if "NOT APPROVED" in raw or "STOP WORK" in raw:
        return True
    return False
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    Pre-issuance / review labels that already carry PermitIssuedDate are
    promoted to Active (post-issuance workflow still open).
    """
    raw = _raw_status(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Sausalito permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Sausalito".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Sausalito") & (df["STATE"] == "CA")].copy()

    print(f"Sausalito records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_sausalito_repaired.parquet"
    ...
    n = len(repaired)
    print(f"  {n_has:>4,} / {n:>4,} ({n_has / n:.1%})")

    # Chronology checks
    both = repaired[repaired["PERMIT_DATE"].notna() & repaired["FILE_DATE"].notna()]
    inv = (both["PERMIT_DATE"].dt.normalize() < both["FILE_DATE"].dt.normalize()).sum()
    print(f"\nPERMIT < FILE inversions: {inv}")
    bothf = repaired[repaired["FINAL_DATE"].notna() & repaired["PERMIT_DATE"].notna()]
    invf = (bothf["FINAL_DATE"].dt.normalize() < bothf["PERMIT_DATE"].dt.normalize()).sum()
    print(f"FINAL < PERMIT inversions: {invf}")
