"""Data repair for Riverside County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Riverside County DATA is a Tyler EnerGov-style payload with top-level
...
Known issues repaired:
  - 13 Clearances Required rows with missing STATUS_NORMALIZED → FILLED
    as In Review.
  - Status remaps where CaseStatus was miscategorized relative to the
    permit lifecycle (Paid / Filed / Credited → Final; Renewal Due →
    Active; Administratively Closed / Annexed → Inactive) → FIXED.
  - 6 Issued rows missing PERMIT_DATE despite entity.IssueDate → FILLED.
  - Non-Final rows carrying FINAL_DATE copied from entity.FinalDate
    (Paid before remap, Void, Filed, History, Approved, etc.) → cleared
    when effective status is not Final; kept when remapped to Final.
...
Not repairable / left as-is:
  ...
  - Where PERMIT_DATE and IssueDate both exist they always match;
    Final / Completed rows with no IssueDate (and Issued=False) have
    no alternate issuance field → PERMIT_DATE left missing.
  - Administratively Closed and a handful of Paid / Filed / Credited
    rows lack FinalDate → FINAL_DATE left missing after status repair.
...
_STATUS_MAP = {
    # Final — completed / terminal paid or filed outcomes
    "Final": "Final",
    "Completed": "Final",
    "Paid": "Final",
    "Credited": "Final",
    "Filed": "Final",
    # Active — issued / approved / issued-awaiting-renewal
    "Issued": "Active",
    "Approved": "Active",
    "Renewal Due": "Active",
    # Inactive — expired, voided, withdrawn, transferred, closed without final
    "Expired": "Inactive",
    "ENC Expired": "Inactive",
    "Void": "Inactive",
    "Canceled": "Inactive",
    "Withdrawn": "Inactive",
    "History": "Inactive",
    "Refunded": "Inactive",
    "Administratively Closed": "Inactive",
    "Annexed": "Inactive",
    # In Review — pre-issuance / plan check / hold / payment pending
    "Applied": "In Review",
    "Applied Online": "In Review",
    "PLN CK Approved": "In Review",
    "Pend Correction": "In Review",
    "Pending": "In Review",
    "Assigned": "In Review",
    "Payment Pending": "In Review",
    "Clearances Required": "In Review",
    "Plan Check": "In Review",
    "Hold": "In Review",
    "Correction Alert": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Riverside County record."""
    ...
    if effective_status == "Final":
        ...
        # while CaseStatus is Issued / Void / Approved / etc.).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Riverside County permit records using information from the raw DATA
    JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Riverside County".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Riverside County") & (df["STATE"] == "CA")
    ].copy()

    print(f"Riverside County records: {len(city):,}\n")

    repaired = data_repair(city)

    if AGENT_DATA_PATH:
        out_path = os.path.join(
            AGENT_DATA_PATH, "riverside_county_repaired_sample.parquet"
        )
    ...
    print("\nSTATUS_NORMALIZED change summary (CaseStatus → before → after):")
