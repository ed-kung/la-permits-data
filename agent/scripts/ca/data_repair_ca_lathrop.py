"""Data repair for Lathrop (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lathrop DATA is a legacy Logos / citizen portal payload with top-level
keys ``Permit Summary``, ``Payment Summary``, ``Permit Details``,
``Inspections``, ``Location``, etc. Optional sections produce six
key-set variants recorded in ``INFERRED_SCHEMA``:

  - portal:                 core sections only
  - portal_bv:              + Business Valuation
  - portal_coo:             + Certificate of Occupancy (+ BV)
  - portal_prod:            + Permit Category / Production Permits
  - portal_bv_prod:         + BV + Category / Production
  - portal_bv_prod_coo:     + BV + Category / Production + COO
...
Known issues repaired:
  - StatusValue/STATUS_NORMALIZED mismatches: Completed→Active/In Review,
    Issued→In Review, Expired→Active or missing → FIXED/FILLED.
  - Pending / Created rows missing FILE_DATE → FILLED from StatusValue.
  - Issued rows missing PERMIT_DATE → FILLED from Issued StatusValue date.
  ...
  - Completed mislabeled Active/In Review missing FINAL_DATE → FILLED
    from Completed StatusValue date; one Final FINAL_DATE that disagrees
    with StatusValue → FIXED.

Not repairable / left as-is:
  - Issued / Completed / Expired FILE_DATE: no application/submittal date
    in StatusValue (Issued/Completed dates are not filing dates; Expired
    date is an expiry stamp). Existing FILE_DATE values that predate
    issuance are retained.
  - Completed rows where PaidValue is after the completion date, or
    PaidValue is missing / "Not paid" → PERMIT_DATE stays missing.
  - Inspections shells in this extract are empty request placeholders
    (no usable inspection dates).
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Lathrop permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lathrop". Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Lathrop") & (df["STATE"] == "CA")].copy()

    print(f"Lathrop records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_csv = os.path.join(AGENT_DATA_PATH, "lathrop_repair_summary.csv")
