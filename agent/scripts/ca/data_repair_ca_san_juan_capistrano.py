"""Data repair for San Juan Capistrano (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Juan Capistrano DATA is a civic portal payload (same shape as Dana
Point / San Clemente / Hemet). All sample rows share the same top-level
...
Content variants (same keys; differ by which fields are populated):
  ...
  - permit_info_empty:          empty CONVERTED shells (no status/dates)

Known issues repaired:
  - Blank PermitStatus on GRADING shells with PermitAppliedDate →
    STATUS_NORMALIZED FILLED as In Review.
  - Stale STATUS_ORIGINAL-driven labels that disagree with PermitStatus
    (FINALED left Active; EXPIRED left Active/In Review; ISSUED left
    In Review; WAIVED left In Review) → FIXED.
  - Non-inactive rows with PermitFinaledDate (APPROVED / ACTIVE) →
    FIXED to Final.
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    present, or Issued present after status promotion → FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present (incl.
    after status fix) or a final inspection exists → FILLED.
  - Spurious FINAL_DATE on Inactive EXPIRED shells (closure stamps) →
    cleared (FIXED).

Not repairable / left as-is:
  - 67 empty CONVERTED shells (blank PermitStatus and all dates) →
    STATUS_NORMALIZED / dates stay missing.
  - FILE_DATE already matches PermitAppliedDate whenever both present;
    rows with blank AppliedDate stay missing (ApprovedDate / IssuedDate
    are not used as FILE substitutes).
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if not raw_status:
        ...
        if not any(str(pi.get(k) or "").strip() for k in pi):
            return "permit_info_empty"
        # PermitType/Desc present but no status/dates (empty CONVERTED).
        return "permit_info_empty"
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then EXPIRED* / CANCELLED* / WITHDRAW* prefixes."""
    if not raw:
        return None
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if (
        ...
        or raw.startswith("EXPIRED")
        or raw.startswith("CANCEL")
        or raw.startswith("WITHDRAW")
    ):
    ...
    # Blank status: infer from dates (GRADING shells with AppliedDate).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Juan Capistrano permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Juan Capistrano".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "San Juan Capistrano") & (df["STATE"] == "CA")
    ].copy()

    print(f"San Juan Capistrano records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_juan_capistrano_repaired.parquet"
