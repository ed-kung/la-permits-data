"""Data repair for Corte Madera (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Corte Madera DATA is an Accela Citizen Access scrape (same family as
Cupertino / Menlo Park). Most sample rows share core top-level keys
(``address``, ``date``, ``status``, ``tasks``, ``search_data``, …);
a minority also carry ``attachments`` / unpaid-fee blocks, and a few
TMP shells are ``search_data``-only. Content variants (INFERRED_SCHEMA):

  - portal_issued_finaled:   Permit Issuance Issued + final inspection
                             Complete (Bldg Final / Final Inspection)
  - portal_issued:           Issued present, no final-inspection date
  - portal_final_insp_only:  Final inspection Complete, no Issued
  - portal_application_only: Application Submittal / top-level date only
  - portal_empty_tasks:      tasks present but undated (TBD / empty)
  - search_data_only:        TMP Residential Resale shells
  - missing
...
Canonical mappings:
  ...
  - Earliest of DATA.date / search_data.Date / Application
    Submittal Accepted*                                      → FILE_DATE
  - Earliest Permit Issuance Marked as Issued                → PERMIT_DATE
  - Earliest Inspection Marked as Bldg Final Inspection
    Complete / Final Inspection Complete (fallback: Close
    Marked as Closed)                                        → FINAL_DATE

Known issues repaired:
  - STATUS_ORIGINAL / upstream map treated agency ``Finalled``
    (spelling variant of Finaled) as In Review → FIXED to Final
    (160 rows). Those rows already carry PERMIT_DATE / FINAL_DATE.
  - DATA.status lagging STATUS_ORIGINAL: Expired still Active;
    Finaled still Active; Issued still In Review / Inactive → FIXED.
  - In Review rows with a dated Permit Issuance Issued event → FIXED
    to Active.
  - Approved (plans approved, not issued) previously Active → FIXED
    to In Review.
  - Blank-status Comment Letter Sent / Application Pending → FILLED
    In Review; blank-status TMP search_data shells → FILLED In Review.
  - Spurious FINAL_DATE on Inactive Expired (and any remaining
    non-Final after status repair) → cleared.
  - Active/Final missing PERMIT_DATE when Issued events present
    → FILLED (rare; most legacy Finaled shells have TBD-only tasks).

Not repairable / left as-is:
  - Vast majority of older Finaled / Expired shells have empty or
    TBD-only Permit Issuance / Inspection tasks → PERMIT_DATE and
    FINAL_DATE stay missing.
  - Ready to Issue is not treated as issuance.
  - Final inspection Complete does not promote Issued → Final when
...
def _file_date_from_data(d: dict):
    ...
    earliest Application Submittal Accepted* mark. Accela sometimes bumps
    the top-level date on re-open while the original submittal remains in
    the task history.
    """
    candidates = []
...
def _final_date_from_data(d: dict):
    """Earliest building-final inspection Complete mark.

    Corte Madera uses ``Bldg Final Inspection Complete`` (plus a rare
    ``Final Inspection Complete``). When the Inspection task is empty but
    Close was marked Closed (common on ``Finalled`` shells), use that as
    a fallback sign-off date.
    """
    tasks = d.get("tasks") or []
    final = _first_event_date(tasks, {"Inspection"}, _FINAL_MARKS)
    if final is not pd.NaT:
        return final
    return _first_event_date(tasks, {"Close"}, _CLOSE_MARKS)
...
_STATUS_MAP = {
    # Final (Finalled is Corte Madera's spelling variant of Finaled)
    "Finaled": "Final",
    "Finalled": "Final",
    "Complete": "Final",
    "CLOSED": "Final",
    ...
    "Canceled": "Inactive",
    "Cancelled": "Inactive",
    # In Review (including plans-approved / fee / fire-precheck stages)
    ...
    "Comment Letter Sent": "In Review",
    "Fire Inspection Complete": "In Review",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels are sticky. DATA.status Finaled/Finalled/
    Closed maps to Final. A dated Permit Issuance Issued promotes
    In Review → Active. Final inspection Complete alone does **not**
    promote Issued → Final when the portal status is still Issued.
    """
    ...
    # Blank portal status but a filed Accela / TMP record → in-review shell.
    if (
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Corte Madera permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Corte Madera".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Corte Madera") & (df["STATE"] == "CA")].copy()

    print(f"Corte Madera records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_corte_madera_repaired.parquet"
