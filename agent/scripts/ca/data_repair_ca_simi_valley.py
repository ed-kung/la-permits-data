"""Data repair for Simi Valley (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Simi Valley DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - ``Estimate`` shells incorrectly normalized to Final
    (STATUS_ORIGINAL=estimate) → FIXED to In Review.
  - ``Approved`` shells without IssueDate left Active → FIXED to
    In Review (plan approval, not issuance).
  - ``Approved`` / ``Issued`` / ``Case Opened`` shells with IssueDate
    left In Review or missing status → FIXED/FILLED to Active;
    PERMIT_DATE FILLED when missing.
  - ``Finaled`` shells left Active / In Review (STATUS_ORIGINAL lagged
    as issued / submitted online) → FIXED to Final; FINAL_DATE FILLED
    from FinalDate/FinalizeDate.
  ...
  - ``Expired`` shell left Active (STATUS_ORIGINAL=issued) → FIXED to
    Inactive.
  - Junk FINAL_DATE on non-Final rows (Expired / Void same-day or
    closure stamps; Issued same-day FinalDate) → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate at calendar-day
    resolution for every sample row.
  - Two Finaled Water Will-Serve Letter shells with null IssueDate →
  ...
  - processing_status is null/empty for every sample row (no inspection
...
def _final_inspection_date(d: dict):
    ...
    ``Final``, ``Approved``, or ``Passed``. Simi Valley's sample has
    empty processing_status everywhere; kept for schema parity.
    """
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Simi Valley permit records using information from the raw DATA JSON
    column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Simi Valley".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Simi Valley") & (df["STATE"] == "CA")
    ].copy()

    print(f"Simi Valley records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_simi_valley_repaired.parquet"
