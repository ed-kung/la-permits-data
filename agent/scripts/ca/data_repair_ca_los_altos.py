"""Data repair for Los Altos (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Los Altos DATA is a civic portal payload (same shape as Chico / Saratoga /
Oceanside). All sample rows share the same top-level keys:
...
Known issues repaired:
  - Stale STATUS_ORIGINAL lagging PermitStatus: FINALED still Active
    (STATUS_ORIGINAL=issued) → FIXED to Final; ISSUED / APPROVED still
    In Review → FIXED to Active.
  - Blank PermitStatus (104 rows, mostly EXCAVATION / ENCROACHMENT)
    inferred from dates → FILLED Active (Issued) or In Review (Applied).
  - Non-inactive rows carrying PermitFinaledDate (APPROVED + Finaled)
    → FIXED to Final.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    → FILLED; one Active ISSUED with wrong PERMIT_DATE (Approved date
    used instead of Issued) → FIXED.
  - Final missing FINAL_DATE filled from PermitFinaledDate or latest
    passed BUILDING FINAL** / similar inspection → FILLED.
  - Spurious FINAL_DATE on Inactive CANCELED (and any remaining
    non-Final after status repair) → cleared.

Not repairable / left as-is:
  - 71 FILE_DATE gaps lack PermitAppliedDate (and search_data has no
    Application key) → stay missing; Issued is not used as FILE_DATE.
  - Many Final / finaled-only rows lack both Issued and Approved
    → PERMIT_DATE stays missing.
  - One FINALED row lacks PermitFinaledDate and a passed final
    inspection (only CORRECTION NOTICE) → FINAL_DATE stays missing.
  - Two blank-status shells with no dates → STATUS stays missing.
"""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Los Altos permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Los Altos".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Los Altos") & (df["STATE"] == "CA")].copy()

    print(f"Los Altos records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_los_altos_repaired.parquet"
