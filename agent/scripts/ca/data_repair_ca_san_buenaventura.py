"""Data repair for San Buenaventura (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Buenaventura DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - ``Final`` / ``Complete`` shells left Active (STATUS_ORIGINAL=issued)
    → FIXED to Final; FINAL_DATE FILLED from FinalDate/FinalizeDate.
  - ``Final`` shell left Inactive (STATUS_ORIGINAL=expired) → FIXED to
    Final.
  - ``Issued`` shells with FinalDate strictly after IssueDate left
    Active → FIXED to Final.
  - ``Issued`` / ``Revision Issued`` shells left In Review or missing
    status (STATUS_ORIGINAL lagged: fees due/paid, in review, returned
    to applicant, other - see comments) → FIXED/FILLED to Active;
    PERMIT_DATE FILLED when missing.
  - ``Expired`` / ``Cancelled`` shells left Active or In Review
    (STATUS_ORIGINAL lagged) → FIXED to Inactive.
  - ``Approved`` without IssueDate left Active → FIXED to In Review
    (plan-approval, not issuance).
  - ``Other - See Comments`` left missing STATUS_NORMALIZED → FILLED
    to In Review.
  - Active Issued shell missing PERMIT_DATE while IssueDate is present
    → FILLED.
  - Junk FINAL_DATE on non-Final rows (Cancelled / Plan Approval
    Expired / Returned to Applicant / Fees Due / Issued same-day or
    inverted FinalDate stamps) → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate at calendar-day
    resolution for every sample row.
  - One Final shell (WBFP-03-22-0062) with null IssueDate → PERMIT_DATE
    stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Cancelled / Plan Approval Expired is a
  ...
  - Issued shells with FinalDate equal to or before IssueDate stay
    Active; inverted / same-day stamps are not treated as completion.
  - processing_status is null for every sample row (no inspection
    fallback available in this extract).
"""
...
def _final_inspection_date(d: dict):
    ...
    ``Final``, ``Approved``, or ``Passed``. San Buenaventura's sample
    has null processing_status everywhere; kept for schema parity.
    """
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Cancelled / Denied / Plan
    Approval Expired) are sticky even when FinalDate is present as a
    case-closure stamp. Explicit Final / Complete labels → Final.
    Otherwise a FinalDate/FinalizeDate strictly after IssueDate → Final
    (stale Issued shells). IssueDate → Active overrides review-pipeline
    labels (In Review / Fees Due / Returned to Applicant / Approved /
    Revision Issued / Other - See Comments when CaseStatus is Issued).
    """
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Buenaventura permit records using information from the raw DATA
    JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Buenaventura".  Must
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "San Buenaventura") & (df["STATE"] == "CA")
    ].copy()

    print(f"San Buenaventura records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_buenaventura_repaired.parquet"
