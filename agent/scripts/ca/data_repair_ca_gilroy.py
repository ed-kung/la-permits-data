"""Data repair for Gilroy (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Gilroy DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Missing STATUS_NORMALIZED for CaseStatus ``Legacy - Open``
    (issued → Active; finaled → Final; else In Review).
  - One Issued / Active row missing PERMIT_DATE despite IssueDate in
    entity/details (entity stamp lacked a ``Z`` suffix) → FILLED.
  - Spurious FINAL_DATE on Inactive / In Review rows copied from
    entity.FinalDate as a case-closure stamp → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Mapped CaseStatus values other than Legacy - Open already match
    STATUS_NORMALIZED.
  - 32 Complete / Final rows with null IssueDate (many share a
    2023-06-13 bulk FinalDate stamp) → PERMIT_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Void / Expired / Fees Due is a case-closure stamp,
    not a permit finaled date.
"""
...
def _base_status(d: dict) -> Optional[str]:
    for raw in _status_strings(d):
        ...
        if mapped is not None:
            return mapped
        # Legacy - Open needs issuance / final evidence (handled below).
        if raw == "Legacy - Open":
            return "legacy_open"
        lower = raw.lower()
        ...
        if any(
            tok in lower
            for tok in ("review", "submitted", "fees due", "fees paid", "pending")
        ):
            return "In Review"
    return None
...
def _expected_status(d: dict) -> Optional[str]:
    """Map CaseStatus / PermitStatus; resolve Legacy - Open from dates.

    Inactive terminal labels are sticky. Legacy - Open shells with a
    FinalDate promote to Final; with IssueDate / Issued to Active;
    otherwise In Review.
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Gilroy record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Gilroy permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Gilroy".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Gilroy") & (df["STATE"] == "CA")
    ].copy()

    print(f"Gilroy records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_gilroy_repaired.parquet"
