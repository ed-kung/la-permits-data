"""Data repair for Chico (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Chico DATA is a civic portal payload (same shape as Saratoga / Oceanside /
Merced County). All sample rows share the same top-level keys:
...
Known issues repaired:
  - Stale STATUS_ORIGINAL lagging PermitStatus: FINALED still Active /
    In Review; ISSUED still In Review / Inactive; CERTIFICATE OF OCC
    still Active / Inactive; EXPIRED still Active → FIXED.
  - Blank PermitStatus with Applied only → FILLED as In Review.
  - Non-inactive rows carrying PermitFinaledDate (esp. sewer PAID /
    NOT PAID shells) → FIXED to Final.
  - MASTER PLAN APPROVED wrongly Final (approved only, never finaled)
    → FIXED to Active; PERMIT_DATE filled from Approved.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    (oversized-load ISSUED Approved-only; FINALED Approved-only) →
    FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present (incl.
    after status fix) → FILLED.
  - Spurious FINAL_DATE on Inactive WITHDRAWN (and any remaining
    non-Final after status repair) → cleared.

Not repairable / left as-is:
  - 10 FILE_DATE gaps (CLONE / VOID / APPLIED shells) lack Applied
    and search Application → stay missing.
  - A handful of Active / Final rows lack both Issued and Approved
    → PERMIT_DATE stays missing.
  - A few FINAL / FINALED / MASTER-remapped rows lack PermitFinaledDate
    and a usable final inspection → FINAL_DATE stays missing.
"""
...
_STATUS_MAP = {
    ...
    "CERTOFOC": "Final",
    "SEWER IN-LIEU EXECUTED": "Final",
    # Active (approval / issuance; not completion)
    "ISSUED": "Active",
    "MASTER PLAN APPROVED": "Active",
    # In Review
    "APPLIED": "In Review",
...
def _is_inactive_keep(raw: str) -> bool:
    ...
    if "EXPIRE" in raw or raw.startswith("VOID") or "CANCEL" in raw:
        return True
    if "WITHDRAW" in raw or raw == "REVOKED":
        return True
    return False
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Chico permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Chico".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Chico") & (df["STATE"] == "CA")].copy()

    print(f"Chico records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_chico_repaired.parquet"
