"""Data repair for Brentwood (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Brentwood DATA is a civic portal payload (same shape as San Carlos /
Los Altos / Dana Point). When present, rows share top-level keys:
...
A large share of sample rows have null DATA (``missing`` schema) and
cannot be repaired from JSON. Content variants when DATA is present:
...
Known issues repaired:
  - Missing STATUS_NORMALIZED for fee-stage statuses (F_PROJECTION /
    FEE PROJECTION), empty-status encroachment/grading shells, and
    misc labels (BRENTWOOD REPRO, TO BE BILLED) → FILLED.
  - FEE ESTIMATE previously mapped to Inactive → FIXED to In Review.
  - Non-inactive rows carrying PermitFinaledDate (ISSUED / APPROVED /
    ACCEPTED) → FIXED to Final.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    → FILLED (including blank-status shells inferred Active).
  - Spurious FINAL_DATE on Inactive WITHDRAWN (and any remaining
    non-Final after status repair) → cleared.

Not repairable / left as-is:
  - 1,185 rows with null DATA (status/dates already set upstream from
    STATUS_ORIGINAL; no JSON to validate or fill against).
  - 3 legacy shells with blank status and no Applied/Issued/Approved
    dates → STATUS and FILE_DATE stay missing.
  - FINALED rows lacking PermitFinaledDate and a passed final
    inspection (often Encroachment Inspect with empty Result) →
    FINAL_DATE stays missing.
  - One Active ISSUED row with neither Issued nor Approved →
...
_STATUS_MAP = {
    ...
    "PAID": "In Review",
    "FEES PAID": "In Review",
    "F_PROJECTION": "In Review",
    "FEE PROJECTION": "In Review",
    "FEE ESTIMATE": "In Review",
    "BRENTWOOD REPRO": "In Review",
    # Inactive
    "CANCELED": "Inactive",
    "CANCELLED": "Inactive",
    "WITHDRAWN": "Inactive",
...
def _lookup_status(raw: str) -> Optional[str]:
    ...
    if "PROJECTION" in raw or "ESTIMATE" in raw:
        return "In Review"
    if "REPRO" in raw:
        return "In Review"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Brentwood permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Brentwood".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Brentwood") & (df["STATE"] == "CA")].copy()

    print(f"Brentwood records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_brentwood_repaired.parquet"
