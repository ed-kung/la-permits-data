"""Data repair for Belmont (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Belmont DATA is a civic portal payload. All sample rows share the same
top-level keys: ``fees``, ``contacts``, ``site_info``, ``inspections``,
``permit_info``, ``search_data``. Canonical fields live under
``permit_info`` (``search_data`` only mirrors Address / RECORDID /
Permit Number — no date mirrors):
...
Known issues repaired:
  - 40 blank / DUPLICATE/CANCELED PermitStatus rows left
    STATUS_NORMALIZED null; fill from dates or cancel label.
  - FINALED mislabeled Active (12); ISSUED/ACTIVE/APPROVED*
    mislabeled In Review; EXPIRED mislabeled Active → FIXED.
  - APPROVED / similar rows with PermitFinaledDate present → Final
    (upstream left them Active while copying FINAL_DATE).
  - NO CALL FOR FIN mislabeled In Review → Final.
...
Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate wherever Applied exists;
    13 rows lack Applied (8 FINALED with only Issued/Finaled; 5
    status-only shells). Do not backfill FILE_DATE from Issued.
  - ~50 Final / finalish rows lack PermitFinaledDate and usable final
    inspections (mostly NO FINAL CALL legacy).
  - Active/Final rows with neither Issued nor Approved → PERMIT_DATE
    stays missing.
  - Blank-status shells with only Applied stay In Review with no
...
# PermitStatus (uppercased / prefix-normalized) → STATUS_NORMALIZED
_STATUS_MAP = {
    "FINALED": "Final",
    "FINAL": "Final",
    "NO FINAL CALL": "Final",
    "NO CALL FOR FIN": "Final",
    "NO CALL FINAL": "Final",
    "COMPLETED TRANSACTION": "Final",
    "ACTIVE": "Active",
    "ISSUED": "Active",
    "APPROVED": "Active",
    "ALARM PERMIT ISSUED": "Active",
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then truncated APPROVED* / CANCELED* prefixes."""
    if not raw:
        return None
    ...
    if raw.startswith("APPROVED"):
        return "Active"
    if raw.startswith("CANCELED") or raw.startswith("CANCELLED"):
        return "Inactive"
    return None
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if raw in _INACTIVE_KEEP or (
        raw.startswith("CANCELED") or raw.startswith("CANCELLED")
    ):
...
def _final_from_inspections(d: dict):
    """Latest completion date from a passed FINAL inspection (dict rows)."""
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Belmont record."""
    # -- STATUS_NORMALIZED --
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Belmont permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Belmont".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Belmont") & (df["STATE"] == "CA")].copy()

    print(f"Belmont records: {len(city):,}\n")

    repaired = data_repair(city)

    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "belmont_repaired_sample.parquet")
