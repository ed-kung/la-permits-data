"""Data repair for Oceanside (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Oceanside DATA is a civic portal payload (same shape as Novato /
San Clemente / Belmont). All sample rows share the same top-level keys:
...
``permit_info``:
  ...
  - PermitAppliedDate (fallback:
    search_data.Application)              → FILE_DATE
...
Content variants (same keys; differ by which fields are populated):
  ...
  - permit_info_empty:          blank permit_info shell

Known issues repaired:
  - Stale STATUS_NORMALIZED vs PermitStatus (FINALED still Active
    because STATUS_ORIGINAL lagged at issued; ISSUED still In Review
    because STATUS_ORIGINAL lagged at received; APPROVED / WITHDRAWN
    similarly lagging) → FIXED.
  - Blank PermitStatus with Applied date only → FILLED as In Review.
  - Non-inactive rows with PermitFinaledDate (ISSUED / UNDER REVIEW)
    promoted to Final.
  ...
  - Final missing FINAL_DATE when PermitFinaledDate present (incl.
    after status fix from stale ISSUED) → FILLED; one FINALED row
    recovered from a passed **905 FINAL SFR inspection.
  - Spurious FINAL_DATE on Inactive EXPIRED row → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate wherever Applied
    exists; 4 rows lack Applied and search_data.Application → stay
    missing.
  - ~7 FINALED rows lack PermitFinaledDate and a usable final
    inspection → FINAL_DATE stays missing.
  - Some Final / Active rows lack both Issued and Approved →
    PERMIT_DATE stays missing.
  - One empty FIRE shell has no status or dates → STATUS stays null.
  - PAID / READY TO BILL water permits with Issued dates stay
    In Review (status is payment workflow, not issuance).
"""
...
_STATUS_MAP = {
    ...
    "PAID": "In Review",
    "UNPAID": "In Review",
    "READY TO BILL": "In Review",
    "RTRND FOR CORRECTION": "In Review",
    "NSF-RTND CHECK": "In Review",
    "EXPIRED": "Inactive",
    "WITHDRAWN": "Inactive",
    "VOID": "Inactive",
    "CANCELED": "Inactive",
...
# Oceanside uses coded finals like "**905 FINAL SFR" / "**920F FINAL".
_FINAL_TITLE_RE = re.compile(
    r"(?i)("
    r"\*\*\d*\s*f?\s*final\b|"
    r"final\s*inspection|permit\s*final|building\s*final|"
    r"final\s*building|final\s*bldg|\*\*final\b|"
    r"\bfinal\s*(sfr|bldg|building|elec|electrical|fire|roof)?\b|"
    r"final-complete|final-building|"
...
def _preferred_file_date(d: dict):
    """Application / submittal date from PermitAppliedDate, else search Application."""
    applied = _pi_date(d, "PermitAppliedDate")
    if applied is not pd.NaT:
        return applied
    sd = d.get("search_data")
    if isinstance(sd, dict):
        for key in ("Application", "APPLIED", "Applied"):
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Oceanside permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Oceanside".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Oceanside") & (df["STATE"] == "CA")].copy()

    print(f"Oceanside records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_oceanside_repaired.parquet"
