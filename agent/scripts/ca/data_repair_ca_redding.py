"""Data repair for Redding (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Redding DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED null for Waiting for Resubmit (upstream mapper
    missed this) → FILLED as In Review.
  - Stale STATUS_NORMALIZED vs DATA (Active while Finaled or Expired;
    In Review while Void) when STATUS_ORIGINAL lagged CaseStatus
    → FIXED.
  - Missing PERMIT_DATE on Issued/Active with IssueDate present
    → FILLED.
  - Missing FINAL_DATE on rows corrected Active→Final where FinalDate
    is present → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Void / Expired / Issued
    shells that still carry entity.FinalDate) → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Three Finaled rows (FIRE / GRAD) have Issued=False and null
    IssueDate → PERMIT_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Void / Expired is an application-close stamp, not a
    permit finaled date, and must not promote status to Final.
"""
...
_STATUS_MAP = {
    ...
    "Void": "Inactive",
    "Denied": "Inactive",
    "Fee Estimate": "Inactive",
    # In Review (pre-issuance / application)
    "Pending/In Plan Review": "In Review",
    "Ready to Issue": "In Review",
    "Online Permit (Pending Review)": "In Review",
    "Waiting for Resubmit": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Redding record."""
    ...
    # Note: non-Finaled statuses (Void/Expired/Issued) sometimes carry
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Redding permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Redding".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Redding") & (df["STATE"] == "CA")].copy()

    print(f"Redding records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_redding_repaired.parquet"
