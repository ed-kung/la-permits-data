"""Data repair for Arcadia (CA) permit records.
...
Known issues repaired:
  - STATUS_NORMALIZED incorrectly set to Final for CaseStatus=Estimate
    (25 sample records). Estimates are pre-issuance water/utility quotes
    with Issued=False and null IssueDate/FinalDate → In Review.

Not repairable from DATA:
  - 13 Active (Approved) and 8 Final (Complete, legacy) records have null
    IssueDate and Issued=False, so PERMIT_DATE cannot be filled.
    ApplyDate≈IssueDate is common (~80%) but not reliable enough to use
    FILE_DATE as a proxy (same convention as Alhambra).
  - 3 Final (Complete) records have FinalDate / FinalizeDate in years
    2205–5201 (corrupt sentinel values). Upstream correctly left
    FINAL_DATE null; these are rejected by a plausibility gate.
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Arcadia record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Arcadia permit records using information from the raw DATA JSON column.
...
if __name__ == "__main__":
    ...
    arcadia = df[(df["JURISDICTION"] == "Arcadia") & (df["STATE"] == "CA")].copy()

    print(f"Arcadia records: {len(arcadia):,}\n")

    repaired = data_repair(arcadia)
    ...
    for field in ["STATUS_NORMALIZED", "FILE_DATE", "PERMIT_DATE", "FINAL_DATE"]:
        ...
        before_missing = arcadia[field].isna().sum()
    ...
    print("  Before:")
    for s, c in arcadia["STATUS_NORMALIZED"].value_counts(dropna=False).items():
