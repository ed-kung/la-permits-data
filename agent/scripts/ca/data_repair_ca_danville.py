"""Data repair for Danville (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Danville DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - CaseStatus Finaled shells left Active with null FINAL_DATE
    → FIXED to Final; FINAL_DATE FILLED from FinalDate / FinalizeDate.
  - Stale CaseStatus Issued shells that already carry FinalizeDate
    (entity.FinalDate null) → FIXED to Final; FINAL_DATE FILLED.
  - Approved (plans approved, not issued) previously Active
    → FIXED to In Review unless IssueDate / FinalDate evidence upgrades.
  - Expired / Voided / Canceled mislabeled Active or In Review
    → FIXED to Inactive; spurious FINAL_DATE cleared.
  - Issued mislabeled In Review → FIXED to Active; PERMIT_DATE FILLED.
  - FINAL_DATE day mismatch vs entity.FinalDate → FIXED.
  - Spurious FINAL_DATE on Inactive (Expired / Canceled closure stamps)
    → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Finaled / Final rows with null IssueDate → PERMIT_DATE stays missing;
    DATA has no alternate issuance stamp (common on older electrical).
...
_STATUS_MAP = {
    ...
    # In Review (Approved = plan approval, not issuance, unless
    # IssueDate / FinalDate evidence upgrades the status)
    "Approved": "In Review",
    "Received - CAP": "In Review",
    "Applied": "In Review",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Voided / Canceled / Withdrawn /
    Approval Expired / Application Expired) are sticky even when
    FinalDate is present as a case-closure stamp. Credible FinalDate /
    FinalizeDate → Final overrides stale Issued / Approved / Finaled-
    mislabeled Active. CaseStatus Finaled / Complete stays Final even
    when FinalDate is absent. Otherwise IssueDate → Active overrides
    review-pipeline labels (including Approved).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Danville record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Danville permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Danville".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Danville") & (df["STATE"] == "CA")
    ].copy()

    print(f"Danville records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_danville_repaired.parquet"
