"""Data repair for Rocklin (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Rocklin DATA is a single flat portal schema (all sample rows share the
...
Known issues repaired:
  - 7 blank PermitStatus rows (PRE* permit requests) → FILLED as
    In Review from presence of PermitAppliedDate only.
  - 1 APPROVED W/COND row incorrectly mapped to In Review → FIXED to
    Active.
  - 9 Active / In Review rows with a true PermitFinaledDate but stale
    ISSUED / PENDING status → FIXED to Final (VOID stays Inactive).
  - Active/Final rows missing PERMIT_DATE with empty Issued but
    populated Approved → FILLED from PermitApprovedDate (~59).
  - Spurious FINAL_DATE on non-Final rows (legacy VOID with a finaled
    stamp after status upgrade of the rest) → cleared (FIXED).

Not repairable from DATA:
  - 22 FILE_DATE gaps (empty PermitAppliedDate / APPLIED; mostly VOID).
  - ~7 Active and ~10 Final rows lack both Issued and Approved →
    PERMIT_DATE stays missing.
  - ~12 Final rows (COMPLETED police/fee shells, CLOSED legacy, one
    FINALED SFR) lack PermitFinaledDate and usable finaling
...
def _map_permit_status(raw) -> Optional[str]:
    ...
    if not key:
        return None
    if "FINAL" in key or key.startswith("COMPLET"):
        return "Final"
    if "APPROV" in key:
        return "Active"
...
def _derive_status(pi: dict) -> Optional[str]:
    ...
    (VOID close stamps are not treated as building-permit sign-offs).

    In Review stays In Review unless a true PermitFinaledDate is present
    (legacy PENDING shells that were finaled). ISSUED / APPROVED / blank
    with a PermitFinaledDate are remapped to Final.
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Rocklin record."""
    pi = _permit_info(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Rocklin permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Rocklin".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Rocklin") & (df["STATE"] == "CA")].copy()

    print(f"Rocklin records: {len(city):,}\n")
