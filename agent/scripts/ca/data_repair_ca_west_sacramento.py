"""Data repair for West Sacramento (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

West Sacramento DATA is an Accela Citizen Access scrape. All sample rows
share top-level keys (``address``, ``date``, ``status``, ``tasks``,
``search_data``, …); a handful omit ``inspections`` / ``conditions``.
Workflow events use ``Marked as <status> on <date>`` (HTML spans and/or
structured keys), not Menlo Park's ``Completed on … as …`` form.
...
Canonical mappings:
  ...
  - Earliest of DATA.date / search_data.Date / Application
    Submittal first-touch marks (Route / Accepted / …)      → FILE_DATE
  - Earliest Ready to Issue / Application Submittal Issued*
    (fallback: any Issued* / Renewal Issued / TCO Issued)   → PERMIT_DATE
  - Earliest Finaled (any task); else C of O Issued; else
    Final Processing Final Approved; else Closed/Close on
    Closed; else final-titled inspection Status Date        → FINAL_DATE

Known issues repaired:
  - Estimate shells labeled Final despite empty Issued /
    Finaled workflow → FIXED to In Review.
  - Temp C of O Issued labeled Final → FIXED to Active.
  - Revision Pending / other In Review labels with dated
    Issued* → FIXED to Active (status lag).
  - FILE_DATE lagging earlier Application Submittal Route /
    Accepted marks after Accela bumped DATA.date → FIXED.
  - PERMIT_DATE often set to Processing Ready to Issue (or
    Building Approved) rather than Ready to Issue Issued → FIXED.
  - Missing PERMIT_DATE on Active / Final / post-issue Inactive
    when Issued* exists → FILLED.
  - Spurious PERMIT_DATE on In Review without Issued evidence
    → cleared.
  - FINAL_DATE systematically set to Issued / Ready-to-Issue
    rather than Finaled / Final Approved → FIXED.
  - Missing FINAL_DATE on Final when Finaled / Final Approved /
    Closed evidence exists → FILLED.
  - Spurious FINAL_DATE on non-Final rows → cleared.

Not repairable / left as-is:
  - Many older Finaled / Issued shells have empty or TBD-only
    task histories → PERMIT_DATE / FINAL_DATE stay missing.
  - Estimate / Applied / In Review shells with no dated Issued
    marks correctly keep PERMIT_DATE / FINAL_DATE missing.
"""
...
# Accela HTML: Marked as <span>…</span> on <span>…</span>
_MARKED_ON_RE = re.compile(
    r"Marked as\s*<span[^>]*>([^<]*)</span>"
    r"\s*on\s*<span[^>]*>([^<]*)</span>",
    re.I,
)

# Fallback: Completed on <span>…</span> … as <span>…</span>
...
_FINAL_APPROVED_MARKS = {"Final Approved"}
_CO_MARKS = {"C of O Issued"}
_CLOSE_MARKS = {"Closed", "Close"}
_FILE_MARKS = {
    "Route",
    "Route Application",
    "Accepted for Review",
...
def _event_mark_and_date(event: dict):
    """Return (marked_as, event_date) for West Sacramento Accela events.

    Prefer HTML ``Marked as … on …`` spans; also accept structured
    ``Marked as`` / ``on`` keys and Menlo-style ``Completed on … as …``.
    """
    if not isinstance(event, dict):
        return None, pd.NaT

    # Structured keys first (``Marked as`` / ``on``). Do NOT use the free-text
    # ``status`` field here — it is often "Marked as X on DATE by USER" and
    # would shadow the clean HTML span value.
    mark = _event_field(event, "Marked as", "as")
    on = _safe_to_datetime(_event_field(event, "on", "Completed on", "Completed On"))

    html = event.get("html") or ""
    if isinstance(html, str) and html:
        m = _MARKED_ON_RE.search(html)
        if m:
            html_mark = m.group(1).strip()
            html_on = _safe_to_datetime(m.group(2))
            if html_mark and html_mark.upper() != "TBD":
                mark = html_mark
            if html_on is not pd.NaT:
                on = html_on
        else:
            m2 = _COMPLETED_AS_RE.search(html)
            if m2:
                html_mark = m2.group(2).strip()
                html_on = _safe_to_datetime(m2.group(1))
                if html_mark and html_mark.upper() != "TBD":
    ...
    if isinstance(mark, str):
        mark = mark.strip()
        if not mark or mark.upper() == "TBD":
            mark = None
        # Guard against leftover free-text blobs.
        elif mark.lower().startswith("marked as "):
            mark = None
    else:
        mark = None
    return mark, on
...
_STATUS_MAP = {
    ...
    "Closed": "Final",
    "Complete": "Final",
    # Active (open / issued / enforcement)
    "Issued": "Active",
    "Issued w/Revisions": "Active",
    "Active": "Active",
    "Enforcement": "Active",
    "Inspections": "Active",
    "Renewed": "Active",
    "Temp C of O Issued": "Active",
    # Inactive
    "Expired": "Inactive",
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Complete maps to Final. A dated Issued* promotes In Review → Active.
    A dated Finaled / Final Approved promotes In Review → Final. Final
    evidence alone does **not** promote Issued → Final when the portal
    status is still Issued / Active.
    """
...
def _file_date_from_data(d: dict):
    ...
    earliest Application Submittal / Distribution first-touch marks.
    Accela sometimes bumps the top-level date on re-open while the
    original submittal remains in the task history.
    """
    candidates = []
...
def _permit_date_from_data(d: dict):
    """Earliest true issuance date from Accela workflow marks.

    Prefer ``Ready to Issue`` / ``Issued``, then OTC ``Application
    Submittal`` / ``Issued``, then any Issued* / Renewal Issued /
    TCO Issued mark.
    """
    tasks = d.get("tasks") or []

    rti = _first_event_date(tasks, {"Ready to Issue", "Issue"}, {"Issued", "Issued w/Revisions"})
    if rti is not pd.NaT:
        return rti

    otc = _first_event_date(tasks, {"Application Submittal"}, {"Issued", "Issued w/Revisions"})
    if otc is not pd.NaT:
        return otc

    return _first_mark_date(tasks, _ISSUE_MARKS)
...
def _final_date_from_data(d: dict):
    """Final / completion / signoff date for West Sacramento Accela records.

    Prefer any task ``Finaled`` mark, then ``C of O Issued``, then
    ``Final Processing`` / ``Final Approved``, then ``Closed`` /
    ``Close`` on the Closed task, then a final-titled inspection
    Status Date. Do **not** treat Ready-to-Issue / Issued as final.
    """
    tasks = d.get("tasks") or []
    ...
    co = _first_mark_date(tasks, _CO_MARKS)
    if co is not pd.NaT:
        return co

    fa = _first_event_date(tasks, {"Final Processing"}, _FINAL_APPROVED_MARKS)
    if fa is not pd.NaT:
        return fa

    # Broader Final Approved (e.g. Certificate of Occupancy task)
    fa_any = _first_mark_date(tasks, _FINAL_APPROVED_MARKS)
    if fa_any is not pd.NaT:
        return fa_any
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if file_date is not pd.NaT:
        if pd.isna(row["FILE_DATE"]):
            ...
            # Only pull FILE_DATE earlier (Accela re-open bump).
    ...
    if not pd.isna(current_permit):
        if issued is not pd.NaT and not _dates_equal(current_permit, issued):
            ...
            # Spurious permit stamp from Ready-to-Issue / Approved.
        ...
        # Expired / withdrawn after issuance — still record issue date.
        repairs["PERMIT_DATE"] = issued
        repairs["PERMIT_DATE_FLAG"] = "FILLED"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    West Sacramento permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "West Sacramento". Must contain
    ...
    # Normalize date columns up front so repairs don't mix datetime.date
    # with Timestamps (breaks parquet).
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "West Sacramento") & (df["STATE"] == "CA")
    ].copy()

    print(f"West Sacramento records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_west_sacramento_repaired.parquet"
