"""Data repair for San Clemente (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Clemente DATA is a civic portal payload (same shape as Hemet /
Mendocino County / Belmont). All sample rows share the same top-level
...
Known issues repaired:
  - Blank PermitStatus (TRANSPORTATION shells with Issued) left
    STATUS_NORMALIZED null → FILLED to Active.
  - Stale STATUS_NORMALIZED vs PermitStatus (FINALED mislabeled Active
    because STATUS_ORIGINAL lagged at 'issued') → FIXED to Final.
  - Rows with PermitFinaledDate but non-Final PermitStatus
    (OPENED / APPROVED) → FIXED to Final.
  - STOP WORK mapped as In Review upstream → FIXED to Inactive.
...
Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate on every sample row.
  - CLOSED / legacy letter-code O Final rows lack PermitFinaledDate and
    usable final inspections → FINAL_DATE stays missing (~80 rows).
  - Some Final rows lack both Issued and Approved → PERMIT_DATE stays
    missing.
  - Legacy single-letter statuses P/G/I stay In Review (and N Inactive,
    O Final) to match the upstream CRW mapping, even when an Issued date
    is present.
"""
...
_STATUS_MAP = {
    "FINALED": "Final",
    "CLOSED": "Final",
    "O": "Final",  # legacy CRW code
    "ISSUED": "Active",
    "APPROVED": "Active",
    "ACTIVE": "Active",
    "PLAN CHECK": "In Review",
    "P": "In Review",  # legacy CRW
    "G": "In Review",  # legacy CRW
    "I": "In Review",  # legacy CRW
    "OPENED": "In Review",
    "ON HOLD": "In Review",
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Clemente permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Clemente".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "San Clemente") & (df["STATE"] == "CA")].copy()

    print(f"San Clemente records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_clemente_repaired.parquet"
