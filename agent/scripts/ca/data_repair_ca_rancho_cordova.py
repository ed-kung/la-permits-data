"""Data repair for Rancho Cordova (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Rancho Cordova DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED on ``Missing Information``, ``Field Acceptance``,
    ``In Punch List``, and ``Warranty Accepted – Closeout Items Due``
    → FILLED (In Review, or Final when a credible FinalDate exists).
  - ``Issued`` shells with FinalDate/FinalizeDate strictly after IssueDate
    (or PermitStatus ``Final``) left Active → FIXED to Final.
  - Post-issuance ``Fees Due`` / ``Fees Paid`` / ``On Hold`` / ``Warranty``
    / ``Stop Work Order`` / ``Plan Approved`` rows with IssueDate left
    In Review → FIXED to Active (or Final when FinalDate > IssueDate).
  - Junk FinalDate stamps on non-Final rows (Issued with FinalDate ≤
    IssueDate; Inactive / In Review closure stamps) → FINAL_DATE cleared.
  - Two ``Issued`` Active rows with IssueDate but null PERMIT_DATE → FILLED.
...
Not repairable / left as-is:
  ...
  - ``Admin Closed`` / ``Recorded`` Final shells and a few Issued /
    In Construction Active shells lack IssueDate and/or FinalDate in
    DATA → PERMIT_DATE / FINAL_DATE stay missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Void is a case-closure stamp, not a permit
...
_STATUS_MAP = {
    ...
    "Denied": "Inactive",
    "Rejected": "Inactive",
    # In Review (pre-issuance / plan pipeline)
    "Submitted": "In Review",
    ...
    "Warranty": "In Review",
    "Missing Information": "In Review",
    "Field Acceptance": "In Review",
    "In Punch List": "In Review",
    "Warranty Accepted – Closeout Items Due": "In Review",
    "Warranty Accepted - Closeout Items Due": "In Review",
}
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Rancho Cordova occasionally leaves CaseStatus as Issued / Fees Due /
    Fees Paid while FinalDate/FinalizeDate is strictly after IssueDate.
    Require either an explicit Final / Completed Project / Admin Closed /
    Recorded label, or FinalDate/FinalizeDate strictly after IssueDate.
    """
...
def _mapped_status(d: dict) -> Optional[str]:
    for raw in _status_strings(d):
        ...
        if mapped is not None:
            return mapped
        # Normalize en-dash / hyphen variants for warranty-accepted shells.
        raw_norm = raw.replace("–", "-").replace("—", "-")
        mapped = _STATUS_MAP.get(raw_norm)
        if mapped is not None:
            return mapped
        lower = raw.lower()
        if (
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Void / Denied / Plan Approval
    Expired) are sticky even when FinalDate is present as a case-closure
    stamp. Explicit Final / Completed Project / Admin Closed / Recorded
    labels (from either CaseStatus or PermitStatus) → Final. Otherwise a
    FinalDate/FinalizeDate strictly after IssueDate → Final (stale Issued
    / Fees Due / Fees Paid shells). IssueDate → Active overrides
    review-pipeline labels (Fees Due / Fees Paid / On Hold / Warranty /
    Stop Work Order / Plan Approved / Missing Information).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Rancho Cordova record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Rancho Cordova permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Rancho Cordova".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Rancho Cordova") & (df["STATE"] == "CA")
    ].copy()

    print(f"Rancho Cordova records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_rancho_cordova_repaired.parquet"
    ...
    print("\nFinal still missing FINAL_DATE (by CaseStatus / CaseType):")
    gap = Counter()
    for idx in repaired.index:
        ...
        d = _safe_parse(city.at[idx, "DATA"])
        entity = (d or {}).get("entity") or {}
        gap[(entity.get("CaseStatus"), entity.get("CaseType"))] += 1
    for k, v in gap.most_common():
        print(f"  {k}: {v}")
