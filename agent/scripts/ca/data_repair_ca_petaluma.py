"""Data repair for Petaluma (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Petaluma DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Missing STATUS_NORMALIZED for CaseStatus ``Utility Restoration``
    (issued, no final) → FILLED as Active.
  - Stale CaseStatus on issued / finaled shells still labeled Issued,
    Fees Due, Fees Paid, or In Review → FIXED using FinalDate /
    IssueDate evidence (Inactive terminal labels stay sticky).
  - Spurious FINAL_DATE on Inactive rows (Void / Expired closure
    stamps copied from entity.FinalDate) → cleared.
...
Not repairable / left as-is:
  ...
  - Mapped dates that are already populated match entity stamps at
    day resolution (no date FIXED values in the sample).
  - 152 Complete / Final rows with null IssueDate (mostly
    encroachment / utility / tree-removal shells) → PERMIT_DATE
    stays missing; DATA has no alternate issuance stamp.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Void / Expired is a case-closure stamp, not a
...
def _mapped_status(d: dict) -> Optional[str]:
    for raw in _raw_labels(d):
        ...
        if any(
            tok in lower
            for tok in ("expired", "void", "denied", "withdrawn", "revoked")
        ):
            return "Inactive"
        if lower.startswith("issued") or "utility restoration" in lower:
            return "Active"
        if any(
            tok in lower
            for tok in (
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    when FinalDate is present as a case-closure stamp. Otherwise
    FinalDate → Final and IssueDate → Active override stale review /
    issued labels (e.g. Fees Due or Issued shells that already carry
    FinalDate).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Petaluma record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Petaluma permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Petaluma".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Petaluma") & (df["STATE"] == "CA")
    ].copy()

    print(f"Petaluma records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_petaluma_repaired.parquet"
