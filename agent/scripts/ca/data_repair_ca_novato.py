"""Data repair for Novato (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Novato DATA is a civic portal payload (same shape as San Clemente /
...
Known issues repaired:
  - Missing STATUS_NORMALIZED for INFO OR FEE REQ / PLAN CHK IN
    PROGRESS (and blank-status shells with Issued/Approved) → FILLED.
  - Stale STATUS_NORMALIZED vs PermitStatus (FINALED still Active or
    In Review because STATUS_ORIGINAL lagged; ISSUED still In Review
    or null) → FIXED.
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    present (common on PERFORMED / CLOSED resale reports) → FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present (incl.
    after status fix from stale ISSUED/PENDING) → FILLED.
  - Spurious FINAL_DATE on non-Final rows (REFUNDED / INACTIVE) →
    cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate wherever Applied
    exists; 5 rows lack Applied (and search_data APPLIED) → stay
    missing.
  - PERFORMED / CLOSED / COMPLETE Final rows often lack Issued,
    Approved, and PermitFinaledDate → PERMIT_DATE / FINAL_DATE stay
    missing (~50–100 rows).
  - Two FINALED rows lack PermitFinaledDate and a passed **FINAL
    inspection → FINAL_DATE stays missing.
  - UNLOCK resale scheduling shells stay In Review (approved date
    only; no issuance).
"""
...
_STATUS_MAP = {
    ...
    "APPROVED": "Active",
    "ACTIVE": "Active",
    "UNLOCK": "In Review",  # resale unlock scheduling; no issuance
    "INFO OR FEE REQ": "In Review",
    "PENDING": "In Review",
    "PLAN CHECKING": "In Review",
    "PLAN CHK IN PROGRESS": "In Review",
    "PLAN CHECK": "In Review",
    "READY TO ISSUE": "In Review",
    "TECH REVIEW COMPLETE": "In Review",
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then EXPIRED* / CANCELLED* / FINAL* prefixes."""
    if not raw:
        return None
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if raw in _INACTIVE_KEEP or raw.startswith("EXPIRED") or raw.startswith(
        "CANCEL"
    ):
    ...
    if raw:
        if "FINAL" in raw or "COMPLETED" in raw or "CLOSED" in raw or "PERFORMED" in raw:
            return "Final"
        if (
            "EXPIRE" in raw
            or "VOID" in raw
            ...
            return "Inactive"
        if "ACTIVE" in raw or "ISSUE" in raw or "APPROV" in raw or "REISSUE" in raw:
            return "Active"
        if (
            "REVIEW" in raw
            or "PENDING" in raw
...
def _preferred_file_date(d: dict):
    """Application / submittal date from PermitAppliedDate, else search APPLIED."""
    applied = _pi_date(d, "PermitAppliedDate")
    if applied is not pd.NaT:
        return applied
    sd = d.get("search_data")
    if isinstance(sd, dict):
        return _safe_to_datetime(sd.get("APPLIED"))
    return pd.NaT
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Novato permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Novato".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Novato") & (df["STATE"] == "CA")].copy()

    print(f"Novato records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_novato_repaired.parquet"
