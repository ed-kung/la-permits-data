"""Data repair for Merced County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Merced County DATA is a civic portal payload (same shape as Oceanside /
Novato / San Clemente). All sample rows share the same top-level keys:
...
Known issues repaired:
  - ESTIMATE / RED TAG wrongly mapped to Final (no finaling
    evidence) → FIXED to In Review / Inactive.
  - RETIRED mapped to In Review → FIXED to Inactive.
  - Stale STATUS_ORIGINAL=issued while PermitStatus=FINALED
    (and ISSUED/ACTIVE rows carrying PermitFinaledDate)
    → FIXED to Final.
  - Blank PermitStatus with Applied date only → FILLED as
    In Review.
  - PERMIT_DATE set to Applied instead of Issued (1 ACTIVE
    row) → FIXED from PermitIssuedDate.
  - Active/Final missing PERMIT_DATE when Issued or Approved
    present → FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present
    (incl. after status fix) → FILLED; a few FINALED rows
    recovered from passed Final Inspection / FINAL-FIRE.
  - Spurious FINAL_DATE on non-Final rows → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate wherever
    Applied exists; 4 rows lack Applied → stay missing.
  - ~3 FINALED rows lack PermitFinaledDate and a usable
    final inspection → FINAL_DATE stays missing.
  - Some Active / Final rows lack both Issued and Approved
    → PERMIT_DATE stays missing (ESTIMATE reclassed away
    from Final accounts for most of the prior Final gap).
  - APPLIED / In Review rows that carry Issued dates in
    DATA keep those PERMIT_DATE values (source quirk).
"""
...
_STATUS_MAP = {
    # Final
    "FINALED": "Final",
    "FINALED/ DONE 1": "Final",
    # Active
    "ISSUED": "Active",
    "ISSUED - 1ST EXT": "Active",
    "ISSUED - 2ND EX": "Active",
    "ISSUED/REINSTAT": "Active",
    "ACTIVE": "Active",
    "APPROVED": "Active",
    "30(+) DAYS PAST DUE": "Active",
    # In Review
    "APPLIED": "In Review",
    "ESTIMATE": "In Review",
    "APPROVED (PLANS)": "In Review",
    "MASTERPLANS": "In Review",
    # Inactive
    "PERMIT EXPIRED": "Inactive",
    "APPLICATION EXPIRED": "Inactive",
    "APPLICATION EXP": "Inactive",
    "APPL EXPIRED/DE": "Inactive",
    "CANCELED": "Inactive",
    "CANCELLED": "Inactive",
    "RETIRED": "Inactive",
    "RED TAG": "Inactive",
...
# Merced final inspection titles (plain "Final Inspection", FINAL-FIRE, etc.).
_FINAL_TITLE_RE = re.compile(
    r"(?i)("
    r"^final\s*inspection$|"
    r"^final-|"
    r"final\s*inspection|"
    r"permit\s*final|"
    r"building\s*final|"
    r"final\s*building|"
    r"final\s*bldg|"
    r"\bfinal\s*(sfr|bldg|building|elec|electrical|fire|roof|eh)?\b|"
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then EXPIRED* / CANCELLED* / FINAL* / ISSUED* prefixes."""
    if not raw:
        return None
...
def _is_inactive_keep(raw: str) -> bool:
    ...
    if raw in _INACTIVE_KEEP:
        return True
    if "EXPIRE" in raw or raw.startswith("CANCEL"):
        return True
    if raw == "RED TAG" or raw == "RETIRED":
        return True
    return False
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if raw:
        ...
        if (
            ...
            return "Inactive"
        if "ISSUE" in raw or raw == "ACTIVE" or "APPROV" in raw:
            return "Active"
        if (
            "APPLI" in raw
            or "REVIEW" in raw
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Merced County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Merced County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Merced County") & (df["STATE"] == "CA")].copy()

    print(f"Merced County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_merced_county_repaired.parquet"
