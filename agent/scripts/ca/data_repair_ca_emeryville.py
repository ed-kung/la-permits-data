"""Data repair for Emeryville (CA) permit records.
...
Emeryville's Dewey / sample payload has **no usable DATA JSON** on any
row (confirmed in both ``permits_ca_sample.parquet`` and the upstream
``building-permits-united-states_0_5_0.snappy.parquet`` shard). All
records therefore receive ``INFERRED_SCHEMA = "missing"``.

Without agency JSON, date fields cannot be validated or filled from
source. The only structured agency signal available is
``STATUS_ORIGINAL``, which is used to correct a known
``STATUS_NORMALIZED`` mis-map:

  - ``tco`` (Temporary Certificate of Occupancy) was normalized to
    ``In Review``; TCO is post-issuance occupancy and maps to ``Active``.

Canonical ``STATUS_ORIGINAL`` → ``STATUS_NORMALIZED`` map (for fill /
fix):

  - finaled                         → Final
  - issued, approved, tco           → Active
  - under review, on hold, pending  → In Review
  - expired, withdrawn, void        → Inactive

Known issues repaired:
  - ``tco`` rows left In Review → FIXED to Active.
...
Not repairable / left as-is:
  ...
  - FINAL_DATE missing on every row, including all Final / finaled
    shells (no Finaled / completion field and no DATA).
  - PERMIT_DATE already present on every sample row, including In Review
    / Inactive; without DATA it cannot be confirmed or corrected.
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    """Emeryville sample has no DATA; any future payload → unknown."""
...
def _expected_status_from_original(status_original) -> Optional[str]:
    ...
    if "final" in key:
        return "Final"
    if key.startswith("issued") or key in {"active", "tco"}:
        return "Active"
...
def _repair_record(row, d: Optional[dict], repairs: dict):
    """Populate *repairs* for a single Emeryville record.
    ...
    STATUS_ORIGINAL. Date fields are only touched if DATA later exposes
    canonical keys (none known today).
    """
    ...
    # No agency date fields are available in DATA for Emeryville. Leave
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Emeryville permit records.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Emeryville".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Emeryville") & (df["STATE"] == "CA")
    ].copy()

    print(f"Emeryville records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_emeryville_repaired.parquet"
