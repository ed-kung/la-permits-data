"""Data repair for Westminster (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Westminster DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - ``Issued`` / ``Approved`` shells with FinalDate/FinalizeDate
    strictly after IssueDate left Active → FIXED to Final.
  - ``Approved`` shells with no IssueDate coded Active → FIXED to
    In Review (plan approval, not issuance).
  - ``Submitted`` / ``On Hold`` / ``Stop Work Order`` shells that
    already carry IssueDate left In Review → FIXED to Active.
  - Junk FINAL_DATE on non-Final rows (Expired closure stamp; Active
    rows before status promotion) → cleared or retained after status
    fix as appropriate.
  - Sentinel IssueDate years (e.g. 3017 / 9018) ignored as null.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Active/Final rows with null IssueDate (Approved-without-issue
    remapped away; Complete/Legacy/Issued shells with blank IssueDate,
    including Issued=False investigation/solar shells and one 3017
    sentinel) → PERMIT_DATE stays missing.
  - Complete / Legacy Final rows with null FinalDate/FinalizeDate →
    FINAL_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired is a case-closure stamp, not a permit finaled
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Westminster occasionally leaves CaseStatus as Issued/Approved while
    FinalDate/FinalizeDate is strictly after IssueDate. Require either
    an explicit Complete / Closed / Finaled / Legacy label, or
    FinalDate/FinalizeDate strictly after IssueDate.
    """
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    / Approved shells). IssueDate → Active overrides review-pipeline
    labels (Submitted / On Hold / Stop Work Order / Approved). Approved
    without IssueDate stays In Review.
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Westminster record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Westminster permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Westminster".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Westminster") & (df["STATE"] == "CA")
    ].copy()

    print(f"Westminster records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_westminster_repaired.parquet"
