"""Data repair for Vacaville (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Vacaville DATA is a civic portal payload. All sample rows share the same
top-level keys: ``fees``, ``contacts``, ``site_info``, ``inspections``,
``permit_info``, ``search_data``. Canonical fields live under
``permit_info`` (with ``search_data`` as a redundant mirror for ISSUED /
FINALED):

  - PermitStatus                          → STATUS_NORMALIZED
  - PermitAppliedDate                     → FILE_DATE
  - PermitIssuedDate (fallback: Approved,
    then search_data ISSUED)              → PERMIT_DATE
  - PermitFinaledDate (fallback: SD
    FINALED, then passed FINAL insp)      → FINAL_DATE
...
Known issues repaired:
  - Stale STATUS_ORIGINAL lags PermitStatus: FINALED still labeled
    issued→Active (3), ISSUED still labeled ready-to-issue/applied→
    In Review (2). Remap from PermitStatus.
  - ONLINE SUBMITTAL left STATUS_NORMALIZED null → FILLED In Review.
  - FEE ESTIMATE mapped to Inactive → FIXED to In Review.
  - INSPECTION PHASE row with PermitFinaledDate kept as Active →
    FIXED to Final (finaled date is authoritative for non-inactive).
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    (or SD ISSUED) present → FILLED.
  - Final missing FINAL_DATE with PermitFinaledDate / SD FINALED /
    passed FINAL inspection → FILLED.
  - Spurious FINAL_DATE on Inactive (CANCELED close timestamp) →
    cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate for all sample rows.
  - ~2 Final FINALED rows have neither Issued nor Approved →
    PERMIT_DATE stays missing.
  - ~41 legacy Final rows (mostly admin fee / interim development
    shells) have blank PermitFinaledDate and no dated final
...
_STATUS_MAP = {
    ...
    "FEES PAID NOT ISSUED": "In Review",
...
_INACTIVE_KEEP = {"CANCELED", "CANCELLED", "EXPIRED", "VOID", "WITHDRAWN"}
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"final\s*building|final\s*bldg|^final$|"
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Vacaville record."""
    pi = _permit_info(d)
    ...
    if effective_status == "Final":
        ...
        # Spurious FINAL_DATE on non-Final rows (e.g. CANCELED close date).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Vacaville permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Vacaville".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Vacaville") & (df["STATE"] == "CA")].copy()

    print(f"Vacaville records: {len(city):,}\n")

    repaired = data_repair(city)

    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "vacaville_repaired_sample.parquet")
