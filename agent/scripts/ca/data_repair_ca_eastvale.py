"""Data repair for Eastvale (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Eastvale DATA is an Accela Citizen Access scrape (same family as Corte
Madera / Cupertino). Most sample rows share core top-level keys
(``address``, ``date``, ``status``, ``tasks``, ``inspections``,
``search_data``, …); a minority of shells are ``search_data``-only.
...
Known issues repaired:
  - Blank DATA.status / STATUS_ORIGINAL (104 rows: 84 full portal
    shells with TBD-only Application Submittal, 20 search_data-only)
    → FILLED In Review.
  - In Review rows with a dated Permit Issuance Issued event
    (Ready to Issue / Plan Review / Pending / Resubmittal Required)
    → FIXED to Active (10 rows).
  - Finaled shells missing FINAL_DATE while a Passed Final inspection
    exists → FILLED (3 rows).
  - Active Issued shell missing PERMIT_DATE when Issued events are
    present → FILLED (rare; one Issued row has no dated issuance).

Not repairable / left as-is:
  - FILE_DATE already matches DATA.date / search_data.Date for every
    sample row; Application Submittal is never earlier.
  - One Active ``Issued`` row has no Permit Issuance task history →
    PERMIT_DATE stays missing.
  - Passed Final inspections on still-Issued / Ready-to-Issue shells
    do **not** promote status to Final (agency CaseStatus lag).
  - Expired stays Inactive; ExpireDate is not a completion date.
"""
...
def _file_date_from_data(d: dict):
    ...
    earliest Application Submittal Accepted* mark.
    """
    candidates = []
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels are sticky. Closed - Complete / Finaled map
    to Final. A dated Permit Issuance Issued promotes In Review → Active.
    Passed Final inspections alone do **not** promote Issued → Final when
    the portal status is still Issued / Permit Issued.
    """
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Eastvale permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Eastvale".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Eastvale") & (df["STATE"] == "CA")].copy()

    print(f"Eastvale records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_eastvale_repaired.parquet"
