"""Data repair for Woodside (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Woodside DATA is a civic portal payload (same shape as Brentwood /
...
Known issues repaired:
  - Blank PermitStatus (108 rows, mostly REVISION shells) with no
    STATUS_NORMALIZED → FILLED from dates (Issued/Approved → Active,
    Finaled → Final, Applied only → In Review).
  - ISSUED rows that already carry PermitFinaledDate but remain Active
    → FIXED to Final.
  - UNKNOWN mapped upstream to Final with no dates → FIXED to Inactive.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    (especially APPROVED tree/revision permits) → FILLED.
  - Spurious FINAL_DATE on Inactive EXPIRED/VOID (and any remaining
    non-Final after status repair) → cleared.

Not repairable / left as-is:
  - 135 legacy rows with blank PermitAppliedDate (and blank
    search_data.Application); FILE_DATE stays missing.
  - ACTIVE / APPROVED rows lacking both Issued and Approved →
    PERMIT_DATE stays missing.
  - FINALED rows with neither Issued nor Approved → PERMIT_DATE stays
    missing (Finaled is completion, not issuance).
  - UNKNOWN empty-date shell lacks PermitFinaledDate and a passed final
    inspection → FINAL_DATE stays missing after status fix.
"""
...
_STATUS_MAP = {
    ...
    "ABANDONED": "Inactive",
    "ABANDON": "Inactive",
    # Unknown / empty shells with no workflow dates
    "UNKNOWN": "Inactive",
}
...
def _is_inactive_keep(raw: str) -> bool:
    ...
    if "WITHDRAW" in raw or "ABANDON" in raw or "DENIED" in raw:
        return True
    if "UNKNOWN" in raw or "ERROR" in raw:
        return True
    return False
...
def _result_ok(result: str) -> bool:
    """Accept PASS / PASSED / Y / PASS W/CONDITIONS and other ok finals."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Woodside permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Woodside".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Woodside") & (df["STATE"] == "CA")].copy()

    print(f"Woodside records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_woodside_repaired.parquet"
