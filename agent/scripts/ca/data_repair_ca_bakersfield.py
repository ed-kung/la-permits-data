"""Data repair for Bakersfield (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Bakersfield DATA is a city permit-portal scrape. Two sub-schemas:
...
Canonical mappings:
  - Status for Permit Number (fallback: Application Status)
      with REVOKED/EXPIRED Application Status override → STATUS_NORMALIZED
  - detail['Application Date']                       → FILE_DATE
  - permit_status_detail['Issue Date']
      (fallback: 'Permit Date')                      → PERMIT_DATE
  - Latest APPROVED FINAL* / CERTIFICATE inspection
      on/after FILE_DATE; else latest APPROVED insp  → FINAL_DATE

Known issues repaired:
  - 22 detail_only rows with null STATUS_ORIGINAL / STATUS_NORMALIZED
    → FILLED from Application Status.
  - Application Status REVOKED/EXPIRED still labeled Active / In Review
  ...
  - PERMIT_DATE often taken from Permit Date, which on finaled rows is
    frequently overwritten to the finalization date (~825 rows where
    Permit Date == FINAL_DATE). Prefer Issue Date → FIXED.
  - Spurious PERMIT_DATE on In Review (plan check / to be issued) and
    never-issued revoked shells → cleared (FIXED).
  - Missing FINAL_DATE on Final rows with dated APPROVED inspections
    (e.g. temporary C.O.) → FILLED.

Not repairable / left as-is:
  - FILE_DATE already matches Application Date for all sample rows.
  - ~29 Final rows with empty insp_status_detail → FINAL_DATE stays
    missing.
  - detail_only Active/Final candidates have no Issue/Permit Date →
...
# terminal override for REVOKED / EXPIRED.
_APP_STATUS_MAP = {
    "finaled": "Final",
    "c of o": "Final",
    "temporary certificate": "Final",
    # Approved without a permit block: no evidence of issuance.
    "approved": "In Review",
    "pending": "In Review",
    "pending verification": "In Review",
    "plan check correction": "In Review",
...
def _is_final_insp_name(name: str) -> bool:
    n = name.upper().strip()
    if n.startswith("FINAL INSPECTION"):
        return True
    if n == "CERTIFICATE OF OCCUPANCY":
        return True
    if n == "MASONRY WALL FINAL":
        return True
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Bakersfield record."""
    ...
    if effective_status in ("Active", "Final"):
        ...
        # Application revoked/expired after an Issue Date existed.
    ...
    # -- FINAL_DATE --
    # Canonical: latest APPROVED FINAL* / CERTIFICATE on/after FILE_DATE;
    # fallback to latest any APPROVED on/after FILE_DATE. Existing values
    # that already equal the latest APPROVED (>= FILE_DATE) are left alone
    # even if a earlier FINAL INSPECTION exists (avoids stale prior-job
    # finals on solar / add-on permits).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Bakersfield permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Bakersfield".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Bakersfield") & (df["STATE"] == "CA")].copy()

    print(f"Bakersfield records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "bakersfield_repaired_sample.parquet")
