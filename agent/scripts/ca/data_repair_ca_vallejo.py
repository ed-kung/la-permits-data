"""Data repair for Vallejo (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Vallejo DATA is a single flat portal schema (all sample rows share the
...
search_data, site_info). Canonical fields:
  ...
  - permit_info.PermitFinaledDate  → FINAL_DATE
      (fallback for Final rows: latest successful finaling /
       public-works inspection Completed date)

Known issues repaired:
  - 137 COMPLIED rows (mostly WEED-IMPA abatement) incorrectly mapped
    to In Review → FIXED to Final (nearly all have PermitFinaledDate).
  - 13 APPROVED rows with a true PermitFinaledDate still labeled Active
    → FIXED to Final.
  - 61 rows with unmapped PermitStatus (COMPLANT, LETR1BCK, REF-*,
    WORKORDR, TOWED, etc.) → FILLED from an explicit status map and/or
    date inference.
  - Active/Final rows missing PERMIT_DATE with empty Issued but
    populated Approved → FILLED from PermitApprovedDate (~15).
  - Final rows missing FINAL_DATE with a usable finaling inspection
    Completed date → FILLED (~17).
  - Spurious FINAL_DATE on non-Final rows (ABATED close stamps, stray
    APPLIED/HOLD finaled dates, etc.) → cleared (FIXED).

Not repairable from DATA:
  - 10 FILE_DATE gaps (empty PermitAppliedDate; no alternate application
    date in permit_info).
  - ~17 Active and ~215 Final rows lack both Issued and Approved →
    PERMIT_DATE stays missing.
  - ~34 Final rows (mostly FINALED excavation/encroachment and CLOSED
    investigations) lack PermitFinaledDate and usable finaling
...
# Vallejo sample includes mid-century legacy records.
_MIN_YEAR = 1950
_MAX_YEAR = 2035
...
_STATUS_MAP = {
    ...
    "LETTER1": "In Review",
    "LETR1BCK": "In Review",
    "COMPLANT": "In Review",  # open complaint (not COMPLIED)
    "RESOLVED": "In Review",
    "TEST": "In Review",
    "VIO": "In Review",
    "REF-ABVH": "In Review",
...
def _map_permit_status(raw) -> Optional[str]:
    ...
    if not key:
        return None
    if "FINAL" in key or "COMPLI" in key:
        return "Final"
    if "EXPIRE" in key or "VOID" in key or "CANCEL" in key or "WITHDRAW" in key:
        return "Inactive"
    return None


def _has_true_finaled(pi: dict) -> bool:
    """PermitFinaledDate present (Vallejo does not use expire-as-finaled stamps)."""
...
def _derive_status(pi: dict) -> Optional[str]:
    ...
    (ABATED / EXPIRED close dates are not building-permit sign-offs).

    In Review labels stay In Review, except COMPLIED which is mapped to
    Final above. ISSUED / APPROVED / ACTIVE (or blank / unmapped) with a
    PermitFinaledDate are remapped to Final. Unmapped code-enforcement
    statuses with a FinaledDate are also treated as Final.
    """
    ...
    if mapped == "In Review":
        # Completed work-order / citation shells with a finaled stamp.
        key = _normalize_status_key(pi.get("PermitStatus"))
        if key in {"WORKORDR", "1ST CIT", "REF-BLDG"} and _has_true_finaled(pi):
            return "Final"
        return "In Review"
...
def _finaled_from_inspections(d: dict) -> Optional[date]:
    ...
    for item in inspections:
        ...
        result_is_final = result in {"FINALED", "FINAL"} or "FINALED" in result
        type_is_final = "FINAL" in typ
        public_works_done = typ == "PUBLIC WORKS" and result in _FINAL_INSP_OK_RESULTS and result != ""
        if result_is_final:
            ok = True
        elif type_is_final and result in _FINAL_INSP_OK_RESULTS:
            ok = True
        elif public_works_done:
            ok = True
        else:
            ok = False
        if not ok:
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Vallejo record."""
    pi = _permit_info(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Vallejo permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Vallejo".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Vallejo") & (df["STATE"] == "CA")].copy()

    print(f"Vallejo records: {len(city):,}\n")
    ...
    # Spot-check remaining Active/Final gaps
