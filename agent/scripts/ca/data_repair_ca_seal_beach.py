"""Data repair for Seal Beach (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Seal Beach DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Stale CaseStatus on issued / finaled shells still labeled Issued or
    In Review (and PermitStatus Complete with FinalizeDate only) →
    FIXED to Final using FinalDate / FinalizeDate evidence.
  - Review-pipeline rows (In Review / On Hold) that already carry
    IssueDate but no final stamp → FIXED to Active.
  - Approved shells with no IssueDate left as Active → FIXED to
    In Review (plan approval, not issuance).
  - Spurious FINAL_DATE on Inactive rows (Void closure stamps copied
    from entity.FinalDate) → cleared.
  - CaseStatus Complete stays Final even when FinalDate is absent
    (do not demote issued-but-unfinaled Complete shells to Active).
...
Not repairable / left as-is:
  ...
  - Mapped dates that are already populated match entity stamps at
    day resolution (no date FIXED values in the sample beyond clearing
    spurious finals / filling FinalizeDate-only finals).
  - 149 Complete / Final rows with null FinalDate / FinalizeDate →
    FINAL_DATE stays missing; DATA has no alternate completion stamp.
  - 137 Complete / Final rows with null IssueDate → PERMIT_DATE stays
    missing; DATA has no alternate issuance stamp.
...
# Terminal Inactive and Complete/Issued are authoritative when no
...
_STATUS_MAP = {
    ...
    "Withdrawn": "Inactive",
    "Denied": "Inactive",
    # In Review (Approved = plan approval, not issuance)
    "Approved": "In Review",
    "In Review": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
...
def _mapped_status(d: dict) -> Optional[str]:
    for raw in _raw_labels(d):
        ...
        if mapped is not None:
            return mapped
        lower = raw.lower()
        if "complete" in lower or "finaled" in lower:
            return "Final"
        if any(
            tok in lower
            for tok in (
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Void / Canceled / Withdrawn) are
    sticky even when FinalDate is present as a case-closure stamp.
    FinalDate / FinalizeDate → Final overrides stale Issued / review
    labels. CaseStatus Complete stays Final even when FinalDate is
    absent (common on older encroachment shells). Otherwise IssueDate
    → Active overrides review-pipeline labels.
    """
    ...
    # Agency "Complete" is authoritative even without a FinalDate stamp.
    if mapped == "Final":
        return "Final"
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Seal Beach record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Seal Beach permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Seal Beach".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Seal Beach") & (df["STATE"] == "CA")
    ].copy()

    print(f"Seal Beach records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_seal_beach_repaired.parquet"
