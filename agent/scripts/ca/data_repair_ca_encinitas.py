"""Data repair for Encinitas (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Encinitas DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Issued shells whose details.PermitStatus is Finaled (FinalizeDate
    present) left Active → FIXED to Final; FINAL_DATE FILLED.
  - Issued shells with FinalDate/FinalizeDate strictly after IssueDate
    left Active → FIXED to Final.
  - Pending Payment / Under Review / Ready To Issue rows with IssueDate
    left In Review → FIXED to Active (or Final when a credible final
    stamp is also present).
  - Junk FinalDate stamps on non-Final rows (Issued with FinalDate ≤
    IssueDate; Inactive Expired / Void / Abandoned closure stamps;
    In Review Pending Payment without promotion) → FINAL_DATE cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - 2 Finaled shells have null IssueDate → PERMIT_DATE stays missing.
  - 1 Finaled shell has null FinalDate and null FinalizeDate →
    FINAL_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Void / Abandoned / Withdrawn / Denied is a
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Encinitas often stamps FinalDate on still-Issued / Expired shells
    where FinalDate ≤ IssueDate (or the stamp is a post-expiry closure).
    Those are junk / case-closure stamps, not completion. Require either
    an explicit Finaled label, or FinalDate/FinalizeDate strictly after
    IssueDate (and not on an Inactive terminal label — handled upstream).
    """
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Abandoned) are sticky even when FinalDate is present as a
    case-closure stamp. Explicit Finaled labels (from either CaseStatus
    or PermitStatus) → Final. Otherwise a FinalDate/FinalizeDate
    strictly after IssueDate → Final (stale Issued / Pending Payment
    shells). IssueDate → Active overrides review-pipeline labels
    (Pending Payment / Under Review / Ready To Issue).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Encinitas record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Encinitas permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Encinitas".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Encinitas") & (df["STATE"] == "CA")
    ].copy()

    print(f"Encinitas records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_encinitas_repaired.parquet"
    ...
    for idx in repaired.index:
        ...
        print(
            f"  {entity.get('CaseNumber')}: CaseStatus={entity.get('CaseStatus')} "
            f"PermitStatus={details.get('PermitStatus')} "
            f"Type={entity.get('CaseType')}"
        )
