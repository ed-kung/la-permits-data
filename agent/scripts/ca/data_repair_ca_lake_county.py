"""Data repair for Lake County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Lake County DATA is an Accela Citizen Access scrape (same family as
Eastvale / Corte Madera). Most sample rows share core top-level keys
(``address``, ``date``, ``status``, ``tasks``, ``inspections``,
``search_data``, …). A minority of Closed / Online Permit shells have
``tasks`` null and empty ``inspections``. Content variants
(INFERRED_SCHEMA):

  - portal_issued_finaled:   Permit Issuance Issued + final inspection
                             Complete / passed Final inspection
  ...
  - portal_empty_tasks:      tasks null/empty or undated (TBD only)
  - missing

Canonical mappings:
  - DATA.status / search_data.Status                                 → STATUS_NORMALIZED
  - Earliest of DATA.date / search_data.Date / Application
    Submittal|Acceptance Accepted*                                   → FILE_DATE
  - Earliest Permit Issuance / Ready to issue permit
    Issued|Permit Issued                                             → PERMIT_DATE
  - Earliest Inspection Marked as Final Inspection Complete
    (fallback: earliest Passed inspection titled Final*)             → FINAL_DATE

Known issues repaired:
  - Blank STATUS_NORMALIZED for ``Appl Complete- Routed for Rev`` /
    ``Zon Approved`` (19 rows) → FILLED In Review.
  - STATUS_ORIGINAL lag: DATA.status ``Finaled`` while ORIG was
    issued / permit issued → FIXED Active → Final (9 rows).
  - DATA.status ``Issued`` while ORIG was expired → FIXED Inactive →
    Active (1 row).
  - Finaled shells missing FINAL_DATE while a Passed Final inspection
    exists → FILLED (~662 rows).
  - Spurious FINAL_DATE on still-Issued Active shells → cleared FIXED.
  - Permit Issued / Permit issued marks (not just ``Issued``) treated
    as issuance stamps for PERMIT_DATE consistency.

Not repairable / left as-is:
  - FILE_DATE already matches DATA.date for every sample row;
    Application Submittal is never earlier.
  - Active Issued / Finaled shells with no dated issuance task history
    → PERMIT_DATE stays missing (~42 Active + ~1,088 Final in sample).
  - Closed Online Permit shells and Finaled rows with no Final*
    inspection / Final Inspection Complete mark → FINAL_DATE stays
    missing.
  - Passed Final inspections on still-Issued shells do **not** promote
    status to Final (agency CaseStatus lag).
  - Expired stays Inactive; Expiration Date is not a completion date.
"""
...
_FINAL_TASKS = {"Inspection", "Inspection(s)", "Inspections"}
_FINAL_MARKS = {
...
def _passed_final_inspection_dates(d: dict) -> list:
    """Dates from inspections[] where Status is Passed and title is Final*."""
    dates = []
    for insp in d.get("inspections") or []:
        ...
        if status not in {"passed", "approved", "ap"}:
            continue
...
def _file_date_from_data(d: dict):
    ...
    earliest Application Submittal|Acceptance Accepted* mark.
    """
    candidates = []
...
def _permit_date_from_data(d: dict):
    """Earliest Permit Issuance / Ready to issue permit Issued date."""
    tasks = d.get("tasks") or []
    return _first_event_date(tasks, _ISSUE_TASKS, _ISSUE_MARKS)


def _final_date_from_data(d: dict):
    """Earliest Final Inspection Complete mark, else Passed Final inspection."""
    tasks = d.get("tasks") or []
    final = _first_event_date(tasks, _FINAL_TASKS, _FINAL_MARKS)
...
_STATUS_MAP = {
    ...
    "New": "In Review",
    "Appl Complete- Routed for Rev": "In Review",
    "Zon Approved": "In Review",
}
...
def _base_status(d: dict) -> Optional[str]:
    ...
    for k, v in _STATUS_MAP.items():
        if k.lower() == raw.lower():
            return v
    # Prefix fallback: "Closed - …" → Final
    if raw.lower().startswith("closed"):
        return "Final"
    if raw.lower().startswith("appl complete"):
        return "In Review"
    return None
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels are sticky when DATA.status itself is
    Inactive. Closed / Finaled / CofO Issued map to Final. A dated
    issuance promotes In Review → Active. Passed Final inspections
    alone do **not** promote Issued → Final when the portal status is
    still Issued / Permit Issued.
    """
    ...
    # Blank portal status but a filed Accela / search shell → In Review.
    if (
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Lake County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Lake County") & (df["STATE"] == "CA")].copy()

    print(f"Lake County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_lake_county_repaired.parquet"
    ...
    print("\nDate order checks (after repair):")
