"""Data repair for Foster City (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Foster City DATA is a civic portal payload (same shape as Brentwood /
San Carlos / Los Altos). All sample rows share top-level keys:
...
Known issues repaired:
  - STATUS_NORMALIZED missing or stale vs PermitStatus when
    STATUS_ORIGINAL lagged (e.g. FINALED still Active/In Review, or
    ISSUED/EXPIRED/UNDER REVIEW left null under "waiting for applicant
    response") → FILLED / FIXED from PermitStatus (and Final when a
    non-inactive row carries PermitFinaledDate).
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    → FILLED (Approved-only for ACTIVE / APPROVED shells).
  - Final missing FINAL_DATE when PermitFinaledDate present, or when a
    passed final inspection date exists → FILLED.
  - Implausible Issued/Approved years (e.g. 2049 OCR/migration sentinel)
    are ignored so they are not written into PERMIT_DATE.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate on every sample row.
  - Many legacy CLOSED/FINALED shells lack Issued, Approved, and
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Foster City permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Foster City".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Foster City") & (df["STATE"] == "CA")].copy()

    print(f"Foster City records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_foster_city_repaired.parquet"
