"""Data repair for Laguna Hills (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Laguna Hills DATA is a Tyler EnerGov-style payload with top-level keys
...
Canonical fields live under ``entity`` (with details fallbacks):
  ...
  - FinalDate (fallback: details.FinalizeDate;
    then passed Final* processing_status) → FINAL_DATE

Agency CaseStatus strings are inconsistently cased (``FINALED``,
``CANCELLED``, ``READY TO ISSUE`` vs title case). Matching is
case-insensitive.

Known issues repaired:
  - Junk FINAL_DATE on Inactive Void / Expired rows whose FinalDate is
    a case-closure stamp, not a permit finaled date → cleared.

Not repairable / left as-is:
  - STATUS_NORMALIZED already matches CaseStatus for every sample row
    (no missing or mislabeled statuses under case-insensitive mapping).
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Active ``Issued`` shells with Issued=False and null IssueDate →
    PERMIT_DATE stays missing.
  - FINALED shells with null IssueDate → PERMIT_DATE stays missing.
  - FINALED / Complete shells with null FinalDate/FinalizeDate and
    null processing_status → FINAL_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Void is a case-closure stamp (status stays
...
# ── Status mapping (case-insensitive keys) ───────────────────────────────────
...
def _final_inspection_date(d: dict):
    ...
    for item in ps:
        ...
        if "final" not in desc.lower() or status != "passed":
            continue
...
def _has_final_label(d: dict) -> bool:
    return any(_norm_label(label) in _FINAL_LABELS for label in _status_strings(d))


def _has_credible_final_stamp(d: dict) -> bool:
    """True when FinalDate/FinalizeDate is credible completion evidence.

    Require either an explicit Complete / Finaled label, or
    FinalDate/FinalizeDate strictly after IssueDate (stale Issued /
    Fees Due shells).
    """
...
def _is_inactive_label(d: dict) -> bool:
    return any(_norm_label(label) in _INACTIVE_LABELS for label in _status_strings(d))


def _mapped_status(d: dict) -> Optional[str]:
    for raw in _status_strings(d):
        mapped = _STATUS_MAP.get(_norm_label(raw))
        if mapped is not None:
            return mapped
        lower = _norm_label(raw)
        if (
            "finalized" in lower
            or "finaled" in lower
            or lower == "final"
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Finaled labels → Final. Otherwise a FinalDate/FinalizeDate strictly
    after IssueDate → Final. IssueDate → Active overrides review-pipeline
    labels.
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Laguna Hills record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Laguna Hills permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Laguna Hills".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Laguna Hills") & (df["STATE"] == "CA")
    ].copy()

    print(f"Laguna Hills records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_laguna_hills_repaired.parquet"
    ...
    print("\nFINAL_DATE FIXED examples (cleared junk):")
    mask = repaired["FINAL_DATE_FLAG"] == "FIXED"
    for idx in repaired.index[mask][:10]:
        d = _safe_parse(city.at[idx, "DATA"])
        entity = (d or {}).get("entity") or {}
        print(
            f"  {city.at[idx, 'PERMIT_NUMBER']}: "
            f"CaseStatus={entity.get('CaseStatus')!r} "
            f"before={city.at[idx, 'FINAL_DATE']} "
            f"after={repaired.at[idx, 'FINAL_DATE']}"
        )
