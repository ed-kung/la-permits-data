"""Data repair for Butte County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Butte County DATA is a single GIS / open-data portal schema (same family
as Shasta County). All sample rows share top-level keys ``contacts``,
``fees``, ``inspections``, ``permit_info``, ``search_data``, and
``site_info``. Content variants (used as INFERRED_SCHEMA) differ by
which sources are populated:
  ...
  - permit_info_applied_only:     only PermitAppliedDate
  - permit_info_empty:            no applied / issued / approved / finaled
                                  dates and no inspections

Canonical fields (from ``permit_info``):

  - PermitStatus                         → STATUS_NORMALIZED
  - PermitAppliedDate (fallback:
    PermitIssuedDate)                    → FILE_DATE
  - PermitIssuedDate (fallback:
    PermitApprovedDate)                  → PERMIT_DATE
  - PermitFinaledDate (fallback: latest
    passed PERMIT / BUILDING / trade
    FINAL inspection)                    → FINAL_DATE

Known issues repaired:
  - ISSUED CWIF DUE labeled In Review despite an Issued date → FIXED to
    Active.
  - Empty PermitStatus shells with only an Applied date → FILLED as
    In Review; shells with no usable dates left missing.
  - FILE_DATE missing on ~650 legacy rows with empty Applied but a
    populated Issued date → FILLED from Issued.
  - Active / Final rows missing PERMIT_DATE when Issued is empty but
    Approved is present (APPROVED / CLOSED / COMPLETED shells) → FILLED
    from Approved; rare Issued present but PERMIT null → FILLED.
  - Final rows missing FINAL_DATE with a usable final inspection →
    FILLED from that Completed date.
  - EXPIRED rows carrying PermitFinaledDate / FINAL_DATE → FINAL_DATE
    cleared (FIXED); status stays Inactive.

Not repairable from DATA:
  - ~230 Final CLOSED / COMPLETED / FINALED shells have neither
    PermitFinaledDate nor a usable final inspection.
  - ~25 empty PermitStatus archive stubs have no dates at all.
  - RECEIPT ISSUED Active rows have neither Issued nor Approved.
"""
...
# Historical permits go back to the 1970s; do not reject those years.
_MIN_YEAR = 1900
_MAX_YEAR = 2035
...
_STATUS_MAP = {
    ...
    # Active
    "ISSUED": "Active",
    "ISSUED CWIF DUE": "Active",
    "APPROVED": "Active",
    "RECEIPT ISSUED": "Active",
    # In Review
    "APPLIED": "In Review",
    ...
    "CANCELLED": "Inactive",
    "CANCELED": "Inactive",
    "REFUND REQ RECD": "Inactive",
}

# Terminal inactive labels: PermitFinaledDate on these is not evidence the
# permit should be treated as Final (often an expire / close timestamp).
_INACTIVE_KEEP = {
    "EXPIRED",
    "VOID",
    "CANCELED",
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"final\s*bldg|electric(?:al)?\s*final|mechanical\s*final|plumbing\s*final|"
    r"grading\s*final|fire\s*final|septic\s*final|pw\s*final|"
    r"cal\s*fire\s*final|ehd\s*final|planning\s*final|"
...
def _derive_status(pi: dict) -> Optional[str]:
    """Map PermitStatus; fall back to date evidence when status is blank."""
    ...
    if raw:
        if "FINAL" in raw or raw in {"COMPLETE", "COMPLETED", "CLOSED"}:
            return "Final"
        if raw.startswith("ISSUED") or raw.startswith("APPROVED"):
            return "Active"
        if "EXPIRE" in raw or "VOID" in raw or "CANCEL" in raw or "DENY" in raw:
            return "Inactive"
        if "REVIEW" in raw or raw in {"APPLIED", "RENEWABLE"}:
            return "In Review"

    # Empty PermitStatus: infer lightly from dates when possible.
...
def _preferred_file_date(pi: dict) -> Optional[date]:
    ...
    if applied is not None:
        return applied
    # Legacy shells often store only Issued; use it so FILE_DATE is populated.
    return _as_date(pi.get("PermitIssuedDate"))
...
def _final_from_inspections(d: dict) -> Optional[date]:
    """Latest completion date from a passed final-type inspection."""
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Butte record."""
    pi = _permit_info(d)
    ...
    if effective_status != "Final":
        # Expired / void / cancel close timestamps are not completion finalings.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Butte County permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Butte County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Butte County") & (df["STATE"] == "CA")].copy()

    print(f"Butte County records: {len(city):,}\n")

    repaired = data_repair(city)

    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "butte_county_repaired_sample.parquet")
