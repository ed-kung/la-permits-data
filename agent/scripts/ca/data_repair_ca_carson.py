"""Data repair for Carson (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Carson DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED is null for two CaseStatus values that upstream
    never mapped (293 sample records): Additional Documents Required
    (291) and Planning Cleared (2) → In Review.

Not repairable from DATA:
  - 46 Final records (mostly CaseStatus=Complete miscellaneous /
    address-assignment applications) have null IssueDate and
    Issued=False, so PERMIT_DATE cannot be filled. FILE_DATE is not a
    safe proxy for those CaseTypes.
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Carson record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Carson permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Carson".  Must contain
...
if __name__ == "__main__":
    ...
    carson = df[(df["JURISDICTION"] == "Carson") & (df["STATE"] == "CA")].copy()

    print(f"Carson records: {len(carson):,}\n")

    repaired = data_repair(carson)
    ...
    for field in ["STATUS_NORMALIZED", "FILE_DATE", "PERMIT_DATE", "FINAL_DATE"]:
        ...
        before_missing = carson[field].isna().sum()
    ...
    print("  Before:")
    for s, c in carson["STATUS_NORMALIZED"].value_counts(dropna=False).items():
