"""Data repair for Mill Valley (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Mill Valley DATA is a civic portal payload (same shape as Rohnert Park /
...
``permit_info`` (``search_data`` only carries Address / Permit Number /
RECORDID in this sample):
...
Known issues repaired:
  - STATUS_NORMALIZED null for blank PermitStatus (legacy issued shells)
    and unmapped labels (DPW FEES, APPLICANT * SUBMITTAL REQUIRED,
    APPROVED READY TO PAY, HARD COPIES REQUIRED, PREPARING FOR
    ISSUANCE, RESALE APPLIED ONLINE, REVISION UNDER REVIEW) → FILLED.
  - ISSUED / PAID ONLINE shells that already carry PermitFinaledDate
    left Active / In Review → FIXED to Final.
  - Pre-issuance labels (READY TO PAY, PAID ONLINE, ON HOLD-PENDING
    ITEM, PENDING PLAN SUBMITL, etc.) that already carry Issued →
    FIXED to Active.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    → FILLED (common on FINALED FIRE shells with Approved only).
  - Final missing FINAL_DATE when a passed final inspection date
    exists → FILLED.
  - Spurious FINAL_DATE on non-Final rows → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate whenever Applied is
    present; 252 shells lack any application date (legacy blank-status
    Issued shells, ISSUED without Applied, empty FIRE finals) → stay
    missing.
  - FINALED FIRE / empty shells with neither Issued, Approved, nor a
    usable final inspection → PERMIT_DATE / FINAL_DATE stay missing.
"""
...
_STATUS_MAP = {
    ...
    "ISSUED": "Active",
    "APPROVED": "Active",
    "APPROVED W/ COND": "Active",
    "ACTIVE": "Active",
    # In Review (intake / fee / pre-issuance workflow)
    "SUBMITTED": "In Review",
    "UNDER REVIEW": "In Review",
    "REVISION UNDER REVIEW": "In Review",
    "PENDING PLAN SUBMITL": "In Review",
    "APPLIED": "In Review",
    "APPLIED ONLINE": "In Review",
    "RESALE APPLIED ONLINE": "In Review",
    "READY TO PAY": "In Review",
    "APPROVED READY TO PAY": "In Review",
    "PAID ONLINE": "In Review",
    "DPW FEES": "In Review",
    "PREPARING FOR ISSUANCE": "In Review",
    "APPLICANT RESUBMITTAL REQUIRED": "In Review",
    "APPLICANT SUBMITTAL REQUIRED": "In Review",
    "APPLICANT REVISION SUBMITTAL REQUIRED": "In Review",
    "ON HOLD-PENDING ITEM": "In Review",
    "HARD COPIES REQUIRED": "In Review",
    # Inactive
    "CANCELLED": "Inactive",
    "CANCELED": "Inactive",
    "EXPIRED": "Inactive",
    "STOP WORK ISSUED": "Inactive",
    "WITHDRAWN": "Inactive",
    "VOID": "Inactive",
    "DENIED": "Inactive",
}
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Mill Valley permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Mill Valley".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Mill Valley") & (df["STATE"] == "CA")].copy()

    print(f"Mill Valley records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_mill_valley_repaired.parquet"
