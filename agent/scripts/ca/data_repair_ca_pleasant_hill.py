"""Data repair for Pleasant Hill (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Pleasant Hill DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Missing STATUS_NORMALIZED for CaseStatus ``No Action Taken``
    (915 rows; upstream left blank). Issued shells → Active; shells
    with FinalDate / PermitStatus Complete / Passed Final Building
    (or trade-permit Final inspection) → Final; unissued → In Review.
  - Issued still Active while PermitStatus=Complete (and/or
    FinalizeDate / Passed Final Building present) → FIXED to Final.
  - Spurious FINAL_DATE on Inactive Expired / Void copied from
    entity.FinalDate (case-closure stamp) → cleared.
  - Missing FINAL_DATE on status catch-up Final rows filled from
    FinalizeDate or Passed Final inspection date.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - PERMIT_DATE already matches IssueDate when present; In Review /
    pre-issuance Inactive shells correctly lack PERMIT_DATE.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Void is a case-closure stamp, not a
    permit finaled date.
  - Building permits with only trade Final inspections (no Final
    Building) stay Active — partial finals are not treated as
    completion.
"""
...
_STATUS_MAP = {
    ...
    "Fees Due": "In Review",
    "On Hold": "In Review",
    # Handled specially in _expected_status
    "No Action Taken": None,
}

# Prefer Complete over Issued when CaseStatus and PermitStatus disagree.
_STATUS_RANK = {
    "Final": 4,
    "Active": 3,
    "In Review": 2,
...
# CaseType substrings that are single-trade / specialty permits where any
# Passed Final* inspection is enough to treat the permit as Final.
_TRADE_CASETYPE_TOKENS = (
    "plumbing",
    "mechanical",
    "electrical",
    "re-roof",
...
def _passed_final_items(d: dict) -> list[dict]:
    ...
    for item in ps:
        ...
        if "final" in desc.lower() and status == "passed":
            out.append(item)
    return out


def _final_inspection_date(d: dict):
    """Latest Passed processing_status item whose description mentions Final."""
    best = pd.NaT
    for item in _passed_final_items(d):
...
def _has_completion_evidence(d: dict) -> bool:
    """True when DATA indicates the permit was finaled / completed."""
    if _entity_date(d, "FinalDate", "FinalizeDate") is not pd.NaT:
        return True

    details = d.get("details") if isinstance(d.get("details"), dict) else {}
    if str(details.get("PermitStatus") or "").strip() == "Complete":
        return True

    finals = _passed_final_items(d)
    if not finals:
        return False

    descs = [str(item.get("description") or "").lower() for item in finals]
    # Whole-building / pool finals are strong completion signals.
    if any("building" in desc or "pool" in desc for desc in descs):
        return True

    # Single-trade permits: any Passed Final* inspection is enough.
    entity = d.get("entity") if isinstance(d.get("entity"), dict) else {}
    case_type = str(entity.get("CaseType") or "").lower()
    if any(tok in case_type for tok in _TRADE_CASETYPE_TOKENS):
        return True
...
def _expected_status(d: dict) -> Optional[str]:
    """Map CaseStatus / PermitStatus; apply No Action Taken heuristics."""
    strings = _status_strings(d)
    best = None
    best_rank = -1
    saw_no_action = False
    ...
    if best is None and saw_no_action:
        if _has_completion_evidence(d):
            return "Final"
        if _is_issued(d):
            return "Active"
        return "In Review"

    # Issued (or other Active) shells that were actually completed.
    if best == "Active" and _has_completion_evidence(d):
        return "Final"
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Pleasant Hill record."""
    ...
    # Note: non-Final statuses (Expired / Void) sometimes carry FinalDate
    # as a case-closure stamp; only keep FINAL_DATE when status is Final.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Pleasant Hill permit records using information from the raw DATA JSON
    column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Pleasant Hill".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Pleasant Hill") & (df["STATE"] == "CA")
    ].copy()

    print(f"Pleasant Hill records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_pleasant_hill_repaired.parquet"
