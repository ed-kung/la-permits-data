"""Data repair for Milpitas (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Milpitas DATA is a single flat portal schema (all sample rows share the
...
Canonical mappings:
  ...
  - permit_info.PermitFinaledDate  → FINAL_DATE
      (fallback for Final rows: latest inspection Completed date
       where Type contains FINAL and Result is PASS/PASSED/…,
       or Result contains FINALED)

``PermitExpirationDate`` is a permit-validity window, not a finaling /
completion date. ``search_data`` only carries identifiers (RECORDID,
Permit Number, Address) — no date fields. Legacy CLOSED rows often have
FOLLOW UP / Complete inspections decades after issuance; those are
administrative cleanup and are not used as FINAL_DATE.

Known issues repaired:
  - STATUS_NORMALIZED stale vs PermitStatus on ~11 rows (FINALED /
    CLOSED mislabeled Active/Inactive; ISSUED / APPROVED mislabeled
    In Review) and empty PermitStatus on 2 CONV_* shells → FILLED /
    FIXED.
  - Rows with PermitFinaledDate but non-Final STATUS_NORMALIZED
    (including stale ISSUED and CANCELED/Withdrawn closures) → Final.
  - PERMIT_DATE missing on Active/Final when Issued is empty but
    Approved is present, or when status remaps expose Issued → FILLED.
  - FINAL_DATE missing when PermitFinaledDate is present (status was
    wrong) or when Final FINALED rows lack PermitFinaledDate but have
    a finaling inspection → FILLED.
...
Not repairable from DATA:
  - FILE_DATE already matches PermitAppliedDate whenever Applied is
    present; 8 shells have neither.
  - Most CLOSED Final rows lack PermitFinaledDate and a usable finaling
    inspection (only late FOLLOW UP Complete) → FINAL_DATE stays missing.
  - Some Active/Final rows (legacy Approved / CLOSED) lack both Issued
    and Approved → PERMIT_DATE stays missing.
"""
...
def _map_permit_status(raw) -> Optional[str]:
    ...
    if "FINALED" in key or key in ("CLOSED", "COMPLETE", "COMPLETED"):
        return "Final"
    if key in ("EXPIRED", "WITHDRAWN", "DENIED") or "CANCEL" in key:
        return "Inactive"
    if "APPR" in key and "COND" in key:
        return "In Review"
    return None
...
def _derive_status(pi: dict) -> Optional[str]:
    ...
    # stale ISSUED / empty / cancel-closure portal label.
...
# itself is a final-* inspection (FINALED, FINAL ELECTRICAL, FIN, etc.).
...
def _finaled_from_inspections(d: dict) -> Optional[date]:
    ...
    Accepts either:
      ...
      - Type containing FINAL / trailing FIN token (e.g. ACC-FINALS,
        B B099 Re-Roof - Fin) with a successful / neutral Result.
    ...
    for item in inspections:
        ...
        # FINAL covers FINALS / FINAL ELECTRICAL; also bare FIN token
        # used on older re-roof final inspections.
        type_is_final = (
            "FINAL" in typ
            or typ.endswith(" FIN")
            or " - FIN" in typ
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Milpitas record."""
    pi = _permit_info(d)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Milpitas permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Milpitas".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Milpitas") & (df["STATE"] == "CA")].copy()

    print(f"Milpitas records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        out_path = os.path.join(AGENT_DATA_PATH, "milpitas_repaired_sample.parquet")
