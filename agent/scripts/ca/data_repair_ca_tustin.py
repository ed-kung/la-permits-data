"""Data repair for Tustin (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Tustin DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED on ``Review Approved`` shells (and one
    Issued / Finalized row whose STATUS_ORIGINAL lagged) → FILLED.
  - Stale STATUS_ORIGINAL-driven labels that disagree with CaseStatus
    (Finalized left Active/In Review; Closed left Inactive; Issued left
    In Review; Expired left Active/In Review) → FIXED.
  - ``Approved`` coded Active despite no IssueDate → FIXED to In Review.
  - Credible FinalDate (strictly after IssueDate) on Issued / Review
    Approved shells → FIXED to Final; FINAL_DATE kept/filled.
  - Junk FinalDate stamps on Issued / Approved / Inactive rows
    (often FinalDate ≤ IssueDate, or FinalDate with no IssueDate) →
    FINAL_DATE cleared when status stays non-Final.
  - Issued / Submitted shells missing PERMIT_DATE while IssueDate is
    present → FILLED.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Closed / Finalized / Issued shells with null IssueDate →
    PERMIT_DATE stays missing (no alternate issuance stamp in DATA).
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Void / Withdrawn is a case-closure stamp,
    not a permit finaled date (status stays Inactive; FINAL_DATE
    cleared).
"""
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Tustin often stamps FinalDate on still-Issued / Approved shells where
    FinalDate ≤ IssueDate (or IssueDate is absent). Those are junk
    workflow stamps, not completion. Require either an explicit
    Finalized/Closed label, or FinalDate strictly after IssueDate.
    """
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Void / Withdrawn) are sticky
    even when FinalDate is present as a case-closure stamp. Explicit
    Finalized / Closed labels → Final. Otherwise a FinalDate strictly
    after IssueDate → Final (stale Issued / Review Approved shells).
    IssueDate → Active overrides review-pipeline labels. Approved /
    Review Approved without IssueDate stay In Review.
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Tustin record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Tustin permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Tustin".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Tustin") & (df["STATE"] == "CA")
    ].copy()

    print(f"Tustin records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_tustin_repaired.parquet"
