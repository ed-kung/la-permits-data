"""Data repair for Rohnert Park (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Rohnert Park DATA is a civic portal payload (same shape as Foster City /
...
``permit_info`` (with ``search_data`` fallbacks):

  - PermitStatus (fallback: search_data.Status) → STATUS_NORMALIZED
  - PermitAppliedDate (fallback: search Applied) → FILE_DATE
  - PermitIssuedDate (fallback: PermitApprovedDate,
    then search ISSUED)                    → PERMIT_DATE
...
Content variants (INFERRED_SCHEMA):
  ...
  - legacy_no_status / empty variants for blank PermitStatus shells

Known issues repaired:
  - STATUS_NORMALIZED null for unmapped labels (APPROVED - WAITING FOR
    SIGNATURE) or blank PermitStatus → FILLED from PermitStatus /
    search Status / date inference.
  - Active / In Review rows that already carry PermitFinaledDate
    → FIXED to Final (inactive labels kept Inactive).
  - MASTER PLAN upstream-mapped to Final despite no Finaled date
    → FIXED to Active.
  ...
  - Final missing FINAL_DATE when a passed final inspection date
    exists → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Expired / Approved / etc.)
    → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate whenever Applied is
    present; 28 shells lack any application date → stay missing.
  - Many FINALED / APPROVED / ISSUED shells lack Issued, Approved, and
    Finaled timestamps (and often have no usable final inspection)
...
_STATUS_MAP = {
    ...
    "APPROVED": "Active",
    "ACTIVE": "Active",
    "APPROVED - WAITING FOR SIGNATURE": "Active",
    "MASTER PLAN": "Active",
    ...
    "RECEIVED": "In Review",
    "ACCEPTED": "In Review",
    "WAITING FOR DOCUMENTS": "In Review",
    "WAITING FOR APPLICANT RESPONSE": "In Review",
    ...
    "ABANDONED": "Inactive",
    "ABANDON": "Inactive",
    "DESTROYED PER RETENTION POLICY": "Inactive",
}
...
_INACTIVE_KEEP = {
    ...
    "ABANDONED",
    "ABANDON",
    "DESTROYED PER RETENTION POLICY",
}
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"plumbing|plumb|mechanical|mech|interior|exterior|planning)?\b|"
    r"building\s*pmt\s*final|"
    r"admin\s*final|"
    r"eng\s*admin\s*final|"
...
def _normalize_status_key(raw) -> str:
    ...
    if s in {"<NONE>", "NONE", "NULL", "N/A", "UNKNOWN"}:
        return ""
    # Truncated search_data labels (e.g. "...SIGNATU...")
    if s.startswith("APPROVED - WAITING FOR SIGNATU"):
        return "APPROVED - WAITING FOR SIGNATURE"
    return s
...
def _lookup_status(raw: str) -> Optional[str]:
    ...
    if "REVOKE" in raw or "WITHDRAW" in raw or "ABANDON" in raw:
        return "Inactive"
    if "DENIED" in raw or "ERROR" in raw or "DESTROYED" in raw:
        return "Inactive"
    ...
    if raw.startswith("ISSUED") or raw == "ACTIVE":
        return "Active"
    # Approved-but-waiting-for-signature still has issuance in RP data.
    if "APPROV" in raw:
        return "Active"
    if "INSPECT" in raw or "MASTER PLAN" in raw:
        return "Active"
    if (
        "SUBMIT" in raw
        or "REVIEW" in raw
...
def _is_inactive_keep(raw: str) -> bool:
    ...
    if "REVOKE" in raw or "WITHDRAW" in raw or "ABANDON" in raw or "DENIED" in raw:
        return True
    if "ERROR" in raw or "DESTROYED" in raw:
        return True
    return False
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    # Blank status: infer from dates (permit_info, then search_data).
...
def _preferred_permit_date(d: dict):
    ...
    if approved is not pd.NaT:
        return approved
    sd = _search_data(d)
    for key in ("ISSUED", "Issued", "Issue Date"):
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    # -- PERMIT_DATE (issuance; fallback Approved / search ISSUED) --
    issued = _pi_date(d, "PermitIssuedDate")
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Rohnert Park permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Rohnert Park".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Rohnert Park") & (df["STATE"] == "CA")].copy()

    print(f"Rohnert Park records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_rohnert_park_repaired.parquet"
