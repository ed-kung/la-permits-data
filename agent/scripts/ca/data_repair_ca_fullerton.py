"""Data repair for Fullerton (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Fullerton DATA is a Tyler EnerGov-style payload with top-level keys
...
Canonical fields live under ``entity`` (with details fallbacks):
  - CaseStatus / details.PermitStatus  → STATUS_NORMALIZED
    (identical in the sample; map either when present)
  - ApplyDate                          → FILE_DATE
  - IssueDate                          → PERMIT_DATE
  - FinalDate (fallback: details.FinalizeDate) → FINAL_DATE

Known issues repaired:
  - STATUS_NORMALIZED null for Invoiced - Pending Payment and
    Pre-Issuance Status (upstream mapper missed these) → FILLED as
    In Review.
  - Stale STATUS_NORMALIZED vs DATA (Active while Expired; Active /
    In Review / Inactive while Complete; In Review while Issued or
    Cancelled; Inactive while In Review) → FIXED.
  - Missing PERMIT_DATE / FINAL_DATE on rows whose status was
    corrected to Active / Final and IssueDate / FinalDate are present
    → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Issued / Expired /
    Cancelled / In Review shells that still carry entity.FinalDate)
    → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - ~35 Complete rows (mostly Plan Revision / Miscellaneous) have
    Issued=False and null IssueDate → PERMIT_DATE stays missing.
  - One Issued Plan Revision shell also lacks IssueDate.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on non-Complete statuses is not treated as a permit
    finaled date and must not promote status to Final.
"""
...
_STATUS_MAP = {
    ...
    "Cancelled": "Inactive",
    "Withdrawn": "Inactive",
    # In Review (pre-issuance / application)
    "In Review": "In Review",
    "Received": "In Review",
    "Received - Online": "In Review",
    "Ready to Issue": "In Review",
    "Invoiced - Pending Payment": "In Review",
    "With Applicant": "In Review",
    "Pre-Issuance Status": "In Review",
    "PI - Pending Applicant Signature": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Fullerton record."""
    ...
    # Note: non-Complete statuses sometimes carry FinalDate; only keep
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Fullerton permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Fullerton".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Fullerton") & (df["STATE"] == "CA")].copy()

    print(f"Fullerton records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_fullerton_repaired.parquet"
