"""Data repair for Ojai (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Ojai DATA is an Accela Citizen Access scrape (same family as Yuba City /
Pittsburg / Martinez). Most sample rows share the full portal key set
(``address``, ``date``, ``status``, ``tasks``, ``inspections``,
``fees_details``, ``search_data``, …); a handful omit fees/inspections
or contacts. Content variants (INFERRED_SCHEMA):

  - portal_issued_finaled:   Permit Issuance Issued + final-completion
                             evidence
...
Canonical mappings:
  - DATA.status / search_data.Status (+ workflow upgrades when Issued /
    Final Inspection Complete / Final CO Issued exists)
                                                         → STATUS_NORMALIZED
  - Earliest of DATA.date / search_data.Date / Application
    Acceptance|Submittal Accepted*|Submitted*            → FILE_DATE
  - Earliest Permit Issuance Issued                      → PERMIT_DATE
  - Earliest Inspection Final Inspection Complete
    (fallback: Certificate of Occupancy Final CO Issued) → FINAL_DATE

Known issues repaired:
  - ``Meter Released`` shells left In Review despite Issued + Final
    Inspection Complete (STATUS_ORIGINAL never mapped past review)
    → FIXED to Final; existing FINAL_DATE retained.
  - ``Issued`` shells with Final Inspection Complete left Active
    (portal lag) → FIXED to Final.
  - FILE_DATE equal to portal ``date`` but later than Application
    Acceptance/Submittal Accepted* → FIXED to the earlier stamp.
  - Spurious FINAL_DATE on non-Final after status repair → cleared.

Not repairable / left as-is:
  - 171 Finaled shells with Inspection TBD / no Final Inspection
    Complete or Final CO Issued → FINAL_DATE stays missing.
  - 1 CofO Issued shell with Inspection TBD and no Final CO Issued
    event → FINAL_DATE stays missing.
  - 1 Issued shell (OJ 24-346) with empty Permit Issuance events and
    open Zoning/Utilities review → stays Active; PERMIT_DATE stays
    missing (portal status Issued trusted over workflow lag).
  - Inactive Expired shells correctly lack FINAL_DATE; PERMIT_DATE
    present only when Permit Issuance Issued exists.
"""
...
_ACTIVE_STATUSES = {
    "Issued",
    "Permit Issued",
    # Post-issuance / post-final workflow label; upgraded to Final when
    # Final Inspection Complete (or Final CO) evidence exists.
    "Meter Released",
}
...
def _final_task_date(d: dict):
    """Completion date from workflow tasks (status-promotion bar)."""
    tasks = d.get("tasks") or []
...
def _final_date_from_data(d: dict):
    """Prefer Inspection Final Inspection Complete, then Final CO Issued."""
    return _final_task_date(d)
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    issued = _has_issuance(data_dict)
...
_STATUS_MAP = {
    ...
    "CofO Issued": "Final",
    # Active (Meter Released upgraded to Final when completion evidence exists)
    "Issued": "Active",
...
def _base_status(d: dict) -> Optional[str]:
    ...
    if "cofo" in raw.lower() or "c of o" in raw.lower():
        return "Final"
    if "meter" in raw.lower() and "release" in raw.lower():
        return "Active"
    return None
...
def _status_original_hint(status_original) -> Optional[str]:
    ...
    if so in {"finaled", "finalled", "complete", "closed", "cofo issued"}:
        return "Final"
    if so in {"issued", "permit issued", "meter released"}:
        return "Active"
...
def _expected_status(d: dict, status_original=None) -> Optional[str]:
    ...
    Issued / Meter Released → Active unless a dated final-completion mark
    exists (portal lag / incomplete STATUS_ORIGINAL mapping), in which
    case → Final. In Review upgrades to Final / Active from workflow.
    """
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Ojai permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Ojai".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Ojai") & (df["STATE"] == "CA")
    ].copy()

    print(f"Ojai records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_ojai_repaired.parquet"
    ...
    for idx in repaired.index:
        ...
        gap[_raw_status(d) if d else None] += 1
    for k, v in gap.most_common():
        print(f"  {k}: {v}")

    print("\nFinal still missing FINAL_DATE (by DATA.status):")
    gap = Counter()
    for idx in repaired.index:
