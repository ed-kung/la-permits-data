"""Data repair for San Anselmo (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Anselmo DATA is a civic portal payload (same shape as Mill Valley /
Hillsborough). All sample rows share top-level keys:
``fees``, ``contacts``, ``site_info``, ``inspections``,
``permit_info``, ``search_data``. Canonical fields live under
``permit_info`` (``search_data`` mostly carries Address / Permit Number /
RECORDID; a minority also expose an ``ISSUED`` stamp):

  - PermitStatus                         → STATUS_NORMALIZED
  - PermitAppliedDate                    → FILE_DATE
  - PermitIssuedDate (fallback
    PermitApprovedDate / search_data.ISSUED)
                                         → PERMIT_DATE
...
Known issues repaired:
  - STATUS_NORMALIZED derived from stale STATUS_ORIGINAL while
    ``permit_info.PermitStatus`` has moved on (FINALED left Active,
    ISSUED left In Review/Inactive, EXPIRED left In Review, etc.) → FIXED.
  - Pre-issuance labels (READY TO PAY, ACTION REQUIRED, RECEIVED, …)
    that already carry PermitIssuedDate → FIXED to Active.
  - Non-inactive shells that already carry PermitFinaledDate → FIXED
    to Final.
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    → FILLED (common when STATUS_ORIGINAL lagged PermitStatus).
  - Final missing FINAL_DATE when PermitFinaledDate or a passed final
    inspection exists → FILLED.
  - Spurious FINAL_DATE on non-Final rows → cleared.

Not repairable / left as-is:
  - 2 RECEIVED shells with blank PermitAppliedDate → FILE_DATE stays
    missing.
  - Active/Final shells with neither Issued nor Approved → PERMIT_DATE
    stays missing.
  - FINALED/COMPLETE shells with neither PermitFinaledDate nor a usable
    final inspection → FINAL_DATE stays missing.
  - PermitFinaledDate on CANCELLED / EXPIRED / VOID / WITHDRAWN /
    REVOKED is a close/void stamp, not a permit finaled date (status
    stays Inactive; FINAL_DATE cleared if present).
"""
...
_STATUS_MAP = {
    ...
    "APPROVED": "Active",
    "ACTIVE": "Active",
    # In Review (intake / fee / pre-issuance workflow)
    "RECEIVED": "In Review",
    "RECEIVED AND PAID": "In Review",
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"plumbing|plumb|mechanical|mech|interior|exterior|planning|approval)?\b|"
...
def _lookup_status(raw: str) -> Optional[str]:
    ...
    if raw.startswith("FINAL") or "CERTIFICATE OF OCC" in raw:
        return "Final"
    if raw == "COMPLETE" or raw == "CLOSED" or raw.startswith("CLOSE"):
        return "Final"
    if raw.startswith("ISSUED") or raw == "ACTIVE":
        return "Active"
    if "APPROV" in raw and "READY TO PAY" not in raw:
        return "Active"
    if (
        "SUBMIT" in raw
        or "REVIEW" in raw
        ...
        or raw == "HOLD"
        or "PAID" in raw
        or "READY TO PAY" in raw
        or "ACTION REQUIRED" in raw
        or "RECEIVED" in raw
        or "FEES" in raw
        or "APPLIED" in raw
    ):
...
def _preferred_permit_date(d: dict):
    ...
    if issued is not pd.NaT:
        return issued
    sd = _search_data(d)
    sd_issued = _safe_to_datetime(sd.get("ISSUED") or sd.get("Issued") or sd.get("Issued Date"))
    if sd_issued is not pd.NaT:
        return sd_issued
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    to Active (post-issuance workflow still open).
    """
    raw = _raw_status(d)
    ...
    # Only promote on a true issuance stamp, not Approved-only.
    issued_only = _pi_date(d, "PermitIssuedDate")
    if issued_only is pd.NaT:
        sd = _search_data(d)
        issued_only = _safe_to_datetime(
            sd.get("ISSUED") or sd.get("Issued") or sd.get("Issued Date")
        )
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Anselmo permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Anselmo".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "San Anselmo") & (df["STATE"] == "CA")].copy()

    print(f"San Anselmo records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_anselmo_repaired.parquet"
    ...
    both_f = repaired[repaired["FINAL_DATE"].notna() & repaired["PERMIT_DATE"].notna()]
    inv_f = (both_f["FINAL_DATE"].dt.normalize() < both_f["PERMIT_DATE"].dt.normalize()).sum()
    print(f"FINAL < PERMIT inversions: {inv_f}")
