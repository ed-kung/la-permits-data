"""Data repair for Temecula (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Temecula DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED on ``Pending Approvals`` → FILLED In Review.
  - Stale STATUS_ORIGINAL-driven statuses lagging CaseStatus/PermitStatus
    in DATA (Finaled left Active/In Review/Inactive; Issued left In
    Review; Void/Cancel left In Review) → FIXED.
  - Issued shells whose PermitStatus is Finaled (FinalizeDate present)
    left Active → FIXED to Final; FINAL_DATE filled from FinalizeDate.
  - Stale PERMIT_DATE that no longer matches IssueDate → FIXED.
  - Junk FINAL_DATE on Expired (case-closure stamp) → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Finaled shells with null FinalDate/FinalizeDate (39 in sample,
    mostly older FIRE / Temporary Sign / Special Event cases) →
    FINAL_DATE stays missing.
  - Issued / Finaled shells with null IssueDate → PERMIT_DATE stays
    missing (Fire hourly, temporary signs, special events often issue
    without an IssueDate stamp).
...
_STATUS_MAP = {
    ...
    "Issued - Revision in Progress": "Active",
    "Active": "Active",
    # Inactive
    "Expired": "Inactive",
    ...
    "Denied": "Inactive",
    "Rejected": "Inactive",
    # In Review (pre-issuance / plancheck pipeline)
    "Applied": "In Review",
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Temecula occasionally leaves CaseStatus as Issued while PermitStatus
    is Finaled (or FinalizeDate is present strictly after IssueDate).
    Require either an explicit Finaled/Complete/Closed label, or
...
def _mapped_status(d: dict) -> Optional[str]:
    for raw in _status_strings(d):
        ...
        if any(
            ...
            return "In Review"
        # Temecula "Out" = corrections / plan review out
        if lower == "out":
            return "In Review"
    return None
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Complete / Closed labels → Final. Otherwise a FinalDate/FinalizeDate
    strictly after IssueDate → Final (stale Issued shells). IssueDate →
    Active overrides review-pipeline labels (Applied / In Plancheck /
    Out / Ready for Issuance).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Temecula record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Temecula permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Temecula".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Temecula") & (df["STATE"] == "CA")
    ].copy()

    print(f"Temecula records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_temecula_repaired.parquet"
