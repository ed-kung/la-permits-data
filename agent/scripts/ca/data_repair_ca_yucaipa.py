"""Data repair for Yucaipa (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Yucaipa DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - Null STATUS_NORMALIZED on ``Pending Master Plan Approval`` → FILLED
    as In Review.
  - ``Issued`` shells whose details.PermitStatus is ``Finaled`` (and
    FinalizeDate is present) left Active → FIXED to Final.
  - Issued / Fees Due / Fees Paid shells with FinalDate/FinalizeDate
    strictly after IssueDate left Active/In Review → FIXED to Final.
  - Submitted / Fees Due / Fees Paid rows with IssueDate left In Review
    → FIXED to Active.
  - Junk FINAL_DATE on non-Final rows (Expired / Void closure stamps;
    Issued with FinalDate ≤ IssueDate; In Review fee shells) → cleared.
  - Newly promoted Final rows missing FINAL_DATE while FinalizeDate is
    present → FILLED.
  - Finaled shells missing FinalDate/FinalizeDate but with a passed
    Final* inspection → FILLED from processing_status.
...
Not repairable / left as-is:
  ...
  - Active ``Issued`` shells with Issued=False and null IssueDate
    (mostly legacy 2007–2008 CityView shells) → PERMIT_DATE stays
    missing.
  - Finaled / Complete shells with null IssueDate → PERMIT_DATE stays
    missing.
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Yucaipa occasionally leaves CaseStatus as Issued / Fees Due / Fees
    Paid while FinalDate/FinalizeDate is strictly after IssueDate.
    Require either an explicit Complete / Closed / Finaled / Legacy
    label, or FinalDate/FinalizeDate strictly after IssueDate.
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Yucaipa record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Yucaipa permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Yucaipa".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Yucaipa") & (df["STATE"] == "CA")
    ].copy()

    print(f"Yucaipa records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_yucaipa_repaired.parquet"
