"""Data repair for San Mateo (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Mateo DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED Active while details.PermitStatus is Finaled
    (CaseStatus still Issued) → FIXED to Final.
  - Missing FINAL_DATE on Finaled rows with null FinalDate but a
    Passed Building/Electrical Final inspection → FILLED.
  - Missing FINAL_DATE after Active→Final status catch-up when
    FinalizeDate is present → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Issued / Pending /
    Cancelled shells that still carry entity.FinalDate) → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Five Finaled "Converted" historical shells have Issued=False and
    null IssueDate → PERMIT_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Issued / Pending / Cancelled is not a permit finaled
    date under CaseStatus, and must not promote status to Final on
    its own (PermitStatus=Finaled still wins via rank).
"""
...
_STATUS_MAP = {
    ...
    "Ready for Issuance (Revision)": "In Review",
    "Pending": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single San Mateo record."""
    ...
    # Note: non-Finaled statuses (Issued/Pending/Cancelled) sometimes carry
    # FinalDate; only keep FINAL_DATE when status is Final.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Mateo permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Mateo".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "San Mateo") & (df["STATE"] == "CA")].copy()

    print(f"San Mateo records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_mateo_repaired.parquet"
