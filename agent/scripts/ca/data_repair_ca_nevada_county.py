"""Data repair for Nevada County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Nevada County DATA is an Accela Citizen Access scrape. Nearly all sample
rows share the full key set (``address``, ``date``, ``status``,
``tasks``, ``inspections``, ``search_data``, ``more_details``, …); one
sparse row omits optional blocks. Content variants (INFERRED_SCHEMA):

  - accela_full_issued_finaled:  Ready-to-Issue Issued (or Issue Date) +
                                 Inspection(s) Finaled / final PASS insp
...
Canonical mappings:
  - DATA.status / search_data.Status
    (+ Ready to Issue Issued upgrade; Inspection Finaled upgrade)
                                                      → STATUS_NORMALIZED
  - DATA.date / search_data.Date                      → FILE_DATE
  - Ready to Issue Marked as Issued
    (fallback: more_details … Issue Date)             → PERMIT_DATE
  - Inspection(s) Marked as Finaled
    (fallback: approved Final* inspection Status Date)
                                                      → FINAL_DATE

Known issues repaired:
  - About to Expire left Inactive → FIXED to Active (still issued).
  - Plan Check About to Expire left Inactive → FIXED to In Review.
  - Received left Active (STATUS_ORIGINAL=issued shell) → FIXED to
    In Review; spurious PERMIT_DATE cleared.
  - In Review shells with Ready to Issue Issued events → FIXED to Active.
  - Issued + Inspections Finaled left Active → FIXED to Final.
  - FINAL_DATE used the first Finaled mark when later Finaled events
    exist → FIXED to the latest Finaled date.
  - Final rows with empty Ready-to-Issue tasks but ASI Issue Date
    → PERMIT_DATE FILLED.
  - CLOSED / Closed / Finaled shells missing FINAL_DATE → FILLED from
    Final* PASS inspections when available.
  - FINAL_DATE on non-Final statuses → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches DATA.date for every sample row.
  - ~460 Final shells (mostly legacy CLOSED conversions) have neither
    Ready-to-Issue Issued nor Issue Date → PERMIT_DATE stays missing.
  - ~74 Final shells have no Finaled task and no post-file Final*
    PASS inspection → FINAL_DATE stays missing.
  - Five ``Add a contractor`` shells with null DATA.status and no
    issuance / Finaled evidence remain STATUS_NORMALIZED null.
"""
...
_ISSUE_TASK_NAMES = {"Ready to Issue"}
_ISSUE_MARKS = {"Issued"}
_FINAL_TASK_NAMES = {"Inspections", "Inspection"}
...
def _as_date(val):
    """Normalize a parsed timestamp to datetime.date (None if missing)."""
...
def _walk_find_key(obj, key_names_lower: set[str]):
    """Depth-first search for a scalar whose key matches *key_names_lower*."""
    if isinstance(obj, dict):
        for k, v in obj.items():
            if str(k).strip().lower() in key_names_lower and not isinstance(
                v, (dict, list)
            ):
                dt = _safe_to_datetime(v)
                if dt is not pd.NaT:
                    return dt
            found = _walk_find_key(v, key_names_lower)
            if found is not pd.NaT:
                return found
    elif isinstance(obj, list):
        for item in obj:
            found = _walk_find_key(item, key_names_lower)
            if found is not pd.NaT:
                return found
    return pd.NaT
...
_STATUS_MAP = {
    ...
    "Hold": "In Review",
    "Plan Check About to Expire": "In Review",
}
...
def _has_finaled_task(d: dict) -> bool:
    tasks = d.get("tasks") or []
    return _latest_event_date(tasks, _FINAL_TASK_NAMES, _FINAL_MARKS) is not pd.NaT


def _expected_status(d: dict) -> Optional[str]:
    """Map DATA.status, then apply issuance / finaled upgrades.

    Inactive terminal labels are sticky. CLOSED / Closed / Finaled map to
    Final. Issued / About to Expire map to Active. Ready to Issue Issued
    promotes In Review → Active. Inspection(s) Finaled promotes Active
    (Issued) → Final. Final* inspection PASS alone does not promote.
    """
    ...
    # Inspections Finaled is the Accela completion mark — stronger than
    # DATA.status still reading Issued.
    if _has_finaled_task(d) and mapped in ("Active", "In Review", None):
        return "Final"
    ...
    # No DATA.status (e.g. Add-a-contractor shells): only infer when
    # issuance or Finaled-task evidence is present.
    if _has_finaled_task(d):
        return "Final"
...
def _issue_date_from_more_details(d: dict):
    """ASI / form Issue Date nested under more_details."""
    return _walk_find_key(
        d.get("more_details"),
        {"issue date", "issued date", "date issued", "permit issue date"},
    )
...
def _permit_date_from_data(d: dict):
    ...
    Prefer Ready to Issue Marked as Issued. Fall back to more_details
    ``Issue Date`` (present on many CLOSED conversion shells).
    """
    tasks = d.get("tasks") or []
    issued = _first_event_date(tasks, _ISSUE_TASK_NAMES, _ISSUE_MARKS)
    if issued is not pd.NaT:
        return issued
    return _issue_date_from_more_details(d)


def _final_insp_date(d: dict, floor=None):
    """Latest approved Final* inspection Status Date.

    Prefer Final Building / Permit Final / Final Occupancy titles; else
...
def _final_date_from_data(d: dict):
    ...
    Prefer the latest Inspection(s) ``Finaled`` mark (upstream sometimes
    stamped the first Finaled, which can coincide with a failed final
    inspection). Fall back to an approved Final* inspection Status Date
    on/after issuance (or file date when issuance is unknown).
    """
    tasks = d.get("tasks") or []
    finaled = _latest_event_date(tasks, _FINAL_TASK_NAMES, _FINAL_MARKS)
...
def _repair_record(row, d: dict, repairs: dict):
    ...
    if file_date is not pd.NaT:
        file_date_d = _as_date(file_date)
        if pd.isna(row["FILE_DATE"]):
            repairs["FILE_DATE"] = file_date_d
    ...
    if effective_status == "Final":
        ...
        if final_date is not pd.NaT:
            final_date_d = _as_date(final_date)
            if pd.isna(current_final):
                repairs["FINAL_DATE"] = final_date_d
                repairs["FINAL_DATE_FLAG"] = "FILLED"
            elif not _dates_equal(current_final, final_date):
                repairs["FINAL_DATE"] = final_date_d
                repairs["FINAL_DATE_FLAG"] = "FIXED"
    elif not pd.isna(current_final):
        repairs["FINAL_DATE"] = None
        repairs["FINAL_DATE_FLAG"] = "FIXED"
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Nevada County permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Nevada County".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Nevada County") & (df["STATE"] == "CA")].copy()

    print(f"Nevada County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_nevada_county_repaired.parquet"
    ...
    both_fp = f.notna() & p.notna()
    print(f"  PERMIT < FILE:  {(both_fp & (p < f)).sum()}")
    both_pf = p.notna() & fin.notna()
    print(f"  FINAL < PERMIT: {(both_pf & (fin < p)).sum()}")
    both_ff = f.notna() & fin.notna()
    print(f"  FINAL < FILE:   {(both_ff & (fin < f)).sum()}")
