"""Data repair for Cupertino (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Cupertino DATA is an Accela Citizen Access scrape. All sample rows share
core top-level keys (``address``, ``date``, ``status``, ``tasks``,
``search_data``, …); a few sparse rows omit optional blocks
...
Canonical mappings:
  ...
  - Earliest Inspection Marked as Final Inspection Complete  → FINAL_DATE

Known issues repaired:
  - STATUS_ORIGINAL lagging DATA.status: Finaled still Active /
    In Review; Issued still In Review / plan-review labels → FIXED.
  - Approved (plans approved, not issued) previously Active → FIXED
    to In Review.
  - In Review rows with a dated Permit Issuance Issued event
    (Pending / Submitted / Plan Review / Ready to Issue) → FIXED
    to Active.
  - Blank-status SolarApp shell → FILLED In Review from portal
    presence.
  ...
  - PERMIT_DATE missing on status-lagged Issued / Finaled rows that
    already have Issued events → FILLED after status promotion.
  - FINAL_DATE missing on status-lagged Finaled rows that already
    have Final Inspection Complete → FILLED.
  - Spurious FINAL_DATE on Active Issued rows that have a final
    inspection but whose DATA.status is still Issued → cleared.

Not repairable / left as-is:
  - Vast majority of older Finaled shells (esp. pre-2019) have empty
    or TBD-only Permit Issuance / Inspection tasks → PERMIT_DATE and
...
_ISSUE_MARKS = {"Issued"}
_FINAL_MARKS = {"Final Inspection Complete"}
_FILE_MARKS = {
...
_STATUS_MAP = {
    ...
    "Canceled": "Inactive",
    "Cancelled": "Inactive",
    # In Review (including plans-approved / fee / bond stages)
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels are sticky. DATA.status Finaled/Complete/CLOSED
    maps to Final. A dated Permit Issuance Issued promotes In Review → Active.
    Final Inspection Complete alone does **not** promote Issued → Final when
    ...
    # Blank portal status but a filed Accela record → treat as in-review shell.
...
def _final_date_from_data(d: dict):
    """Earliest Final Inspection Complete mark.

    Cupertino upstream FINAL_DATE matches the first Final Inspection
    Complete event when multiple exist (later marks appear to be
    reopen / follow-up inspections).
    """
    tasks = d.get("tasks") or []
    return _first_event_date(tasks, {"Inspection"}, _FINAL_MARKS)
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Cupertino permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Cupertino".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Cupertino") & (df["STATE"] == "CA")].copy()

    print(f"Cupertino records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_cupertino_repaired.parquet"
