"""Data repair for San Luis Obispo County (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Luis Obispo County DATA is a Tyler EnerGov-style payload with
top-level keys ``entity``, ``details``, ``contacts``, ``fees``, and
...
Known issues repaired:
  - STATUS_ORIGINAL lagging CaseStatus (Issued still In Review;
    Finaled still Active / Inactive / In Review; Expired / Void /
    Withdrawn still In Review or Active) → FIXED from CaseStatus.
  - Administrative Close was upstream-mapped to Final despite almost
    never carrying FinalDate / Issued (mostly Septic Inspection
    shells) → FIXED to Inactive (same treatment as Riverside County
    Administratively Closed / Daly City Admin Closed).
  - Monitoring (issued condition-compliance) kept as In Review →
    FIXED to Active.
  - Missing PERMIT_DATE on Active/Final after status catch-up when
    IssueDate is present → FILLED.
  - Missing FINAL_DATE on Finaled after status catch-up when
    FinalDate / FinalizeDate is present → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Void / Withdrawn / Expired
    / Issued / Monitoring / On Hold / Administrative Close) copied
    from entity.FinalDate → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Administrative Close and some Completed / Finaled shells lack
    IssueDate (Issued=False) → PERMIT_DATE stays missing.
  - Administrative Close remapped to Inactive lacks FinalDate →
    FINAL_DATE stays missing (and any stale FINAL_DATE is cleared).
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Void / Withdrawn / Expired / Issued is a case-closure
...
_STATUS_MAP = {
    ...
    "Withdrawn": "Inactive",
    "Void": "Inactive",
    "Administrative Close": "Inactive",
    # In Review (pre-issuance / application / hold)
    "In Review": "In Review",
    "Intake": "In Review",
...
# Prefer Finaled over Issued when CaseStatus and PermitStatus disagree.
_STATUS_RANK = {
    "Final": 4,
    "Active": 3,
    "In Review": 2,
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single SLO County record."""
    ...
    # Note: non-Finaled statuses (Void/Withdrawn/Expired/Issued/…) sometimes
    # carry FinalDate as a case-closure stamp; only keep FINAL_DATE when
    # status is Final.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Luis Obispo County permit records using information from the raw
    DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Luis Obispo County".
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "San Luis Obispo County") & (df["STATE"] == "CA")
    ].copy()

    print(f"San Luis Obispo County records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_luis_obispo_county_repaired.parquet"
