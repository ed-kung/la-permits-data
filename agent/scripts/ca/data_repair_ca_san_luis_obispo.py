"""Data repair for San Luis Obispo (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

San Luis Obispo DATA is a Tyler EnerGov-style payload (same family as
San Luis Obispo County / Poway / Danville) with top-level keys
``entity``, ``details``, ``contacts``, ``processing_status``, optional
``fees``, and an optional reviews bundle (``reviews`` / ``holds`` /
``attachments`` / ``more_info``). Key-set variants:

  - entity_fees:          entity + details + fees (+ contacts,
                          processing_status)
  - entity_fees_reviews:  entity_fees plus reviews/holds/attachments/
                          more_info
  - entity_basic:         entity + details without fees
...
Known issues repaired:
  - STATUS_ORIGINAL lagging CaseStatus (Finaled still Active with
    ORIG=issued; Issued still In Review with ORIG=on-line submission)
    → FIXED from CaseStatus.
  - Estimate shells upstream-mapped to Final despite no IssueDate /
    FinalDate → FIXED to In Review.
  - Issued (Legacy Only) upstream-mapped to Final despite no FinalDate
    → FIXED to Active.
  - Missing PERMIT_DATE on Active after Issued catch-up when IssueDate
    is present → FILLED.
  - Missing FINAL_DATE on Finaled after status catch-up when FinalDate
    is present → FILLED.
  - Spurious FINAL_DATE on non-Final rows (Issued / Expired) copied
    from entity.FinalDate closure stamps → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Two Finaled encroachment / EPM shells lack IssueDate (Issued=False)
    → PERMIT_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Issued / Expired is a case-closure stamp, not a
    permit finaled date.
"""
...
_STATUS_MAP = {
    ...
    # Active
    "Issued": "Active",
    "Issued (Legacy Only)": "Active",
    # Inactive
    "Expired": "Inactive",
    "Withdrawn": "Inactive",
    "Void": "Inactive",
    # In Review (pre-issuance / application / hold / estimate)
    "Under Review": "In Review",
    "On-Line Submission": "In Review",
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single SLO city record."""
    ...
    # Note: non-Finaled statuses (Expired / Issued) sometimes carry
    # FinalDate as a case-closure stamp; only keep FINAL_DATE when
    # status is Final.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    San Luis Obispo permit records using information from the raw DATA
    JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "San Luis Obispo".
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "San Luis Obispo") & (df["STATE"] == "CA")
    ].copy()

    print(f"San Luis Obispo records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_san_luis_obispo_repaired.parquet"
