"""Data repair for Hemet (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hemet DATA is a civic portal payload (same shape as Mendocino County /
Belmont). All sample rows share the same top-level keys: ``fees``,
``contacts``, ``site_info``, ``inspections``, ``permit_info``,
``search_data``. Canonical fields live under ``permit_info``
(``search_data`` only mirrors Address / RECORDID / Permit #):
...
Known issues repaired:
  - Unmapped PermitStatus left STATUS_NORMALIZED null (CREATED IN ERROR,
    OUT TO OWNER, SIGNATURE REQUIRED) → FILLED.
  - Stale STATUS_NORMALIZED vs PermitStatus (FINALED mislabeled
    Active/In Review; ACTIVE/EXPIRED mislabeled In Review/Active;
    REFUNDED mislabeled Active) → FIXED.
  - Rows with PermitFinaledDate but non-Final PermitStatus
    (ACTIVE / PENDING) → FIXED to Final (portal status lagged).
  - Active/Final missing PERMIT_DATE when Issued blank but Approved
    present → FILLED; Issued present but PERMIT blank on status-fixed
    rows → FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present → FILLED;
    blank FinaledDate but a final / C of O inspection → FILLED.
  - Spurious FINAL_DATE on non-Final (esp. Inactive EXPIRED) rows →
    cleared (FIXED).

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate wherever Applied exists;
    1 VOID shell lacks Applied (and any dates) in DATA.
  - ~50 Final rows lack PermitFinaledDate and a usable final inspection.
  - Some Active/Final rows with neither Issued nor Approved →
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    ...
    if not raw_status and not (has_issued or has_finaled or has_approved or has_applied):
...
_STATUS_MAP = {
    ...
    "OUT TO OWNER": "In Review",
    "SIGNATURE REQUIRED": "In Review",
    "EXPIRED": "Inactive",
    "EXPIRED-RENEWED": "Inactive",
    "CREATED IN ERROR": "Inactive",
    "DENIED": "Inactive",
    "REFUNDED": "Inactive",
}
...
_FINAL_TITLE_RE = re.compile(
    ...
    r"final-fire|final\s*-\s*pw|monument\s*final|"
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then EXPIRED* / CANCELLED* prefixes."""
    if not raw:
        return None
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hemet permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hemet".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Hemet") & (df["STATE"] == "CA")].copy()

    print(f"Hemet records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_hemet_repaired.parquet"
