"""Data repair for Santa Cruz (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Santa Cruz DATA is a civic portal payload (same shape as Willows /
Mill Valley / Hillsborough). All sample rows share top-level keys:
``fees``, ``contacts``, ``site_info``, ``inspections``,
``permit_info``, ``search_data``. Canonical fields live under
``permit_info``; ``search_data`` mirrors Status / Applied Date /
Issued Date / Finaled Date exactly when both are present.
...
Known issues repaired:
  - Stale STATUS_ORIGINAL mapping: ``FINALED`` shells left Active /
    Inactive / In Review → FIXED to Final.
  - ``PAYMENT AUTHORIZED`` / ``UPLOAD AUTHORIZED`` incorrectly mapped
    to Final (pre-issuance workflow) → FIXED to In Review (or Active
    when PermitIssuedDate is present).
  - ``EXPIRED`` / ``WITHDRAWN`` left Active → FIXED to Inactive.
  - ``ISSUED`` left In Review / Final → FIXED to Active.
  - Non-inactive shells with PermitFinaledDate left Active → Final.
  - Active / Final missing PERMIT_DATE when Issued or Approved is
    present → FILLED.
  - Final missing FINAL_DATE while PermitFinaledDate or a passed
    job-final inspection exists → FILLED.
  - Spurious FINAL_DATE on Inactive (Withdrawn / Void closure stamps)
    → cleared.

Status is not promoted to Final from intermediate inspections alone
(roof / plumbing / planning finals are common on still-ISSUED shells).

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate for every sample row.
  - Some FINALED / APPROVED Active-Final shells lack Issued and
    Approved → PERMIT_DATE stays missing.
  - Some FINALED shells lack PermitFinaledDate and have no usable
    final inspection → FINAL_DATE stays missing.
  - PermitFinaledDate on Withdrawn / Void / Expired is a case-closure
    stamp, not a permit finaled date (status stays Inactive;
    FINAL_DATE cleared).
"""
...
_STATUS_MAP = {
    ...
    "APPROVED": "Active",
    "ACTIVE": "Active",
    "REVIEWING REVISIONS": "Active",
    # In Review (intake / fee / upload / pre-issuance)
    "UNDER REVIEW": "In Review",
    "APPLICATION INCOMPLE": "In Review",  # truncated
    "APPLICATION INCOMPLETE": "In Review",
    "PAID": "In Review",
    "PAYMENT AUTHORIZED": "In Review",
    "UPLOAD AUTHORIZED": "In Review",
    "UPLOAD RECEIVED": "In Review",
...
# Empty Result is intentionally excluded — Santa Cruz schedules many
# "* FINAL" inspections with blank Result before they are completed.
_FINAL_INSP_OK = {
    "PASS",
    "PASSED",
    "PASS WITH COMMENTS",
...
def _lookup_status(raw: str) -> Optional[str]:
    ...
    if raw.startswith("ISSUED") or raw == "ACTIVE" or "NOT FINAL" in raw:
        return "Active"
    if "APPROV" in raw and "PAYMENT" not in raw and "UPLOAD" not in raw:
        return "Active"
    if (
        "SUBMIT" in raw
        or "REVIEW" in raw
...
def _preferred_permit_date(d: dict):
    ...
    if approved is not pd.NaT:
        return approved
    return _sd_date(d, "Issued Date", "Issued")
...
def _preferred_file_date(d: dict):
    ...
    if applied is not pd.NaT:
        return applied
    return _sd_date(d, "Applied Date", "Application")
...
def _is_job_final_inspection(item: dict) -> bool:
    """Whole-job / C-of-O finals only (not roof/plumbing/dept finals)."""
...
def _canonical_final_date(d: dict):
    """PermitFinaledDate / search Finaled Date only (no inspection fallback)."""
    finaled = _pi_date(d, "PermitFinaledDate")
    if finaled is not pd.NaT:
        return finaled
    return _sd_date(d, "Finaled Date", "FINALED", "Finaled")
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    Intermediate final inspections (roof / plumbing / planning) are common
    on still-ISSUED Santa Cruz shells and must not override PermitStatus.
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Santa Cruz record."""
    # -- STATUS_NORMALIZED --
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Santa Cruz permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Santa Cruz".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Santa Cruz") & (df["STATE"] == "CA")
    ].copy()

    print(f"Santa Cruz records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_santa_cruz_repaired.parquet"
