"""Data repair for La Habra (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

La Habra DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - 12 In Review (CaseStatus=Fees Due) rows carry FINAL_DATE copied from
    entity.FinalDate while the permit is still open for fee collection.
    Those stamps are not sign-offs under the authoritative CaseStatus
    → cleared.
  - 1 Final (CaseStatus=Finaled) row has a plausible entity.FinalDate
    (2028-12-06) that upstream left null → filled.
...
Not repairable from DATA:
  ...
  - 85 Final rows (83 CaseStatus=Complete, 2 Finaled) have null
    IssueDate and Issued=False, so PERMIT_DATE cannot be filled.
  - 83 Final (Complete) rows also lack FinalDate / FinalizeDate, so
...
_STATUS_MAP = {
    ...
    "Cancelled": "Inactive",
    "Denied": "Inactive",
    # In Review (pre-issuance / fees / enforcement)
    "In Review": "In Review",
    "Fees Due": "In Review",
    "Fees Paid": "In Review",
    "Submitted": "In Review",
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single La Habra record."""
    ...
    # -- FINAL_DATE (finaled / FinalDate; not ExpireDate / fee-due stamps) --
    # Prefer entity.FinalDate. On non-Final rows FinalDate can appear while
    # CaseStatus is still Fees Due (open for collection) and must not remain
    # on FINAL_DATE.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    La Habra permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "La Habra".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "La Habra") & (df["STATE"] == "CA")].copy()

    print(f"La Habra records: {len(city):,}\n")
