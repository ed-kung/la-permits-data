"""Data repair for Daly City (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Daly City DATA is a Tyler EnerGov-style payload with top-level keys
...
Known issues repaired:
  - STATUS_NORMALIZED is missing for 39 CaseStatus values the upstream
    mapper did not know (Admin Final, Expired–Closed, Closeout Pending).
  - Admin Closed / Superseded were labeled Final despite cancelled /
    unissued / replaced work (descriptions: expired plan check, never
    picked up, superseded). Remap to Inactive and clear spurious
    FINAL_DATE.
  - Extended was labeled In Review despite having IssueDate → Active.
  - Stop Work Order was labeled In Review → Inactive.
  - After Inactive remaps, clear FINAL_DATE copied from entity.FinalDate
    on Expired–Closed / Admin Closed / Superseded rows (not a sign-off
    for Inactive status).
  - Reject implausible FinalDate years (e.g. 2091) via year bounds.

Not repairable from DATA:
  - FILE_DATE already matches the UTC calendar date of entity.ApplyDate
    for all sample records.
  - ~1 Finaled row has null IssueDate (Issued=False) so PERMIT_DATE
    cannot be filled. FILE_DATE is not used as a proxy.
  - ~1 Finaled row has null FinalDate / FinalizeDate, so FINAL_DATE
    cannot be filled. Admin Closed / Superseded remapped to Inactive
    also lack usable issuance/final dates.
"""
...
# entity.CaseStatus (portal labels, as in DATA) → STATUS_NORMALIZED
# Expired–Closed uses an en-dash in the source data; hyphen variant included
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Daly City record."""
    ...
    if effective_status == "Final":
        ...
        # Spurious FINAL_DATE on non-Final rows (Admin Closed / Expired–
        # Closed / Superseded remapped to Inactive, etc.).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Daly City permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Daly City".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Daly City") & (df["STATE"] == "CA")].copy()

    print(f"Daly City records: {len(city):,}\n")
