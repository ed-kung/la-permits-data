"""Data repair for Hanford (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Hanford DATA is a civic portal payload (same shape as Tracy / Brentwood /
Dana Point). All sample rows share top-level keys: ``fees``, ``contacts``,
``site_info``, ``inspections``, ``permit_info``, ``search_data``.
Canonical fields live under ``permit_info``:
...
Content variants (INFERRED_SCHEMA) differ by which permit_info dates are
populated:
...
Known issues repaired:
  - One FINALED row mislabeled Active (STATUS_ORIGINAL=active) → FIXED
    to Final.
  - Non-inactive rows carrying PermitFinaledDate (ACTIVE / APPROVED /
    HOLD) → FIXED to Final.
  - Blank PermitStatus legacy shells inferred from dates / VOID address
    → FILLED Final / Active / In Review / Inactive.
  ...
  - Spurious FINAL_DATE on remaining non-Final rows (EXPIRED / VOID /
    and any leftover Active without upgrade) → cleared.

Not repairable from DATA:
  - 5 shells with empty PermitAppliedDate (and no search Application)
    → FILE_DATE stays missing.
  - Some Active/Final rows lack both Issued and Approved → PERMIT_DATE
    stays missing.
  - Some FINALED rows lack PermitFinaledDate and a usable finaling
    inspection (TEMP POLE / HVAC / COMPLIANCE-only sign-offs) →
    FINAL_DATE stays missing.
  - EXPIRED / VOID / WITHDRAWN keep Inactive even when PermitFinaledDate
    is a close stamp; that stamp is not treated as a true Final.
"""
...
# Plausible calendar-year range (Hanford sample includes a 1959 shell).
_MIN_YEAR = 1950
_MAX_YEAR = 2035
...
def _is_void_address(d: dict) -> bool:
    addr = _search_data(d).get("Address")
    if not isinstance(addr, str):
        return False
    return "VOID" in addr.upper()
...
_STATUS_MAP = {
    ...
    "PLAN REVIEW FEE DUE": "In Review",
    "RDY TO ISSUE": "In Review",
    "READY TO ISSUE": "In Review",
    "COND. APPROVAL": "In Review",
...
def _lookup_status(raw: str) -> Optional[str]:
    ...
    if "CANCEL" in raw or raw.startswith("VOID"):
        return "Inactive"
    if "EXPIRE" in raw or "WITHDRAW" in raw or "DENIED" in raw:
        return "Inactive"
    if raw.startswith("FINAL") or raw in ("CLOSED", "COMPLETE", "COMPLETED"):
        return "Final"
    if raw in ("ISSUED", "ACTIVE", "APPROVED") or "ISSUED" in raw or "APPROV" in raw:
        return "Active"
    if (
        "REVIEW" in raw
        or "PENDING" in raw
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if _is_void_address(d) and not raw:
        return "Inactive"
    ...
    # Blank / unmapped status: infer from dates.
...
def _result_ok(result: str) -> bool:
    ...
    if result_u in _FINAL_INSP_OK:
        return True
    if result_u.startswith("PASS") or result_u.startswith("PARTIAL"):
        return True
    return False
...
def _is_final_inspection(item: dict) -> bool:
    ...
    if "FOLLOW" in typ.upper():
        return False
    if result_u in ("FINAL", "FINALED", "FIN"):
        return True
    if not _FINAL_TITLE_RE.search(typ):
        return False
    # Require a completed / successful result for final-* types.
    if not result:
        return False
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Hanford record."""
    # -- STATUS_NORMALIZED --
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Hanford permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Hanford".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Hanford") & (df["STATE"] == "CA")].copy()

    print(f"Hanford records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_hanford_repaired.parquet"
