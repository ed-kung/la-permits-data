"""Data repair for Saratoga (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Saratoga DATA is a civic portal payload (same shape as Oceanside /
Merced County / Novato). All sample rows share the same top-level keys:
...
Known issues repaired:
  - Short codes ``AP`` (approved) and ``PP`` (issued/printed) wrongly
    normalized to Final → FIXED to Active (or Final when
    PermitFinaledDate is present).
  - Stale STATUS_ORIGINAL (issued / approved / under review) while
    PermitStatus is FINALED or ISSUED, and ISSUED/APPROVED rows
    carrying PermitFinaledDate → FIXED to Final / Active.
  ...
  - Active/Final missing PERMIT_DATE when Issued or Approved present
    (esp. CL closed rows with Approved-only, APPROVED, AP) → FILLED.
  - Final missing FINAL_DATE when PermitFinaledDate present (incl.
    after status fix) → FILLED; a few FINALED rows recovered from
    passed final inspection.
  - Spurious FINAL_DATE on Inactive EXPIRED rows → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches PermitAppliedDate on 1,999 / 2,000;
    one FINALED encroachment lacks Applied → stays missing.
  - Closed (CL) / FI Final rows almost never carry PermitFinaledDate
    or a titled final inspection → FINAL_DATE stays missing for most
    legacy closed records.
  - A handful of Active / Final rows lack both Issued and Approved
...
_STATUS_MAP = {
    # Final
    "FINALED": "Final",
    "CL": "Final",  # Closed (legacy)
    "FI": "Final",  # Final / finaled abbrev
    # Active
    "ISSUED": "Active",
    "APPROVED": "Active",
    "AP": "Active",  # Approved abbrev (approved date, no final)
    "PP": "Active",  # Permit printed / issued (issued date present)
    # In Review
    "APPLIED": "In Review",
    "E-APPLIED": "In Review",
...
def _lookup_status(raw: str) -> Optional[str]:
    """Exact map, then EXPIRED* / VOID* / FINAL* / ISSUED* prefixes."""
    if not raw:
        return None
    if raw in _STATUS_MAP:
        return _STATUS_MAP[raw]
    if raw.startswith("VOID") or "CANCEL" in raw:
        return "Inactive"
    if "EXPIRE" in raw:
        return "Inactive"
    if raw.startswith("FINAL") or raw in ("CL", "FI"):
        return "Final"
...
def _is_inactive_keep(raw: str) -> bool:
    ...
    if raw in _INACTIVE_KEEP:
        return True
    if "EXPIRE" in raw or raw.startswith("VOID") or "CANCEL" in raw:
        return True
    return False
...
def _derive_status(d: dict) -> Optional[str]:
    ...
    if raw:
        if "FINAL" in raw or raw in ("CL", "FI"):
            return "Final"
        if (
            "EXPIRE" in raw
            or "VOID" in raw
            ...
            return "Inactive"
        if "ISSUE" in raw or raw in ("AP", "PP") or "APPROV" in raw:
            return "Active"
        if (
            "APPLI" in raw
            or "REVIEW" in raw
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Saratoga permit records using information from the raw DATA JSON.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Saratoga".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Saratoga") & (df["STATE"] == "CA")].copy()

    print(f"Saratoga records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_saratoga_repaired.parquet"
