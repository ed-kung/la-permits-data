"""Data repair for Upland (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Upland DATA is a Tyler EnerGov-style payload with top-level keys
...
Canonical fields live under ``entity`` (with details fallbacks):
  ...
  - FinalDate (fallback: details.FinalizeDate;
    then Final* processing_status with status Final/Passed/Approved)
    → FINAL_DATE

Known issues repaired:
  - ``Finaled`` shells left Active (STATUS_ORIGINAL=issued) → FIXED to
    Final; FINAL_DATE FILLED from FinalDate/FinalizeDate.
  - ``Issued`` / ``Approved`` / ``Submitted`` / ``On Hold/Pending`` /
    ``Online Submission`` rows with IssueDate left In Review → FIXED to
    Active; PERMIT_DATE FILLED when missing.
  - ``Issued`` shells with PermitStatus ``Finaled`` and FinalizeDate
    strictly after IssueDate left Active → FIXED to Final.
  - ``Issued`` shells with FinalDate/FinalizeDate strictly after
    IssueDate left Active → FIXED to Final.
  - ``Stop Work Order`` shells with FinalDate after IssueDate left In
    Review → FIXED to Final.
  - ``Expired`` shells left Active (STATUS_ORIGINAL=issued) → FIXED to
    Inactive.
  - ``Voided`` left In Review → FIXED to Inactive.
  - ``On Hold/Pending`` left Inactive (STATUS_ORIGINAL=denied) → FIXED
    to Active when IssueDate is present.
  - Junk FINAL_DATE on non-Final rows (Voided / Refunded / Cancelled /
    Denied / Stop Work without promotion / Issued same-day stamps) →
    cleared.
  - Active Issued shells missing PERMIT_DATE while IssueDate is present
    → FILLED.
  - Finaled shells missing FINAL_DATE but with a Final* inspection
    whose status is Final/Passed/Approved → FILLED.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Finaled shells with null IssueDate → PERMIT_DATE stays missing.
  - Finaled shells with neither FinalDate/FinalizeDate nor a usable
    Final* inspection → FINAL_DATE stays missing (none observed after
    inspection fallback in the sample).
  - ExpireDate is a validity window, not a completion date.
  - FinalDate on Expired / Voided / Refunded / Cancelled / Denied is a
...
_STATUS_MAP = {
    ...
    "Pending": "In Review",
    "Pending Master Plan Approval": "In Review",
    "Approved": "In Review",
...
# Upland Final* inspection outcomes that count as completion evidence.
...
def _final_inspection_date(d: dict):
    ...
    Upland uses inspection ``status`` values ``Final``, ``Approved``, and
    (rarely) ``Passed`` rather than a uniform ``Passed`` label.
    """
    ...
    for item in ps:
        ...
        if status not in _FINAL_INSPECTION_OK:
            continue
        # Exclude partial approvals (status already filtered, but be safe).
...
def _has_credible_final_stamp(d: dict) -> bool:
    ...
    Upland occasionally leaves CaseStatus as Issued while PermitStatus is
    Finaled (FinalizeDate present), or leaves FinalDate strictly after
    IssueDate on Issued / Approved / Stop Work Order shells. Require
    either an explicit Finaled/Complete label, or FinalDate/FinalizeDate
    strictly after IssueDate.
    """
...
def _mapped_status(d: dict) -> Optional[str]:
    for raw in _status_strings(d):
        ...
        if any(
            ...
            return "Inactive"
        if lower.startswith("issued") or lower == "active" or "post-issuance" in lower:
            return "Active"
        if any(
            tok in lower
            for tok in (
...
def _expected_status(d: dict) -> Optional[str]:
    ...
    Inactive terminal labels (Expired / Voided / Refunded / Revoked /
    Cancelled / Denied) are sticky even when FinalDate is present as a
    case-closure stamp. Explicit Finaled / Complete labels → Final.
    Otherwise a FinalDate/FinalizeDate strictly after IssueDate → Final
    (stale Issued / Approved / Stop Work shells). IssueDate → Active
    overrides review-pipeline labels (Submitted / Online Submission /
    On Hold/Pending / Approved / Plan Check).
    """
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Upland record."""
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Upland permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Upland".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Upland") & (df["STATE"] == "CA")
    ].copy()

    print(f"Upland records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_upland_repaired.parquet"
