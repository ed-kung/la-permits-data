"""Data repair for Union City (CA) permit records.
...
Union City's Dewey / sample payload has **no usable DATA JSON** on any
row (confirmed in both ``permits_ca_sample.parquet`` and the upstream
Dewey shards ``building-permits-united-states_0_5_17.snappy.parquet``
and ``building-permits-united-states_2_5_4.snappy.parquet`` — 14,892
Union City CA jurisdiction rows, all with null DATA). All records
therefore receive ``INFERRED_SCHEMA = "missing"``.

``STATUS_ORIGINAL`` is also null on every sample (and upstream) row.
Without agency JSON or an original status string, date fields cannot be
validated or filled from source. The only structured agency signal
available is presence of ``PERMIT_DATE``, which is used to fill blank
``STATUS_NORMALIZED`` shells:

  - Blank ``STATUS_ORIGINAL`` / ``STATUS_NORMALIZED`` shells that
    already carry a ``PERMIT_DATE`` → ``Active`` (same convention as
    Newark blank fire/ops shells with an issuance stamp).

If ``STATUS_ORIGINAL`` appears in a future feed, the canonical map
below is applied for fill / fix:
...
Known issues repaired:
  - All 2,000 sample rows have null STATUS_NORMALIZED and null
    STATUS_ORIGINAL but a populated PERMIT_DATE → FILLED Active.

Not repairable / left as-is:
  - FILE_DATE missing on every row (no Applied / Submitted field and no
    DATA). Permit-number year tokens (e.g. BLDG-21-…) often match or
    precede PERMIT_DATE by one year but are not a calendar application
    date and are not used.
  - FINAL_DATE missing on every row (no Finaled / completion field and
    no DATA). No rows can be promoted to Final without a terminal status
    or finaling stamp.
...
def _classify_schema(data_dict: Optional[dict]) -> str:
    """Union City sample has no DATA; any future payload → unknown."""
...
def _repair_record(row, d: Optional[dict], repairs: dict):
    """Populate *repairs* for a single Union City record.
    ...
    # Active (Union City sample: every row; same as Newark blank shells).
    ...
    # No agency date fields are available in DATA for Union City. Leave
    # existing values unchanged (including systematically missing
    # FILE_DATE / FINAL_DATE).
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Union City permit records.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Union City".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[
        (df["JURISDICTION"] == "Union City") & (df["STATE"] == "CA")
    ].copy()

    print(f"Union City records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_union_city_repaired.parquet"
