"""Data repair for Clovis (CA) permit records.

Repairs STATUS_NORMALIZED, FILE_DATE, PERMIT_DATE, and FINAL_DATE using
the raw DATA JSON column. Creates {FIELD}_FLAG columns with "FILLED" or
"FIXED" annotations for every value that was changed.

Clovis DATA is a Tyler EnerGov-style payload with top-level keys
...
Canonical fields live under ``entity`` (with details fallbacks):
  - CaseStatus / details.PermitStatus  → STATUS_NORMALIZED
    ...
    e.g. PermitStatus Finaled over CaseStatus Issued)
  - ApplyDate                          → FILE_DATE
  - IssueDate                          → PERMIT_DATE
  - FinalDate (fallback: details.FinalizeDate) → FINAL_DATE

Known issues repaired:
  - STATUS_NORMALIZED null for Applied for Online (upstream mapper
    missed this) → FILLED as In Review.
  - Stale STATUS_NORMALIZED vs DATA (Active / In Review while
    CaseStatus is Finaled; Active while Expired; In Review while
    Nullified) → FIXED.
  - Final missing FINAL_DATE when FinalDate is present (incl. after
    status fix from stale Issued/Submitted) → FILLED.
  - Spurious FINAL_DATE on non-Final rows (esp. Application Approved
    permit-application shells whose entity.FinalDate is an application
    close timestamp, plus Nullified / Comments Out) → cleared.

Not repairable / left as-is:
  - FILE_DATE already matches entity.ApplyDate for every sample row.
  - Nine Finaled rows have Issued=False and null IssueDate →
    PERMIT_DATE stays missing.
  - Two older Finaled Combination S.F. Dwelling shells lack
    FinalDate / FinalizeDate and have empty processing_status →
    FINAL_DATE stays missing.
  - ExpireDate is a validity window, not a completion date.
  - Application Approved FinalDate is not a permit finaled date and
    must not promote status to Final.
"""
...
_STATUS_MAP = {
    ...
    "Refunded": "Inactive",
    "Nullified": "Inactive",
    # In Review (pre-issuance / application / comments)
    "Application Approved": "In Review",
    "Submitted": "In Review",
    "Plan Check Approved": "In Review",
    "Comments Out": "In Review",
    "Comments Ready": "In Review",
    "Payment Pending": "In Review",
    "Applied for Online": "In Review",
    "Under Review": "In Review",
    "Waiting for Other Division": "In Review",
    "Re-Submitted": "In Review",
}
...
def _repair_record(row, d: dict, repairs: dict):
    """Populate *repairs* with corrected values for a single Clovis record."""
    ...
    # Note: Application Approved shells often carry FinalDate as an
    # application-close timestamp; only keep FINAL_DATE when status is Final.
...
def data_repair(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Clovis permit records using information from the raw DATA JSON column.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame filtered to JURISDICTION == "Clovis".  Must contain
...
if __name__ == "__main__":
    ...
    city = df[(df["JURISDICTION"] == "Clovis") & (df["STATE"] == "CA")].copy()

    print(f"Clovis records: {len(city):,}\n")
    ...
    if AGENT_DATA_PATH:
        ...
        out_path = out_dir / "permits_ca_clovis_repaired.parquet"
