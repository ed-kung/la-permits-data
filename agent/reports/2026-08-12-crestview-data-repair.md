# Crestview (FL) data repair

**Summary:** First FL sample jurisdiction without an existing repair script was **Crestview**. DATA is a flat MyGovernmentOnline (MGO) project payload (`ProjectStatus` / `DateCreated` / `DateIssued`). `STATUS_NORMALIZED` already matched portal status on every row; `FILE_DATE` already matched `DateCreated` on all 2,000 rows. `DateIssued` and `DateUpdated` are the `.NET` sentinel `0001-01-01` on every row, and no completion/CO date exists, so `PERMIT_DATE` and `FINAL_DATE` remain universally missing. After repair: STATUS fully populated; FILE_DATE 100%; Active/Final PERMIT_DATE 0%; Final FINAL_DATE 0%.

## Jurisdiction selection

Ordered `(JURISDICTION, STATE)` pairs in `MY_DATA_PATH/processed_data/permits_fl_sample.parquet` were checked against `agent/scripts/{state}/data_repair_{state}_{city}.py` (slug via `[^a-z0-9]+` → `_`). First missing: **Crestview, FL** → `agent/scripts/fl/data_repair_fl_crestview.py` (2,000 sample rows).

## DATA schemas (`INFERRED_SCHEMA`)

| Schema | n | Notes |
| --- | ---: | --- |
| `mgo_ppm` | 1,328 | Includes `PaymentProcessorModule` (value `MGO`) |
| `mgo_base` | 672 | Same flat key set without that key |

No nested inspections/fees objects. All rows have `ProjectType == "Permit"`. `TypeList` holds fee-description strings (e.g. Roof Permit, Mechanical/Gas Standalone Permit).
...
| --- | ---: | --- | --- |
| Project Closed/Complete | 1,201 | Final | Correct |
| Permit Issued | 390 | Active | Correct |
| Pending (Under Review) | 210 | In Review | Correct |
| Expired | 173 | Inactive | Correct |
| Cancelled | 26 | Inactive | Correct |
...
- Before: missing on **all 2,000** rows, including all Active (390) and Final (1,201).
...
- Root cause: MGO extract does not populate issuance timestamps in this city sample (same pattern as Pensacola / Santa Rosa County / Escambia County).
...
Status distribution unchanged: Final 1,201; Active 390; In Review 210; Inactive 199.

## Artifacts

- Script: `agent/scripts/fl/data_repair_fl_crestview.py`
- Repaired sample: `$AGENT_DATA_PATH/crestview_repaired_sample.parquet`
