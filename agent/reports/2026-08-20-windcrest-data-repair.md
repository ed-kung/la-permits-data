...
| --- | ---: | --- |
| `mgo_ppm` | 1,993 | `PaymentProcessorModule == "MGO"` |
| `mgo_base` | 7 | same keys, no `PaymentProcessorModule` |
...
| `RequestInspections` | Always `false` in sample; no inspection dates |
...
| --- | --- | --- | ---: |
| closed | Closed | Final | 788 |
| permit issued | Permit Issued | Active | 728 |
| open | Open | In Review | 482 |
| expired | Expired | Inactive | 1 |
...
- Missing before repair: **2,000 / 2,000** (including all 728 Active + 788 Final)
...
- Missing before repair: **2,000 / 2,000** (including all 788 Final)
...
- Path: `agent/scripts/tx/data_repair_tx_windcrest.py`
...
- Status map covers Windcrest’s observed values (`Closed`, `Permit Issued`, `Open`, `Expired`, `Withdrawn`) plus common MGO siblings
...
- Script: `agent/scripts/tx/data_repair_tx_windcrest.py`
- Repaired sample parquet: `AGENT_DATA_PATH/repaired/permits_tx_windcrest_repaired.parquet`
